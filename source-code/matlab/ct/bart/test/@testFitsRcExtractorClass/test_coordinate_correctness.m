function self = test_coordinate_correctness(self)
%
...
%
    ...
    % Test that the output image does not have row/column swapping for the pixel values
    ...
    for channel = 1:25:84
        ...
        for ii = 1:100
            ...
            assert(image(rows(ipix), cols(ipix)) == data(ipix));
            assert(image(rows(ipix), cols(ipix)) == data(ipix));
        end
