...
% 3 pixels on each CCD, no orientation errors, plate scale error.
% 
...
% state for them...
  ...
  fpgTest2 = set(fpgTest2,'randState',rand('twister')) ;
  fpgTest2 = set(fpgTest2,'randnState',randn('state')) ;
  
% assign the centroiding error to the canonical 20 micro-pixels

  fpgTest2 = set(fpgTest2,'centroidErrorPixels',20e-6) ;
...
% assign the CCD errors, plate scale error, orientation error to zero
  ...
  fpgTest2 = set(fpgTest2,'ccdError',[ccdPointError ccdPointError ccdRotError]) ;
  fpgTest2 = set(fpgTest2,'overallOrientationError',[0 0 0]) ;
  fpgTest2 = set(fpgTest2,'dPlateScale',plateScaleError) ;
  
% there is only one cadence, and it is at the reference orientation

  fpgTest2 = set(fpgTest2,'cadencedOrientation',[0 ; 0 ; 0]) ;
  
% cadence pointing error isn't used if the only orientation is 0 0 0, but put it in as
% zeroes anyway

  fpgTest2 = set(fpgTest2,'cadencePointingError',[0 0 0 0 0]) ;
  
% the date:  according to my copy of Mission Plan rev C, April 15 2009 looks like a good
% day (ie, no rolls planned that day)

  fpgTest2 = set(fpgTest2,'calendarDate','15-apr-2009 12:00:00') ;
...
% 275, 500, 725, 950.
  ...
  fpgTest2 = set(fpgTest2,'posRowCol',[row' col']) ;
    
% generate the test data!

  fpgTest2 = generateData(fpgTest2) ;
