...
% state for them...
  ...
  fpgTest7 = set(fpgTest7,'randState',rand('twister')) ;
  fpgTest7 = set(fpgTest7,'randnState',randn('state')) ;
  
% assign the centroiding error to the canonical 20 micro-pixels

  fpgTest7 = set(fpgTest7,'centroidErrorPixels',20e-6) ;
...
% set the overall orientation error
  ...
  fpgTest7 = set(fpgTest7,'ccdError',[ccdPointError ccdPointError ccdRotError]) ;
  fpgTest7 = set(fpgTest7,'overallOrientationError',[raError decError rollError]) ;
  fpgTest7 = set(fpgTest7,'dPlateScale',plateScaleError) ;
...
%                 0 * ovec ...
 ...
 % dColScanVec = [0.5 0.25 0 -0.25 -0.5] ;
  dRowScan = [0.5*ovec 3.33*ovec 1.67*ovec 0*ovec -1.67*ovec -3.33*ovec -5*ovec] ;
  dColScanVec = [0.5 3.33 1.67 0 -1.67 -3.33 -5] ;
  dColScan = [dColScanVec dColScanVec dColScanVec dColScanVec dColScanVec dColScanVec dColScanVec] ;
  ...
  fpgTest7 = set(fpgTest7,'cadencedOrientation',[dRAdDec ; zeros(1,49) ]) ;
  
% cadence pointing errors -- put in errors at the 0.1 pixel / 0.1% level

  fpgTest7 = set(fpgTest7,'cadencePointingError',[1.11e-6 1.11e-6 1.15e-5 0.001 -0.001]) ;
 
% the date:  according to my copy of Mission Plan rev C, April 15 2009 looks like a good
% day (ie, no rolls planned that day)

  fpgTest7 = set(fpgTest7,'calendarDate','15-apr-2009 12:00:00') ;
...
% 275, 500, 725, 950.
  ...
  fpgTest7 = set(fpgTest7,'posRowCol',[row' col']) ;
    
% generate the test data!

  fpgTest7 = generateData(fpgTest7) ;
  [drow,dcol] = fpgTestDataCheck(fpgTest7) ;
