% script to construct fpgTestData objects for test 5:  4 cadences, errors of up to
...
% state for them...
  ...
  fpgTest5 = set(fpgTest5,'randState',rand('twister')) ;
  fpgTest5 = set(fpgTest5,'randnState',randn('state')) ;
  
% assign the centroiding error to the canonical 20 micro-pixels

  fpgTest5 = set(fpgTest5,'centroidErrorPixels',20e-6) ;
...
% set the overall orientation error
  ...
  fpgTest5 = set(fpgTest5,'ccdError',[ccdPointError ccdPointError ccdRotError]) ;
  fpgTest5 = set(fpgTest5,'overallOrientationError',[raError decError rollError]) ;
  fpgTest5 = set(fpgTest5,'dPlateScale',plateScaleError) ;
...
%              dColScanVec dColScanVec dColScanVec dColScanVec dColScanVec ] ;
  ...
  dRowScan = [0.5 -0.5 0  0.5 -0.5] ;
  dColScan = [0.5  0.5 0 -0.5 -0.5] ;
  ...
  fpgTest5 = set(fpgTest5,'cadencedOrientation',[dRAdDec ; zeros(1,5) ]) ;
  
% cadence pointing errors -- put in errors at the 0.1 pixel / 0.1% level

  fpgTest5 = set(fpgTest5,'cadencePointingError',[1.11e-6 1.11e-6 1.15e-5 0.001 -0.001]) ;
 
% the date:  according to my copy of Mission Plan rev C, April 15 2009 looks like a good
% day (ie, no rolls planned that day)

  fpgTest5 = set(fpgTest5,'calendarDate','15-apr-2009 12:00:00') ;
...
% 275, 500, 725, 950.
  ...
  fpgTest5 = set(fpgTest5,'posRowCol',[row' col']) ;
    
% generate the test data!

  fpgTest5 = generateData(fpgTest5) ;
  [drow,dcol] = fpgTestDataCheck(fpgTest5) ;
