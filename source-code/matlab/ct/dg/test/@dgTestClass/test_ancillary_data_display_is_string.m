function self = test_ancillary_data_display_is_string(self)
% mlunit for testing if string ancillary data gets tabled and displayed
% correctly
% 
...
% expectedOutputType is 'table' since we are testing for ancillary data
% that comes in string values
expectedOutputType = 'table';

% create structure with single entry of string values and save in mat
% format
anc.mnemonic = 'FOC_STATUS';
anc.timestamps = (50000:51000)';
anc.values = [];
anc.stringValues = repmat({'on'},1001,1);
anc.uncertainties = [];
...
save ancTestStringData.mat anc
...
dgAncObj = dgAncillaryClass({'hello'}, 100, 200, 'ancTestStringData.mat');

% call the display method and observe that a figure is not displayed
