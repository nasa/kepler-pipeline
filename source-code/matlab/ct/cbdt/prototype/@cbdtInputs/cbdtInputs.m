function obj = cbdtInputs(varargin)

% Constructor for a cbdtInputs class object.
...
if nargin == 0 % Used when objects are loaded from disk
  obj = init_fields;
  obj = class(obj, 'cbdtInputs');
  return;
end

firstArg = varargin{1};
if isa(firstArg, 'cbdtInputs') %  used when objects are passed as arguments
  obj = firstArg;
  return;
end
...
obj.originalFFI = single( zeros(FFI_ROWS, FFI_COLS, CBD_FFI_NO) );  % 3 tuple input FFI images
