function obj = cbdtParams(varargin)

% Constructor for a cbdtParams class object.
...
if nargin == 0 % Used when objects are loaded from disk
  obj = init_fields;
  obj = class(obj, 'cbdtParams');
  return;
end

firstArg = varargin{1};
if isa(firstArg, 'cbdtParams') %  used when objects are passed as arguments
  obj = firstArg;
  return;
end
...
% the parameters associated with the algorithms
obj.smoothRadius = uint32(zeros(1));
