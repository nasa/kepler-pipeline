function obj = cbdBadPixelsClass(varargin)

% Constructor for a cbdBadPixelsClass class object.
...
if nargin == 0 % Used when objects are loaded from disk
    obj = init_fields;
    obj = class(obj, 'cbdBadPixelsClass');
    return;
elseif nargin == 1

    firstArg = varargin{1};
    if isa(firstArg, 'cbdBadPixelsClass') %  used when objects are passed as arguments
        obj = firstArg;
        return;
    end
else
    error('cbdBadPixelsClass: too many input');
end
...
obj.badPixelMap = []; % 2D mask of the bad, FGS and gap pixels
