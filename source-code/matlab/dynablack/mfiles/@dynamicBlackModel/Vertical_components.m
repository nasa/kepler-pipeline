function valDN = Vertical_components(obj, row_list, lc_list)
% function valDN = Vertical_components(obj, row_list, lc_list)
% 
% Vertical_components (multiple pixel & LC case)
...
% THESE ELEMENTS MUST MATCH WHAT WAS FIT (seeA1_parameter_init.m)
% 
% We fit nominally 6 rows 2,3,4,5,9,10
% Note coadd normalization for the collateral black is contained in sub matrices (e.g. allpix_const, allpix_logrow, ...)
% for the fit so we will *not* include that normalization while retrieving (evaluating) the models for single (non-coadded) pixels.
% 
...
if nargin > 0
    ...
    valDN = zeros(row_count,lc_count);
    ...
    for row_ID = 1:row_count
        ...
        coefficients = obj.Vertical_parameters.estimates( obj.Predictors, 2:(size(predictors,2)+1), lc_list )';
        valDN(row_ID,1:lc_count) = sum( coefficients.*predictors, 2 );
    end
end
