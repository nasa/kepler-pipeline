...
rand('seed',0); %reset the random number generator
...
bitxor(cadusMinusASMs,repmat(pseudoRandSeq,size(cadus,1),1));

% map to RS basis
cadusMinusASMsDerandomizedRS = RSstruct.berlekamp_to_RS(...
    double(cadusMinusASMsDerandomized)+1);
...
testVcdu = (1:1115*2);
numVcduBlocks = 2;
testVcduInterleaved = interleave_vcdus(testVcdu,...
    rsInfoBlockLength, interleavingFactor);
