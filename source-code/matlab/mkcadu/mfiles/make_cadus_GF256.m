function cadus = make_cadus_GF256(vcdus)
% cadus = ccsds_vcdu_GF256(vcdus)
...
% note that vcdusInRS is a GF256 array
vcdusInRS = berlekamp_to_RS(GF256(vcdusInterleaved));

% Pad out vcdus to 255 (rsCodeBlockLength)
vcdusInRSPadded = [vcdusInRS,zeros(size(vcdusInRS,1),numCheckBytes)];

%% generate checkbits
%RScheckbits = modG(vcdusInRS);

RScheckbits = generate_rs_checkbitsGF256(vcdusInRS);

%% tranform Checkbits back to dual basis
checkbits = RS_to_berlekamp(RScheckbits);
...
cadusRandomized = cadusDeinterleaved+...
