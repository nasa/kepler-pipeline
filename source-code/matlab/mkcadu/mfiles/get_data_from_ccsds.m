function data = get_data_from_ccsds(ccsdsFileName)
...
secondaryHeaderLength = 8;
scienceHeaderLength = 16;
...
for count = 1:10000
    ...
    headersToSkip = ccsdsHeaderLength + scienceHeaderLength;
    dataLength = packetLength - headersToSkip;
    ...
    % now get the data
    % seek to the beginning of that data from the start of the packet
    fseek(fid, headersToSkip, 'cof');
    data = [data fread(fid, [1, dataLength], 'uint8=>uint8')];
    % seek back to the beginning
    status = fseek(fid, -(dataLength + headersToSkip), 'cof');
    if status == -1
        break;
    end
