function browse_flux_compare3(inputsStruct1,outputsStruct1,outputsStruct2,outputsStruct3,title1,title2,title3,varargin)
% function browse_flux_compare3(inputsStruct1,outputsStruct1,outputsStruct2,outputsStruct3,title1,title2,title,3varargin)
% 
...
%
    ...
    kIds3 = [ outputsStruct3.targetResultsStruct().keplerId ];
...
%             str = [ '   Target: ' int2str(k) '   Quarter: ' int2str(q) '   Mod.Out: ' int2str(inputsStruct1.ccdModule) '.' int2str(inputsStruct1.ccdOutput) ];
            ...
            if (NANGAPS)
                x = t;
                x(gaps) = nan;
                y = outputsStruct3.targetResultsStruct(k).correctedFluxTimeSeries.values;
                y(gaps) = nan;
            else
                x = t(~gaps);
                y = outputsStruct3.targetResultsStruct(k).correctedFluxTimeSeries.values(~gaps);
            end
            plot( x , y , LINESTYLE );
            if (PLOTGAPS)
                x = t(gaps);
            ...
            ylim3 = get(a_(3),'ylim');
            ylim4 = get(a_(4),'ylim');
            ylim(1) = min([ylim2(1) ylim3(1) ylim4(1)]);
            ylim(2) = max([ylim2(2) ylim3(2) ylim4(2)]);
            for j=2:4
                set(a_(j),'ylim',ylim);
            end
            linkaxes(a_,'x');
            ...
            pdc_display_target_diagnostics(k, outputsStruct1, outputsStruct2, outputsStruct3);
...
%             pause;
            ...
            disp(['no data found for target ' int2str(k) ' in second or third dataset.']);
        end
	end;
