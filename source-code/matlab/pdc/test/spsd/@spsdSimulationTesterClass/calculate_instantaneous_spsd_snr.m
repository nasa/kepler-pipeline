function snr = calculate_instantaneous_spsd_snr( obj, events )
%
% This function calculates the instantaneous signal to noise of the SPSD 
% detection filter response to a simulated SPSD.
%
% 
...
%
    ...
    for i = 1:numel(events)
        ...
        reponseToCleanFlux    = conv(cleanConditioned, kernel, 'valid');

        snrArray = abs(reponseToAdditiveSpsd ./ reponseToCleanFlux);
        snr(i) = max( snrArray( max(1, events(i).cadence - obj.LOCALIZATION_TOLERANCE):min(length(snrArray), events(i).cadence + obj.LOCALIZATION_TOLERANCE) ) );
