function self = test_retrieve_ts_SAP_centroids_uncertainties(self)
%KamalUddin
% 
...
% Kamal Uddin
display('test_retrieve_ts_SAP_centroids_uncertainties');
display(datestr(now()));
...
sapCols=retrieve_ts('SapCentroidCols', keplerIds, startCadenceq4, endCadenceq4);

if (isempty(sapCols))
    assert(1, 0, 'The retrieved structure is empty.');
end

sapColsUncert = retrieve_ts('SapCentroidColsUncert', keplerIds, startCadenceq4, endCadenceq4);
if (isempty(sapColsUncert))
    assert(1, 0, 'The retrieved structure is empty.');
end

sapRows       = retrieve_ts('SapCentroidRows',       keplerIds, startCadenceq4, endCadenceq4);
if (isempty(sapRows))
    assert(1, 0, 'The retrieved structure is empty.');
end
...
toc;
clear sapCols sapColsUncert sapRows sapColsUncert ;
display('*********Done*******');
