function self = test_retrieve_ts_flux_weighted(self)
%kamalUddin
% 
...
% Kamal Uddin
display('test_retrieve_ts_flux_weighted');
display(datestr(now()));
...
%tsFlux = retrieve_ts('tsPrfCentRow', keplerIds, startCadence, endCadence);
fwcRows       = retrieve_ts('FluxWeightedCentroidRows',       keplerIds, startCadenceq4, endCadenceq4);
if (isempty(fwcRows))
    assert(1, 0, 'The retrieved structure is empty.');
end

fwcRowsUncert = retrieve_ts('FluxWeightedCentroidRowsUncert', keplerIds, startCadenceq4, endCadenceq4);

if (isempty(fwcRowsUncert))
    assert(1, 0, 'The retrieved structure is empty.');
end

fwcCols       = retrieve_ts('FluxWeightedCentroidCols',       keplerIds, startCadenceq4, endCadenceq4);
if (isempty(fwcCols))
    assert(1, 0, 'The retrieved structure is empty.');
end

fwcColsUncert = retrieve_ts('FluxWeightedCentroidColsUncert', keplerIds, startCadenceq4, endCadenceq4);
...
clear fwcRows fwcRowsUncert fwcCols fwcColsUncert ;
display('*********Done*******');
end
