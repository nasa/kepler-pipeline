function self = test_retrieve_categories_q6(self)
%kamal
% 
...
display('test_retrieve_categories_q5(self)');
% 4. retrieve available data range, input 1 for short cadence, 0 or empty is
%for long cadence

display(datestr(now()));

%q6targetListSetName='astero_lc_quarter6_summer2010_trimmed.txt';

%'quarter5_spring2010_untrimmed_lc';
 ...
 categories = retrieve_categories(q6targetListSetName);
 
 
if (isempty(categories))
    %assert(false , 'The retrieved structure is empty.');
    assert(false, 'The retrieved structure is empty.');
