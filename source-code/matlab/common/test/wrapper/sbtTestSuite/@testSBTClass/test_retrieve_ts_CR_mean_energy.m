function self = test_retrieve_ts_CR_mean_energy(self)
...
display('test_retrieve_ts_CR_mean_energy');
display(datestr(now()));
%mdj is for q3 and month 1
 ...
 ccdOut = 2;
          ...
          ts = retrieve_ts('PaLcCosmicRayMeanEnergy', ccdMod, ccdOut,  startCadenceq3, endCadenceq3 );
          if (isempty(ts))
            assert(1, 0, 'The retrieved structure is empty.');
          end
          
toc;
clear ts;display('*********Done*******');
end
