function self = test_retrieve_kepler_ids_by_label_labelCategories(self)
% Kamal Uddin
% 
...
%
tic;
display('test_retrieve_kepler_ids_by_label_labelCategories');
display(datestr(now()));

% targetListSetName= 'rc-flight-lc_v2';
...
matchingTargets = retrieve_kepler_ids_by_label(q4targetListSetName, 'labels', labels, 'categories',categories);

 
 
if (isempty(matchingTargets))
    assert_equals(1, 0, 'The retrieved structure is empty.');
end
