function self = test_retrieve_filestore_ffi_mjdModOutRawdataSave0(self)
% Kamal Uddin
% 
...
%
tic;
display('test_retrieve_filestore_ffi_mjdModOutRawdataSave0');
display(datestr(now()));
...
saveLocalCopies=0;
 ...
 ffi_data_struct =retrieve_filestore_ffi(mjdStartq4, mjdEndq4, modules, outputs, get_raw_data,saveLocalCopies);


if (isempty(ffi_data_struct))
    assert(false, 'The retrieved structure is empty.');
end

toc;
clear ffi_data_struct;
display('*****Done*********');
end
