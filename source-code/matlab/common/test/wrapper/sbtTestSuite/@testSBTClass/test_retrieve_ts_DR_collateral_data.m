function self = test_retrieve_ts_DR_collateral_data(self)
...
display('test_retrieve_ts_DR_collateral_data');
display(datestr(now()));
...
%    Get the DR_collateral_data for mod 7/out 3, coordinate 37 for cadences 0-400:
ts1 = retrieve_ts('DrCollateralLongVirtualSmear', ccdMod, ccdOut,  coordinate,  startCadenceq2, endCadenceq2 );
if (isempty(ts1))
    assert(1, 0, 'The retrieved structure is empty.');
end

ts2 = retrieve_ts('DrCollateralLongMaskedSmear', ccdMod, ccdOut,  coordinate,  startCadenceq2, endCadenceq2 );
if (isempty(ts2))
    assert(1, 0, 'The retrieved structure is empty.');
end
ts3 = retrieve_ts('DrCollateralLongBlack', ccdMod, ccdOut,  coordinate,  startCadenceq2, endCadenceq2 );
