function self = test_retrieve_target_time_series_lcCalibrated(self)
...
display('test_retrieve_target_time_series_lcCalibrated');
display(datestr(now()));
...
%    Get the DR_collateral_data for mod 7/out 3, coordinate 37 for cadences 0-400:
 ...
 targetTimeSeries = retrieve_target_time_series(ccdModules, ccdOutputs, startCadenceq3, endCadenceq3, isLongCadence, isCalibrated);
  


if (isempty(targetTimeSeries))
    assert(1, 0, 'The retrieved structure is empty.');
end


toc;
clear targetTimeSeries;
