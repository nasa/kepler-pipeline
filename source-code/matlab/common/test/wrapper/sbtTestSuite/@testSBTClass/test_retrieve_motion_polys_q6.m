function self = test_retrieve_motion_polys_q6(self)
% Kamal Uddin
...
%
display('test_retrieve_motion_polys_q6');
display(datestr(now()));
% isLongCadence=1;
tic;

display(' test_retrieve_motion_polys_sdf_q6(self)');
...
% endMjd  = 55380.03206960;
 ...
 motion_poly_struct =  retrieve_motion_polys(mjdStartq6, mjdEndq6);


 
if (isempty(motion_poly_struct))
    assert(1, 0, 'The retrieved structure is empty.');
end


toc;
clear motion_poly_struct;
display('*********Done*******');
end