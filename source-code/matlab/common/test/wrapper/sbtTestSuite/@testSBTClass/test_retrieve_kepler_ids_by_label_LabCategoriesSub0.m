function self = test_retrieve_kepler_ids_by_label_LabCategoriesSub0(self)
% Kamal Uddin
% 
...
%
tic;
display('test_retrieve_kepler_ids_by_label_LabCategoriesSub0');
display(datestr(now()));

% targetListSetName= 'quarter4_winter2009_lc';

labels = {'TAD_ONE_HALO'};

categories = {'GO_LC'};

labelsAndCategoriesAreSubstrings =0;
test_inputs
 ...
 matchingTargets = retrieve_kepler_ids_by_label(q4targetListSetName, 'labels', labels, 'categories', categories, labelsAndCategoriesAreSubstrings);

 
if (isempty(matchingTargets))
    assert(1, 0, 'The retrieved structure is empty.');
end

toc;
clear matchingTargets;
display('*********Done*******');
end