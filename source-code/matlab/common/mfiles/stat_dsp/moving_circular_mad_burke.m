...
% the decimated regime covers the same number of undecimated samples.
  ...
  nAverageDecimated = max([round( nAverage / decimationFactor ) 7]) ;
...
%     if median subtracted is desired, compute the median; otherwise, set equal to zero
      ...
      if subtractMedianFlag
          medianValue = medfilt1( decimatedTimeSeries, nAverageDecimated, nAverageDecimated ) ;
      else
...
%     the specified blksz
      ...
      madValueDecimated = medfilt1( abs( decimatedTimeSeries - medianValue ), ...
          nAverageDecimated,  nAverageDecimated ) ;
