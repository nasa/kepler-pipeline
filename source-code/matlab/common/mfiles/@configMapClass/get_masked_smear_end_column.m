function [maskedSmearEndColumns, closestConfigMapTimeStamps] = get_masked_smear_end_column(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [maskedSmearEndColumns, closestConfigMapTimeStamps] = get_masked_smear_end_column(configMapObject, timeStamps)
%
%
...
maskedSmearEndColumns = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('maskedRegionEndColumn', allNames, 'exact');
    if(~isempty(indexMatched))
        maskedRegionEndColumn = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field maskedRegionEndColumn - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end


    maskedSmearEndColumns(jTimeStamp) = maskedRegionEndColumn + 1;  % converts to 1-based column value
