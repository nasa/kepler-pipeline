function [exposuresPerFFI, closestConfigMapTimeStamps] = get_number_of_exposures_per_ffi(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [exposuresPerFFI, closestConfigMapTimeStamps] = get_number_of_exposures_per_ffi(configMapObject, timeStamps)
%
%
...
exposuresPerFFI = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('numberOfIntegrationsInFFI', allNames, 'exact');
    if(~isempty(indexMatched))
        exposuresPerFFI(jTimeStamp) = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field numberOfIntegrationsInFFI - for time stamp ' num2str(timeStamps(jTimeStamp))]);
    end
