function [exposureTimes, closestConfigMapTimeStamps] = get_exposure_time(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [exposureTimes, closestConfigMapTimeStamps] = get_exposure_time(configMapObject, timeStamps)
%
%
...
exposureTimes = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    % KSOC-399 get_exposure_time method for configMapClass has inappropriate
    % definition of "exposure time"
    % JVC Commented: The interval between reads of a given pixel of a CCD is
    % the integration time, which is in turn composed of a selectable exposure
    % time and fixed readout time of 0.520 s. The set of pixels collected by
    % one repetition of the CCD clocking cycle is called a frame, and frame and
    % integration time are synonymous, though the former is preferred where
    % applicable since it is less multivalent. The flight exposure time is 6.02
    % s, giving an integration time = exposure + readout times = 6.54 s.


    exposureTimes(jTimeStamp) = (integrationTimeInFGSframes*FGSframePeriodInMsec); %
    exposureTimes(jTimeStamp) = exposureTimes(jTimeStamp)*1e-3; % converts to seconds

    if(exposureTimes(jTimeStamp) > 8 ||exposureTimes(jTimeStamp) < 2.49) % from the CDPP spread sheet maintained on LiveLink
        error('configMapClass:get_exposure_time:inavlidExposreTime', ...
            ['configMapClass:retrieved config map is invalid - computed exposure time ' num2str(exposureTimes(jTimeStamp)) ' is not within range as specified in the CDPP spreadsheet']);
    end
