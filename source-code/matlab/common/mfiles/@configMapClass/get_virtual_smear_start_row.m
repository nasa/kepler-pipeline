function [virtualSmearStartRows, closestConfigMapTimeStamps] = get_virtual_smear_start_row(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [virtualSmearStartRows, closestConfigMapTimeStamps] = get_virtual_smear_start_row(configMapObject, timeStamps)
%
%
...
virtualSmearStartRows = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('smearRegionStartRow', allNames, 'exact');
    if(~isempty(indexMatched))
        smearRegionStartRow = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field smearRegionStartRow - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end

    virtualSmearStartRows(jTimeStamp) = smearRegionStartRow + 1; % converts to 1-based row value
