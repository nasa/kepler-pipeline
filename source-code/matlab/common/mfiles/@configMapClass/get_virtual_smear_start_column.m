function [virtualSmearStartColumns, closestConfigMapTimeStamps] = get_virtual_smear_start_column(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [virtualSmearStartColumns, closestConfigMapTimeStamps] = get_virtual_smear_start_column(configMapObject, timeStamps)
%
%
...
virtualSmearStartColumns = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('smearRegionStartColumn', allNames, 'exact');
    if(~isempty(indexMatched))
        smearRegionStartColumn = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field smearRegionStartColumn - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end

    virtualSmearStartColumns(jTimeStamp) = smearRegionStartColumn + 1;  % converts to 1-based column value
