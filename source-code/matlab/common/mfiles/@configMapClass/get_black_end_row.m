function [blackEndRows, closestConfigMapTimeStamps] = get_black_end_row(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [blackEndRows, closestConfigMapTimeStamps] = get_black_end_row(configMapObject, timeStamps)
%
%
...
blackEndRows = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('blackRegionEndRow', allNames, 'exact');
    if(~isempty(indexMatched))
        blackRegionEndRow = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field blackRegionEndRow - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end

    blackEndRows(jTimeStamp) = blackRegionEndRow + 1;  % converts to 1-based column value
