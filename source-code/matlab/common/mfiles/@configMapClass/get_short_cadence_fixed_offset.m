function requantTableFixedOffsets = get_short_cadence_fixed_offset(configMapObject, timeStamps)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function requantTableFixedOffsets = get_fixed_offset(configMapObject, timeStamps)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%  This function currently returns a hardcoded constant for the fixed
%  offset value as the mnemonic for requantization table fixed offset has
%  not been added yet. Once that menmonic has beed added, this method needs
%  to be rewritten to fetch the value of fixedOffset from the configmaps.
%
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
% FDMSCOFFSET      requantization table fixed offset                       # (ADU?)        INT16             offset added to requantized values to keep them positive
% 
...
requantTableFixedOffsets = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('requantTableScFixedOffset', allNames, 'exact');

    if(~isempty(indexMatched))
        requantTableFixedOffsets(jTimeStamp) = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field requantTableScFixedOffset - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end
