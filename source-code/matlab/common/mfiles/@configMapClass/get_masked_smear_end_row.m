function [maskedlSmearEndRows, closestConfigMapTimeStamps] = get_masked_smear_end_row(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [maskedlSmearEndRows, closestConfigMapTimeStamps] = get_masked_smear_end_row(configMapObject, timeStamps)
%
%
...
maskedlSmearEndRows = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('maskedRegionEndRow', allNames, 'exact');
    if(~isempty(indexMatched))
        maskedRegionEndRow = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field maskedRegionEndRow - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end

    maskedlSmearEndRows(jTimeStamp) = maskedRegionEndRow + 1;  % converts to 1-based row value
