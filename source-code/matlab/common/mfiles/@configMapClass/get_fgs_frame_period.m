function [fgsFramePeriodInMsec, closestConfigMapTimeStamps] = ... 
    get_fgs_frame_period(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [fgsFramePeriodInMsec, closestConfigMapTimeStamps] = ...
% get_fgs_frame_period(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
tempFgsFramePeriodInMsec = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    if  numel(unique(allTimeStamps - timeStamps(jTimeStamp)))==1
        closestTimeIndex = jTimeStamp;
    else
    ...
    if(~isempty(indexMatched))
        tempFgsFramePeriodInMsec(jTimeStamp)= configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
...
if numel(tempFgsFramePeriodInMsec) ~= 1 % Multiple configMaps retrieved

    if numel(unique(tempFgsFramePeriodInMsec)) ~= 1 % Check for consistency;
        error('Inconsistent FGS frame periods in configMap')
    else
        tempFgsFramePeriodInMsec = unique(tempFgsFramePeriodInMsec);
    end
end

fgsFramePeriodInMsec = tempFgsFramePeriodInMsec;
