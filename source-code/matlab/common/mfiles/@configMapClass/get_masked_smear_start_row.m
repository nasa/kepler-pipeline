function [maskedSmearStartRows, closestConfigMapTimeStamps] = get_masked_smear_start_row(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [maskedSmearStartRows, closestConfigMapTimeStamps] = get_masked_smear_start_row(configMapObject, timeStamps)
%
%
...
maskedSmearStartRows = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('maskedRegionStartRow', allNames, 'exact');
    if(~isempty(indexMatched))
        maskedRegionStartRow = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field maskedRegionStartRow - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end


    maskedSmearStartRows(jTimeStamp) = maskedRegionStartRow + 1;  % converts to 1-based row value
