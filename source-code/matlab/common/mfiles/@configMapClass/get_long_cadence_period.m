function [longCadencePeriods, closestConfigMapTimeStamps] = get_long_cadence_period(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [longCadencePeriod, closestConfigMapTimeStamps] = get_long_cadence_period(configMapObject, timeStamps)
%
%
%
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
longCadencePeriods = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('longCadencePeriodInShortCadences', allNames, 'exact');

    if(~isempty(indexMatched))
        longCadencePeriodInShortCadences = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field longCadencePeriodInShortCadences - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end

    indexMatched = strmatch('shortCadencePeriodInIntegrations', allNames, 'exact');

    if(~isempty(indexMatched))
        shortCadencePeriodInIntegrations = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field shortCadencePeriodInIntegrations - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end



    indexMatched = strmatch('integrationTimeInFGSframes', allNames, 'exact');

    if(~isempty(indexMatched))
        integrationTimeInFGSframes = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field integrationTimeInFGSframes - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end

    indexMatched = strmatch('FGSframePeriodInMsec', allNames, 'exact');

    if(~isempty(indexMatched))
        FGSframePeriodInMsec = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field FGSframePeriodInMsec - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end

    indexMatched = strmatch('readoutPeriodInMsec', allNames, 'exact');

    if(~isempty(indexMatched))
        readoutPeriodInMsec = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field readoutPeriodInMsec - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end


    integrationTime = (integrationTimeInFGSframes*FGSframePeriodInMsec) + readoutPeriodInMsec;% exposure time
    integrationTime = integrationTime*1e-3; % converts to seconds


    shortCadencePeriodInSeconds = shortCadencePeriodInIntegrations*integrationTime;

    longCadencePeriods(jTimeStamp) = longCadencePeriodInShortCadences*shortCadencePeriodInSeconds;
