function [maskedSmearStartColumns, closestConfigMapTimeStamps] = get_masked_smear_start_column(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [maskedSmearStartColumns, closestConfigMapTimeStamps] = get_masked_smear_start_column(configMapObject, timeStamps)
%
%
...
maskedSmearStartColumns = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('maskedRegionStartColumn', allNames, 'exact');
    if(~isempty(indexMatched))
        maskedRegionStartColumn = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field maskedRegionStartColumn - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end


    maskedSmearStartColumns(jTimeStamp) = maskedRegionStartColumn + 1;  % converts to 1-based column value
