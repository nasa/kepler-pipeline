function [blackEndColumns, closestConfigMapTimeStamps] = get_black_end_column(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [blackEndColumns, closestConfigMapTimeStamps] = get_black_end_column(configMapObject, timeStamps)
%
%
...
blackEndColumns = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('blackRegionEndColumn', allNames, 'exact');
    if(~isempty(indexMatched))
        blackRegionEndColumn = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field blackRegionEndColumn - for time stamp ' num2str(timeStamps(jTimeStamp))]);
    end

    blackEndColumns(jTimeStamp) = blackRegionEndColumn + 1;  % converts to 1-based column value
