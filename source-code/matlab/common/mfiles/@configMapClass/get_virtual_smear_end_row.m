function [virtualSmearEndRows, closestConfigMapTimeStamps] = get_virtual_smear_end_row(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [virtualSmearEndRows, closestConfigMapTimeStamps] = get_virtual_smear_end_row(configMapObject, timeStamps)
%
%
...
virtualSmearEndRows = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('smearRegionEndRow', allNames, 'exact');
    if(~isempty(indexMatched))
        smearRegionEndRow = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field smearRegionEndRow - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end

    virtualSmearEndRows(jTimeStamp) = smearRegionEndRow + 1; %  % converts to 1-based row value
