function [blackStartRows, closestConfigMapTimeStamps] = get_black_start_row(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [blackStartRows, closestConfigMapTimeStamps] = get_black_start_row(configMapObject, timeStamps)
%
%
...
blackStartRows = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('blackRegionStartRow', allNames, 'exact');
    if(~isempty(indexMatched))
        blackRegionStartRow = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field blackRegionStartRow - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end

    blackStartRows(jTimeStamp) = blackRegionStartRow + 1;  % converts to 1-based row value
