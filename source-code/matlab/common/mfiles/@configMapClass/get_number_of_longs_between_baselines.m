function [numberOfLongsBetweenBaselines, closestConfigMapTimeStamps] = get_number_of_longs_between_baselines(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [numberOfLongsBetweenBaselines, closestConfigMapTimeStamps] = get_number_of_longs_between_baselines(configMapObject, timeStamps)
%
%
...
numberOfLongsBetweenBaselines = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('numberOfLCperiodsBetweenBaselines', allNames, 'exact');
    if(~isempty(indexMatched))
        numberOfLCperiodsBetweenBaselines = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field numberOfLCperiodsBetweenBaselines - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end

    numberOfLongsBetweenBaselines(jTimeStamp) = numberOfLCperiodsBetweenBaselines;
