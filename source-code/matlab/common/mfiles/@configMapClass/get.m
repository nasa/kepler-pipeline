function configMapStruct = get(configMapObject, field, timeStamp)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function configMapStruct = get(configMapObject, field, timeStamp)
%  GET Get configMapObjectClass property from the specified object
%  and return the value.
%
%  No error is generated if field does not refer to a valid member -
%  instead all the fields are returned in the form of a structure.
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% The get method provides a way to access the data in the configMapObject
% object using a "property name" style interface. While in this example the
% property names are similar to the structure field name, they can be quite
% different. You could also choose to exclude certain fields from access via
% the get method or return the data from the same field for a variety of
% property names, if such behavior suits your design.
%
% 
...
% bugzilla #738 
% FDMLCOFFSET      requantization table fixed offset                       # (ADU?)        INT16             offset added to requantized values to keep them positive 
...
% requantTableFixedOffset
%
if(exist('timeStamp', 'var'))
    if(length(timeStamp) > 1)
        warning('configMapClass:get:tooManyTimestamps',...
            ['More than one time stamp found - will return results for  ' num2str(timeStamp(1))]);
...
if(~exist('field','var') && ~exist('timeStamp','var'))
    field = '';
    timeStamp = [];
...
if(isempty(field) && isempty(timeStamp))

    configMapStruct = struct(configMapObject);

elseif(isempty(field) && ~isempty(timeStamp))
    % look for the time stamp or the nearest time stamp (closest earlier
    % timestamp) and return the structure

    closestTimeIndex = find((allTimeStamps - timeStamp) < 0, 1,'last');

    if(isempty(closestTimeIndex))
        error('configMapClass:get:invalidTimestamp',...
            ['no match found for the given time stamp ' num2str(timeStamp)]);
    end

    configMapStruct = struct(configMapObject(closestTimeIndex));

elseif(~isempty(field) && isempty(timeStamp))

    % add the description string to each mnemonic in the 'entries' structure
    nConfigMaps = length(configMapObject);
    configMapStruct = struct(configMapObject);
    for jMap = 1:nConfigMaps
        % keep only the requested field
        allNames = cat(1,configMapObject(jMap).entries.name);
        indexMatched = strmatch(field, allNames, 'exact');
        if(~isempty(indexMatched))

            configMapStruct(jMap).entries(1:indexMatched-1) = [];
            configMapStruct(jMap).entries(2:end) = [];
        else
            warning('configMapClass:get:invalidFieldName',...
                ['no match found for the given field ' field '- for the time stamp ' num2str(timeStamp)]);
            configMapStruct(jMap).entries = [];
        end
...
elseif(~isempty(field) && ~isempty(timeStamp))
    ...
    allNames = cat(1, configMapObject(closestTimeIndex).name);
    indexMatched = strmatch(field, allNames, 'exact');

    if(~isempty(indexMatched))
        configMapStruct = struct(configMapObject(closestTimeIndex).entries(indexMatched));
    else

        warning('configMapClass:get:invalidFieldName',...
            ['no match found for the given field ' field '- so returning all the fields for the time stamp ' num2str(timeStamp)]);
        configMapStruct = struct(configMapObject(closestTimeIndex));
