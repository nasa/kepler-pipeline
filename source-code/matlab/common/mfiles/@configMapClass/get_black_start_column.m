function [blackStartColumns, closestConfigMapTimeStamps] = get_black_start_column(configMapObject, timeStamps)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [blackStartColumns, closestConfigMapTimeStamps] = get_black_start_column(configMapObject, timeStamps)
%
%
...
blackStartColumns = -1*ones(nTimeStamps,1);
...
for jTimeStamp = 1:nTimeStamps
    ...
    indexMatched = strmatch('blackRegionStartColumn', allNames, 'exact');
    if(~isempty(indexMatched))
        blackRegionStartColumn = configMapObject(closestTimeIndex).entries(indexMatched).value;
    else
        error('configMapClass:get:invalidFieldName',...
            ['no match found for the given field blackRegionStartColumn - for the time stamp ' num2str(timeStamps(jTimeStamp))]);
    end

    blackStartColumns(jTimeStamp) = blackRegionStartColumn + 1;  % converts to 1-based column value
