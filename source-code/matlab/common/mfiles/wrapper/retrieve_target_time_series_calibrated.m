function time_series_struct = retrieve_target_time_series_calibrated(modules, outputs, start_mjd, end_mjd, get_long_cadence)
%
...
%
% target_time_series_struct = retrieve_target_time_series_calibrated(modules, outputs, start_mjd, end_mjd)
% or
% target_time_series_struct = retrieve_target_time_series_calibrated(modules, outputs, start_mjd, end_mjd, get_long_cadence)
%
    ...
    for ittl = 0:targetTableLogs.size() - 1 % N.B. Java indexing
        ...
        for itargdef = 0:targetDefinitions.size()-1 % N.B. Java indexing
            ...
            for ioffset = 0:offsets.size()-1 % N.B. Java indexing
                ...
                fs_id = CalFsIdFactory.getTimeSeriesFsId( ...
                        PixelTimeSeriesType.SOC_CAL_UNCERTAINTIES, SbtFcOperations.getTargetType('LC'), ...
                        ccd_module, ccd_output, ...
                        row, column);
                fs_ids_uncert(end+1) = fs_id;
            end
            ...
            timeSeriesesUncert = tsOps.readPixelTimeSeriesAsFloat(fs_ids_uncert, loop_start_cadence, loop_end_cadence);
            
            if timeSerieses.length ~= length(fs_ids_data)
                error('timeSerieses/fs_ids length mismatch!');
            end
            
            for itimeseries = 1:timeSerieses.length
                ...
                one_channel_time_series_struct.keplerIdTimeSeriesStruct(itargdef).uncertainties(:, itimeseries) = timeSeriesesUncert(itimeseries).getUncertainty();
            end
