function keplerIdTimeSeriesStruct = retrieve_pixel_time_series_work_uncalibrated_target(observedTarget, ccdModule, ccdOutput, targetType, startCadence, endCadence, fsclient)
%
...
%
    ...
    if isempty(aperture)
        warning('MATLAB:SBT:wrapper:retrieve_pixel_time_series_work_uncalibrated_target', ...
    ...
    timeSeriesType = SbtFcOperations.getDrTimeSeriesType('ORIG');
    ...
    for ipix = 1:size(uniqRowCol, 1)
        fsIdsRaw.add(DrFsIdFactory.getSciencePixelTimeSeries(timeSeriesType, targetType, ccdModule, ccdOutput, row, col));
    ...
    fsIdsArrayRaw = SbtFcOperations.makeArrayFromList(fsIdsRaw);

    % The name tses* indicates multiple timeseries:
    %
    tsesRaw = fsclient.readTimeSeriesAsInt(fsIdsArrayRaw, startCadence, endCadence, false);

    if length(tsesRaw) ~= length(fsIdsArrayRaw)
        error('MATLAB:SBT:wrapper:retrieve_pixel_time_series', 'timeSerieses/fs_ids length mismatch!');
    end

    
    nTimeSerieses = length(tsesRaw);
    nData = length(tsesRaw(1).iseries);
    
    keplerIdTimeSeriesStruct.timeSeries    = zeros(nData, nTimeSerieses);
    keplerIdTimeSeriesStruct.gapIndicators = zeros(nData, nTimeSerieses);
    keplerIdTimeSeriesStruct.uncertainties = zeros(nData, nTimeSerieses);
    
    for itimeseries = 1:length(tsesRaw)
        data   = tsesRaw(itimeseries).iseries;
        gaps   = tsesRaw(itimeseries).getGapIndicators();
