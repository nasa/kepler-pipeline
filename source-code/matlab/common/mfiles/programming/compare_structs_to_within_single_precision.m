function  status  = compare_structs_to_within_single_precision(s1,s2, nameOfStruct1, nameOfStruct2)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function status  = compare_structs_to_within_single_precision(s1,s2, nameOfStruct1, nameOfStruct2)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% Decription:
% This function compares two given structures one of whose fields (could be
% some) are in double precision and the other in single precision. This
% utility comes in handy when we need to compare two structures which are
% identical in all respects except for the data type - one holding double
% precision and the other single precision. This situation is encountered
% when we read from the datastore or write to the data store the Matlab
% data structure which is double precision as single precision
%
...
%
% Inputs:
%       s1, s2 - structures one of whose fields (could be some) are in
%       double precision and the othe rin single precision
...
%       same to within single precision, false otherwise.
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% 
...
status = test_structs(s1,s2, nameOfStruct1, nameOfStruct2);

return



function status = test_structs(s1,s2, nameOfStruct1, nameOfStruct2)
        ...
        for k = 1:nFields
            ...
            if(~isstruct(s1(j).(namesOfFields{k})))
                if(isfloat(s1(j).(namesOfFields{k})))
                    ...
                    v1(isnan(v1)) = -sqrt(pi);
                    v2(isnan(v2)) = -sqrt(pi);
                    compare = isequal(single(v1), single(v2));
                    if ~compare,
                        % print error message
                        newNameOfStruct1 = [nameOfStruct1 '(' num2str(j) ').' (namesOfFields{k})];
                        newNameOfStruct2 = [nameOfStruct2 '(' num2str(j) ').' (namesOfFields{k})];
                        fprintf('%s is not equal to %s at the single precision level \n', newNameOfStruct1, newNameOfStruct2);
                        status = false;
                        %break
                    end
                end
                ...
                newNameOfStruct1 = [nameOfStruct1 '(' num2str(j) ').' (namesOfFields{k})];
                newNameOfStruct2 = [nameOfStruct2 '(' num2str(j) ').' (namesOfFields{k})];
                compare = test_structs(s1(j).(namesOfFields{k}),s2(j).(namesOfFields{k}), newNameOfStruct1, newNameOfStruct2);
                if ~compare
                    fprintf('%s is not equal to %s at the single precision level \n', newNameOfStruct1, newNameOfStruct2);
                    status = false;
                    %break
