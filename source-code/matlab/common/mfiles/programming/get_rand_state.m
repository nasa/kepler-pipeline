function randState = get_rand_state
%
% get_rand_state -- determine the state of the random number generator, and its method.
%
% randState = get_rand_state determines the method of the random number generator, and the
%    state of the generator, at the time of calling.  The method is returned as a string
%    in randState.method; the state is returned as a numeric vector in randState.vector.
%
% See also:  set_rand_state.
%
...
% populate the list of known possible rand methods, and capture their state values
  ...
  methodList = {'twister','state','seed'} ;
  methodState = cell(size(methodList)) ;
  for iList = 1:length(methodList)
      methodState{iList} = rand(methodList{iList}) ;
  end
...
% method setting
  ...
  for iList = 1:length(methodList)
      rand(methodList{iList},methodState{iList}) ;
      methodRandValue(iList) = rand(1) ;
  end
