function [beta,r,J,Sigma,mse] = kepler_user_nonlinearfit(X,y,model,beta,options,userStruct)
% function [beta,r,J,Sigma,mse] = kepler_user_nonlinearfit(X,y,model,beta,options,userStruct)
%
% This is a version of the nlinfit function that accepts a user structure
% that is passed as the third argument to model which now has the signature
% model(beta, X, userStruct).  Otherwise this function is the same as
% nlinfit.
%
...
if strcmp(options.Robust,'off')
    ...
    [beta,J,lsiter,cause] = LMfit(X,y, model,beta,options,verbose,maxiter,userStruct);
else
    ...
    % limit.
    [beta_ls,J] = LMfit(X,y, model,beta,options,0,maxiter,userStruct);
    res = y - model(beta_ls,X,userStruct);
    res(isnan(res)) = [];
    ...
    [beta,J,sig,cause] = nlrobustfit(X,y,beta,model,J,ols_s,options,verbose,maxiter,userStruct);  
end;
...
%----------------------------------------------------------------------
function  [beta,J,iter,cause] = LMfit(X,y, model,beta,options,verbose,maxiter,userStruct) 
...
while iter < maxiter
    ...
    J = getjacobian(beta,fdiffstep,model,X,yfit,nans,userStruct);
...
%--------------------------------------------------------------------------
function [beta,J,sig,cause]=nlrobustfit(x,y,beta,model,J,ols_s,options,verbose,maxiter,userStruct)
...
while maxiter>0
    ...
    sw = sqrt(w);
    yw = y .* sw;
    modelw = @(b,x) sqrt(w).*model(b,x,userStruct);
...
fdiffstep = options.DerivStep;
J = getjacobian(beta,fdiffstep,model,x,yfit,~ok,userStruct);
...
% ---------------------- Jacobian
function J = getjacobian(beta,fdiffstep,model,X,yfit,nans,userStruct)
p = numel(beta);
...
for k = 1:p
    ...
    yplus = model(beta+delta,X,userStruct);
    dy = yplus(:) - yfit(:);
    dy(nans) = [];
    J(:,k) = dy/delta(k);
    delta(k) = 0;
