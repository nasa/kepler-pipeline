...
static void estimate_mes_distribution_parameters( double *correlationTimeSeries, 
        ...
        double threshold, double *nIterations, double maxIterations ) ;
...
/* Main Function */
void estimate_sample_size( double* correlationTimeSeries, double* normalizationTimeSeries, 
        int nPulses, unsigned int nTransits, double meanEstimateTolerance, unsigned int* lengthSES,
        double threshold, unsigned int* startIndices, double* sampleSize, double maxIterations ) {
...
nTransits, lengthSES, sampleSize, nPulses, maxIterations ) \
private( iPulse )
    {
#pragma omp for
    ...
    for(iPulse = 0; iPulse < nPulses; iPulse++){
        ...
        estimate_mes_distribution_parameters( correlationTimeSeries, normalizationTimeSeries, 
                nTransits, lengthSES[iPulse], nSamples, threshold, sampleSize+iPulse, maxIterations ) ;
...
void estimate_mes_distribution_parameters( double *correlationTimeSeries, 
        double *normalizationTimeSeries, unsigned int nTransits, 
        unsigned int lengthSES, unsigned long long int nSamples, 
        double threshold, double *sampleSize, double maxIterations ) {
    ...
    mpf_t mpMaxIterations;
    
    mpf_init(mpMaxIterations);
    ...
    mpf_set_d(mpMaxIterations, maxIterations);
    ...
    mpf_div(mpProbability, mpMaxIterations, mpProbability);    
    ...
    *sampleSize = mpf_get_d(mpProbability);
    *sampleSize = *sampleSize * 0.5;
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
    ...
    maxIterations = mexGetScalar( prhs[7] ) ;
    ...
    sampleSize = mxGetPr( plhs[0] ) ;

    estimate_sample_size( correlationTimeSeries, normalizationTimeSeries, nPulses, nTransits, 
            meanEstimateTolerance, lengthSES, threshold, startIndices, sampleSize, maxIterations ) ;
