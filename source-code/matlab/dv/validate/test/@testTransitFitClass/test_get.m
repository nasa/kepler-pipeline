...
%
% test_get -- test the transitFitClass get method
%
...
%
%      run(text_test_runner, testTransitFitClass('test_get'));
%
...
%=========================================================================================
  ...
  disp('... testing transitFitClass get method ... ')
...
% Test 1:  get the list of members
  ...
  assert_equals( memberList, memberList2, ...
      'get using "?" and "help" not identical in transitFitClass' ) ;
  
% test 2:  get all of the members at once

  transitFitStruct1 = get( transitFitObject1, '*' ) ;
  assert_equals( fieldnames(transitFitStruct1), memberList, ...
...
% test 3:  get the members one at a time and compare to the all-members approach
  ...
  for iMember = 1:length(memberList)
      ...
      assert_equals( memberValue, transitFitStruct1.(memberName), ...
          ['Member "',memberName,'" not equivalent to value obtained with "*" get'] ) ;
  end
  
% test 4:  try to get with an invalid member name

  try_to_catch_error_condition( 'memberValue = get( transitFitObject1 ,''phony'' ) ; ', ...
      'badFieldName', 'caller' ) ;
  disp(' ') ;
