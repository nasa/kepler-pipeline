function self = test_generate_transit_fit_plots( self ) 
%
% test_generate_transit_fit_plots -- test the transitFitClass method which generates
% diagnostic and reporting plots
%
% This is a unit test of the generate_transit_fit_plots method of the transitFitClass.  It
% tests the following functionality:
%
% ==> That plots are generated with correct names and paths for all valid values of the
%     oddEvenFlag
%
% This test is intended to be executed in the mlunit context.  For standalone execution
% use the following syntax:
%
%      run(text_test_runner, testTransitFitClass('test_generate_transit_fit_plots'));
%
...
%=========================================================================================
  ...
  disp('... testing transitFitClass master plot generator ... ')
  
  testTransitFitClass_initialization ;
  
  load(fullfile(testDataDir,'target-flux-time-series')) ;
...
% create the necessary subfolders
  ...
  planetSearchDirName = ['planet-',num2str(iPlanet, '%02d'), filesep, ...
...
% create the figures for the all-transits fit
  ...
  generate_transit_fit_plots( transitFitObject1, targetFluxTimeSeries, testDir, ...
      keplerId, iPlanet, true ) ;
  
% check to see that the figures are all present

  figureNameBasis = sprintf('%09d-%02d-all-*.fig', keplerId, iPlanet);
  figuresDirResults = dir( fullfile( allTransitsDir, figureNameBasis ) ) ;
  mlunit_assert( length(figuresDirResults) == 8, ...
      'all-transits fit figures not all present in correct directory' ) ;
  
% create the figures for the odd-even transits fit -- this requires making a new transit
% fit object and performing a new fit
  ...
  generate_transit_fit_plots( transitFitObject2, targetFluxTimeSeries, testDir, ...
      keplerId, iPlanet, true ) ;
  
% check to see that the figures are all present

  figureNameBasis = sprintf('%09d-%02d-odd-even-*.fig', keplerId, iPlanet);
  figuresDirResults = dir( fullfile( oddTransitsDir, figureNameBasis ) ) ;
  mlunit_assert( length(figuresDirResults) == 8, ...
      'odd-transits fit figures not all present in correct directory' ) ;
...
% exercise the error statement
  ...
  errorString = ['generate_transit_fit_plots( transitFitObject3, targetFluxTimeSeries, ', ...
      'testDir, keplerId, iPlanet, true) ; '] ;
