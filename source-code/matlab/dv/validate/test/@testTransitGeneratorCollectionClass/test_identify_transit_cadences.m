...
%
% test_identify_transit_cadences -- unit test for identify_transit_cadences method of
...
% following syntax:
%
%   run(text_test_runner, testTransitGeneratorCollectionClass('test_identify_transit_cadences'));
%
...
% oddEvenFlag == 0 case
  ...
  transitCadences = identify_transit_cadences( transitObject, ...
      cadenceTimes, 1.0 ) ;
  transitCadences2 = identify_transit_cadences( get( transitObject, ...
      'transitGeneratorObjectVector' ), cadenceTimes, 1.0 ) ;
  assert_equals( transitCadences, transitCadences2, ...
      'Cadences for oddEvenFlag == 0 case not correct' ) ;
...
% oddEvenFlag == 1 case -- put an offset of ~1 cadence between the two objects
  ...
  transitCadences2 = identify_transit_cadences( ...
      transitGeneratorObjectVector(1), cadenceTimes, 1.0 ) ;
  transitCadences2 = find( mod(transitCadences2,2) == 1 ) ;
  transitCadences3 = identify_transit_cadences( ...
      transitGeneratorObjectVector(2), cadenceTimes, 1.0 ) ;
  transitCadences3 = find( mod(transitCadences3,2) == 0 & transitCadences3 > 0 ) ;
  transitCadences2 = sort( [transitCadences2 ; transitCadences3] ) ;
  
  assert_equals( transitCadences, transitCadences2, ...
      'Cadences for oddEvenFlag == 1 case not correct' ) ;
...
% oddEvenFlag == 2 case -- put an offset of ~1 cadence in the 3rd object
  ...
  for iObject = 1:length(transitGeneratorObjectVector)
      ...
      transitCadences3 = identify_transit_cadences( ...
          transitGeneratorObjectVector(iObject), cadenceTimes, 1.0 ) ;
      transitCadences2 = [transitCadences2 ; find(transitCadences3 == iObject)] ;
      
  end
  
  assert_equals( transitCadences, transitCadences2, ...
      'Cadences for oddEvenFlag == 2 case not correct' ) ;
