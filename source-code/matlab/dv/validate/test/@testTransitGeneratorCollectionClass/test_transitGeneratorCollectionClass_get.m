function self = test_transitGeneratorCollectionClass_get( self )
%
% test_transitGeneratorCollectionClass_get -- unit test for the get method of the
% transitGeneratorCollectionClass
%
% test_transitGeneratorCollectionClass_get tests the following features of the get method
...
% following syntax:
%
%   run(text_test_runner, testTransitGeneratorCollectionClass('test_transitGeneratorCollectionClass_get'));
%
...
% identify the member names which should produce something other than the value which is
% in the embedded transitGeneratorClass single object

  planetModel = get( transitObject0, 'planetModel' ) ;
  planetModelFields = fieldnames( planetModel ) ;
  fieldsWithVectorOutput = [planetModelFields ; 'planetModel' ; 'planetRadiusMeters' ; ...
...
% test 1:  'help' and '?'
  ...
  assert_equals( helpReturn, [transitGeneratorClassMembers ; 'transitGeneratorObjectVector' ; ...
      'oddEvenFlag'], '''?'' argument produces incorrect output' ) ;
  helpReturn = get( transitObject, 'help' ) ;
  assert_equals( helpReturn, [transitGeneratorClassMembers ; 'transitGeneratorObjectVector' ; ...
      'oddEvenFlag'], '''help'' argument produces incorrect output' ) ;
  helpReturn = get( transitObject, 'HELP' ) ;
  assert_equals( helpReturn, [transitGeneratorClassMembers ; 'transitGeneratorObjectVector' ; ...
      'oddEvenFlag'], '''HELP'' argument produces incorrect output' ) ;
...
% test 3:  transitGeneratorObjectVector
  ...
  assert_equals( size( tgovReturn ), [6 1], ...
      'Size of transitGeneratorObjectVector wrong in get' ) ;
  for iObject = 1:length( tgovReturn )
      assert_equals( struct( tgovReturn(iObject) ), struct( transitObject0 ), ...
          'transitGeneratorObjectVector values wrong in get' ) ;
  end
...
% test 4: "non-vector" returns
  ...
  for iMember = 1:length( fieldsWithoutVectorOutput )
      ...
      assert_equals( retval, get( transitObject0, fieldsWithoutVectorOutput{iMember} ), ...
          ['Member ''', fieldsWithoutVectorOutput{iMember}, ''' value wrong in get'] ) ;
  end
...
% test 5:  vector returns
  ...
  for iMember = 1:length( fieldsWithVectorOutput )
      ...
      assert_equals( retval, repmat(retval0, 6, 1), ...
          ['Member ''', fieldsWithVectorOutput{iMember}, ''' value wrong in get'] ) ;
  end

% test 6:  error on '*'

  try_to_catch_error_condition( ...
      'retval=get(transitObject,''*'');', 'wildCardNotValid', 'caller' ) ;
