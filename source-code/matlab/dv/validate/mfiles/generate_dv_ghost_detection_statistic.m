function [targetResults, alertsOnly] = generate_dv_ghost_detection_statistic(whitenerResultsStruct,...
                                                                                iTarget,...
                                                                                targetResults,...
                                                                                alertsOnly)
% function [targetResults, alertsOnly] = generate_dv_ghost_detection_statistic(whitenerResultsStruct,...
...
%                                                                                alertsOnly)
% 
% Adapted from generate_dv_pixel_detection_statistic.
% In this adaptation, the .ra and .dec fields of whitenerResultsStruct are 'repurposed' to contain the 
% whitened results for core aperture flux, and halo aperture flux, respectively.
% We develop a statistic and its significance for the correlation of each flux timeseries with the transit model 
% 
...
%==========================================================================
% This block of comments is from generate_dv_pixel_detection_statistic: 
...
%                   rmsResidual: 1.2013
%
% whitenerResultsStruct.dec has a structure that is similar to whitenerResultsStruct.ra

% Get the whitened residual and original gap indicators used in the whitener robust
% fit, for core and halo apertures. Assumes whitenedResidual is not empty.

for iAperture = 1:2
    switch iAperture
        case 1
            resultLabel = 'coreApertureCorrelationStatistic';
            % Get whitened core aperture information from ra field
            pixelResidual = whitenerResultsStruct.ra.whitenedResidual;
            pixelGaps = whitenerResultsStruct.ra.whitenedGaps;
            robustWeights = whitenerResultsStruct.ra.robustWeights;
            whitenedDesignMatrix = whitenerResultsStruct.ra.whitenedDesignMatrix;
            validDesignColumn = whitenerResultsStruct.ra.validDesignColumn;
            coefficients = whitenerResultsStruct.ra.coefficients;
        case 2
            resultLabel = 'haloApertureCorrelationStatistic';
            % Get whitened halo aperture information from dec field
            pixelResidual = whitenerResultsStruct.dec.whitenedResidual;
            pixelGaps = whitenerResultsStruct.dec.whitenedGaps;
            robustWeights = whitenerResultsStruct.dec.robustWeights;
            whitenedDesignMatrix = whitenerResultsStruct.dec.whitenedDesignMatrix;
            validDesignColumn = whitenerResultsStruct.dec.validDesignColumn;
            coefficients = whitenerResultsStruct.dec.coefficients;
    end
    
    % Eliminate gaps from the robustWeights
    ...
    while( iPlanet < nPlanets )
        ...
        % Increment fitted planet index
        iPlanet = iPlanet + 1;
        
        % Compute and save the ghost detection statistic value and
        % significance if there is a valid DV transit model, or else
        % set an alert and return a warning message saying that default
        ...
        if( validDesignColumn(iPlanet) )
            ...
            % Check that both the statistic (T) and its significance are finite.
            % If not, issue a warning that default values are being returned for
            % this candidate.
            if( isfinite( significance ) && isfinite( T ) )
                ...
                targetResults.planetResultsStruct(iPlanet).ghostDiagnosticResults.(resultLabel).value = T;
                targetResults.planetResultsStruct(iPlanet).ghostDiagnosticResults.(resultLabel).significance = significance;
                
            else
                
                % Throw alert with warning that test results are not valid
                messageString = [resultLabel,' results are not valid -- ghost diagnostic test statistic and significance set to default values for planet ',num2str(iPlanet,'%d')];
                disp(messageString);
                alertsOnly = add_dv_alert(alertsOnly, 'ghost diagnostic test', 'warning',...
                    messageString, iTarget, targetResults.keplerId, iPlanet);
                
            end
            
            
        else % Throw alert with warning that the test wasn't done
            
            messageString = [resultLabel, ': no valid DV transit model -- ghost diagnostic test statistic and significance set to default values for planet ',num2str(iPlanet,'%d')];
            disp(messageString);
            alertsOnly = add_dv_alert(alertsOnly, 'ghost diagnostic test', 'warning',...
                messageString, iTarget, targetResults.keplerId, iPlanet);
            
        end % if(validDesignColumn(iPlanet))
        
    end % while loop over planets
    
end % loop over core and halo apertures
