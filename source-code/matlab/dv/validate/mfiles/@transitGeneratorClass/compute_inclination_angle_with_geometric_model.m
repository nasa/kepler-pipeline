function [inclinationAngle] = compute_inclination_angle_with_geometric_model( transitModelObject, unitString )
%
      ...
      switch lower(unitString)
          ...
          otherwise
              error('dv:transitGeneratorClass:computeInclinationAngle:unitStringNotRecognized', ...
                  ['compute_inclination_angle_with_geometric_model:  unrecognized unit string ''', ...
                  unitString, ''' detected']) ;
...
% get the parameters out of the planetModel
  ...
  ratioSemiMajorAxisToStarRadius  = transitModelObject.planetModel.ratioSemiMajorAxisToStarRadius;
...
% which has the correct unit conversion
  ...
  beta0 = [ratioSemiMajorAxisToStarRadius; minImpactParameter];
  inclination_angle = @(beta) unitConversion * acos( beta(2)/ beta(1) );
