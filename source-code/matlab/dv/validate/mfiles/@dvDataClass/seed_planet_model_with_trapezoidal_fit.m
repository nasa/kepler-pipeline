function [planetModel] = seed_planet_model_with_trapezoidal_fit( dvDataObject, dvResultsStruct, iTarget, iPlanet, planetModel, reducedParameterFitsEnabled, impactParameterSeed )
% function [planetModel] = seed_planet_model_with_trapezoidal_fit( dvDataObject, dvResultsStruct, iTarget, iPlanet, planetModel, reducedParameterFitsEnabled, impactParameterSeed )
%
% This function seeds the planet model with the trapezoidal fit parameters.
%
% Version date:  2014-October-23.
%
% 
...
%   2014-October-23, JL:
%       Add inputs reducedParameterFitsEnabled and impactParameterSeed
%   2014-August-12, JL:
%       Initial release.
%
%=========================================================================================
...
% Seed the planet model with the trapezoidal fit parameters

modelParameters     = dvResultsStruct.targetResultsStruct(iTarget).planetResultsStruct(iPlanet).trapezoidalFit.modelParameters;
...
if reducedParameterFitsEnabled
    ...
    disp(' ');
    disp(['  Seed planet model with trapezoidal fit parameters with fixed impact parameter ' num2str(planetModel.minImpactParameter)]);
    disp(' ');

else
    
    disp(' ');
    disp('  Seed planet model with trapezoidal fit parameters');
    disp(' ');
