function [pixelCorrelationMotionResults, diagnostics, alertString] = ...
perform_dv_correlation_image_centroid_fit_and_offset_analysis(dvDataObject, ...
pixelCorrelationResults, differenceImageResults, pixelCorrelationMotionResults, ...
allTransitsFit)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function [pixelCorrelationMotionResults, diagnostics, alertString] = ...
% perform_dv_correlation_image_centroid_fit_and_offset_analysis(dvDataObject, ...
...
% and correlation images respectively (if the model fit SNR is below a
% specified threshold) to estimate the out of transit and correlation image
% centroid positions in sky coordinates. The out of transit centroid marks
% the position of the target; the correlation image centroid marks the
% position of the transit source.
%
% Compute the offset between the multi-quarter correlation image and out of
...
% in the pixel correlation motion results structure. Also return a
...
%
% Likewise compute the offset between the multi-quarter correlation image
...
% pixel correlation motion results structure accordingly.
%
% The dvDataObject, pixelCorrelationResults, differenceImageResults,
% pixelCorrelationMotionResults and allTransitsFit structures and structure
% arrays are defined in the headers of the dv_matlab_controller and
% validate_dv_inputs functions. The module parameter
% 'singlePrfFitSnrThreshold' which determines whether or not the bootstrap
% multi-quarter PRF fit is performed may be found in the
...
% above the specified threshold, if none of the quarterly images is valid
% or if there are no PRF models.
...
mjdTimestamps = [pixelCorrelationResults.mjdTimestamp];

if modelChiSquare == GAP_VALUE
    alertString = 'Multi-quarter PRF fitting and offset analysis will not be performed because all-transits fit failed';
    return
...
elseif ~any(mjdTimestamps > 0)
    alertString = 'Multi-quarter PRF fitting and offset analysis will not be performed because there are no valid control or correlation images';
    return
...
correlationImageData = controlImageData;

isValidCorrelationImage = true([1, nTables]);

for iTable = 1 : nTables
    
    rows = ...
        [pixelCorrelationResults(iTable).pixelCorrelationStatisticStruct.ccdRow]';
    columns = ...
        [pixelCorrelationResults(iTable).pixelCorrelationStatisticStruct.ccdColumn]';
    
    values = [pixelCorrelationResults(iTable).pixelCorrelationStatisticStruct.value]';
    uncertainties = [pixelCorrelationResults(iTable).pixelCorrelationStatisticStruct.significance]';
    uncertainties(uncertainties ~= GAP_VALUE) = 1;
    ...
    correlationImageData(iTable).ccdRow = rows;
    correlationImageData(iTable).ccdColumn = columns;
    correlationImageData(iTable).values = values;
    correlationImageData(iTable).uncertainties = uncertainties;
    
    if isempty(values) || isempty(uncertainties)
        isValidCorrelationImage(iTable) = false;
    end % if

end % for iTable

isInvalid = mjdTimestamps == 0 | ...
    ~isValidControlImage | ~isValidCorrelationImage;
controlImageData(isInvalid) = [];
correlationImageData(isInvalid) = [];
pixelCorrelationResults(isInvalid) = [];
...
if ~isempty(pixelCorrelationResults) && ...
        pixelCorrelationResults(1).kicReferenceCentroid.raHours.uncertainty ~= GAP_VALUE && ...
        pixelCorrelationResults(1).kicReferenceCentroid.decDegrees.uncertainty ~= GAP_VALUE
    referenceRaHours = ...
        pixelCorrelationResults(1).kicReferenceCentroid.raHours.value;
    referenceRaDegrees = referenceRaHours * DEGREES_PER_HOUR;
    referenceDecDegrees = ...
        pixelCorrelationResults(1).kicReferenceCentroid.decDegrees.value;
else
    referenceRaDegrees = [];
    referenceDecDegrees = [];
end % if / else
...
% correlation images to estimate the RA/Dec coordinates of the associated
% centroids and the covariance for each.
[centroidStatus, controlCentroidRaDegrees, controlCentroidDecDegrees, ...
    controlCentroidCovariance, correlationCentroidRaDegrees, ...
    correlationCentroidDecDegrees, correlationCentroidCovariance, diagnostics] = ...
	bootstrap_multi_quarter_prf_fit(controlImageData, correlationImageData, ...
...
correlationCentroidRaHours = correlationCentroidRaDegrees * HOURS_PER_DEGREE;
correlationCentroidCovariance = T_RA_HOURS_DEC_DEGREES * ...
    correlationCentroidCovariance * T_RA_HOURS_DEC_DEGREES';

pixelCorrelationMotionResults.mqControlImageCentroid.raHours.value = ...
    controlCentroidRaHours;
pixelCorrelationMotionResults.mqControlImageCentroid.raHours.uncertainty = ...
    sqrt(controlCentroidCovariance(1, 1));
pixelCorrelationMotionResults.mqControlImageCentroid.decDegrees.value = ...
    controlCentroidDecDegrees;
pixelCorrelationMotionResults.mqControlImageCentroid.decDegrees.uncertainty = ...
    sqrt(controlCentroidCovariance(2, 2));

pixelCorrelationMotionResults.mqCorrelationImageCentroid.raHours.value = ...
    correlationCentroidRaHours;
pixelCorrelationMotionResults.mqCorrelationImageCentroid.raHours.uncertainty = ...
    sqrt(correlationCentroidCovariance(1, 1));
pixelCorrelationMotionResults.mqCorrelationImageCentroid.decDegrees.value = ...
    correlationCentroidDecDegrees;
pixelCorrelationMotionResults.mqCorrelationImageCentroid.decDegrees.uncertainty = ...
    sqrt(correlationCentroidCovariance(2, 2));

% Estimate the pixel correlation image centroid offsets and associated
...
[pixelCorrelationMotionResults] = ...
    ...
    pixelCorrelationMotionResults, mqOffsetConstantUncertainty, ...
    singlePrfFitForCentroidPositionsEnabled);
