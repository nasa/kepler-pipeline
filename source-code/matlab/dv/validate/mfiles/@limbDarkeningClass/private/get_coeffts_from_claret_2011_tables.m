...
% pre-defined Claret 2011 tabular data
%
%
% INPUTS:
%
...
%
%    2011-Nov-7, EQ:
%        initial release to support 2011 Claret limb darkening model



% extract the (Teff, logg, Z) and nonlinear coefficient arrays, where
%   stellarTable = [teff logg z];
%   nonlinearLdCoefftsTable = [c1 c2 c3 c4];

[stellarTable, nonlinearLdCoefftsTable] = get_claret_model_2011();

effectiveTempVector       = stellarTable(:, 1);
log10SurfaceGravityVector = stellarTable(:, 2);
metallicityVector         = stellarTable(:, 3);

c1Vector = nonlinearLdCoefftsTable(:, 1);
c2Vector = nonlinearLdCoefftsTable(:, 2);
c3Vector = nonlinearLdCoefftsTable(:, 3);
c4Vector = nonlinearLdCoefftsTable(:, 4);


% ensure that the (Teff, logg, Z) are within the valid ranges
