...
% get a valid bpa input structure
bpaParameterStruct = setup_valid_bpa_test_struct();

% first we test the high level field names
test_missing_fields(bpaParameterStruct, []);

function test_missing_fields(bpaParameterStruct, subStruct);
% get a list of its fields
if isempty(subStruct)
    names = fieldnames(bpaParameterStruct);
else
    names = fieldnames(bpaParameterStruct.(subStruct));
end
nNames = length(names);
for j=1:nNames
    ...
    if isempty(subStruct)
        bpaData = rmfield(bpaData, names(j));
    else
        bpaData.(subStruct) = rmfield(bpaData.(subStruct), names(j));
    end
    ...
    try
        ...
        caughtErrorFlag = 1;
%         print_error(err, 'bpa missing field test');    
    end;
    if(~caughtErrorFlag)
