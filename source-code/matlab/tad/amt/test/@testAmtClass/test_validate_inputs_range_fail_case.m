...
% set up input structure for the amtConfigurationStruct
testAmtConfigFields = cell(9,5);
% each cell row contains the following:
% {fieldName;   Nan;    Inf;    out of left bound;  out of right bound}
testAmtConfigFields(1,:) = {'maxMasks'; aNaN; anInf; -1; 1e6 + 3};
testAmtConfigFields(2,:) = {'maxPixelsInMask'; aNaN; anInf; -1; 1e6 + 3};
testAmtConfigFields(3,:) = {'maxMaskRows'; aNaN; anInf; -1; 2313};
testAmtConfigFields(4,:) = {'maxMaskCols'; aNaN; anInf; -1; 2313};
testAmtConfigFields(5,:) = {'centerRow'; aNaN; anInf; -1; 2313};
testAmtConfigFields(6,:) = {'centerCol'; aNaN; anInf; -1; 2313};
testAmtConfigFields(7,:) = {'minEccentricity'; aNaN; anInf; -1; 2};
testAmtConfigFields(8,:) = {'maxEccentricity'; aNaN; anInf; -1; 2};
testAmtConfigFields(9,:) = {'stepEccentricity'; aNaN; anInf; -1; 2};
testAmtConfigFields(9,:) = {'stepInclination'; aNaN; anInf; -1; 12};

test_range(testAmtConfigFields, amtParameterStruct, 'amtConfigurationStruct', []);

function test_range(testFields, amtParameterStruct, subStructName, index)
% if subStructName = [] then we are testing the fields of
% amtParameterStruct.
% if subStructName is not empty then we are testing the fields of
% amtParameterStruct.subStructName.
% if index is not empty we are testing the fields in 
% amtParameterStruct.subStructName[index].
    ...
    for j=1:nFields
        ...
        if(~isempty(testFields{j,k+1})) % for some parameters, need to check only one bound (either <= or >= some value)
            ...
            if isempty(subStructName) % testing the top level structure
                amtData.(testFields{j,1}) = testFields{j,k+1};
            else
                if isempty(index) % if there is no index
                    amtData.(subStructName).(testFields{j,1}) = testFields{j,k+1};
                else % there is an index on the sub-structure
                    amtData.(subStructName)(index).(testFields{j,1}) = testFields{j,k+1};
                end
            end;
