    ...
    try
        ...
        switch (iTest)
            case 1                              % startMjd is a vector
                ...
                endMjd   = 54595.0;
                retrievedReceivedFiles = retrieve_all_received_files(startMjd, endMjd);
...
% Test allReceivedFileClass constructor for illegal inputs
...
for iField = 1:length(fieldString)
    fieldsAndBounds( 2,:) = { 'dispatcherType';         [];         [];      { dispatcherType{iField} } };

    if ( ~isempty(allReceivedFile.(fieldString{iField}).mjdSocIngestTime) )
        assign_illegal_value_and_test_for_failure(allReceivedFile.(fieldString{iField}), ['allReceivedFile.' fieldString{iField}], ...
