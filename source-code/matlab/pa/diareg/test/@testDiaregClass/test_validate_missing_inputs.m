...
% get a valid diareg input structure
diaregParameterStruct = setup_valid_diareg_test_struct();

% first we test the high level field names
test_missing_fields(diaregParameterStruct, []);
% then check the sub-structures
test_missing_fields(diaregParameterStruct, 'backgroundConfigurationStruct', [], [], []);
test_missing_fields(diaregParameterStruct, 'cosmicRayConfigurationStruct', [], [], []);
test_missing_fields(diaregParameterStruct, 'targetStarStruct', 2, [], []);
test_missing_fields(diaregParameterStruct, 'targetStarStruct', 2, 'pixelTimeSeriesStruct', 2);
test_missing_fields(diaregParameterStruct, 'backgroundCoeffStruct', 2, [], []);

function test_missing_fields(diaregParameterStruct, subStruct, index, subStruct2, index2)
% get a list of its fields
if isempty(subStruct)
    names = fieldnames(diaregParameterStruct);
elseif isempty(subStruct2)
    if isempty(index)
        names = fieldnames(diaregParameterStruct.(subStruct));
    else
        names = fieldnames(diaregParameterStruct.(subStruct)(index));
    end
else
    if isempty(index2)
        names = fieldnames(diaregParameterStruct.(subStruct)(index).(subStruct2));
    else
        names = fieldnames(diaregParameterStruct.(subStruct)(index).(subStruct2)(index2));
    end
end
nNames = length(names);
for j=1:nNames
    ...
    if isempty(subStruct)
        diaregData = rmfield(diaregData, names(j));
    elseif isempty(subStruct2)
        diaregData.(subStruct) = rmfield(diaregData.(subStruct), names(j));
    else
        diaregData.(subStruct)(index).(subStruct2) = ...
            rmfield(diaregData.(subStruct)(index).(subStruct2), names(j));
    end
...
%                 display(['looking for ' char(names(j)) ' in ' err.identifier]);
        ...
        caughtErrorFlag = 1;
%         print_error(err, 'diareg missing field test');    
    end;
    if(~caughtErrorFlag)
