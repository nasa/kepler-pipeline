...
% get a valid bpp input structure
bppParameterStruct = setup_valid_bpp_test_struct();

% first we test the high level field names
test_missing_fields(bppParameterStruct, []);
% then check the sub-structures
test_missing_fields(bppParameterStruct, 'backgroundConfigurationStruct');
test_missing_fields(bppParameterStruct, 'cosmicRayConfigurationStruct');
test_missing_fields(bppParameterStruct, 'backgroundStruct');

function test_missing_fields(bppParameterStruct, subStruct);
% get a list of its fields
if isempty(subStruct)
    names = fieldnames(bppParameterStruct);
else
    names = fieldnames(bppParameterStruct.(subStruct));
end
nNames = length(names);
for j=1:nNames
    ...
    if isempty(subStruct)
        bppData = rmfield(bppData, names(j));
    else
        bppData.(subStruct) = rmfield(bppData.(subStruct), names(j));
    end
...
%                 display(['looking for ' char(names(j)) ' in ' err.identifier]);
        ...
        caughtErrorFlag = 1;
%         print_error(err, 'bpp missing field test');    
    end;
    if(~caughtErrorFlag)
