...
if navg <= 0, error('movstd:  # points to consider less than or equal 0!'), end

if fix(navg/2)==navg/2, error('movstd: navg must be odd!'), end

favg = movavg(f,navg); % moving average of f with a window size of navg points

f2avg = movavg(f.^2,navg); % moving average of f^2 with a window size of navg points
fstd = sqrt(f2avg-favg.^2); % calculation of moving standard deviation from mean and mean square values
