...
% set up input structure for the tpp level
testTppFields = cell(1,5);
% each cell row contains the following:
% {fieldName;   Nan;    Inf;    out of left bound;  out of right bound}
testTppFields(1,:) = {'debugFlag'; aNaN; anInf; []; []};

testTppConfigFields = cell(6,5);
% each cell row contains the following:
% {fieldName;   Nan;    Inf;    out of left bound;  out of right bound}
testTppConfigFields(1,:) = {'startCadence'; aNaN; anInf; -1; 2e9};
testTppConfigFields(2,:) = {'endCadence'; aNaN; anInf; -1; 2e9};
testTppConfigFields(3,:) = {'cadenceType'; aNaN; anInf; -1; 2};
testTppConfigFields(4,:) = {'ccdModule'; aNaN; anInf; -1; 25};
testTppConfigFields(5,:) = {'ccdOutput'; aNaN; anInf; -1; 5};
testTppConfigFields(6,:) = {'cleanCosmicRays'; aNaN; anInf; -1; 2};
...
% set up input structure for the ancillaryDataStruct
testAncValueFields = cell(1,5);
% each cell row contains the following:
% {fieldName;   Nan;    Inf;    out of left bound;  out of right bound}
testAncValueFields(1,:) = {'values'; aNaN; anInf; -2e15; 2e15};

% set up input structure for the ancillaryDataStruct
testAncUncertaintiesFields = cell(1,5);
% each cell row contains the following:
% {fieldName;   Nan;    Inf;    out of left bound;  out of right bound}
testAncUncertaintiesFields(1,:) = {'uncertainties'; aNaN; anInf; -2e15; 2e15};

% set up input structure for the ancillaryDataStruct
testAncTimeStampFields = cell(1,5);
% each cell row contains the following:
% {fieldName;   Nan;    Inf;    out of left bound;  out of right bound}
testAncTimeStampFields(1,:) = {'timestamps'; aNaN; anInf; -2; 2e9};

% set up input structure for the ancillaryDataStruct
testAncGapFields = cell(1,5);
% each cell row contains the following:
% {fieldName;   Nan;    Inf;    out of left bound;  out of right bound}
testAncGapFields(1,:) = {'dataGapIndicators'; aNaN; anInf; -1; 2};


test_range(testTppFields, tppParameterStruct, [], [], [], [], []);
test_range(testTppConfigFields, tppParameterStruct, 'tppConfigurationStruct', [], [], [], []);
test_range(testBgConfigFields, tppParameterStruct, 'backgroundConfigurationStruct', [], [], [], []);
test_range(testCrConfigFields, tppParameterStruct, 'cosmicRayConfigurationStruct', [], [], [], []);
test_range(testTargetPixStructFields, tppParameterStruct, 'targetStarStruct', 2, ...
    'pixelTimeSeriesStruct', 2, []);
% look at target 2, pixel 2, series entry 4
test_range(testTimeSeriesFields, tppParameterStruct, 'targetStarStruct', 2, ...
    'pixelTimeSeriesStruct', 2, 4);
test_range(testUncertaintiesFields, tppParameterStruct, 'targetStarStruct', 2, ...
    'pixelTimeSeriesStruct', 2, 4);
% look at target 2, pixel 2, series gap 2
test_range(testGapListFields, tppParameterStruct, 'targetStarStruct', 2, ...
    'pixelTimeSeriesStruct', 2, 2);
test_range(testAncValueFields, tppParameterStruct, 'ancillaryDataStruct', 2, [], [], []);
% look at target 2, pixel 2, series entry 4
test_range(testAncUncertaintiesFields, tppParameterStruct, 'ancillaryDataStruct', 2, [], [], []);
test_range(testAncTimeStampFields, tppParameterStruct, 'ancillaryDataStruct', 2, [], [], []);
% look at target 2, pixel 2, series gap 2
test_range(testAncGapFields, tppParameterStruct, 'ancillaryDataStruct', 2, [], [], []);



function test_range(testFields, tppParameterStruct, subStructName, index, ...
...
% tppParameterStruct.subStructName.
% if index is not empty we are testing the fields in 
% tppParameterStruct.subStructName[index].
    ...
    for j=1:nFields
        ...
        if(~isempty(testFields{j,k+1})) % for some parameters, need to check only one bound (either <= or >= some value)
            ...
            if isempty(subStructName) % testing the top level structure
                tppData.(testFields{j,1}) = testFields{j,k+1};
            elseif isempty(subStruct2Name) % there is no subsubstructure 
                if isempty(index) % if there is no index
                    tppData.(subStructName).(testFields{j,1}) = testFields{j,k+1};
                else % there is an index on the sub-structure
                    tppData.(subStructName)(index).(testFields{j,1}) = testFields{j,k+1};
                end
            else
                if isempty(index3) % if there is end index
                    tppData.(subStructName)(index).(subStruct2Name)(index2).(testFields{j,1}) = testFields{j,k+1};
                else % there is an index on the sub-structure
                    tppData.(subStructName)(index).(subStruct2Name)(index2).(testFields{j,1})(index3) = testFields{j,k+1};
                end
            end;
