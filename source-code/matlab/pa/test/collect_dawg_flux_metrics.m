function F = collect_dawg_flux_metrics( pathName, channelList, quarter, month )
...
for iChannel = 1:length(channelList)
    ...
    if( ~isempty(channelDir) )
        ...
        counter = counter + 1;
        filename = fullfile(channelPath, PA_DAWG_FLUX);
        
        if( exist( filename, 'file') )
            s = load(filename);

            F{counter} = s.fluxOutputStruct;
            F{counter}.taskFileDirectory = [channelPath,'/'];
        else
            F{counter} = [];
