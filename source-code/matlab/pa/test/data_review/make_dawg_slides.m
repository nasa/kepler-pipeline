...
dataLocation = ['/path/to/pa/q0-q17-r9.2-dawg/q' num2str(quarter) '/'];
disp(dataLocation);
...
fid = fopen('pa_latex_presentation/report-definitions.sty', 'w');
...
fid = fopen('pa_latex_presentation/lcDynamicFluxSlides.tex', 'w');
...
if ~isempty(negativeStars)
    ...
    for i=1:length(negativeStars)
        kicIdx = find(newKic.kepid == negativeStars(i));
        [m o r c] = ra_dec_2_pix(raDec2PixObject, 15*newKic.ra(kicIdx), ...
            newKic.dec(kicIdx), mjd);
...
fid = fopen('pa_latex_presentation/lcDynamicCentroidSlides.tex', 'w');
...
fid = fopen('pa_latex_presentation/scDynamicFluxSlides.tex', 'w');
