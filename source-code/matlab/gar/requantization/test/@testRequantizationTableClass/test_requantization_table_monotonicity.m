function self = test_requantization_table_monotonicity(self)
...
%         Example: run(text_test_runner, testRequantizationTableClass('test_requantization_table_monotonicity'));
% 
...
% this collects only those entries that <= quantizationFraction
...
testResult = any(differenceTable <= 0); % no entry should be negative

expectedResult = 0;

% you can only check whether the indices are equal
messageOut = 'Entries in the requantization table are not in strict monotonic increasing order';
