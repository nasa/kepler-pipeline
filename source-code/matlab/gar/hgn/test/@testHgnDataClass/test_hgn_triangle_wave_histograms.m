function [self] = test_hgn_triangle_wave_histograms(self)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% [self] = test_hgn_triangle_wave_histograms(self)
%
% This test generates a triangle wave with a period of 16 cadences, and
...
% A bias is added to the triangle wave which should have no effect on the
% resulting histograms (of the differences between pixel values and
% baseline values).
%
%  Example
%  =======
%  Use a test runner to run the test method:
%      run(text_test_runner, testHgnDataClass('test_hgn_triangle_wave_histograms'));
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% 
...
for i = 1 : nCadences
    ...
    hgnDataStruct.cadencePixels(i).pixelValues = triangle(i);
...
for i = 1 : 8
    huffmanHistograms(zeroOffsetResults - i, 1) = 1 * nPeriods;
    huffmanHistograms(zeroOffsetResults + i, 1) = 1 * nPeriods;
end

huffmanHistograms(zeroOffsetResults    , 2) = 1 * nPeriods;
for i = 1 : 7
    huffmanHistograms(zeroOffsetResults + i, 2) = 2 * nPeriods;
end
huffmanHistograms(zeroOffsetResults + 8, 2) = 1 * nPeriods;

huffmanHistograms(zeroOffsetResults    , 3) = (2 * nPeriods) / 3;
for i = 1 : 7
    huffmanHistograms(zeroOffsetResults - i, 3) = 1 * nPeriods;
    huffmanHistograms(zeroOffsetResults + i, 3) = 1 * nPeriods;
end
huffmanHistograms(zeroOffsetResults - 8, 3) = (2 * nPeriods) / 3;
huffmanHistograms(zeroOffsetResults + 8, 3) = (2 * nPeriods) / 3;

% Compare results for each baseline interval with expected histograms.
for i = 1 : nIntervals
    messageOut = sprintf( ...
        'Triangle test failed (interval = %d) - generated and expected histograms are not identical!', ...
