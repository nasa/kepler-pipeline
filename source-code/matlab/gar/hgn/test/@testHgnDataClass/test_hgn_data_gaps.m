function [self] = test_hgn_data_gaps(self)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% [self] = test_hgn_data_gaps(self)
%
...
% 8, 16 and 24 cadences when gaps are inserted in the pixel values. One of
% the gaps in inserted such that a baseline value is lost for the shortest
% baseline interval of interest. The results for each baseline should be
% different.
%
%  Example
%  =======
%  Use a test runner to run the test method:
%      run(text_test_runner, testHgnDataClass('test_hgn_data_gaps'));
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% 
...
% Set up a square wave with the desired pixel values. An additional
% sample is needed at the start to set the initial baseline for each
% of the baseline intervals.
s = ones([1, period / 2]);
s = [s -s];
s = repmat(s, [1, nPeriods]);
squareWave = squareWaveMagnitude*[-1 s] + zeroOffsetData;

% Set up the gap indicators with missing pixel values.
gaps = false([1, nCadences]);
gaps(1 + baselineIntervals(1)) = true;
gaps(2 + baselineIntervals(3)) = true;
gaps(3 + baselineIntervals(3)) = true;
...
for i = 1 : nCadences
    ...
    hgnDataStruct.cadencePixels(i).gapIndicators = gaps(i);
end
...
huffmanHistograms(zeroOffsetResults - 2*squareWaveMagnitude, 1) = ...
    ((nCadences - 1) / 2) - 10;   % includes loss of full baseline interval
huffmanHistograms(zeroOffsetResults + 2*squareWaveMagnitude, 1) = ...
    ((nCadences - 1) / 2) - 1;

huffmanHistograms(zeroOffsetResults                        , 2) = ...
    ((nCadences - 1) / 2) - 2;
huffmanHistograms(zeroOffsetResults + 2*squareWaveMagnitude, 2) = ...
    ((nCadences - 1) / 2) - 1;

huffmanHistograms(zeroOffsetResults - 2*squareWaveMagnitude, 3) = ...
    ((nCadences - 1) / 3) - 2;
huffmanHistograms(zeroOffsetResults                        , 3) = ...
    (nCadences - 1) / 3;
huffmanHistograms(zeroOffsetResults + 2*squareWaveMagnitude, 3) = ...
    ((nCadences - 1) / 3) - 1;
...
for i = 1 : nIntervals
    messageOut = sprintf( ...
        'Gaps test failed (interval = %d) - generated and expected histograms are not identical!', ...
