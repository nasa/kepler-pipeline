...
%      run(text_test_runner, testHacDataClass('test_validate_inputs_range_fail_case'));
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% 
...
% Assign invalid fields and check for failures in hacDataStruct.
%
% If fields are structures, they are not included here (they are validated
% within the hacDataClass, and also tested for missing fields in
% test_validate_missing_inputs).
%--------------------------------------------------------------------------
fieldsAndBounds = cell(6,4);
%fieldsAndBounds(1,:)  = { 'fcConstants'; []; []; []};
fieldsAndBounds(1,:)  = { 'invocationCcdModule'; '>= 2'; '<= 24'; []};
fieldsAndBounds(2,:)  = { 'invocationCcdOutput'; '>= 1'; '<= 4'; []};
fieldsAndBounds(3,:)  = { 'cadenceStart'; '> - 2^20'; '< 2^20'; []};  % for now
fieldsAndBounds(4,:)  = { 'cadenceEnd'; '> -2^20'; '< 2^20'; []};     % for now
fieldsAndBounds(5,:)  = { 'firstMatlabInvocation'; '>= 0'; '<= 1'; []};
%fieldsAndBounds(6,:)  = { 'histograms'; []; []; []};
fieldsAndBounds(6,:)  = { 'debugFlag'; '>= 0'; '<= 3'; []};  % 3 levels max

assign_illegal_value_and_test_for_failure(hacDataStruct, 'hacDataStruct', ...
    hacDataStruct, 'hacDataStruct', 'hacDataClass', fieldsAndBounds, ...
...
% Assign invalid fields and check for failures in hacDataStruct.histograms.
%--------------------------------------------------------------------------
...
fprintf('\n');
fprintf('Checking for illegal data range in structure in %d of hacDataStruct.histograms....(display suppressed)\n', ...
    nStructures);
...
for j = 1 : nStructures
    ...
    lowLevelStructName =  ['hacDataStruct.histograms(' num2str(j) ')'];

    assign_illegal_value_and_test_for_failure(hacDataStruct.histograms(j), ...
        lowLevelStructName, hacDataStruct, 'hacDataStruct', 'hacDataClass', ...
