function [prfArray, prfRow, prfColumn] = make_array_pixel(prfObject, pixelsToDraw, resolution, reverse, offset)
%
...
for r = 1:resolution
    for c = 1:resolution
        ...
        if ismember(pixelIndex, pixelsToDraw)
...
prfArray(prfArray == -1) = [];
prfRow(prfRow == -1) = [];
prfColumn(prfColumn == -1) = [];

prfArray = reshape(prfArray, sqrt(length(prfArray)), sqrt(length(prfArray)));
prfRow = reshape(prfRow, sqrt(length(prfRow)), sqrt(length(prfRow)));
prfColumn = reshape(prfColumn, sqrt(length(prfColumn)), sqrt(length(prfColumn)));
