function fixedPrfPolyStruct = draw_prf_pixel(prfPolyStruct, pixNum)
%
...
fixedPrfPolyStruct = prfPolyStruct;

pStruct = prfPolyStruct(pixNum);
...
for r = 1:rSize
    for c = 1:cSize
        ...
        if prfPolyStruct(pixel, subRow, subCol).c.order > 5
            fixedPrfPolyStruct(pixel, subRow, subCol).c = weighted_polyfit2d(R(:), C(:), V, 1, 2);
        end
        V = reshape(V, 10, 10);
        mesh(R, C, V, 'EdgeColor', 'b');
        
        newV = weighted_polyval2d(R(:), C(:), fixedPrfPolyStruct(pixel, subRow, subCol).c);
        newV = reshape(newV, 10, 10);
        mesh(R, C, newV, 'EdgeColor', 'r');
        title(['polynomial fit and fitted data for pixel' num2str(pixNum)]);
