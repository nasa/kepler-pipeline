%% test_pdq_generate_reports
%
% function [self] = test_pdq_generate_reports(self)
%
% This test validates PDQ report generation.
%
%  Example
%  =======
%  Use a test runner to run the test method:
%      run(text_test_runner, testPdqClass('test_pdq_generate_reports'));
%%
% 
...
% later.
try
    fprintf('\nGenerating report based upon %s\n', pdqDataFilename);
    
    % Load previously generated data structure.
    load(fullfile(testDataDir, pdqDataFilename));
    
    % Generate reports.
    reportFilename = pdq_generate_reports(pdqInputStruct, channelsProcessed, figures);
    
    % Check that returned reportFilename variable has been set.
    if (isempty(reportFilename))
        fprintf('Expected a non-null report filename\n');
        failed = true;
    end
