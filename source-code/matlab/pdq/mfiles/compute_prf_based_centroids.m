function pdqTempStruct = compute_prf_based_centroids(pdqTempStruct, fcConstantsStruct)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% function pdqTempStruct = compute_prf_based_centroids(pdqTempStruct)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
%
% 12/11/13: If a preliminary attitude solution was provided in the PDQ
% inputs, it is used to generate initial guesses at the centroid row and
% column positions.
% 
% Adds the field 'centroidUncertaintyStruct' to pdqTempStruct and modifies
% the following fields: 
% 
% centroidRows
% centroidCols
% centroidRowUncertainties 
% centroidColumnUncertainties
% 
% eePixelExclusionIndicators
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% 
...
for cadenceIndex = 1 : numCadences
    ...
    for targetIndex = 1 : numTargets
        ...
        % Provide a seed point for PRF centroiding. If a preliminary
        % attitude solution was provided as input, use it to derive the
        % seeds. Otherwise seed with the flux-weighted centroids.
        %
        % Note that this code will work with a multi-cadence input, but it
        % is assumed that a single preliminary attitude solution is
        % inserted by Quasar and that this solution can be used to seed all
        % cadences.
        if isfield(pdqTempStruct, 'preliminaryAttitudeSolutionStruct')
            raDec2PixObject = raDec2PixClass(pdqTempStruct.raDec2PixModel, 'one-based');
            attitude = pdqTempStruct.preliminaryAttitudeSolutionStruct;
                        
            [~, ~, seedRow, seedColumn] = ra_dec_2_pix_absolute(raDec2PixObject, ...
                    pdqTempStruct.raStars(targetIndex), ...
                    pdqTempStruct.decStars(targetIndex), ...
                    attitude.mjd,  ...
                    attitude.raDegrees, ...
                    attitude.decDegrees, ...
                    attitude.rollDegrees);
                
            % Clip the seed point such that it lies within the pixel mask.
            if seedRow < min(targetRows)
                seedRow = min(targetRows);
            end
            if seedRow > max(targetRows)
                seedRow = max(targetRows);
            end
            if seedColumn < min(targetColumns)
                seedColumn = min(targetColumns);
            end
            if seedColumn > max(targetColumns)
                seedColumn = max(targetColumns);
            end
            
            prfCentroidRowGuesses(targetIndex, cadenceIndex)    = seedRow;
            prfCentroidColumnGuesses(targetIndex, cadenceIndex) = seedColumn;
        else
        ...
        %----------------turn off; not needed for regular runs, a time hog ---------
        % ----from PRF, compute the ee radius, against which we can compare
        % the variations in computed metric over the focal plane ---------

        % default resolution of 100 pixels for now
        %         [prfArray, prfSubPixelRows, prfSubPixelColumns] = make_array(prfObject);
        %
        %         prfArray = prfArray/sum(sum(prfArray));
        %
        %         % convert to absolute co-ordinatesprfSubPixelRows run from 0 through 11 with the first pixel being
