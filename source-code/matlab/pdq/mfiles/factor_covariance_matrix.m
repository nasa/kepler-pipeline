%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% from the built-in function lscov (simplified version)
%--------------------------------------------------------------------------
% 
...
function [T,p] = factor_covariance_matrix(V)
...
if (any(any(V ~= V')))
    error('MATLAB:lscov:InvalidCovMatSymV', 'V must be symmetric.');
end

if (any(any(~isfinite(V))))
    error('MATLAB:lscov:InvalidCovMatV', 'V must not contain Inf or NaN');
end
if (any(any(~isreal(V))))
    error('MATLAB:lscov:InvalidCovMatSymV', 'V must not be complex.');
end
