function collect_data_from_available_channels(cadenceTypeString, dataDir, ...
    figurePath, taskfileMap, savePixelDataFlag, channelArray)
%
...
% determine maximum number of available channels
[channelList, summaryStruct] = ...
    get_channels_from_taskfile_dir(taskfileMap, 'cal', dataDir);
...
rowsAndColumnsStruct = repmat(struct('ccdChannel', [], 'blackRows', [], ...
    ...
    'mSmearOutputCols', [], 'vSmearOutputCols', [], 'commonOutputCols', []), numChannels, 1);

% keep track of channels to ensure data from at least one invocation per available
% channel is collected
listOfCollectedChannels = cell(numChannels, 1);

for i = 1:numChannels
    
    cd(dataDir)
    
    thisChannel = channelList(i);
    ...
    % try first invocation
    taskFilename = validTaskFilenames{1};
    ...
    if exist([ taskFilename '/st-0/cal-inputs-0.mat'], 'file') && ...
            ...
            isempty(listOfCollectedChannels{i})
        
        display(['Keeping invocation 1 of ' num2str(numInvocations) ' for Channel ' num2str(thisChannel)])
        
    else
        
        for j = 1:numInvocations
            
            taskFilename = validTaskFilenames{j};
            ...
            if exist([ taskFilename '/st-0/cal-inputs-0.mat'], 'file') && ...
                ...
                display(['Keeping invocation ' num2str(j) ' of ' num2str(numInvocations) ' for Channel ' num2str(thisChannel)])
            end
        end
    end
    ...
    if i == 1
        ...
        %------------------------------------------------------------------
        medBlackAcrossRows = nan(numChannels, numCadences);
        medBlackAcrossTime = nan(numChannels, numCcdRows);
        
        stdBlackAcrossRows = nan(numChannels, numCadences);
        stdBlackAcrossTime = nan(numChannels, numCcdRows);
        
        
        %------------------------------------------------------------------
        % allocate memory for med/std black corrected masked smear pixels
        %------------------------------------------------------------------
        medMsmearAcrossCols = nan(numChannels, numCadences);
        medMsmearAcrossTime = nan(numChannels, numCcdCols);
        
        stdMsmearAcrossCols = nan(numChannels, numCadences);
        stdMsmearAcrossTime = nan(numChannels, numCcdCols);
        
        
        %------------------------------------------------------------------
        % allocate memory for med/std black corrected virtual smear pixels
        %------------------------------------------------------------------
        medVsmearAcrossCols = nan(numChannels, numCadences);
        medVsmearAcrossTime = nan(numChannels, numCcdCols);
        
        stdVsmearAcrossCols = nan(numChannels, numCadences);
        stdVsmearAcrossTime = nan(numChannels, numCcdCols);
        
        
        %------------------------------------------------------------------
        % allocate memory for med/std black corrected smear *difference*
        %------------------------------------------------------------------
        medSmearDiffAcrossCols = nan(numChannels, numCadences);
        medSmearDiffAcrossTime = nan(numChannels, numCcdCols);
        
        stdSmearDiffAcrossCols = nan(numChannels, numCadences);
        stdSmearDiffAcrossTime = nan(numChannels, numCcdCols);
        
        %------------------------------------------------------------------
        % allocate memory to collect all black residuals into on array of
        % size numCadences x numRows x numChannels
        %------------------------------------------------------------------
        allBlackResidualsArray = nan(numCcdRows, numCadences, numChannels);
        
        %------------------------------------------------------------------
        % allocate memory to collect all m-v residuals into on array of
        % size numCadences x numRows x numChannels
        %------------------------------------------------------------------
        allSmearResidualsArray = nan(numCcdCols, numCadences, numChannels);
    end
    ...
    % allBlackResidualsArray = nan(numCcdRows, numCadences, numChannels);
    allBlackResidualsArray(blackOutputRows, :, i) = blackPixels';
    ...
    % allSmearResidualsArray = nan(numCcdCols, numCadences, numChannels);
    allSmearResidualsArray(commonOutputCols, :, i) = mSmearPixels' - vSmearPixels';
...
%--------------------------------------------------------------------------
% save the full numRowsOrCols x numCadences x numChannels arrays for black
% and smear
%--------------------------------------------------------------------------
