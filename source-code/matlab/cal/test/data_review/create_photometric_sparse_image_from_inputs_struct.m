function create_photometric_sparse_image_from_inputs_struct(inputsStruct, oneOrZeroBaseString, plotFlag)
%
% function to create 2D image of available target pixels from a CAL
% inputsStruct
%
% example:
% create_photometric_sparse_image_from_inputs_struct(inputsStruct, 'oneBase', true)
%
% 
...
if strcmpi(oneOrZeroBaseString, 'oneBase')
    [targetCols] = [inputsStruct.targetAndBkgPixels.column];
    [targetRows] = [inputsStruct.targetAndBkgPixels.row];
    
else strcmpi(oneOrZeroBaseString, 'zeroBase')
    [targetCols] = [inputsStruct.targetAndBkgPixels.column]+1;
    [targetRows] = [inputsStruct.targetAndBkgPixels.row]+1;
end

cadences = [inputsStruct.cadenceTimes.midTimestamps];
...
colorbar
title([' Target Pixels (med over time)'])
ylabel('Row Index')
xlabel('Column Index')
