function collect_collateral_data_for_dawg(quarterString, monthString, cadenceTypeString, dataDir, ...
...
%  each mod/out specified in channelArray:
%
...
% parse quarter string to decide if there are 84 or 80 channels
% Q1-Q4 => 84 channels
% Q5-Q17 => 80 channels
switch quarterString
    case {'Q0','Q1','Q2','Q3','Q4'}
        channelArray = 1:84;
        numAllChannels = length(channelArray);
    otherwise
        channelArray =[1:4,9:84];
        numAllChannels = length(channelArray);
end
...
for ichan = 1:numAllChannels
    thisChannel = channelArray(ichan);
    ...
    if isempty(validTaskFilenames)
        ...
        display(['No available data for Channel ' num2str(thisChannel) ' because no validTaskFileName'])
        continue;
    end
    ...
    validInvocation = 1;
    while validInvocation <= numInvocations
        
        taskFilename = [dataDir validTaskFilenames{validInvocation}];
        
        % CHECK FOR STUPID BLOODY SUBTASKS
        
        if exist([taskFilename '/st-0'])
            stFlag = true;
        else
            stFlag = false;
        end
        ...
        if stFlag
            if exist([ taskFilename '/st-0/cal-inputs-0.mat'], 'file') && ...
                ...
                display(['Keeping invocation ' num2str(validInvocation) ' of ' num2str(numInvocations) ' for Channel ' num2str(thisChannel)])
                break
            else
                validInvocation = validInvocation + 1;
            end
        else
            % check for input file
            if exist([ taskFilename '/cal-inputs-0.mat'], 'file') && ...
                    exist([ taskFilename '/cal-outputs-0.mat'], 'file')
                display(['Keeping invocation ' num2str(validInvocation) ' of ' num2str(numInvocations) ' for Channel ' num2str(thisChannel)])
                break
            else
                validInvocation = validInvocation + 1;
    ...
    %    display(['Channel number ' num2str(thisChannel) ' validInvocation = ' num2str(validInvocation) ' num = ' num2str(numInvocations)])
    
    if (validInvocation <= numInvocations)
        ...
        if stFlag
            ...
            load([taskFilename '/cal-inputs-0.mat']);
        end
        ...
        if stFlag
            ...
            load([taskFilename '/cal-outputs-0.mat']);
        end
        ...
        %--------------------------------------------------------------------------
        % create image of gaps for each pixel type
        %--------------------------------------------------------------------------
