...
% correct_smear_pix_for_cosmic_rays()
%
% INPUTS:
%     returnSparse      : If true, construct and return sparse matrices.
%
% OUTPUTS:
%     correctedFluxMat  : An nCadence-by-nCcdColumns real matrix whose 
%                         columns contain corrected pixel values. 
...
%
    ...
    for iPixel = 1:numel(resultPixelArray)
        column = resultPixelArray(iPixel).ccdColumn;
        correctedFluxMat(  :, column ) = resultPixelArray(iPixel).values;
        eventIndicatorMat( :, column ) = ...
