function calImageStruct = create_FFI_from_cal_input(inputsStruct, ccdModule, ...
    ccdOutput, fcConstants, zeroBaseIdxFlag, rawFFI, cadenceIdx, plotsOn)
%function  calImageStruct = create_FFI_from_cal_input(outputsStruct, ccdModule, ...
%   ccdOutput, fcConstants, zeroBaseIdxFlag, rawFFI, cadenceIdx, plotsOn)
%
...
if zeroBaseIdxFlag
    ...
    pixelRows = [inputPixels.row]';               % 1 x nPixels
    pixelCols = [inputPixels.column]';            % 1 x nPixels
end
...
if (plotsOn)
    ...
    if (cadenceIdx > 1)
        title(['CAL Input FFI for Module ' num2str(ccdModule) ' Output ' num2str(ccdOutput) ',    Cad# ' num2str(cadenceIdx)], 'fontsize', 14);
    else
        title(['CAL Input FFI for Module ' num2str(ccdModule) ' Output ' num2str(ccdOutput)], 'fontsize', 14);
    end
    
    xlabel('CCD Column Index', 'fontsize', 14);
    ylabel('CCD Row Index (charge injection rows clipped)', 'fontsize', 14);
    
    fileNameStr = [ 'cal_input_ffi_mod'  num2str(ccdModule) '_out' num2str(ccdOutput) ];
