...
% extract data
%--------------------------------------------------------------------------
blackPixelValues = calIntermediateStruct.blackPixels(:, cadenceIndex);
blackPixelGaps   = calIntermediateStruct.blackGaps(:, cadenceIndex);
...
% if there are no valid black pixels, set values to -1 and gap POU
if isempty(validBlackPixelIndices)
    ...
    OneDBlack_Input = struct('TrailingBlackCollat',   blackPixelValues', ...
    ...
    %  estimate 1D black
    %--------------------------------------------------------------------------
    [calIntermediateStruct, tStruct] = estimate_lc_1dblack(calObject, OneDBlack_Input, calIntermediateStruct, cadenceIndex, tStruct);
    
end

%--------------------------------------------------------------------------
% transfer POU struct to intermediate struct
%--------------------------------------------------------------------------
if pouEnabled
