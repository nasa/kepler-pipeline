function [calObject, calIntermediateStruct, calTransformStruct] = ...
    correct_photometric_pix_for_gain(calObject, calIntermediateStruct, calTransformStruct)
% function [calObject, calIntermediateStruct, calTransformStruct] = ...
%     correct_photometric_pix_for_gain(calObject, calIntermediateStruct, calTransformStruct)
%
...
% is determined such that the full well (~1.3*10^6 electrons) does not exceed the max ADC value of 16,383 (2^14-1), and is of order ~100e-/DN.
% 
...
if numel(gain) > 1
    gain = repmat(gain(:)', length(photometricPixels(:, 1)), 1);
    gain = gain(validPhotometricPixelIndicators);
end

% correct pixels for gain
photometricPixels(validPhotometricPixelIndicators) = photometricPixels(validPhotometricPixelIndicators) .* gain;
...
if pouEnabled
    ...
    for cadenceIndex = 1:nCadences
        ...
        tStruct = append_transformation(tStruct, 'scale', calIntermediateStruct.pixelVariableName, [], gain);
...
if isAvailableFfiPix && enableFfiInform    
    ...
    for iFfi = 1:nFfis
        ...
        calIntermediateStruct.ffiStruct(iFfi).image = gain .* calIntermediateStruct.ffiStruct(iFfi).image;
    end
end
