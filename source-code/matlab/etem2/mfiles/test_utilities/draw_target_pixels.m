function ccdImage = draw_target_pixels(location, requantize, type)
%
...
targetDefs = get_target_definitions(location, type);
pixelData = get_pixel_time_series(location, type, requantize);
maskDefinitions = get_mask_definitions(location, 'targets');

nTargets = length(pixelData);
% nTargets = 7;
...
for target=1:nTargets
    ...
    for p=1:length(pixRow)
        ccdImage(pixRow(p), pixCol(p)) = pixelData(target).pixelValues(1,p);
    end
end
