function draw_target(location, target, cadenceRange)
%
...
while 1
    for c=cadenceRange
        ...
        for i=1:length(pixRow)
            targetImage(pixRow(i) - minRow + 1, pixCol(i) - minCol + 1) = pixelData(target).pixelValues(c,i);
        end

        figure(1);
        subplot(1,2,1);
        imagesc(targetImage, [minVal, maxVal]);
        colormap(hot);
        subplot(1,2,2);
        imagesc(log10(targetImage + 1e-6));
        title(['cadence ' num2str(c)]);
        colormap(hot);
        drawnow;
        pause;
    end
