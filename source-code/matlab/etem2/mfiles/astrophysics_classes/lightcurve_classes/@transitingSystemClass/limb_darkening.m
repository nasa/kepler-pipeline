function limbDarkeningCoef = limb_darkening(transitingSystemObject)
% function transitingSystemObject = limb_darkening(transitingSystemObject)
%
% sets the limb darkening coefficients for the primary object.
% requires that the primary object's effective temperature and log g have
% been set
% 
...
load(transitingSystemObject.limbDarkeningDataPath); 
...
effectiveTemperature = a1(:, effectiveTIndex);
logG = a1(:, logGIndex);
...
limbDarkeningCoef = interpolate_limb_darkening_coefficients(...
    RCoefStruct, logG, effectiveTemperature, myLogG, myEffT);
