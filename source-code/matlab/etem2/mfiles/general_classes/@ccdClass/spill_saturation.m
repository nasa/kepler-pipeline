function ccd = spill_saturation(ccdObject, ccd, numExposures, rowsInCcd, colsInCcd)
...
if nargin > 3
    % the ccd array is a subset of the full ccd with the rows and cols
    % specified by the last two args.  
    % extract the well capacity variation for those rows and cols
    if length(rowsInCcd) ~= size(ccd, 1) || length(colsInCcd) ~= size(ccd, 2)
        error('ccdClass:spill_saturation:rowsInCcd, colsInCcd does not match ccd size');
    end
    wellCapacityVariation = get(ccdObject, 'wellDepthVariation');
    wellCapacity = wellCapacity*wellCapacityVariation(rowsInCcd, colsInCcd);
else
    wellCapacity = wellCapacity*get(ccdObject, 'wellDepthVariation');
end
% size(wellCapacity) should now either be 1,1 or = size(ccd)
...
for k = 1:nsat
    ...
    satchargeup          = (ccd(isat(k),jsat(k))-wellCapacity(isat(k),jsat(k))) *      saturationSpillUpFraction;  % spill half of excess up column
    satchargedn          = (ccd(isat(k),jsat(k))-wellCapacity(isat(k),jsat(k))) * (1 - saturationSpillUpFraction); % spill half of excess down column

    % Set the over-saturated pixel to the saturation value
    ccd(isat(k),jsat(k)) = wellCapacity(isat(k),jsat(k));
    ...
    while i > 1 && satchargeup > 0 
        ...
        chargetospill  = (wellCapacity(i,j) - ccd(i,j)) * (ccd(i,j) < wellCapacity(i,j));
