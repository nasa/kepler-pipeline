function count = write_ccd_poly_segment(ccdPlaneObject, filename, ccdSegment, columns, prfNum)
%
...
ccdSegment = reshape(ccdSegment,[numCcdRows,length(columns),nCoefs]);
...
for i = 1:nCoefs
    ...
    temp = zeros(numCcdRows,numColumns);
    temp(:,relativeColumns) = ccdSegment(:,:,i);
    
    count = fwrite(fid,temp,'float32');% write out so that columns of c_ccd are written contiguously to disk
    count = fseek(fid,skip2,'cof');
end
