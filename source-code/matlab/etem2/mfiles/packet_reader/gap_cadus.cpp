...
int not_a_gap(int caduCount);

int not_a_gap(int caduCount) {
	for (int i=0; i<nGaps; i++) {
		if (caduCount >= gapStart[i] && caduCount <= gapEnd[i])
			return 0;
	}
	return 1;
}
...
int main(int argc, char* const* argv) {
	...
	while (working) {
        ...
        if (nRead != CADU_LENGTH) {
			...
			if (not_a_gap(caduCount)) {
				if (inAGap) {
					cout << "ending gap cadu # " << caduCount - 1 << endl;
					inAGap = false;
				}
				fwrite(caduBytes, sizeof(unsigned char), CADU_LENGTH, outFid);
			} else {
				if (!inAGap) {
					cout << "starting gap vcdu # " << caduCount << endl;
					inAGap = true;
				}
			}
			caduCount++;
