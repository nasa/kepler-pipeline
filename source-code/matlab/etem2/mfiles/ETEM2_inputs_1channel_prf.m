function gloabalConfigurationStruct = ETEM2_inputs_1channel_prf()
...
runParamsData.simulationData.numberOfTargetsRequested=4000; % Number of target stars before downselect

runParamsData.simulationData.runStartDate = '25 April 2009'; % start date of current run
runParamsData.simulationData.runDuration = 1; % length of run in the units defined in the next field
...
runParamsData.simulationData.outputNumber = 4; % legal values: 1-4
...
runParamsData.keplerData.shortsPerLongCadence = 15; % # of short cadences in a long cadence
...
runParamsData.keplerData.supressSmear = 1; % if true don't add smear, for producing clean image
...
%%%%%
% the following lines cause ETEM2 to load tad data from a pre-constructed
% file
tadInputData = pluginList.tadFileReaderData; 
tadInputData.tadFileLocation = '/path/to/matlab/prf/mfiles';
tadInputData.tadFilename = ['prf_tadInputStruct_m' ...
    num2str(runParamsData.simulationData.moduleNumber) 'o' ...
    num2str(runParamsData.simulationData.outputNumber) '.mat'];
...
ccdPlaneData(ccdPlaneCount).psfObjectData.psfFileLocation = '/path/to/prf/v7/';
...
ccdData.blackLevelData = pluginList.dynamicNoiseData;
ccdData.blackLevelData.baseline = 0;
...
% ccdData.blackLevelData.temperatureFunctionData = temperatureFunctionData;
...
% ccdData.targetScienceManagerData.targetSpecifiction(counter).selectionMagnitudeRange = [12 13];
...
% ccdData.targetScienceManagerData.backgroundBinarySpecification.selectionMagnitudeRange = [12 13];
