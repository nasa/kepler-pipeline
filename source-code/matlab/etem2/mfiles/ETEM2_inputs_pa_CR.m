function gloabalConfigurationStruct = ETEM2_inputs_pa_CR()
%
% Creates ETEM2 globalConfigurationStruct for PA cosmic ray test. All
% features that CAL removes from the data except 2D black are ommitted from
% the data set.
%
% 
...
outputDirectory         = 'TC05_2D_ST_sm_dc_nl_lu_ff_RN_QN_SN_ZD_DV_JT_SV_AP_CR_dir';           % s.outputDirectory;

numCadences             = 672;                                          % s.numCadences;
ccdModule               = 7;                                            % s.ccdModule;
ccdOutput               = 3;                                            % s.ccdOutput;

cadenceType             = 'long';                                       % s.cadenceType;

twoDBlackEnabled        = true;                                         % s.twoDBlackEnabled;
starsEnabled            = true;                                         % s.starsEnabled;

smearEnabled            = false;                                        % s.smearEnabled;
darkEnabled             = false;                                        % s.darkEnabled;
darkCurrentValue        = 0;                                            % s.darkCurrentValue;

nonlinearityEnabled     = false;                                        % s.nonlinearityEnabled;
undershootEnabled       = false;                                        % s.undershootEnabled;
flatFieldEnabled        = false;                                        % s.flatFieldEnabled;

readNoiseEnabled        = true;                                         % s.readNoiseEnabled;
quantNoiseEnabled       = true;                                         % s.quantNoiseEnabled;
shotNoiseEnabled        = true;                                         % s.shotNoiseEnabled;

cosmicRaysEnabled       = true;                                         % s.cosmicRaysEnabled;

supressAllMotionFlag    = false;                                        % s.supressAllMotionFlag;
makeCleanFlag           = false;                                        % s.makeCleanFlag;

targetListSetName       = 'q1-lc';                                      % s.targetListSetName;
refPixTargetListSetName = 'q1-rp';                                      % s.refPixTargetListSetName;

runStartDate            = '1-April-2009';                               % s.runStartDate;
requantizationTableId   = 175;                                          % s.requantizationTableId;
