...
/* Computing MAX */
	...
	dn2000 = (c__2000 - 1) * 365 + (c__2000 - 1) / 4 - (c__2000 - 1) / 
		...
		923)] + extra[(i__2 = c__1 - 1) < 12 && 0 <= i__2 ? i__2 : 
		s_rnge("extra", i__2, "ttrans_", (ftnlen)923)] * (max(i__3,
...
/* Computing MAX */
	...
	sunday = (c__1991 - 1) * 365 + (c__1991 - 1) / 4 - (c__1991 - 1) / 
		...
		0 <= i__1 ? i__1 : s_rnge("dpjan0", i__1, "ttrans_", (ftnlen)
		924)] + extra[(i__2 = c__1 - 1) < 12 && 0 <= i__2 ? i__2 : 
		s_rnge("extra", i__2, "ttrans_", (ftnlen)924)] * (max(i__3,
...
/*        times in the TAI table are in increasing order. */
	...
	for (i__ = 2; i__ <= i__1; ++i__) {
	    if (taitab[(i__2 = i__ - 2) < 280 && 0 <= i__2 ? i__2 : s_rnge(
		    "taitab", i__2, "ttrans_", (ftnlen)1188)] >= taitab[(i__3 
		    = i__ - 1) < 280 && 0 <= i__3 ? i__3 : s_rnge("taitab", 
		    i__3, "ttrans_", (ftnlen)1188)]) {
...
/* Computing MAX */
	...
	daynum = (year - 1) * 365 + (year - 1) / 4 - (year - 1) / 100 + (year 
		...
		i__1 : s_rnge("dpjan0", i__1, "ttrans_", (ftnlen)1322)] + 
		extra[(i__2 = month - 1) < 12 && 0 <= i__2 ? i__2 : s_rnge(
		"extra", i__2, "ttrans_", (ftnlen)1322)] * (max(i__3,i__4) - 
...
/* Computing MAX */
	...
	daynum = (year - 1) * 365 + (year - 1) / 4 - (year - 1) / 100 + (year 
		- 1) / 400 + (dpjan0[(i__1 = month - 1) < 12 && 0 <= i__1 ? 
		i__1 : s_rnge("dpjan0", i__1, "ttrans_", (ftnlen)1349)] + 
		extra[(i__2 = month - 1) < 12 && 0 <= i__2 ? i__2 : s_rnge(
		"extra", i__2, "ttrans_", (ftnlen)1349)] * (max(i__3,i__4) - 
		max(i__5,i__6) + max(i__7,i__8)) + day) - 1 + doffst;
	d__1 = d_int(&tvec[2]);
	d__2 = d_int(&tvec[3]);
...
/* Computing MAX */
	...
	daynum = (year - 1) * 365 + (year - 1) / 4 - (year - 1) / 100 + (year 
		- 1) / 400 + (dpjan0[(i__1 = month - 1) < 12 && 0 <= i__1 ? 
		i__1 : s_rnge("dpjan0", i__1, "ttrans_", (ftnlen)1377)] + 
		extra[(i__2 = month - 1) < 12 && 0 <= i__2 ? i__2 : s_rnge(
		"extra", i__2, "ttrans_", (ftnlen)1377)] * (max(i__3,i__4) - 
		max(i__5,i__6) + max(i__7,i__8)) + day) - 1 + doffst;
...
/*        change it if it turns out this is a day with a leapsecond. */
	...
	if (pfrom == 14) {
		...
		daylen = taitab[(i__1 = dayptr) < 280 && 0 <= i__1 ? i__1 : 
			s_rnge("taitab", i__1, "ttrans_", (ftnlen)1392)] - 
			taitab[(i__2 = dayptr - 1) < 280 && 0 <= i__2 ? i__2 :
			 s_rnge("taitab", i__2, "ttrans_", (ftnlen)1392)];
	    }
	    secs = frac * daylen;
	}
    } else if (pfrom == 4) {
...
/* Computing MAX */
	...
	daynum = (year - 1) * 365 + (year - 1) / 4 - (year - 1) / 100 + (year 
		- 1) / 400 + (dpjan0[(i__1 = month - 1) < 12 && 0 <= i__1 ? 
		i__1 : s_rnge("dpjan0", i__1, "ttrans_", (ftnlen)1465)] + 
		extra[(i__2 = month - 1) < 12 && 0 <= i__2 ? i__2 : s_rnge(
		"extra", i__2, "ttrans_", (ftnlen)1465)] * (max(i__3,i__4) - 
...
/* Computing MAX */
	...
	daynum = (year - 1) * 365 + (year - 1) / 4 - (year - 1) / 100 + (year 
		- 1) / 400 + (dpjan0[(i__1 = month - 1) < 12 && 0 <= i__1 ? 
		i__1 : s_rnge("dpjan0", i__1, "ttrans_", (ftnlen)1502)] + 
		extra[(i__2 = month - 1) < 12 && 0 <= i__2 ? i__2 : s_rnge(
		"extra", i__2, "ttrans_", (ftnlen)1502)] * (max(i__3,i__4) - 
		max(i__5,i__6) + max(i__7,i__8)) + c__1) - 1 + doffst;
	i__1 = daynum - sunday;
	rmaini_(&i__1, &c__7, &qint, &dpsun);
	fmday = dpsun + 1;
...
/*        day directly ... */
	...
	if (odd_(&taiptr)) {
	    daynum = daytab[(i__1 = taiptr - 1) < 280 && 0 <= i__1 ? i__1 : 
		    s_rnge("daytab", i__1, "ttrans_", (ftnlen)1550)];
	    secs = tai - taitab[(i__1 = taiptr - 1) < 280 && 0 <= i__1 ? i__1 
		    : s_rnge("taitab", i__1, "ttrans_", (ftnlen)1551)];
...
/*           entry in the TAI table. */
	    ...
	    d__1 = tai - taitab[(i__1 = taiptr - 1) < 280 && 0 <= i__1 ? i__1 
		    : s_rnge("taitab", i__1, "ttrans_", (ftnlen)1570)];
...
/*     time system or not. */
    ...
    if (forml[(i__1 = pto - 1) < 21 && 0 <= i__1 ? i__1 : s_rnge("forml", 
	    i__1, "ttrans_", (ftnlen)1595)] && forml[(i__2 = pfrom - 1) < 21 
...
/* Computing MAX */
	    ...
	    tai = taitab[(i__1 = dayptr - 1) < 280 && 0 <= i__1 ? i__1 : 
		    s_rnge("taitab", i__1, "ttrans_", (ftnlen)1610)] + secs;
...
/*           ...then back to DAYNUM and SECS */
	    ...
	    if (odd_(&taiptr)) {
		daynum = daytab[(i__1 = taiptr - 1) < 280 && 0 <= i__1 ? i__1 
			: s_rnge("daytab", i__1, "ttrans_", (ftnlen)1619)];
		secs = tai - taitab[(i__1 = taiptr - 1) < 280 && 0 <= i__1 ? 
			i__1 : s_rnge("taitab", i__1, "ttrans_", (ftnlen)1620)
			];
	    } else {
		taiptr = max(1,taiptr);
		daynum = daytab[(i__1 = taiptr - 1) < 280 && 0 <= i__1 ? i__1 
			: s_rnge("daytab", i__1, "ttrans_", (ftnlen)1626)];
		d__1 = tai - taitab[(i__1 = taiptr - 1) < 280 && 0 <= i__1 ? 
			i__1 : s_rnge("taitab", i__1, "ttrans_", (ftnlen)1628)
			];
		rmaind_(&d__1, &secspd, &daydp, &secs);
		daynum += (integer) daydp;
	    }
...
/*     increment the day number by one and set SECS to zero. */
    ...
    if (forml[(i__1 = pto - 1) < 21 && 0 <= i__1 ? i__1 : s_rnge("forml", 
	    i__1, "ttrans_", (ftnlen)1651)] && secs > secspd) {
	++daynum;
	secs = 0.;
    }
...
/* Computing MAX */
	...
	if (max(i__1,i__2) - max(i__3,i__4) + max(i__5,i__6) == 0) {
	    ...
	    day = dofyr - dpjan0[(i__1 = month - 1) < 12 && 0 <= i__1 ? i__1 :
		     s_rnge("dpjan0", i__1, "ttrans_", (ftnlen)1691)];
	} else {
	    month = lstlti_(&dofyr, &c__12, dpbegl);
	    day = dofyr - dpbegl[(i__1 = month - 1) < 12 && 0 <= i__1 ? i__1 :
		     s_rnge("dpbegl", i__1, "ttrans_", (ftnlen)1694)];
	}
...
/*     Finally, we convert to the requested output. */
	...
	if (pto == 13) {
	    ...
	    if (odd_(&dayptr)) {
		daylen = taitab[(i__1 = dayptr) < 280 && 0 <= i__1 ? i__1 : 
			s_rnge("taitab", i__1, "ttrans_", (ftnlen)1743)] - 
			taitab[(i__2 = dayptr - 1) < 280 && 0 <= i__2 ? i__2 :
			 s_rnge("taitab", i__2, "ttrans_", (ftnlen)1743)];
	    }
...
/* Computing MAX */
	...
	week = (daynum - ((year - 1) * 365 + (year - 1) / 4 - (year - 1) / 
		...
		1812)] + extra[(i__2 = month - 1) < 12 && 0 <= i__2 ? i__2 : 
		s_rnge("extra", i__2, "ttrans_", (ftnlen)1812)] * (max(i__3,
		i__4) - max(i__5,i__6) + max(i__7,i__8)) + c__1) - 1) - 
		doffst) / 7 + 1;
	i__1 = daynum - sunday;
	rmaini_(&i__1, &c__7, &qint, &day);
...
/* Computing MAX */
	...
	tai = taitab[(i__1 = dayptr - 1) < 280 && 0 <= i__1 ? i__1 : s_rnge(
		"taitab", i__1, "ttrans_", (ftnlen)1836)] + secs;
