...
/* $Procedure GETFOV ( Fetch instrument FOV configuration ) */
...
/*    This subroutine returns the field-of-view (FOV) configuration for */
...
/*        This specification requires keywords in the kernel pool */
/*        that define the shape, boresight, boundary vectors, and */
/*        reference frame of the FOV.  The list of currently */
/*        supported shapes: */
...
/*        This specification requires keywords in the kernel pool */
/*        that define the shape, boresight, reference vector, */
/*        reference and cross angular extents of the FOV.  The list */
/*        of supported shapes is: */
...
/*     longitude of the points where the corners of a rectangular field */
/*     of view intersect the surface of an object.  This routine together */
...
/*     C */
/*     C     Determine the object, instrument and time */
/*     C     at which the latitude and longitude should */
/*     C     be computed. */
/*     C */
...
/*     C */
/*     C     Fetch the instrument, body and epoch of interest. */
/*     C */
/*           CALL GET_PARTICULARS ( INSTID, BODY, ET ) */

/*     C */
/*     C     Get the field of view information */
/*     C */
/*           CALL GETFOV ( INSTID, ROOM, SHAPE, FRAME, */
/*          .              BSIGHT, N,    BOUNDS        ) */

/*     C */
/*     C     Using the normal SPICE convention, the observer code */
/*     C     can be obtained by performing an integer divide */
/*     C     on the instrument ID. */
/*     C */
/*           OBS = INSTID / 1000 */

/*     C */
/*     C     Determine the bodyfixed frame associated with the body */
/*     C     being observed. */
/*     C */
/*           CALL CIDFRM ( BODY, FRCODE, BDYFXD, FOUND ) */

/*           IF ( .NOT. FOUND ) THEN */
/*              WRITE (*,*) 'Could not get the bodyfixed frame for : ', */
/*          .               BODY */
/*              STOP */
/*           END IF */

/*     C */
/*     C     Get the state of the body relative to the bodyfixed frame */
/*     C     corrected for light time.  Then compute the apparent */
/*     C     position of the observer in the bodyfixed frame. */
/*     C */
/*           CALL SPKEZ ( BODY, ET, BDYFXD, 'LT+S', OBS, STATE, LT ) */
/*           CALL VMINUS( STATE, POSOBS ) */

/*     C */
/*     C     Get the transformation from the instrument frame to the */
/*     C     bodyfixed frame of the body. */
/*     C */
/*           CALL SXFORM ( FRAME,  'J2000', ET,    FRM2J2 ) */
/*           CALL SXFORM ( 'J2000', BDYFXD, ET-LT, TSIPM  ) */
/*           CALL MXMG   ( TSIPM,   FRM2J2, 6, 6,  XFORM  ) */

/*     C */
/*     C     We only need the rotation part of the state transformation */
/*     C     matrix. */
/*     C */
/*           DO I = 1,3 */
/*              DO J = 1,3 */
...
/*     C */
/*     C     Rotate the field of view into the bodyfixed frame. */
/*     C */
/*           DO I = 1, N */
/*              CALL MXV ( ROT, BOUNDS(1,I), FOV(1,I) ) */
/*           END DO */

/*     C */
/*     C     Look up the axes of the body. */
/*     C */
/*           CALL BODVCD ( BODY, 'RADII', 3, DIM, ABC ) */

/*     C */
/*     C     We assume that all of the edges of the field of view */
/*     C     intersect the body.  A more robust example would check */
/*     C     the FOUND flag after calling SURFPT. */
/*     C */
/*           WRITE (*,*) 'The latitude and longitude ', */
/*          .            'of the corners are: ' */

/*           DO I = 1, N */
/*              CALL SURFPT ( POSOBS, FOV(1,I), ABC(1), ABC(2), ABC(3), */
/*          .                 POINT,  FOUND ) */
/*              CALL RECLAT ( POINT, LAT, LONG, RADIUS ) */
...
/*     been loaded via a call to FURNSH prior to calling this routine. */
...
/*           rotate the the BSIGHT by REFANG towards REFVEC.  To do */
...
/*           to rotate the the BSIGHT by REFANG towards REFVEC.  To */
...
/*           If we end up here something's is terribly wrong with */
