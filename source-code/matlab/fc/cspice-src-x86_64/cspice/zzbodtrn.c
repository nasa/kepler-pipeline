	...
	logical *found, ftnlen name_len)
{
    ...
    static integer defcod[687], defocd[687];
    static char defnam[36*687];
    ...
    static char defnor[36*687], kernam[36*2000];
    static integer defonr[687];
...
/*        ZZBODC2N exactly returns the string as used in the the */
...
/*        and DEFCOD arrays from the built-in code list. */
	...
	zzbodget_(&c__687, defnam, defnor, defcod, &defsiz, (ftnlen)36, (
		ftnlen)36);
...
/*        output argument and return. */
	...
	if (i__ != 0) {
	    *code = kercod[(i__1 = i__ - 1) < 2000 && 0 <= i__1 ? i__1 : 
		    s_rnge("kercod", i__1, "zzbodtrn_", (ftnlen)1014)];
	    *found = TRUE_;
	    chkout_("ZZBODN2C", (ftnlen)8);
	    return 0;
	}
...
/*     ZZBODDEF mappings for a match. */
    ...
    if (i__ != 0) {
	*code = defcod[(i__1 = i__ - 1) < 687 && 0 <= i__1 ? i__1 : s_rnge(
		"defcod", i__1, "zzbodtrn_", (ftnlen)1030)];
	*found = TRUE_;
    }
    chkout_("ZZBODN2C", (ftnlen)8);
    return 0;
...
/*        Search the the list of codes, KERCOD, using the */
...
/*        output argument and return. */
	...
	if (i__ != 0) {
	    s_copy(name__, kernam + ((i__1 = i__ - 1) < 2000 && 0 <= i__1 ? 
		    i__1 : s_rnge("kernam", i__1, "zzbodtrn_", (ftnlen)1353)) 
...
/*           mappings present in the kernel pool lists. */
	    ...
	    ljust_(defnam + ((i__1 = i__ - 1) < 687 && 0 <= i__1 ? i__1 : 
		    s_rnge("defnam", i__1, "zzbodtrn_", (ftnlen)1380)) * 36, 
...
/*              Return the name. */
		...
		s_copy(name__, defnam + ((i__1 = i__ - 1) < 687 && 0 <= i__1 ?
			 i__1 : s_rnge("defnam", i__1, "zzbodtrn_", (ftnlen)
			1402)) * 36, name_len, (ftnlen)36);
		*found = TRUE_;
	    }
	} else {
...
/*           return the name. */
	    ...
	    s_copy(name__, defnam + ((i__1 = i__ - 1) < 687 && 0 <= i__1 ? 
		    i__1 : s_rnge("defnam", i__1, "zzbodtrn_", (ftnlen)1413)) 
		    * 36, name_len, (ftnlen)36);
	    *found = TRUE_;
	}
    }
...
/*           .TRUE. */
	    ...
	    bodchg = TRUE_;
	    s_copy(defnam + ((i__1 = index - 1) < 687 && 0 <= i__1 ? i__1 : 
		    s_rnge("defnam", i__1, "zzbodtrn_", (ftnlen)1824)) * 36, 
...
/*        out the existing entry. */
	...
	for (i__ = index + 1; i__ <= i__1; ++i__) {
	    s_copy(defnam + ((i__2 = i__ - 2) < 687 && 0 <= i__2 ? i__2 : 
		    s_rnge("defnam", i__2, "zzbodtrn_", (ftnlen)1840)) * 36, 
		    defnam + ((i__3 = i__ - 1) < 687 && 0 <= i__3 ? i__3 : 
		    s_rnge("defnam", i__3, "zzbodtrn_", (ftnlen)1840)) * 36, (
		    ftnlen)36, (ftnlen)36);
	    s_copy(defnor + ((i__2 = i__ - 2) < 687 && 0 <= i__2 ? i__2 : 
		    s_rnge("defnor", i__2, "zzbodtrn_", (ftnlen)1841)) * 36, 
		    defnor + ((i__3 = i__ - 1) < 687 && 0 <= i__3 ? i__3 : 
		    s_rnge("defnor", i__3, "zzbodtrn_", (ftnlen)1841)) * 36, (
		    ftnlen)36, (ftnlen)36);
	    defcod[(i__2 = i__ - 2) < 687 && 0 <= i__2 ? i__2 : s_rnge("defc"
		    "od", i__2, "zzbodtrn_", (ftnlen)1842)] = defcod[(i__3 = 
		    i__ - 1) < 687 && 0 <= i__3 ? i__3 : s_rnge("defcod", 
		    i__3, "zzbodtrn_", (ftnlen)1842)];
	}
    } else {
...
/*     DEFNAM, DEFNOR, and DEFCOD lists. */
    ...
    s_copy(defnam + ((i__1 = defsiz - 1) < 687 && 0 <= i__1 ? i__1 : s_rnge(
	    "defnam", i__1, "zzbodtrn_", (ftnlen)1885)) * 36, name__, (ftnlen)
	    36, name_len);
    s_copy(defnor + ((i__1 = defsiz - 1) < 687 && 0 <= i__1 ? i__1 : s_rnge(
	    "defnor", i__1, "zzbodtrn_", (ftnlen)1886)) * 36, tmpnam, (ftnlen)
	    36, (ftnlen)36);
    defcod[(i__1 = defsiz - 1) < 687 && 0 <= i__1 ? i__1 : s_rnge("defcod", 
	    i__1, "zzbodtrn_", (ftnlen)1887)] = *code;
