...
/*     the parallel array. */
	...
	*frcode = idcode[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 : s_rnge(
		"idcode", i__1, "framex_", (ftnlen)618)];
    } else {
...
/*     Perform any needed first pass initializations. */
	...
	s_copy(frname, name__ + (((i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 
		: s_rnge("name", i__1, "framex_", (ftnlen)859)) << 5), 
...
/*     the parallel array. */
	...
	*cent = center[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 : s_rnge(
		"center", i__1, "framex_", (ftnlen)1164)];
	*class__ = type__[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 : 
		s_rnge("type", i__1, "framex_", (ftnlen)1165)];
	*clssid = typeid[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 : s_rnge(
		"typeid", i__1, "framex_", (ftnlen)1166)];
	*found = TRUE_;
    } else {
...
/*           of built-in names. */
		...
		*frcode = idcode[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 :
			 s_rnge("idcode", i__1, "framex_", (ftnlen)1508)];
		*found = TRUE_;
	    } else {
...
/*     name in the parallel array. */
    ...
    if (item > 0) {
	*frcode = idcode[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 : s_rnge(
		"idcode", i__1, "framex_", (ftnlen)1623)];
	s_copy(frname, name__ + (((i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 
		: s_rnge("name", i__1, "framex_", (ftnlen)1624)) << 5), 
		frname_len, (ftnlen)32);
	*found = TRUE_;
    } else {
...
/*           of built-in names. */
		...
		s_copy(frname, pname, frname_len, (ftnlen)32);
		*frcode = idcode[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 :
			 s_rnge("idcode", i__1, "framex_", (ftnlen)1917)];
		*found = TRUE_;
	    } else {
...
/*     name in the parallel array. */
    ...
    if (item > 0) {
	*frcode = idcode[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 : s_rnge(
		"idcode", i__1, "framex_", (ftnlen)2029)];
	*found = TRUE_;
    } else {
...
/*        if we get a complete match. */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    gipool_(kvbuff + (((i__2 = i__ - 1) < 100 && 0 <= i__2 ? i__2 : 
		    s_rnge("kvbuff", i__2, "framex_", (ftnlen)2284)) << 5), &
		...
		errch_("#", kvbuff + (((i__2 = i__ - 1) < 100 && 0 <= i__2 ? 
			i__2 : s_rnge("kvbuff", i__2, "framex_", (ftnlen)2292)
...
/*              Get the class ID for the current frame. */
		...
		if (! fnd) {
		    ...
		    errch_("#", kvbuff + (((i__2 = i__ - 1) < 100 && 0 <= 
			    ...
			    ftnlen)2316)) << 5), (ftnlen)1, (ftnlen)32);
		    sigerr_("SPICE(INVALIDFRAMEDEF)", (ftnlen)22);
		    chkout_("CCIFRM", (ftnlen)6);
		    return 0;
		}
...
/*                 name keyword. */
		    ...
		    repmc_(kvbuff + (((i__2 = i__ - 1) < 100 && 0 <= i__2 ? 
			    i__2 : s_rnge("kvbuff", i__2, "framex_", (ftnlen)
			    2337)) << 5), "_CLASS", "_NAME", look2, (ftnlen)
			...
			errch_("#", kvbuff + (((i__2 = i__ - 1) < 100 && 0 <= 
				i__2 ? i__2 : s_rnge("kvbuff", i__2, "framex_"
				, (ftnlen)2347)) << 5), (ftnlen)1, (ftnlen)32)
				;
			sigerr_("SPICE(INVALIDFRAMEDEF)", (ftnlen)22);
			chkout_("CCIFRM", (ftnlen)6);
			return 0;
