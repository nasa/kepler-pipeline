...
/*           for the next NODE. */
	    ...
	    i__1, "refchg_", (ftnlen)330)] == 1 || frame[(i__2 = node - 1) < 
...
/*        root classes or we run into FRAME2. */
	...
	rotget_(&frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge(
		"frame", i__1, "refchg_", (ftnlen)344)], et, &rot[(i__2 = (
		node * 3 + 1) * 3 - 12) < 126 && 0 <= i__2 ? i__2 : s_rnge(
		"rot", i__2, "refchg_", (ftnlen)344)], &relto, &found);
	if (found) {
...
/*           rotation from FRAME(NODE) to RELTO. */
	    ...
	    frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge("frame",
		     i__1, "refchg_", (ftnlen)355)] = relto;
	    zzrxr_(&rot[(i__1 = ((node - 1) * 3 + 1) * 3 - 12) < 126 && 0 <= 
		    i__1 ? i__1 : s_rnge("rot", i__1, "refchg_", (ftnlen)356)]
		    , &c__2, tmprot);
	    for (i__ = 1; i__ <= 3; ++i__) {
		for (j = 1; j <= 3; ++j) {
		    rot[(i__1 = i__ + (j + (node - 1) * 3) * 3 - 13) < 126 && 
			    0 <= i__1 ? i__1 : s_rnge("rot", i__1, "refchg_", 
			    (ftnlen)360)] = tmprot[(i__2 = i__ + j * 3 - 4) < 
...
/*        another rotation. */
		...
		"frame", i__1, "refchg_", (ftnlen)370)] == 1 || frame[(i__2 = 
		node - 1) < 10 && 0 <= i__2 ? i__2 : s_rnge("frame", i__2, 
		"refchg_", (ftnlen)370)] == *frame2 || ! found;
    }
...
/*     We now have to do essentially the same thing for FRAME2. */
    ...
    if (frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge("frame", 
	    i__1, "refchg_", (ftnlen)408)] == *frame2) {
...
/*     get from FRAME1 to FRAME2. */
	...
	frmnam_(&frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge(
		"frame", i__1, "refchg_", (ftnlen)541)], name3, (ftnlen)32);
	...
	errint_("#", &frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : 
		s_rnge("frame", i__1, "refchg_", (ftnlen)574)], (ftnlen)1);
	errint_("#", frame2, (ftnlen)1);
...
/*     result from FRAME1 to FRAME2. */
    ...
    xpose_(&rot2[(i__1 = (get * 3 + 1) * 3 - 12) < 18 && 0 <= i__1 ? i__1 : 
	    s_rnge("rot2", i__1, "refchg_", (ftnlen)609)], &rot[(i__2 = (
	    cmnode * 3 + 1) * 3 - 12) < 126 && 0 <= i__2 ? i__2 : s_rnge(
	    "rot", i__2, "refchg_", (ftnlen)609)]);
