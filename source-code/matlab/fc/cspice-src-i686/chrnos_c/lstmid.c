...
/*     This routine find ET of the nearest, previous or next local */
/*     solar time specified as an integer number of seconds past */
/*     midnight of current local solar day.. */
...
/*                    local time to a give input ET time. */
...
/*     Check if input offset within the right bounds. */
	...
	setmsg_("Input offset expressed as count of local seconds must be be"
		"tween 0 and 86000.", (ftnlen)77);
	sigerr_("SPICE(BADTIMEOFFSET)", (ftnlen)20);
    }

/*     Depending of the kind of midnight we need to compute, we */
/*     set initial ET for our iterations. */

    if (s_cmp(mntype, "NEAREST", mntype_len, (ftnlen)7) == 0) {

/*        We don't do anything but assign LSECS to start iterations */
/*        loop. */

	lsecs = 1;
    } else if (s_cmp(mntype, "PREVIOUS", mntype_len, (ftnlen)8) == 0) {

/*        We step back a number of ET second between current local time */
/*        and previous desired local time. */
...
/*        We step forward a number of ET second between current */
/*        local time and following midnight. */
...
/*     We stop iterations when LSECS is exact 0. We can do it because */
/*     ET2LST returns integer number of local hours, minutes and */
/*     seconds :). Before iterating we set counter to 0. */
    i__ = 0;
    while(lsecs != 0) {

/*        Get local time at current ET and convert it to number of ET */
/*        local seconds past previous time of interest. */

	et2lst_(&et, bodyid, lon, "PLANETOCENTRIC", &hr, &mn, &sc, time, ampm,
		 (ftnlen)14, (ftnlen)24, (ftnlen)6);
	lsecs = hr * 3600 + mn * 60 + sc - *secoff;

/*        If next time of interest is closer, adjust local seconds */
...
/*        Increment current ET */

	et -= lsecs * *scrate;

/*        Increase iterations counter and bail out if it's */
/*        over the limit. */
