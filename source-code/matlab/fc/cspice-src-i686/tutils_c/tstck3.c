...
/*     Create the SCLK kernel. */
	...
	r__ = rtrim_(sclkbf + ((i__1 = i__ - 1) < 98 && 0 <= i__1 ? i__1 : 
		s_rnge("sclkbf", i__1, "tstck3_", (ftnlen)452)) * 80, (ftnlen)
		80);
	io___5.ciunit = unit;
	s_wsle(&io___5);
	do_lio(&c__9, &c__1, sclkbf + ((i__1 = i__ - 1) < 98 && 0 <= i__1 ? 
		i__1 : s_rnge("sclkbf", i__1, "tstck3_", (ftnlen)453)) * 80, 
		r__);
	e_wsle();
    }
    cl__1.cerr = 0;
...
/*     The zero point of the clock is 1 Jan 1980 TDB. */
	...
	tick[(i__1 = j - 1) < 205 && 0 <= i__1 ? i__1 : s_rnge("tick", i__1, 
		"tstck3_", (ftnlen)473)] = (et - zeropt) * 1e4;
	++j;
	tick[(i__1 = j - 1) < 205 && 0 <= i__1 ? i__1 : s_rnge("tick", i__1, 
		"tstck3_", (ftnlen)476)] = tick[(i__2 = j - 2) < 205 && 0 <= 
		i__2 ? i__2 : s_rnge("tick", i__2, "tstck3_", (ftnlen)476)] - 
		1.;
	et += -1e7;
    }
    ++j;
    tick[(i__1 = j - 1) < 205 && 0 <= i__1 ? i__1 : s_rnge("tick", i__1, 
	    "tstck3_", (ftnlen)483)] = 0.;
    et = 1e7;
    while(et < maxet) {
	++j;
	tick[(i__1 = j - 1) < 205 && 0 <= i__1 ? i__1 : s_rnge("tick", i__1, 
		"tstck3_", (ftnlen)491)] = (et - zeropt) * 1e4;
	++j;
	tick[(i__1 = j - 1) < 205 && 0 <= i__1 ? i__1 : s_rnge("tick", i__1, 
		"tstck3_", (ftnlen)494)] = tick[(i__2 = j - 2) < 205 && 0 <= 
		i__2 ? i__2 : s_rnge("tick", i__2, "tstck3_", (ftnlen)494)] - 
		1.;
	et += 1e7;
    }
    ++j;
    tick[(i__1 = j - 1) < 205 && 0 <= i__1 ? i__1 : s_rnge("tick", i__1, 
	    "tstck3_", (ftnlen)501)] = (maxet - zeropt) * 1e4;
    nticks = j;
...
/*     Sort the ticks. */
	...
	tk = tick[(i__2 = i__ - 1) < 205 && 0 <= i__2 ? i__2 : s_rnge("tick", 
		i__2, "tstck3_", (ftnlen)519)];
	et = zeropt + tk / 1e4;
	tstatd_(&et, rot, &angvel[(i__2 = i__ * 3 - 3) < 615 && 0 <= i__2 ? 
		i__2 : s_rnge("angvel", i__2, "tstck3_", (ftnlen)521)]);
	m2q_(rot, &quatrn[(i__2 = (i__ << 2) - 4) < 820 && 0 <= i__2 ? i__2 : 
		s_rnge("quatrn", i__2, "tstck3_", (ftnlen)522)]);
    }
    nints = 1;
    starts[0] = 0.;
...
/*     10 million seconds later than those for body -9999 */
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
	tk = tick[(i__2 = i__ - 1) < 205 && 0 <= i__2 ? i__2 : s_rnge("tick", 
		i__2, "tstck3_", (ftnlen)548)];
	et = zeropt + tk / 1e4;
	tstatd_(&et, rot, &angvel[(i__2 = i__ * 3 - 3) < 615 && 0 <= i__2 ? 
		i__2 : s_rnge("angvel", i__2, "tstck3_", (ftnlen)550)]);
	m2q_(rot, &quatrn[(i__2 = (i__ << 2) - 4) < 820 && 0 <= i__2 ? i__2 : 
		s_rnge("quatrn", i__2, "tstck3_", (ftnlen)551)]);
    }
    nints = 1;
    starts[0] = 0.;
    ckw03_(handle, &begtim, &endtim, &inst, ref, &avflg, segid, &nticks, tick,
...
/*     but 100 million seconds later than those for object -9999 */
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
	tk = tick[(i__2 = i__ - 1) < 205 && 0 <= i__2 ? i__2 : s_rnge("tick", 
		i__2, "tstck3_", (ftnlen)577)];
	et = zeropt + tk / 1e4;
	tstatd_(&et, rot, &angvel[(i__2 = i__ * 3 - 3) < 615 && 0 <= i__2 ? 
		i__2 : s_rnge("angvel", i__2, "tstck3_", (ftnlen)579)]);
	m2q_(rot, &quatrn[(i__2 = (i__ << 2) - 4) < 820 && 0 <= i__2 ? i__2 : 
		s_rnge("quatrn", i__2, "tstck3_", (ftnlen)580)]);
    }
    nints = 1;
    starts[0] = 0.;
    ckw03_(handle, &begtim, &endtim, &inst, ref, &avflg, segid, &nticks, tick,
