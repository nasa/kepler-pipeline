/* kilfil.f -- translated by f2c (version 19980913).
...
/* Subroutine */ int kilfil_(char *filnam, ftnlen filnam_len)
{
    ...
    extern logical return_(void), exists_(char *, ftnlen);
...
/*      3)  If the file specified by FILNAM is already open, the file */
/*          is closed with an attempt to delete it. */

/*      4)  If the file doesn't exist, this routine simply returns. */
...
/*     This is a rip off of Kevin's routine DELFIL. */
...
/* -     Test Utility 1.0.0, 30-NOV-1994 (WLT) (KRG) */
...
/*     error and return. */
	...
	chkout_("KILFIL", (ftnlen)6);
	return 0;
    }

/*     If the file doesn't exist, there's nothing to do. */

    if (! exists_(filnam, filnam_len)) {
	chkout_("KILFIL", (ftnlen)6);
	return 0;
    }
