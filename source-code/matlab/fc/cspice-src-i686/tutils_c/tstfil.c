/* tstfil.f -- translated by f2c (version 19980913).
...
/* Subroutine */ int tstfil_(char *pattrn, char *port, char *file, ftnlen 
	pattrn_len, ftnlen port_len, ftnlen file_len)
{
    ...
    extern /* Subroutine */ int tstslf_(char *, ftnlen), fststr_(char *, char 
	    *, ftnlen, ftnlen), tstopn_(char *, char *, ftnlen, ftnlen), 
...
/*     PORT        is the name of an TSTIO port that will be opened */
...
/*     to one of the file ports supported by TSTIO.  In this way */
...
/*     of TSTIO for its IO.  Then you could make the following */
/*     call */

/*        PATTRN = 'util{0-9}{0-9}{0-9}{0-9}.tmp' */

/*        CALL TSTFIL ( PATTRN, 'UTILITY', FILE ) */

/*     If successful, FILE will hold the name of the file that */
/*     was opened and is attached to the UTILITY port of TSTIO. */
...
/*     Local Parameters */
    ...
    chkin_("TSTFIL", (ftnlen)6);
    s_copy(fname, " ", (ftnlen)127, (ftnlen)1);
    s_copy(name__, " ", (ftnlen)127, (ftnlen)1);
    s_copy(this__, " ", (ftnlen)127, (ftnlen)1);
    fststr_(pattrn, fname, pattrn_len, (ftnlen)127);
    s_copy(name__, fname, (ftnlen)127, (ftnlen)127);
    more = TRUE_;
    badopn = 0;
    while(badopn < 20) {
...
/*        Look for a file name that does not already exist. */
	...
	while(exists_(name__, (ftnlen)127) && more) {
	    s_copy(this__, name__, (ftnlen)127, (ftnlen)127);
	    s_copy(name__, " ", (ftnlen)127, (ftnlen)1);
	    nxtstr_(pattrn, this__, name__, pattrn_len, (ftnlen)127, (ftnlen)
		    127);
	    more = s_cmp(name__, fname, (ftnlen)127, (ftnlen)127) != 0;
	}
	if (! more) {
	    ...
	    chkout_("TSTFIL", (ftnlen)6);
	    return 0;
	} else {
	    s_copy(file, name__, file_len, (ftnlen)127);
	}

/*        Ok.  We've got a good candidate, try to attach it to the */
/*        specified port. */

	tstopn_(port, file, port_len, file_len);
	if (failed_()) {
	    ++badopn;
