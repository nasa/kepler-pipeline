...
/* $Procedure      CLCOMM ( Command Line COMMNT program ) */
/* Subroutine */ int clcomm_(void)
{
    ...
    extern /* Subroutine */ int reset_(void), dafacu_(integer *, char *, char 
...
/*     CLCOMMNT is a command line oriented version of the COMMNT */
/*     program. */
...
/*     For additional information, see the CLCOMMNT User's Guide. */
...
/*     Set up the help message. */
    ...
    s_copy(hlpmsg + 320, "If no arguments are specified commnt runsin a menu"
	    " driven,", (ftnlen)80, (ftnlen)58);
...
/*     to mateh FLGMCH characters, including the dash, '-'. */
...
/*     If we didn't find the option, then signal an error. */
	...
	for (i__ = 1; i__ <= 18; ++i__) {
	    tostdo_(hlpmsg + ((i__1 = i__ - 1) < 18 && 0 <= i__1 ? i__1 : 
		    s_rnge("hlpmsg", i__1, "clcomm_", (ftnlen)280)) * 80, (
		    ftnlen)80);
	}
...
/*     for '-a' and '-e'. */
	...
	for (i__ = 1; i__ <= 18; ++i__) {
	    tostdo_(hlpmsg + ((i__1 = i__ - 1) < 18 && 0 <= i__1 ? i__1 : 
		    s_rnge("hlpmsg", i__1, "clcomm_", (ftnlen)293)) * 80, (
		    ftnlen)80);
	}
	byebye_("FAILURE", (ftnlen)7);
    }
...
/*     Set the option. */
    ...
    s_copy(option, optnam + (((i__1 = idxopt - 1) < 5 && 0 <= i__1 ? i__1 : 
	    s_rnge("optnam", i__1, "clcomm_", (ftnlen)342)) << 4), (ftnlen)16,
	     (ftnlen)16);
...
/*        comments, and close the kernel file. */
	...
	if (s_cmp(arch, "DAF", (ftnlen)3, (ftnlen)3) == 0) {
	    ...
	    dafacu_(&comlun, " ", " ", &c_true, &kerhan, (ftnlen)1, (ftnlen)1)
		    ;
	    dafcls_(&kerhan);
...
/*        open and close them. */
	...
	if (numopn > 0) {
		...
		dafcls_(&opnset[(i__2 = i__ + 5) < 7 && 0 <= i__2 ? i__2 : 
			s_rnge("opnset", i__2, "clcomm_", (ftnlen)566)]);
	    }
	}
...
/*        open and close them. */
	...
	if (numopn > 0) {
		...
		dascls_(&opnset[(i__2 = i__ + 5) < 7 && 0 <= i__2 ? i__2 : 
			s_rnge("opnset", i__2, "clcomm_", (ftnlen)585)]);
	    }
	}
