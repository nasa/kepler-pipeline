	...
	char *buffer, logical *done, ftnlen buffer_len)
{
    ...
    static integer lsthan, lstrec[20];
...
/*     marker, and the file table variables. */
	...
	*(unsigned char *)eolmrk = '\0';
	for (i__ = 1; i__ <= 20; ++i__) {
	    filchr[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("filchr"
		    , i__1, "dafec_", (ftnlen)446)] = 0;
	    filcnt[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("filcnt"
		    , i__1, "dafec_", (ftnlen)447)] = 0;
	    filhan[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("filhan"
		    , i__1, "dafec_", (ftnlen)448)] = 0;
	    lstpos[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("lstpos"
		    , i__1, "dafec_", (ftnlen)449)] = 0;
	    lstrec[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("lstrec"
		    , i__1, "dafec_", (ftnlen)450)] = 0;
	}
    }
...
/*        i.e., where we left off when we last read from that file. */
	...
	recno = lstrec[(i__1 = index - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		"lstrec", i__1, "dafec_", (ftnlen)516)];
	curpos = lstpos[(i__1 = index - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		"lstpos", i__1, "dafec_", (ftnlen)517)];
	nchars = filchr[(i__1 = index - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		"filchr", i__1, "dafec_", (ftnlen)518)];
	ncomc = filcnt[(i__1 = index - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
...
/*           Otherwise, just increment the current position. */
	    ...
	    if (curpos == 1000) {
		++recno;
		io___37.ciunit = daflun;
		io___37.cirec = recno;
		iostat = s_rdue(&io___37);
		if (iostat != 0) {
		    goto L100003;
		}
		iostat = do_uio(&c__1, crecrd, (ftnlen)1000);
...
/*           table, and that we are currently reading from one of them. */
		...
		for (k = index; k <= i__1; ++k) {
		    filchr[(i__2 = k - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			    "filchr", i__2, "dafec_", (ftnlen)809)] = filchr[(
			    i__3 = k) < 20 && 0 <= i__3 ? i__3 : s_rnge("fil"
			    "chr", i__3, "dafec_", (ftnlen)809)];
		    filcnt[(i__2 = k - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			    "filcnt", i__2, "dafec_", (ftnlen)810)] = filcnt[(
			    i__3 = k) < 20 && 0 <= i__3 ? i__3 : s_rnge("fil"
			    "cnt", i__3, "dafec_", (ftnlen)810)];
		    filhan[(i__2 = k - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			    "filhan", i__2, "dafec_", (ftnlen)811)] = filhan[(
			    i__3 = k) < 20 && 0 <= i__3 ? i__3 : s_rnge("fil"
			    "han", i__3, "dafec_", (ftnlen)811)];
		    lstrec[(i__2 = k - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			    "lstrec", i__2, "dafec_", (ftnlen)812)] = lstrec[(
			    i__3 = k) < 20 && 0 <= i__3 ? i__3 : s_rnge("lst"
			    "rec", i__3, "dafec_", (ftnlen)812)];
		    lstpos[(i__2 = k - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			    "lstpos", i__2, "dafec_", (ftnlen)813)] = lstpos[(
			    i__3 = k) < 20 && 0 <= i__3 ? i__3 : s_rnge("lst"
			    "pos", i__3, "dafec_", (ftnlen)813)];
		}
		--nfiles;
	    }
	}
...
/*           its information if there is room in the file table. */
	    ...
	    ++nfiles;
	    filchr[(i__1 = nfiles - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "filchr", i__1, "dafec_", (ftnlen)857)] = nchars;
	    filcnt[(i__1 = nfiles - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "filcnt", i__1, "dafec_", (ftnlen)858)] = ncomc;
	    filhan[(i__1 = nfiles - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "filhan", i__1, "dafec_", (ftnlen)859)] = *handle;
	    lstrec[(i__1 = nfiles - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "lstrec", i__1, "dafec_", (ftnlen)860)] = recno;
	    lstpos[(i__1 = nfiles - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "lstpos", i__1, "dafec_", (ftnlen)861)] = curpos;
	    lsthan = *handle;
	} else {
...
/*           so just update its information. */
	    ...
	    filchr[(i__1 = index - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("fil"
		    "chr", i__1, "dafec_", (ftnlen)869)] = nchars;
	    lstrec[(i__1 = index - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("lst"
		    "rec", i__1, "dafec_", (ftnlen)870)] = recno;
	    lstpos[(i__1 = index - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("lst"
		    "pos", i__1, "dafec_", (ftnlen)871)] = curpos;
	    lsthan = *handle;
	}
    }
    chkout_("DAFEC", (ftnlen)5);
