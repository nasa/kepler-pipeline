...
/*     and continue parsing the list. */
	...
	ucase_(words + ((i__2 = i__ - 1) < 5 && 0 <= i__2 ? i__2 : s_rnge(
		"words", i__2, "outmsg_", (ftnlen)515)) * 9, upword, (ftnlen)
		9, (ftnlen)9);
...
/* Writing concatenation */
	    ...
	    i__3[1] = 9, a__1[1] = words + ((i__2 = i__ - 1) < 5 && 0 <= i__2 
		    ? i__2 : s_rnge("words", i__2, "outmsg_", (ftnlen)552)) * 
		    9;
...
/*        needed. */
	...
	i__1 = numwrd;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    nextwd_(lmsg, outwrd, lmsg, (ftnlen)1840, (ftnlen)255, (ftnlen)
		    1840);
	    wrdlen = rtrim_(outwrd, (ftnlen)255);
	    if (start + wrdlen <= 80) {
...
/*                 continue. */
		    ...
		    wrline_(device, line, (ftnlen)255, (ftnlen)80);
		    start = wrdlen + 2;
		    s_copy(line, outwrd, (ftnlen)80, (ftnlen)255);
		} else {
...
/*                 justifying the remaining part before proceeding. */
			...
			ljust_(outwrd, outwrd, (ftnlen)255, (ftnlen)255);
			s_copy(line, " ", (ftnlen)80, (ftnlen)1);
			wrdlen -= remain;
			remain = 80;
			start = 1;
...
/*        Output the default message: */
	...
	for (i__ = 1; i__ <= 4; ++i__) {
	    wrline_(device, defmsg + ((i__1 = i__ - 1) < 4 && 0 <= i__1 ? 
		    i__1 : s_rnge("defmsg", i__1, "outmsg_", (ftnlen)873)) * 
