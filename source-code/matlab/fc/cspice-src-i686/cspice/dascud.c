...
/*     data addition. */
    ...
    last = lastla[(i__1 = *type__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("las"
	    "tla", i__1, "dascud_", (ftnlen)479)];
    dscrec = lastrc[(i__1 = *type__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
	    "lastrc", i__1, "dascud_", (ftnlen)480)];
    if (*type__ == 2) {
	nw = 128;
	rngloc = 5;
    } else if (*type__ == 3) {
...
/*     no directories in the file yet. */
...
/*        The last logical address of TYPE is now MAXADR. */
	...
	lastla[(i__1 = *type__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("lastla",
		 i__1, "dascud_", (ftnlen)585)] = maxadr;
...
/*        free record follows the last data record in use. */
	...
	lastla[(i__1 = *type__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("lastla",
		 i__1, "dascud_", (ftnlen)642)] = maxadr;
	free += needed;
...
/*           directory. */
	    ...
	    dirrec[(i__1 = rngloc - 1) < 256 && 0 <= i__1 ? i__1 : s_rnge(
		    "dirrec", i__1, "dascud_", (ftnlen)687)] = 1;
	    dirrec[(i__1 = rngloc) < 256 && 0 <= i__1 ? i__1 : s_rnge("dirrec"
		    , i__1, "dascud_", (ftnlen)688)] = *nwords;
...
/*           The first free record follows the last data record in use. */
	    ...
	    lastla[(i__1 = *type__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		    "lastla", i__1, "dascud_", (ftnlen)705)] = *nwords;
	    lastrc[(i__1 = *type__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		    "lastrc", i__1, "dascud_", (ftnlen)706)] = recno;
	    lastwd[(i__1 = *type__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		    "lastwd", i__1, "dascud_", (ftnlen)707)] = 10;
	    dasufs_(handle, &nresvr, &nresvc, &ncomr, &ncomc, &free, lastla, 
		    lastrc, lastwd);
	} else if (lword < 256) {
...
/*           the current last descriptor. */
	    ...
	    if (*type__ == next[(i__1 = ltype - 1) < 3 && 0 <= i__1 ? i__1 : 
		    s_rnge("next", i__1, "dascud_", (ftnlen)753)]) {
...
/*           The first free record follows the last data record in use. */
	    ...
	    free += needed;
	    lastla[(i__1 = *type__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		    "lastla", i__1, "dascud_", (ftnlen)793)] = last + *nwords;
	    lastrc[(i__1 = *type__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		    "lastrc", i__1, "dascud_", (ftnlen)794)] = lrec;
	    lastwd[(i__1 = *type__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		    "lastwd", i__1, "dascud_", (ftnlen)795)] = lword + 1;
...
/*           initial data type, and record count. */
	    ...
	    dirrec[0] = lrec;
	    dirrec[(i__1 = rngloc - 1) < 256 && 0 <= i__1 ? i__1 : s_rnge(
		    "dirrec", i__1, "dascud_", (ftnlen)889)] = last + room + 
		    1;
	    dirrec[(i__1 = rngloc) < 256 && 0 <= i__1 ? i__1 : s_rnge("dirrec"
		    , i__1, "dascud_", (ftnlen)890)] = last + *nwords;
	    dirrec[8] = *type__;
	    dirrec[9] = needed;
...
/*           record lies after the added data records. */
	    ...
	    free += needed;
	    lastla[(i__1 = *type__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		    "lastla", i__1, "dascud_", (ftnlen)907)] = last + *nwords;
	    lastrc[(i__1 = *type__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		    "lastrc", i__1, "dascud_", (ftnlen)908)] = recno;
	    lastwd[(i__1 = *type__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		    "lastwd", i__1, "dascud_", (ftnlen)909)] = 10;
	    dasufs_(handle, &nresvr, &nresvc, &ncomr, &ncomc, &free, lastla, 
		    lastrc, lastwd);
	}
    }
