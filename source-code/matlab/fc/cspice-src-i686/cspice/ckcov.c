...
/*     1)  Display the coverage for each object in a specified CK file. */
...
/*              DO I = 1, CARDI( IDS ) */
/*        C */
/*        C        Find the coverage window for the current object. */
/*        C */
...
/*        provided by the set of CK files loaded via a metakernel. */
/*        (The metakernel must also specify leapseconds and */
/*        SCLK kernels.) */
...
/*                 into the window. */
			...
			dctol[(i__1 = i__ - 1) < 2 && 0 <= i__1 ? i__1 : 
				s_rnge("dctol", i__1, "ckcov_", (ftnlen)855)] 
				= et;
		    }
		}
		if (dctol[0] <= dctol[1]) {
