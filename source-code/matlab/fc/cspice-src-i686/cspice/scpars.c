...
/*        'SPICE(NOTSUPPORTED)' is signalled. */
...
/*     partition plus all preceding partitions. */
	...
	d__1 = ptotls[(i__3 = i__ - 2) < 50 && 0 <= i__3 ? i__3 : s_rnge(
		"ptotls", i__3, "scpars_", (ftnlen)574)] + pstop[(i__4 = i__ 
		- 1) < 50 && 0 <= i__4 ? i__4 : s_rnge("pstop", i__4, "scpar"
		"s_", (ftnlen)574)] - pstart[(i__5 = i__ - 1) < 50 && 0 <= 
		i__5 ? i__5 : s_rnge("pstart", i__5, "scpars_", (ftnlen)574)];
	ptotls[(i__2 = i__ - 1) < 50 && 0 <= i__2 ? i__2 : s_rnge("ptotls", 
		i__2, "scpars_", (ftnlen)574)] = d_nint(&d__1);
    }
...
/*           numbers. */
	    ...
	    return 0;
		 ...
		 s_rnge("pstart", i__1, "scpars_", (ftnlen)659)] || ticks > 
		pstop[(i__2 = part - 1) < 50 && 0 <= i__2 ? i__2 : s_rnge(
		"pstop", i__2, "scpars_", (ftnlen)659)]) {
...
/*        clock string converted to. */
	...
	while(part <= nparts && (ticks < pstart[(i__1 = part - 1) < 50 && 0 <=
		 i__1 ? i__1 : s_rnge("pstart", i__1, "scpars_", (ftnlen)688)]
		 || ticks > pstop[(i__2 = part - 1) < 50 && 0 <= i__2 ? i__2 :
		 s_rnge("pstop", i__2, "scpars_", (ftnlen)688)])) {
	    ++part;
	}
	if (part > nparts) {
...
/*     and subtract off the starting ticks value for this partition. */
	...
	*sclkdp = ticks - pstart[(i__1 = part - 1) < 50 && 0 <= i__1 ? i__1 : 
		s_rnge("pstart", i__1, "scpars_", (ftnlen)719)] + ptotls[(
		i__2 = part - 2) < 50 && 0 <= i__2 ? i__2 : s_rnge("ptotls", 
		i__2, "scpars_", (ftnlen)719)];
    } else {
	*sclkdp = ticks - pstart[(i__1 = part - 1) < 50 && 0 <= i__1 ? i__1 : 
		s_rnge("pstart", i__1, "scpars_", (ftnlen)721)];
    }
    *error = FALSE_;
