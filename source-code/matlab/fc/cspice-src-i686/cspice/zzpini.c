...
/* Subroutine */ int zzpini_(logical *first, integer *maxvar, integer *maxval,
	...
	integer *maxagt, integer *mxnote, char *watsym, integer *watptr, char 
	*watval, char *agents, char *active, char *notify, ftnlen begdat_len, 
	ftnlen begtxt_len, ftnlen watsym_len, ftnlen watval_len, ftnlen 
	agents_len, ftnlen active_len, ftnlen notify_len)
{
    ...
    extern /* Subroutine */ int lnkini_(integer *, integer *), ssizec_(
	    integer *, char *, ftnlen), chkout_(char *, ftnlen), ssizei_(
	    integer *, integer *);
...
/*      WATSYM     O   Name array of watcher symbol table */
...
/*      WATSYM     is a symbol table of variables to watch for.  WATSYM */
...
/*     Local Parameters and Variables */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    namlst[(i__2 = i__ - 1) < namlst_dim1 && 0 <= i__2 ? i__2 : 
		    s_rnge("namlst", i__2, "zzpini_", (ftnlen)278)] = 0;
	    datlst[(i__2 = i__ - 1) < datlst_dim1 && 0 <= i__2 ? i__2 : 
		    s_rnge("datlst", i__2, "zzpini_", (ftnlen)279)] = 0;
	}
	...
	ssizec_(maxvar, watsym, watsym_len);
	ssizei_(maxvar, watptr);
	ssizec_(mxnote, watval, watval_len);
	ssizec_(maxagt, agents, agents_len);
	ssizec_(maxagt, active, active_len);
	ssizec_(maxagt, notify, notify_len);
	if (! failed_()) {
	    *first = FALSE_;
	}
