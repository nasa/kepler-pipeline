	...
	ref_len, ftnlen abcorr_len)
{
    ...
    extern /* Subroutine */ int chkout_(char *, ftnlen), spkapp_(integer *, 
...
/*        call SPKSSB, SPKAPP and return. */

	if (type__ == 1) {
	    spkssb_(obs, et, ref, sobs, ref_len);
	    spkapp_(targ, et, ref, sobs, abcorr, starg, lt, ref_len, 
		    abcorr_len);
	    chkout_("SPKEZ", (ftnlen)5);
	    return 0;
	}
...
/*        of the target relative to the observer. */
	...
	spkapp_(targ, et, "J2000", sobs, abcorr, state, lt, (ftnlen)5, 
		abcorr_len);
	if (failed_()) {
	    chkout_("SPKEZ", (ftnlen)5);
	    return 0;
	...
	if (center == *obs) {
	    ...
	    spkapp_(&center, et, "J2000", sobs, abcorr, temp, &ltcent, (
		    ftnlen)5, abcorr_len);
	}
