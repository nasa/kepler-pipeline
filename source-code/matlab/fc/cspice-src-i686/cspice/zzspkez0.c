	...
	ref_len, ftnlen abcorr_len)
{
    ...
    extern /* Subroutine */ int zzfrmch0_(integer *, integer *, doublereal *, 
	    doublereal *), zzspkap0_(integer *, doublereal *, char *, 
...
/*        ZZSPKEZ0 begins by computing the geometric position T(ET) of */
...
/*        for ABCORR, ZZSPKEZ0 computes the position of the target body */
...
/*        selected, ZZSPKEZ0 computes the position of the target body T */
...
/*                  CALL ZZSPKEZ0 ( TARGET, ET, FRAME, ABCORR, OBSRVR, */
/*              .                STATE,  LT                        ) */
...
/*        call ZZSPKSB0, ZZSPKAP0 and return. */

	if (type__ == 1) {
	    zzspksb0_(obs, et, ref, sobs, ref_len);
	    zzspkap0_(targ, et, ref, sobs, abcorr, starg, lt, ref_len, 
		    abcorr_len);
	    chkout_("ZZSPKEZ0", (ftnlen)8);
	    return 0;
	}
...
/*        of the target relative to the observer. */
	...
	zzspkap0_(targ, et, "J2000", sobs, abcorr, state, lt, (ftnlen)5, 
		abcorr_len);
	if (failed_()) {
	    chkout_("ZZSPKEZ0", (ftnlen)8);
	    return 0;
	...
	if (center == *obs) {
	    ...
	    zzspkap0_(&center, et, "J2000", sobs, abcorr, temp, &ltcent, (
		    ftnlen)5, abcorr_len);
	}
