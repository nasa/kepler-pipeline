	...
	ftnlen sclkch_len)
{
    ...
    integer nparts;
    doublereal pstart[50];
    extern logical return_(void);
    doublereal ptotls[50];
    integer pos;
...
/*                using the routines CLPOOL and LDPOOL. */
...
/*        'SPICE(NOPARTITION)' is signalled. */

/*     The following error is signalled by a routine called by SCENCD */
...
/*     CLPOOL and LDPOOL, before calling this routine. */
...
/*     numbers so that logical tests may be performed with them. */
	...
	pstop[(i__2 = i__ - 1) < 50 && 0 <= i__2 ? i__2 : s_rnge("pstop", 
		i__2, "scencd_", (ftnlen)467)] = d_nint(&pstop[(i__3 = i__ - 
		1) < 50 && 0 <= i__3 ? i__3 : s_rnge("pstop", i__3, "scencd_",
		 (ftnlen)467)]);
	pstart[(i__2 = i__ - 1) < 50 && 0 <= i__2 ? i__2 : s_rnge("pstart", 
		i__2, "scencd_", (ftnlen)468)] = d_nint(&pstart[(i__3 = i__ - 
		1) < 50 && 0 <= i__3 ? i__3 : s_rnge("pstart", i__3, "scencd_"
		, (ftnlen)468)]);
    }
...
/*     partition plus all preceding partitions. */
	...
	d__1 = ptotls[(i__3 = i__ - 2) < 50 && 0 <= i__3 ? i__3 : s_rnge(
		"ptotls", i__3, "scencd_", (ftnlen)479)] + pstop[(i__4 = i__ 
		- 1) < 50 && 0 <= i__4 ? i__4 : s_rnge("pstop", i__4, "scenc"
		"d_", (ftnlen)479)] - pstart[(i__5 = i__ - 1) < 50 && 0 <= 
		i__5 ? i__5 : s_rnge("pstart", i__5, "scencd_", (ftnlen)479)];
	ptotls[(i__2 = i__ - 1) < 50 && 0 <= i__2 ? i__2 : s_rnge("ptotls", 
		i__2, "scencd_", (ftnlen)479)] = d_nint(&d__1);
    }
...
/*              of the partitions. */
	...
	if (s_cmp(error, " ", (ftnlen)25, (ftnlen)1) != 0) {
	    ...
	    chkout_("SCENCD", (ftnlen)6);
	    return 0;
	} else if (ticks < pstart[(i__1 = part - 1) < 50 && 0 <= i__1 ? i__1 :
		 s_rnge("pstart", i__1, "scencd_", (ftnlen)542)] || ticks > 
		pstop[(i__2 = part - 1) < 50 && 0 <= i__2 ? i__2 : s_rnge(
		"pstop", i__2, "scencd_", (ftnlen)542)]) {
	...
	part = 1;
	while(part <= nparts && (ticks < pstart[(i__1 = part - 1) < 50 && 0 <=
		 i__1 ? i__1 : s_rnge("pstart", i__1, "scencd_", (ftnlen)559)]
		 || ticks > pstop[(i__2 = part - 1) < 50 && 0 <= i__2 ? i__2 :
		 s_rnge("pstop", i__2, "scencd_", (ftnlen)559)])) {
	    ++part;
	}
	if (part > nparts) {
...
/*     and subtract off the starting ticks value for this partition. */
    ...
    if (part > 1) {
	*sclkdp = ticks - pstart[(i__1 = part - 1) < 50 && 0 <= i__1 ? i__1 : 
		s_rnge("pstart", i__1, "scencd_", (ftnlen)589)] + ptotls[(
		i__2 = part - 2) < 50 && 0 <= i__2 ? i__2 : s_rnge("ptotls", 
		i__2, "scencd_", (ftnlen)589)];
    } else {
	*sclkdp = ticks - pstart[(i__1 = part - 1) < 50 && 0 <= i__1 ? i__1 : 
		s_rnge("pstart", i__1, "scencd_", (ftnlen)591)];
    }
    chkout_("SCENCD", (ftnlen)6);
    return 0;
} /* scencd_ */
