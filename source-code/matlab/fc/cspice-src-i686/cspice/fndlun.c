...
/*     Initialize RESVD if it hasn't already been done. */
	...
	for (i__ = 1; i__ <= 99; ++i__) {
	    resvd[(i__1 = i__ - 1) < 99 && 0 <= i__1 ? i__1 : s_rnge("resvd", 
		    i__1, "fndlun_", (ftnlen)439)] = FALSE_;
	}
	for (i__ = 1; i__ <= 3; ++i__) {
	    resvd[(i__2 = resnum[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : 
		    s_rnge("resnum", i__1, "fndlun_", (ftnlen)443)] - 1) < 99 
...
/*     Initialize RESVD if it hasn't already been done. */
	...
	for (i__ = 1; i__ <= 99; ++i__) {
	    resvd[(i__1 = i__ - 1) < 99 && 0 <= i__1 ? i__1 : s_rnge("resvd", 
		    i__1, "fndlun_", (ftnlen)707)] = FALSE_;
	}
	for (i__ = 1; i__ <= 3; ++i__) {
	    resvd[(i__2 = resnum[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : 
		    s_rnge("resnum", i__1, "fndlun_", (ftnlen)711)] - 1) < 99 
		    && 0 <= i__2 ? i__2 : s_rnge("resvd", i__2, "fndlun_", (
		    ftnlen)711)] = TRUE_;
	}
	first = FALSE_;
...
/*     Initialize RESVD if it hasn't already been done. */
	...
	for (i__ = 1; i__ <= 99; ++i__) {
	    resvd[(i__1 = i__ - 1) < 99 && 0 <= i__1 ? i__1 : s_rnge("resvd", 
		    i__1, "fndlun_", (ftnlen)928)] = FALSE_;
	}
	for (i__ = 1; i__ <= 3; ++i__) {
	    resvd[(i__2 = resnum[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : 
		    s_rnge("resnum", i__1, "fndlun_", (ftnlen)932)] - 1) < 99 
		    && 0 <= i__2 ? i__2 : s_rnge("resvd", i__2, "fndlun_", (
		    ftnlen)932)] = TRUE_;
	}
	first = FALSE_;
...
/*     default, set the corresponding flag to FALSE. */
	...
	for (i__ = 1; i__ <= 3; ++i__) {
	    if (*unit == resnum[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : 
		    s_rnge("resnum", i__1, "fndlun_", (ftnlen)946)]) {
		return 0;
	    }
	}
	resvd[(i__1 = *unit - 1) < 99 && 0 <= i__1 ? i__1 : s_rnge("resvd", 
		i__1, "fndlun_", (ftnlen)951)] = FALSE_;
    }
    return 0;
} /* fndlun_ */
