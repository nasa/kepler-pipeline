...
/*     using the higher-level SPICELIB routine SUBPT.  This routine */
/*     is the mathematical workhorse on which SUBPT relies. */
...
/*     equal to zero. If one is, signal an error and return. */
	...
	setmsg_(mssg + ((i__1 = bad - 1) < 7 && 0 <= i__1 ? i__1 : s_rnge(
		"mssg", i__1, "nearpt_", (ftnlen)548)) * 80, (ftnlen)80);
...
/*     POSITN.  The difference between the the approximated position of */
...
/*     Note the first axis has length 1.D0, so we don't check it. */
	...
	if (axis[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("axis", 
		i__1, "nearpt_", (ftnlen)776)] > toobig) {
	    ...
	    errint_("*", &iorder[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : 
		    s_rnge("iorder", i__1, "nearpt_", (ftnlen)781)], (ftnlen)
		    1);
	    errint_("*", iorder, (ftnlen)1);
	    errdp_("*", &axis[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : 
		    s_rnge("axis", i__1, "nearpt_", (ftnlen)783)], (ftnlen)1);
	    sigerr_("SPICE(BADAXISLENGTH)", (ftnlen)20);
	    chkout_("NEARPT", (ftnlen)6);
	    return 0;
	}
...
/*     the square roots of the factors to TOOBIG. */
	...
	prodct = sqrt(axis[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		"axis", i__1, "nearpt_", (ftnlen)802)]) * sqrt((d__1 = point[(
		i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : s_rnge("point", 
		i__2, "nearpt_", (ftnlen)802)], abs(d__1)));
	if (prodct > toobig) {
	    ...
	    errint_("*", &iorder[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : 
		    s_rnge("iorder", i__1, "nearpt_", (ftnlen)810)], (ftnlen)
		    1);
...
/*        of AXIS, is in fact zero. */
	...
	for (i__ = 1; i__ <= 3; ++i__) {
	    if (point[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("poi"
		    "nt", i__1, "nearpt_", (ftnlen)860)] * .5 + axis[(i__2 = 
		    i__ - 1) < 3 && 0 <= i__2 ? i__2 : s_rnge("axis", i__2, 
		    "nearpt_", (ftnlen)860)] == axis[(i__3 = i__ - 1) < 3 && 
		    0 <= i__3 ? i__3 : s_rnge("axis", i__3, "nearpt_", (
		    ftnlen)860)] || point[(i__4 = i__ - 1) < 3 && 0 <= i__4 ? 
		    i__4 : s_rnge("point", i__4, "nearpt_", (ftnlen)860)] * 
		    .5 - axis[(i__5 = i__ - 1) < 3 && 0 <= i__5 ? i__5 : 
		    s_rnge("axis", i__5, "nearpt_", (ftnlen)860)] == -axis[(
		    i__6 = i__ - 1) < 3 && 0 <= i__6 ? i__6 : s_rnge("axis", 
		    i__6, "nearpt_", (ftnlen)860)]) {
		point[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("poi"
			"nt", i__1, "nearpt_", (ftnlen)863)] = 0.;
	    }
	}
...
/*           term of the expression for Q isn't bigger than 4. */
		...
		tlambd[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
			"tlambd", i__1, "nearpt_", (ftnlen)968)] = ((d__1 = 
			point[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : 
			s_rnge("point", i__2, "nearpt_", (ftnlen)968)], abs(
			d__1)) * .5 - axis[(i__3 = i__ - 1) < 3 && 0 <= i__3 ?
			 i__3 : s_rnge("axis", i__3, "nearpt_", (ftnlen)968)])
			 * axis[(i__4 = i__ - 1) < 3 && 0 <= i__4 ? i__4 : 
			s_rnge("axis", i__4, "nearpt_", (ftnlen)968)];
	    }
/* Computing MAX */
...
/*           one. */
		...
		if (point[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
			"point", i__1, "nearpt_", (ftnlen)1022)] != 0.) {
		    snglpt = i__;
		}
	    }
...
/*           largest term of Q is equal to 4. */
		...
		for (i__ = 1; i__ <= 3; ++i__) {
		    if (point[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : 
			    s_rnge("point", i__1, "nearpt_", (ftnlen)1036)] ==
			     0.) {
			...
			tlambd[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : 
				s_rnge("tlambd", i__1, "nearpt_", (ftnlen)
				1039)] = axis[(i__2 = i__ - 1) < 3 && 0 <= 
				i__2 ? i__2 : s_rnge("axis", i__2, "nearpt_", 
				(ftnlen)1039)] * ((d__1 = point[(i__3 = i__ - 
				1) < 3 && 0 <= i__3 ? i__3 : s_rnge("point", 
				i__3, "nearpt_", (ftnlen)1039)], abs(d__1)) * 
...
/*        values of LAMBDA. */
	...
	for (i__ = 1; i__ <= 3; ++i__) {
	    if (point[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("poi"
		    "nt", i__1, "nearpt_", (ftnlen)1096)] == 0.) {
		term[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("term",
			 i__1, "nearpt_", (ftnlen)1098)] = 0.;
	    } else {
...
/*              division overflows. */
		...
		denom = axis[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : 
			s_rnge("axis", i__1, "nearpt_", (ftnlen)1112)] + 
			lambda / axis[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? 
			i__2 : s_rnge("axis", i__2, "nearpt_", (ftnlen)1112)];
		trim = (d__1 = point[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 
			: s_rnge("point", i__1, "nearpt_", (ftnlen)1114)], 
			abs(d__1)) * .5 > denom;
		if (inside && trim) {
		    factor = 2.;
		} else {
...
/*                 check anyway. */
		    ...
		    factor = point[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : 
			    s_rnge("point", i__1, "nearpt_", (ftnlen)1136)] / 
			    denom;
		}
		term[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("term",
			 i__1, "nearpt_", (ftnlen)1140)] = factor * factor;
	    }
	}
	if (! inside) {
	    qlower = q;
...
/*                       check anyway. */
			...
			term[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : 
				s_rnge("term", i__1, "nearpt_", (ftnlen)1329)]
				 = factor * factor;
		    }
		}
...
/*       this value. */
	...
	for (i__ = 1; i__ <= 3; ++i__) {
	    if (point[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("poi"
		    "nt", i__1, "nearpt_", (ftnlen)1355)] == 0.) {
		spoint[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
			"spoint", i__1, "nearpt_", (ftnlen)1357)] = 0.;
	    } else {
		denom = lambda / axisqr[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? 
			i__1 : s_rnge("axisqr", i__1, "nearpt_", (ftnlen)1361)
			] + 1.;
...
/*             check for this case anyway. */
		...
		spoint[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
			"spoint", i__1, "nearpt_", (ftnlen)1378)] = copy[(
			i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : s_rnge(
			"copy", i__2, "nearpt_", (ftnlen)1378)] / denom;
	    }
	}
...
/*               | POINT(3) | <= c - a*(a/c) */
	    ...
	    if (axis[0] != axis[(i__1 = snglpt - 1) < 3 && 0 <= i__1 ? i__1 : 
		    s_rnge("axis", i__1, "nearpt_", (ftnlen)1535)] && abs(
...
/*           solution point by the computed altitude. */
		...
		err[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("err", 
			i__1, "nearpt_", (ftnlen)1665)] = orignl[(i__2 = i__ 
			- 1) < 3 && 0 <= i__2 ? i__2 : s_rnge("orignl", i__2, 
			"nearpt_", (ftnlen)1665)] - spoint[(i__3 = i__ - 1) < 
			3 && 0 <= i__3 ? i__3 : s_rnge("spoint", i__3, "near"
			"pt_", (ftnlen)1665)] - height * normal[(i__4 = i__ - 
...
/*              ORIGNL - SPOINT - HEIGHT*NORMAL, */
	    ...
	    for (i__ = 1; i__ <= 3; ++i__) {
		err[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("err", 
			i__1, "nearpt_", (ftnlen)1758)] = orignl[(i__2 = i__ 
			- 1) < 3 && 0 <= i__2 ? i__2 : s_rnge("orignl", i__2, 
			"nearpt_", (ftnlen)1758)] - spoint[(i__3 = i__ - 1) < 
			3 && 0 <= i__3 ? i__3 : s_rnge("spoint", i__3, "near"
			"pt_", (ftnlen)1758)] - height * normal[(i__4 = i__ - 
			1) < 3 && 0 <= i__4 ? i__4 : s_rnge("normal", i__4, 
			"nearpt_", (ftnlen)1758)];
	    }
...
/*     and copy the value of BESTHT into the output argument. */
	...
	npoint[(i__2 = iorder[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : 
		s_rnge("iorder", i__1, "nearpt_", (ftnlen)1857)] - 1) < 3 && 
		0 <= i__2 ? i__2 : s_rnge("npoint", i__2, "nearpt_", (ftnlen)
		1857)] = bestpt[(i__3 = i__ - 1) < 3 && 0 <= i__3 ? i__3 : 
		s_rnge("bestpt", i__3, "nearpt_", (ftnlen)1857)];
    }
    *alt = bestht / scale;
    chkout_("NEARPT", (ftnlen)6);
    return 0;
