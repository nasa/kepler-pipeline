...
/*        existance of binary PCK files, search the for */
...
/*        as appropriate. */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    theta = (tcoef[(i__2 = (i__ << 1) - 2) < 200 && 0 <= i__2 ? i__2 :
		     s_rnge("tcoef", i__2, "tisbod_", (ftnlen)887)] + epoch / 
		    t * tcoef[(i__3 = (i__ << 1) - 1) < 200 && 0 <= i__3 ? 
		    i__3 : s_rnge("tcoef", i__3, "tisbod_", (ftnlen)887)]) * 
		    rpd_();
	    dtheta = tcoef[(i__2 = (i__ << 1) - 1) < 200 && 0 <= i__2 ? i__2 :
		     s_rnge("tcoef", i__2, "tisbod_", (ftnlen)888)] / t * 
		    rpd_();
	    sinth[(i__2 = i__ - 1) < 100 && 0 <= i__2 ? i__2 : s_rnge("sinth",
		     i__2, "tisbod_", (ftnlen)890)] = sin(theta);
	    costh[(i__2 = i__ - 1) < 100 && 0 <= i__2 ? i__2 : s_rnge("costh",
		     i__2, "tisbod_", (ftnlen)891)] = cos(theta);
	    dsinth[(i__2 = i__ - 1) < 100 && 0 <= i__2 ? i__2 : s_rnge("dsin"
		    "th", i__2, "tisbod_", (ftnlen)892)] = cos(theta) * dtheta;
	    dcosth[(i__2 = i__ - 1) < 100 && 0 <= i__2 ? i__2 : s_rnge("dcos"
		    "th", i__2, "tisbod_", (ftnlen)893)] = -sin(theta) * 
		    dtheta;
	}
...
/*        Extract the upper and lower left blocks of TSIPM. */
	...
	for (i__ = 1; i__ <= 3; ++i__) {
		...
		tipm[(i__1 = i__ + j * 3 - 4) < 9 && 0 <= i__1 ? i__1 : 
			s_rnge("tipm", i__1, "tisbod_", (ftnlen)991)] = tsipm[
			(i__2 = i__ + j * 6 - 7) < 36 && 0 <= i__2 ? i__2 : 
			s_rnge("tsipm", i__2, "tisbod_", (ftnlen)991)];
		dtipm[(i__1 = i__ + j * 3 - 4) < 9 && 0 <= i__1 ? i__1 : 
			s_rnge("dtipm", i__1, "tisbod_", (ftnlen)992)] = 
			tsipm[(i__2 = i__ + 3 + j * 6 - 7) < 36 && 0 <= i__2 ?
			 i__2 : s_rnge("tsipm", i__2, "tisbod_", (ftnlen)992)]
			;
	    }
	}
...
/*        bodyfixed frame. */
	...
	for (i__ = 1; i__ <= 3; ++i__) {
		...
		tsipm[(i__1 = i__ + j * 6 - 7) < 36 && 0 <= i__1 ? i__1 : 
			s_rnge("tsipm", i__1, "tisbod_", (ftnlen)1013)] = 
			xtipm[(i__2 = i__ + j * 3 - 4) < 9 && 0 <= i__2 ? 
			i__2 : s_rnge("xtipm", i__2, "tisbod_", (ftnlen)1013)]
			;
		tsipm[(i__1 = i__ + 3 + (j + 3) * 6 - 7) < 36 && 0 <= i__1 ? 
			i__1 : s_rnge("tsipm", i__1, "tisbod_", (ftnlen)1014)]
			 = xtipm[(i__2 = i__ + j * 3 - 4) < 9 && 0 <= i__2 ? 
			i__2 : s_rnge("xtipm", i__2, "tisbod_", (ftnlen)1014)]
			;
		tsipm[(i__1 = i__ + 3 + j * 6 - 7) < 36 && 0 <= i__1 ? i__1 : 
			s_rnge("tsipm", i__1, "tisbod_", (ftnlen)1015)] = 
			xdtipm[(i__2 = i__ + j * 3 - 4) < 9 && 0 <= i__2 ? 
			i__2 : s_rnge("xdtipm", i__2, "tisbod_", (ftnlen)1015)
			];
	    }
	}
    }
