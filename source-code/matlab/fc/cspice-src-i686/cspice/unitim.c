...
/*     (normally by loading a leapseconds kernel with LDPOOL) prior to */
...
/*        Fetch all of the time parameters from the kernel pool. */
	...
	rtpool_("DELTET/DELTA_T_A", &n, &dta, found, (ftnlen)16);
	rtpool_("DELTET/K", &n, &k, &found[1], (ftnlen)8);
	rtpool_("DELTET/EB", &n, &eb, &found[2], (ftnlen)9);
	rtpool_("DELTET/M", &n, m, &found[3], (ftnlen)8);
...
/*           needed data. */
	    ...
	    swpool_("UNITIM", &c__4, vars__, (ftnlen)6, (ftnlen)16);
	    setmsg_("The following, needed to convert between the uniform ti"
		    "me scales, could not be found in the kernel pool: #", (
		    ftnlen)106);
	    for (i__ = 1; i__ <= 4; ++i__) {
		if (! found[(i__1 = i__ - 1) < 4 && 0 <= i__1 ? i__1 : s_rnge(
			"found", i__1, "unitim_", (ftnlen)404)]) {
		    errch_("#", missed + ((i__1 = i__ - 1) < 4 && 0 <= i__1 ? 
			    i__1 : s_rnge("missed", i__1, "unitim_", (ftnlen)
			    405)) * 20, (ftnlen)1, (ftnlen)20);
		}
	    }
