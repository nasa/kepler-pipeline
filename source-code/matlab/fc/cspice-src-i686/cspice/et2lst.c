...
static doublereal c_b22 = -43200.;
static doublereal c_b23 = 43200.;
static doublereal c_b24 = 3600.;
static doublereal c_b25 = 60.;
...
/*     Suitable SPK and PCK files must be loaded prior to calling */
/*     this routine so that the body-fixed position of the sun */
/*     relative to BODY can be computed. */
...
/*        the input planetographic longitude. We then find the planeto- */
/*        centric longitude of this point. */
...
/*     Convert the angle to "angle seconds" before or after local noon. */
    ...
    secnds = brcktd_(&secnds, &c_b22, &c_b23);

/*     Get the hour, and minutes components of the local time. */

    rmaind_(&secnds, &c_b24, &hours, &tmpsec);
    rmaind_(&tmpsec, &c_b25, &mins, &secnds);
