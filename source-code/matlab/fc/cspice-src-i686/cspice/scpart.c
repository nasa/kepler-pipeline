	 ...
	 doublereal *pstop)
{
    ...
    static doublereal prtsa[50], prtso[50];
    char kvname[60*2];
    logical update;
...
/*        Make up a list of names of kernel variables that we'll use. */
	...
	for (i__ = 1; i__ <= 2; ++i__) {
	    suffix_("_#", &c__0, kvname + ((i__1 = i__ - 1) < 2 && 0 <= i__1 ?
		     i__1 : s_rnge("kvname", i__1, "scpart_", (ftnlen)255)) * 
		    60, (ftnlen)2, (ftnlen)60);
	    i__3 = -(*sc);
	    repmi_(kvname + ((i__1 = i__ - 1) < 2 && 0 <= i__1 ? i__1 : 
		    s_rnge("kvname", i__1, "scpart_", (ftnlen)256)) * 60, 
		    "#", &i__3, kvname + ((i__2 = i__ - 1) < 2 && 0 <= i__2 ? 
		    i__2 : s_rnge("kvname", i__2, "scpart_", (ftnlen)256)) * 
...
/*        Read the values from the kernel pool. */
	...
	scld01_("SCLK_PARTITION_START", sc, &c__50, &nprtsa, prtsa, (ftnlen)
		20);
	scld01_("SCLK_PARTITION_END", sc, &c__50, &nprtso, prtso, (ftnlen)18);
...
/*        The partition start and stop times must be whole numbers. */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    prtsa[(i__2 = i__ - 1) < 50 && 0 <= i__2 ? i__2 : s_rnge("prtsa", 
		    i__2, "scpart_", (ftnlen)305)] = d_nint(&prtsa[(i__3 = 
		    i__ - 1) < 50 && 0 <= i__3 ? i__3 : s_rnge("prtsa", i__3, 
		    "scpart_", (ftnlen)305)]);
	    prtso[(i__2 = i__ - 1) < 50 && 0 <= i__2 ? i__2 : s_rnge("prtso", 
		    i__2, "scpart_", (ftnlen)306)] = d_nint(&prtso[(i__3 = 
		    i__ - 1) < 50 && 0 <= i__3 ? i__3 : s_rnge("prtso", i__3, 
		    "scpart_", (ftnlen)306)]);
	}
    }
...
/*     Copy the values in local buffers to the output arguments. */
    ...
    *nparts = lstprt;
    i__1 = *nparts;
    for (i__ = 1; i__ <= i__1; ++i__) {
	pstart[i__ - 1] = prtsa[(i__2 = i__ - 1) < 50 && 0 <= i__2 ? i__2 : 
		s_rnge("prtsa", i__2, "scpart_", (ftnlen)317)];
	pstop[i__ - 1] = prtso[(i__2 = i__ - 1) < 50 && 0 <= i__2 ? i__2 : 
		s_rnge("prtso", i__2, "scpart_", (ftnlen)318)];
    }
    chkout_("SCPART", (ftnlen)6);
    return 0;
} /* scpart_ */
