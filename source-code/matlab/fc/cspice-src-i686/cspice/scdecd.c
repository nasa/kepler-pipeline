...
/*     LDPOOL, before calling this routine. */
...
/*     Since the maximum number of partitions is given by MXPART */
/*     as 50, PRTSTR needs at most 2 characters for the partition */
/*     number and one character for the slash. */
...
/*     partition plus all preceding partitions. */
	...
	d__1 = ptotls[(i__3 = i__ - 2) < 50 && 0 <= i__3 ? i__3 : s_rnge(
		"ptotls", i__3, "scdecd_", (ftnlen)488)] + pstop[(i__4 = i__ 
		- 1) < 50 && 0 <= i__4 ? i__4 : s_rnge("pstop", i__4, "scdec"
		"d_", (ftnlen)488)] - pstart[(i__5 = i__ - 1) < 50 && 0 <= 
		i__5 ? i__5 : s_rnge("pstart", i__5, "scdecd_", (ftnlen)488)];
	ptotls[(i__2 = i__ - 1) < 50 && 0 <= i__2 ? i__2 : s_rnge("ptotls", 
		i__2, "scdecd_", (ftnlen)488)] = d_nint(&d__1);
    }
...
/*     negative), or after the last one. */
    ...
    if (ticks == ptotls[(i__1 = nparts - 1) < 50 && 0 <= i__1 ? i__1 : s_rnge(
	    "ptotls", i__1, "scdecd_", (ftnlen)503)]) {
	part = nparts;
    } else {
...
/*     this partition. */
    ...
    if (part == 1) {
	ticks += pstart[(i__1 = part - 1) < 50 && 0 <= i__1 ? i__1 : s_rnge(
		"pstart", i__1, "scdecd_", (ftnlen)528)];
    } else {
	ticks = ticks + pstart[(i__1 = part - 1) < 50 && 0 <= i__1 ? i__1 : 
		s_rnge("pstart", i__1, "scdecd_", (ftnlen)530)] - ptotls[(
		i__2 = part - 2) < 50 && 0 <= i__2 ? i__2 : s_rnge("ptotls", 
		i__2, "scdecd_", (ftnlen)530)];
    }
...
/*     it to the output string. */
    ...
    intstr_(&part, prtstr, (ftnlen)3);
    suffix_("/", &c__0, prtstr, (ftnlen)1, (ftnlen)3);
    prelen = lastnb_(prtstr, (ftnlen)3);
    ...
    prefix_(prtstr, &c__0, sclkch, (ftnlen)3, sclkch_len);
    chkout_("SCDECD", (ftnlen)6);
    return 0;
} /* scdecd_ */
