...
/*        CALL ZZREFCH1 ( FRAME1, FRAME2, ET,   ROTATE ) */
...
/*        Based on SPICELIB Version 1.2.0, 26-APR-2004 (NJB) */
...
/*           for the next NODE. */
	    ...
	    i__1, "zzrefch1_", (ftnlen)326)] == 1 || frame[(i__2 = node - 1) <
	     10 && 0 <= i__2 ? i__2 : s_rnge("frame", i__2, "zzrefch1_", (
	    ftnlen)326)] == *frame2 || ! found;
    while(! done) {
...
/*        root classes or we run into FRAME2. */
	...
	zzrotgt1_(&frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge(
		"frame", i__1, "zzrefch1_", (ftnlen)340)], et, &rot[(i__2 = (
		node * 3 + 1) * 3 - 12) < 126 && 0 <= i__2 ? i__2 : s_rnge(
		"rot", i__2, "zzrefch1_", (ftnlen)340)], &relto, &found);
	if (found) {
...
/*           rotation from FRAME(NODE) to RELTO. */
	    ...
	    frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge("frame",
		     i__1, "zzrefch1_", (ftnlen)351)] = relto;
		...
		for (j = 1; j <= 3; ++j) {
		    rot[(i__1 = i__ + (j + (node - 1) * 3) * 3 - 13) < 126 && 
			    ...
			    , (ftnlen)356)] = tmprot[(i__2 = i__ + j * 3 - 4) 
			    < 9 && 0 <= i__2 ? i__2 : s_rnge("tmprot", i__2, 
			    "zzrefch1_", (ftnlen)356)];
		}
	    }
	}
...
/*        another rotation. */
		...
		"frame", i__1, "zzrefch1_", (ftnlen)366)] == 1 || frame[(i__2 
		= node - 1) < 10 && 0 <= i__2 ? i__2 : s_rnge("frame", i__2, 
		"zzrefch1_", (ftnlen)366)] == *frame2 || ! found;
    }
...
/*     We now have to do essentially the same thing for FRAME2. */
    ...
    if (frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge("frame", 
	    i__1, "zzrefch1_", (ftnlen)404)] == *frame2) {
...
/*     get from FRAME1 to FRAME2. */
	...
	frmnam_(&frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge(
		"frame", i__1, "zzrefch1_", (ftnlen)537)], name3, (ftnlen)32);
	...
	errint_("#", &frame[(i__1 = node - 1) < 10 && 0 <= i__1 ? i__1 : 
		s_rnge("frame", i__1, "zzrefch1_", (ftnlen)570)], (ftnlen)1);
	errint_("#", frame2, (ftnlen)1);
...
/*     result from FRAME1 to FRAME2. */
    ...
    xpose_(&rot2[(i__1 = (get * 3 + 1) * 3 - 12) < 18 && 0 <= i__1 ? i__1 : 
	    s_rnge("rot2", i__1, "zzrefch1_", (ftnlen)605)], &rot[(i__2 = (
	    cmnode * 3 + 1) * 3 - 12) < 126 && 0 <= i__2 ? i__2 : s_rnge(
	    "rot", i__2, "zzrefch1_", (ftnlen)605)]);
