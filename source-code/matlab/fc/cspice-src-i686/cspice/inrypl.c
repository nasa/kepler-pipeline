...
/*         SPICE(ZEROVECTOR) is signalled.  NXPTS and XPT are not */
/*         modified. */


/*     2)  If the ray's vertex is further than DPMAX() / 3 from the */
/*         origin, the error SPICE(VECTORTOOBIG) is signalled.  NXPTS */
...
/*      C */
/*      C     As a final step, tranform XPT to Saturn body-fixed */
/*      C     coordinates. */
/*      C */
/*            IF ( FOUND ) THEN */
...
/*     Find the projection of the direction vector along the the plane's */
/*     normal vector. */
