...
/*         kernel */
/*         CALL LDPOOL   ( '/Users/ewright/lib/geophysical.ker' ) */
/*         CALL LDPOOL   ( '/kernels/gen/lsk/naif0006.tls' ) */
...
/*        arrays. */
	...
	for (i__ = 1; i__ <= 8; ++i__) {
	    if (lstphs[(i__1 = i__ - 1) < 8 && 0 <= i__1 ? i__1 : s_rnge(
		    "lstphs", i__1, "dpspce_", (ftnlen)534)] != geophs[(i__2 =
	...
	for (i__ = 1; i__ <= 10; ++i__) {
	    if (lstelm[(i__1 = i__ - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge(
		    "lstelm", i__1, "dpspce_", (ftnlen)543)] != elems[(i__2 = 
...
/*        Save the geophysical constants for later comparison */
	...
	for (i__ = 1; i__ <= 8; ++i__) {
	    lstphs[(i__1 = i__ - 1) < 8 && 0 <= i__1 ? i__1 : s_rnge("lstphs",
		     i__1, "dpspce_", (ftnlen)578)] = geophs[(i__2 = i__ - 1) 
...
/*        Save the elements for later comparison */
	...
	for (i__ = 1; i__ <= 10; ++i__) {
	    lstelm[(i__1 = i__ - 1) < 10 && 0 <= i__1 ? i__1 : s_rnge("lstelm"
		    , i__1, "dpspce_", (ftnlen)598)] = elems[(i__2 = i__ - 1) 
