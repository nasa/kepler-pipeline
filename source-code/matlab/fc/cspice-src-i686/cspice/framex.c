	...
	found, ftnlen cname_len, ftnlen frname_len)
{
    ...
    static integer corder[117], centrd[117], center[117];
...
/*     --------  ---  -------------------------------------------------- */
/*     CNAME      I   CNMFRM */
/*     FRNAME    I/O  NAMFRM, FRMNAM */
/*     FRCODE    I/O  NAMFRM, FRMNAM, FRINFO, CIDFRM */
/*     CENT      I/O  FRINFO, CIDFRM */
/*     CLASS      O   FRINFO */
/*     CLSSID     O   FRINFO */
/*     FOUND      O   FRINFO */
...
/*     the parallel array. */
	...
	*frcode = idcode[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 : s_rnge(
		"idcode", i__1, "framex_", (ftnlen)592)];
    } else {
...
/*     Perform any needed first pass initializations. */
	...
	s_copy(frname, name__ + (((i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 
		: s_rnge("name", i__1, "framex_", (ftnlen)833)) << 5), 
...
/*        The following errors are now diagosed: */
...
/*     the parallel array. */
	...
	*cent = center[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 : s_rnge(
		"center", i__1, "framex_", (ftnlen)1138)];
	*class__ = type__[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 : 
		s_rnge("type", i__1, "framex_", (ftnlen)1139)];
	*clssid = typeid[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 : s_rnge(
		"typeid", i__1, "framex_", (ftnlen)1140)];
	*found = TRUE_;
    } else {
...
/*           of built-in names. */
		...
		*frcode = idcode[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 :
			 s_rnge("idcode", i__1, "framex_", (ftnlen)1482)];
		*found = TRUE_;
	    } else {
...
/*     name in the parallel array. */
    ...
    if (item > 0) {
	*frcode = idcode[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 : s_rnge(
		"idcode", i__1, "framex_", (ftnlen)1597)];
	s_copy(frname, name__ + (((i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 
		: s_rnge("name", i__1, "framex_", (ftnlen)1598)) << 5), 
		frname_len, (ftnlen)32);
	*found = TRUE_;
    } else {
...
/*           of built-in names. */
		...
		s_copy(frname, pname, frname_len, (ftnlen)32);
		*frcode = idcode[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 :
			 s_rnge("idcode", i__1, "framex_", (ftnlen)1891)];
		*found = TRUE_;
	    } else {
...
/*     name in the parallel array. */
    ...
    if (item > 0) {
	*frcode = idcode[(i__1 = item - 1) < 117 && 0 <= i__1 ? i__1 : s_rnge(
		"idcode", i__1, "framex_", (ftnlen)2003)];
	*found = TRUE_;
    } else {
