	...
	ref_len, ftnlen abcorr_len)
{
    ...
    extern /* Subroutine */ int zzfrmch1_(integer *, integer *, doublereal *, 
	    doublereal *), zzspkap1_(integer *, doublereal *, char *, 
	    doublereal *, char *, doublereal *, doublereal *, ftnlen, ftnlen);
    static doublereal temp[6], sobs[6];
    extern /* Subroutine */ int zzspkgo1_(integer *, doublereal *, char *, 
	    integer *, doublereal *, doublereal *, ftnlen), zzspksb1_(integer 
    ...
    static doublereal xform[36]	/* was [6][6] */;
...
/*        ZZSPKEZ1 begins by computing the geometric position T(ET) of */
...
/*        for ABCORR, ZZSPKEZ1 computes the position of the target body */
...
/*        selected, ZZSPKEZ1 computes the position of the target body T */
...
/*                  CALL ZZSPKEZ1 ( TARGET, ET, FRAME, ABCORR, OBSRVR, */
/*              .                STATE,  LT                        ) */
...
/*       Based on SPICELIB Version 4.1.0, 05-JAN-2005 (NJB) */
...
/*     If we only want geometric states, then compute just that. */

/*     Otherwise, compute the state of the observer relative to */
/*     the SSB.  Then feed that state into SPKAPP to compute the */
/*     apparent state of the target body relative to the observer */
...
/*        call ZZSPKSB1, ZZSPKAP1 and return. */

	if (type__ == 1) {
	    zzspksb1_(obs, et, ref, sobs, ref_len);
	    zzspkap1_(targ, et, ref, sobs, abcorr, starg, lt, ref_len, 
		    abcorr_len);
	    chkout_("ZZSPKEZ1", (ftnlen)8);
	    return 0;
	}
...
/*        Get the "apparent" state of TARG in the intermediary */
...
/*        of the target relative to the observer. */
	...
	zzspkap1_(targ, et, "J2000", sobs, abcorr, state, lt, (ftnlen)5, 
		abcorr_len);
	if (failed_()) {
	    chkout_("ZZSPKEZ1", (ftnlen)8);
	    return 0;
	...
	if (center == *obs) {
	    ...
	    zzspkap1_(&center, et, "J2000", sobs, abcorr, temp, &ltcent, (
		    ftnlen)5, abcorr_len);
	}
