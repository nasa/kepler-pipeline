/* ckgp_o.f -- translated by f2c (version 19980913).
...
/* Subroutine */ int ckgp_o__(integer *inst, doublereal *sclkdp, doublereal *
	tol, char *ref, doublereal *cmat, doublereal *clkout, logical *found, 
	ftnlen ref_len)
{
    logical pfnd, sfnd;
    integer sclk;
...
/*                The C-matrix returned by CKGP_O is the one whose time */
...
/*                algorithm used by CKGP_O to satisfy a pointing */
...
/*     CKGP_O searches through files loaded by FURNSH to locate a */
...
/*                         CKGP_O returns no pointing; the output */
...
/*                   continuous pointing is available. CKGP_O returns */
...
/*                   continuous pointing is *not* available.  CKGP_O */
...
/*     CKGP_O searches through loaded C-kernels to satisfy a pointing */
...
/*     intervals, CKGP_O will return pointing at the request time. When */
...
/*     The following example program uses CKGP_O to get C-matrices for a */
...
/*     C        (units of encoded SCLK) for input to CKGP_O. */
/*     C */
...
/*           DO I = 1, NPICS */
/*     C */
/*     C        CKGP_O requires encoded spacecraft clock. */
/*     C */
/*              CALL SCENCD ( SC, SCLKCH( I ), SCLKDP ) */

/*              CALL CKGP_O ( INST,   SCLKDP, TOLTIK, REF, CMAT, */
/*          .               CLKOUT, FOUND                      ) */
...
/*        value TRUE used to check out with the name 'CKGP_OAV'.  This */
/*        has been changed to a CKGP_O. */
