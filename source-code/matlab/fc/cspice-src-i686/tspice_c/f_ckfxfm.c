/* f_ckfxfm.f -- translated by f2c (version 19980913).
...
/* $Procedure      F_CKFXFM ( Family Test of  CKFXFM ) */
/* Subroutine */ int f_ckfxfm__(logical *ok)
{
    integer frame;
    ...
    extern /* Subroutine */ int chckxc_(logical *, char *, logical *, ftnlen),
	    ...
	    *, ftnlen), kilfil_(char *, ftnlen), ckfxfm_(integer *, 
	    doublereal *, doublereal *, integer *, logical *), namfrm_(char *,
	     integer *, ftnlen), clpool_(void), tparse_(char *, doublereal *, 
	    char *, ftnlen, ftnlen), tstatd_(doublereal *, doublereal *, 
	    doublereal *);
...
/*     Test the routine CKFXFM.  There are several test cases. The first */
/*     few make sure that nothing is found if an instrument is not */
...
/*     Begin every test family with an open call. */
    ...
    topen_("F_CKFXFM", (ftnlen)8);
    kilfil_("TEST.CK", (ftnlen)7);
    kilfil_("TEST.SCLK", (ftnlen)9);
    tcase_("Make sure nothing is found if we don't have a C-kernel loaded or"
	    " SLCK kernel loaded. ", (ftnlen)85);
    tparse_("1-JAN-1990", &et, error, (ftnlen)10, (ftnlen)80);
    ckfxfm_(&c_n9998, &et, xform, &frame, &found);
    chckxc_(&c_false, " ", ok, (ftnlen)1);
    chcksl_("FOUND", &found, &c_false, ok, (ftnlen)5);
    tcase_("Make sure nothing is even if we don't have a C-kernel loaded or "
	    "SLCK kernel loaded. ", (ftnlen)84);
    tstck3_("TEST.CK", "TEST.SCLK", &c_false, &c_true, &c_true, &handle, (
	    ftnlen)7, (ftnlen)9);
    tparse_("1-JAN-1990", &et, error, (ftnlen)10, (ftnlen)80);
    ckfxfm_(&c_n9998, &et, xform, &frame, &found);
    chckxc_(&c_false, " ", ok, (ftnlen)1);
...
/*        Create and load a test C-kernel. */
    ...
    ckfxfm_(&c_n9999, &et, xform, &frame, &found);
    tstatd_(&et, rot, av);
    rav2xf_(rot, av, tmpxfm);
    invstm_(tmpxfm, xform2);
    ...
    ckfxfm_(&c_b51, &et, xform, &frame, &found);
    tstatd_(&et, rot, av);
    rav2xf_(rot, av, tmpxfm);
    invstm_(tmpxfm, xform2);
    ...
    ckfxfm_(&c_b69, &et, xform, &frame, &found);
    tstatd_(&et, rot, av);
    rav2xf_(rot, av, tmpxfm);
    invstm_(tmpxfm, xform2);
