/* t_spkbsr.f -- translated by f2c (version 19980913).
...
/* Subroutine */ int t_sbsr__0_(int n__, char *fname, integer *handle, 
	integer *body, doublereal *et, doublereal *descr, char *ident, 
...
/*     --------  ---  -------------------------------------------------- */
/*     FNAME      I   T_SLEF */
/*     HANDLE    I/O  T_SLEF, T_SUEF, T_SSFS */
/*     BODY       I   T_SSFS */
/*     ET         I   T_SSFS */
/*     DESCR      O   T_SSFS */
/*     IDENT      O   T_SSFS */
...
/*                loaded by T_SLEF at any given time for use by the */
/*                readers. */
...
/*                given time by T_SSFS. */


/* $ Files */

/*     S/P-kernel ephemeris files are indicated by filename before */
/*     loading (see T_SLEF) and handle after loading (all other places). */

/* $ Exceptions */

/*     1) If T_SBSR is called directly, the error 'SPICE(BOGUSENTRY)' */
/*        is signaled. */

/*     2) See entry points T_SLEF, T_SUEF, and T_SSFS for exceptions */
/*        specific to them. */

/* $ Particulars */

/*     T_SBSR serves as an umbrella, allowing data to be shared by its */
/*     entry points: */

/*        T_SLEF       Load ephemeris file. */
/*        T_SUEF       Unload ephemeris file. */
/*        T_SSFS       Select file and segment. */

/*     Before a file can be read by the S/P-kernel readers, it must be */
/*     loaded by T_SLEF, which among other things, loads the file into */
/*     the DAF system. */
...
/*     searching by unloading it via a call to T_SUEF. */

/*     T_SSFS performs the search for segments within a file for the */
...
/*     Information on loaded files is used by T_SSFS to manage a buffer */
...
/*     C */
/*           CALL T_SLEF ( 'PREDICT.SPK',  PRED_HNDL ) */
/*           CALL T_SLEF ( 'UPDATE_1.SPK', UPD1_HNDL ) */
...
/*     C */
/*              CALL T_SSFS ( BODY, ET, HANDLE, DESCR, IDENT, FOUND ) */
...
/*     C */
/*           CALL T_SUEF (  UPD1_HNDL ) */
/*           CALL T_SLEF ( 'UPDATE_2.SPK', UPD2_HNDL ) */
...
/* -    SPICELIB Version 4.1.0, 10-NOV-2005 (NJB) */

/*        Updated to remove non-standard use of duplicate arguments in */
/*        MOVED calls in entry points T_SUEF and T_SSFS. */

/* -    SPICELIB Version 4.0.0, 28-NOV-2001 (NJB) */
...
/*           4) The load routine T_SLEF now resets all file numbers when */
/*              the next file number reaches INTMAX()-1, thereby */
/*              avoiding arithmetic overflow. */

/*           5) The unload routine T_SUEF now calls RETURN() on entry and */
/*              returns if so directed. */

/*           6) In T_SSFS, DAF calls are followed by tests of FAILED() */
/*              in order to ensure that the main state loop terminates. */

/*           7) In T_SSFS, a subscript bound violation in a loop */
...
/*        An interim fix to a bug in T_SBSR was made.  The parameters */
...
/*     1) When loading a file, T_SLEF now checks if the file table is */
...
/*     2) A bug in the way that T_SLEF and T_SUEF clean up the body */
...
/*     Initial values */
    ...
    /* Function Body */
    switch(n__) {
	case 1: goto L_t_slef;
	case 2: goto L_t_suef;
	case 3: goto L_t_ssfs;
	}
...
/*     Nobody has any business calling T_SBSR directly. */
    ...
    chkin_("T_SBSR", (ftnlen)6);
    sigerr_("SPICE(BOGUSENTRY)", (ftnlen)17);
    chkout_("T_SBSR", (ftnlen)6);
    return 0;
...
/*     handle by T_SLEF, which will be used by most other routines to */
/*     refer to it. */

/* $ Particulars */

/*     If there is room for a new file in the file table, T_SLEF creates */
/*     an entry for it and loads the file for reading using DAFOPR. */

/* $ Examples */

/*     See the Example above, in T_SBSR. */
...
/*     intervals are invalid. */
	...
	btchkp[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge("btchkp", 
		i__2, "t_sbsr__", (ftnlen)1031)] = FALSE_;
    }
...
/*        becomes empty, remove that body from the body table. */
	...
	for (i__ = findex; i__ <= i__1; ++i__) {
	    fthan[(i__2 = i__ - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("fthan",
		     i__2, "t_sbsr__", (ftnlen)1073)] = fthan[(i__3 = i__) < 
		    110 && 0 <= i__3 ? i__3 : s_rnge("fthan", i__3, "t_sbsr__"
		    , (ftnlen)1073)];
	    ftnum[(i__2 = i__ - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum",
		     i__2, "t_sbsr__", (ftnlen)1074)] = ftnum[(i__3 = i__) < 
		    110 && 0 <= i__3 ? i__3 : s_rnge("ftnum", i__3, "t_sbsr__"
		    , (ftnlen)1074)];
	}
	i__ = 1;
	while(i__ <= nbt) {
	    p = btbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "btbeg", i__1, "t_sbsr__", (ftnlen)1081)];
	    while(p > 0) {
...
/*              Find the successor of P, if any. */
		...
		if (sthan[(i__1 = p - 1) < 100 && 0 <= i__1 ? i__1 : s_rnge(
			"sthan", i__1, "t_sbsr__", (ftnlen)1089)] == *handle) 
			{
...
/*                 make the successor of P the head of the list. */
		    ...
		    if (p == btbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 :
			     s_rnge("btbeg", i__1, "t_sbsr__", (ftnlen)1099)])
			     {
			btbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("btbeg", i__1, "t_sbsr__", (ftnlen)
				1100)] = nxtseg;
		    }
		}
...
/*           table into the space occupied by the one we've deleted. */
	    ...
	    if (btbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "btbeg", i__1, "t_sbsr__", (ftnlen)1116)] <= 0) {
...
/*                 BTUB */
		...
		btbod[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btbod", i__1, "t_sbsr__", (ftnlen)1130)] = btbod[(
			i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btbod", i__2, "t_sbsr__", (ftnlen)1130)];
		btexp[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btexp", i__1, "t_sbsr__", (ftnlen)1131)] = btexp[(
			i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btexp", i__2, "t_sbsr__", (ftnlen)1131)];
		bthfs[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"bthfs", i__1, "t_sbsr__", (ftnlen)1132)] = bthfs[(
			i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"bthfs", i__2, "t_sbsr__", (ftnlen)1132)];
		btlfs[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btlfs", i__1, "t_sbsr__", (ftnlen)1133)] = btlfs[(
			i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btlfs", i__2, "t_sbsr__", (ftnlen)1133)];
		btbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btbeg", i__1, "t_sbsr__", (ftnlen)1134)] = btbeg[(
			i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btbeg", i__2, "t_sbsr__", (ftnlen)1134)];
		--nbt;
	    } else {
		++i__;
	    }
...
/*           Re-map the HFS table for the Ith body. */
	    ...
	    j = isrchi_(&bthfs[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : 
		    s_rnge("bthfs", i__2, "t_sbsr__", (ftnlen)1196)], &nft, 
		    ftnum);
	    if (j > 0) {
...
/*              in the file table. */
		...
		bthfs[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"bthfs", i__2, "t_sbsr__", (ftnlen)1203)] = j;
	    } else {
...
/*              in the file table. */
		...
		btlfs[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btlfs", i__2, "t_sbsr__", (ftnlen)1226)] = j;
	    } else {
...
/*              making all files "new." */
		...
		btlfs[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btlfs", i__2, "t_sbsr__", (ftnlen)1235)] = 0;
		bthfs[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"bthfs", i__2, "t_sbsr__", (ftnlen)1236)] = 0;
	    }
	}
...
/*        Assign a new file number. */
    ...
    fthan[(i__1 = nft - 1) < 110 && 0 <= i__1 ? i__1 : s_rnge("fthan", i__1, 
	    "t_sbsr__", (ftnlen)1260)] = *handle;
    ftnum[(i__1 = nft - 1) < 110 && 0 <= i__1 ? i__1 : s_rnge("ftnum", i__1, 
	    "t_sbsr__", (ftnlen)1261)] = next;
    chkout_("T_SLEF", (ftnlen)6);
    return 0;
/* $Procedure T_SUEF ( S/P Kernel, Unload ephemeris file ) */
...
/*     1) A bug in the way that T_SLEF and T_SUEF clean up the body */
/*        tables after a file is unloaded was fixed. */
...
/*     before wiping out the handle. */
    ...
    dafcls_(&fthan[(i__1 = findex - 1) < 110 && 0 <= i__1 ? i__1 : s_rnge(
	    "fthan", i__1, "t_sbsr__", (ftnlen)1507)]);
    --nft;
    i__1 = nft;
    for (i__ = findex; i__ <= i__1; ++i__) {
	fthan[(i__2 = i__ - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("fthan", 
		i__2, "t_sbsr__", (ftnlen)1512)] = fthan[(i__3 = i__) < 110 &&
		 0 <= i__3 ? i__3 : s_rnge("fthan", i__3, "t_sbsr__", (ftnlen)
		1512)];
	ftnum[(i__2 = i__ - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", 
		i__2, "t_sbsr__", (ftnlen)1513)] = ftnum[(i__3 = i__) < 110 &&
		 0 <= i__3 ? i__3 : s_rnge("ftnum", i__3, "t_sbsr__", (ftnlen)
		1513)];
    }
...
/*     on each list, having no predecessor, must be handled specially. */
	...
	p = btbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btbeg", 
		i__1, "t_sbsr__", (ftnlen)1524)];
	while(p > 0) {
	    nxtseg = lnknxt_(&p, stpool);
	    if (sthan[(i__1 = p - 1) < 100 && 0 <= i__1 ? i__1 : s_rnge("sth"
		    "an", i__1, "t_sbsr__", (ftnlen)1530)] == *handle) {
		if (p == btbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : 
			s_rnge("btbeg", i__1, "t_sbsr__", (ftnlen)1532)]) {
		    btbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			    "btbeg", i__1, "t_sbsr__", (ftnlen)1533)] = 
			    nxtseg;
		}
		lnkfsl_(&p, &p, stpool);
	    }
...
/*        space occupied by the deleted body. */
	...
	if (btbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btbeg", 
		i__1, "t_sbsr__", (ftnlen)1550)] <= 0) {
	    btbod[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btbod", 
		    i__1, "t_sbsr__", (ftnlen)1552)] = btbod[(i__2 = nbt - 1) 
		    < 20 && 0 <= i__2 ? i__2 : s_rnge("btbod", i__2, "t_sbsr"
		    "__", (ftnlen)1552)];
	    btexp[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btexp", 
		    i__1, "t_sbsr__", (ftnlen)1553)] = btexp[(i__2 = nbt - 1) 
		    < 20 && 0 <= i__2 ? i__2 : s_rnge("btexp", i__2, "t_sbsr"
		    "__", (ftnlen)1553)];
	    bthfs[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("bthfs", 
		    i__1, "t_sbsr__", (ftnlen)1554)] = bthfs[(i__2 = nbt - 1) 
		    < 20 && 0 <= i__2 ? i__2 : s_rnge("bthfs", i__2, "t_sbsr"
		    "__", (ftnlen)1554)];
	    btlfs[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btlfs", 
		    i__1, "t_sbsr__", (ftnlen)1555)] = btlfs[(i__2 = nbt - 1) 
		    < 20 && 0 <= i__2 ? i__2 : s_rnge("btlfs", i__2, "t_sbsr"
		    "__", (ftnlen)1555)];
	    btbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btbeg", 
		    i__1, "t_sbsr__", (ftnlen)1556)] = btbeg[(i__2 = nbt - 1) 
		    < 20 && 0 <= i__2 ? i__2 : s_rnge("btbeg", i__2, "t_sbsr"
		    "__", (ftnlen)1556)];
	    btlb[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btlb", 
		    i__1, "t_sbsr__", (ftnlen)1557)] = btlb[(i__2 = nbt - 1) <
		     20 && 0 <= i__2 ? i__2 : s_rnge("btlb", i__2, "t_sbsr__",
		     (ftnlen)1557)];
	    btub[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btub", 
		    i__1, "t_sbsr__", (ftnlen)1558)] = btub[(i__2 = nbt - 1) <
		     20 && 0 <= i__2 ? i__2 : s_rnge("btub", i__2, "t_sbsr__",
		     (ftnlen)1558)];
	    btprvh[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btprvh"
		    , i__1, "t_sbsr__", (ftnlen)1559)] = btprvh[(i__2 = nbt - 
		    1) < 20 && 0 <= i__2 ? i__2 : s_rnge("btprvh", i__2, 
		    "t_sbsr__", (ftnlen)1559)];
	    s_copy(btprvi + ((i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : 
		    s_rnge("btprvi", i__1, "t_sbsr__", (ftnlen)1560)) * 40, 
		    btprvi + ((i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : 
		    s_rnge("btprvi", i__2, "t_sbsr__", (ftnlen)1560)) * 40, (
		    ftnlen)40, (ftnlen)40);
	    btchkp[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btchkp"
		    , i__1, "t_sbsr__", (ftnlen)1561)] = btchkp[(i__2 = nbt - 
		    1) < 20 && 0 <= i__2 ? i__2 : s_rnge("btchkp", i__2, 
		    "t_sbsr__", (ftnlen)1561)];
	    btruex[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btruex"
		    , i__1, "t_sbsr__", (ftnlen)1562)] = btruex[(i__2 = nbt - 
		    1) < 20 && 0 <= i__2 ? i__2 : s_rnge("btruex", i__2, 
		    "t_sbsr__", (ftnlen)1562)];
	    if (i__ != nbt) {
		moved_(&btprvd[(i__1 = nbt * 5 - 5) < 100 && 0 <= i__1 ? i__1 
			: s_rnge("btprvd", i__1, "t_sbsr__", (ftnlen)1565)], &
			c__5, &btprvd[(i__2 = i__ * 5 - 5) < 100 && 0 <= i__2 
			? i__2 : s_rnge("btprvd", i__2, "t_sbsr__", (ftnlen)
			1565)]);
	    }
	    --nbt;
	} else {
...
/*     that the re-use interval is invalid. */
	...
	if (btchkp[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge("btchkp"
		, i__2, "t_sbsr__", (ftnlen)1587)]) {
	    if (btprvh[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
		    "btprvh", i__2, "t_sbsr__", (ftnlen)1589)] == *handle) {
		btchkp[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btchkp", i__2, "t_sbsr__", (ftnlen)1590)] = FALSE_;
	    }
	}
    }
    chkout_("T_SUEF", (ftnlen)6);
...
/*     1) If an attempt is made to call T_SSFS when there aren't any */
/*        files loaded, the error SPICE(NOLOADEDFILES) is signaled. */

/* $ Files */

/*     All files loaded by T_SLEF are potential search targets for */
/*     T_SSFS. */
...
/*        If an attempt is made to call T_SSFS when there are no files */
...
/*     There must be at least ONE file loaded. */
    ...
    if (nft == 0) {
	setmsg_("At least one SPK file needs to be loaded by T_SLEF before b"
		"eginning a search.", (ftnlen)77);
	sigerr_("SPICE(NOLOADEDFILES)", (ftnlen)20);
	chkout_("T_SSFS", (ftnlen)6);
	return 0;
    }
...
/*        for data for that body.  Check whether this is the case. */
	...
	if (btchkp[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		"btchkp", i__1, "t_sbsr__", (ftnlen)2026)]) {
...
/*           segments. */
	    ...
	    if (*et > btlb[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
		    s_rnge("btlb", i__1, "t_sbsr__", (ftnlen)2039)] && *et < 
		    btub[(i__2 = bindex - 1) < 20 && 0 <= i__2 ? i__2 : 
		    s_rnge("btub", i__2, "t_sbsr__", (ftnlen)2039)]) {
...
/*              the request. */
		...
		*handle = btprvh[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 
			: s_rnge("btprvh", i__1, "t_sbsr__", (ftnlen)2048)];
		s_copy(ident, btprvi + ((i__1 = bindex - 1) < 20 && 0 <= i__1 
			? i__1 : s_rnge("btprvi", i__1, "t_sbsr__", (ftnlen)
			2049)) * 40, ident_len, (ftnlen)40);
		moved_(&btprvd[(i__1 = bindex * 5 - 5) < 100 && 0 <= i__1 ? 
			i__1 : s_rnge("btprvd", i__1, "t_sbsr__", (ftnlen)
			2051)], &c__5, descr);
		*found = TRUE_;
		chkout_("T_SSFS", (ftnlen)6);
		return 0;
...
/*           that component from the expense. */
	    ...
	    btexp[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("bte"
		    "xp", i__1, "t_sbsr__", (ftnlen)2066)] = btexp[(i__2 = 
		    bindex - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge("btexp", 
		    i__2, "t_sbsr__", (ftnlen)2066)] - btruex[(i__3 = bindex 
		    - 1) < 20 && 0 <= i__3 ? i__3 : s_rnge("btruex", i__3, 
		    "t_sbsr__", (ftnlen)2066)];
	    btruex[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "btruex", i__1, "t_sbsr__", (ftnlen)2067)] = 0;
...
/*                recently loaded segment. */
	    ...
	    btchkp[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "btchkp", i__1, "t_sbsr__", (ftnlen)2082)] = FALSE_;
	}
...
/*        expense is reset to 0. */
	...
	if (btbeg[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btb"
		"eg", i__1, "t_sbsr__", (ftnlen)2091)] == 0) {
	    btexp[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("bte"
		    "xp", i__1, "t_sbsr__", (ftnlen)2093)] = 0;
	}
...
/*               a new file. */
	    ...
	    if (bthfs[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "bthfs", i__1, "t_sbsr__", (ftnlen)2120)] < ftnum[(i__2 = 
		    nft - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", i__2,
		     "t_sbsr__", (ftnlen)2120)]) {
...
/*              expensive body in the table and remove it. */
		...
		for (i__ = 2; i__ <= i__1; ++i__) {
		    if (btexp[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : 
			    s_rnge("btexp", i__2, "t_sbsr__", (ftnlen)2169)] <
			     minexp) {
			cheap = i__;
			minexp = btexp[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? 
				i__2 : s_rnge("btexp", i__2, "t_sbsr__", (
				ftnlen)2171)];
		    }
		}
...
/*              list. */
		...
		head = btbeg[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : 
			s_rnge("btbeg", i__1, "t_sbsr__", (ftnlen)2181)];
		if (head > 0) {
...
/*           Set up a body table entry for the new body. */
	    ...
	    btbod[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btbod"
		    , i__1, "t_sbsr__", (ftnlen)2195)] = *body;
	    btexp[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btexp"
		    , i__1, "t_sbsr__", (ftnlen)2196)] = 0;
	    bthfs[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("bthfs"
		    , i__1, "t_sbsr__", (ftnlen)2197)] = ftnum[(i__2 = nft - 
		    1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", i__2, 
		    "t_sbsr__", (ftnlen)2197)];
	    btlfs[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btlfs"
		    , i__1, "t_sbsr__", (ftnlen)2198)] = ftnum[(i__2 = nft - 
		    1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", i__2, 
		    "t_sbsr__", (ftnlen)2198)] + 1;
	    btbeg[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btbeg"
		    , i__1, "t_sbsr__", (ftnlen)2199)] = 0;
	    btchkp[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btc"
		    "hkp", i__1, "t_sbsr__", (ftnlen)2200)] = FALSE_;
...
/*           corresponding file table entry. */
	    ...
	    while(bthfs[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "bthfs", i__1, "t_sbsr__", (ftnlen)2258)] >= ftnum[(i__2 =
		     findex - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", 
		    i__2, "t_sbsr__", (ftnlen)2258)]) {
		++findex;
	    }
	    bthfs[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("bth"
		    "fs", i__1, "t_sbsr__", (ftnlen)2262)] = ftnum[(i__2 = 
		    findex - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", 
		    i__2, "t_sbsr__", (ftnlen)2262)];
	    dafbfs_(&fthan[(i__1 = findex - 1) < 110 && 0 <= i__1 ? i__1 : 
		    s_rnge("fthan", i__1, "t_sbsr__", (ftnlen)2264)]);
	    if (failed_()) {
		chkout_("T_SSFS", (ftnlen)6);
		return 0;
	    }
...
/*              whether we're ready to check the list. */
		...
		btexp[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btexp", i__1, "t_sbsr__", (ftnlen)2306)] = btexp[(
			i__2 = bindex - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btexp", i__2, "t_sbsr__", (ftnlen)2306)] + cost;
	    } else {
		dafgs_(descr);
...
/*              have some files left that have not been seached. */
	    ...
	    while(btlfs[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "btlfs", i__1, "t_sbsr__", (ftnlen)2358)] <= ftnum[(i__2 =
		     findex - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", 
		    i__2, "t_sbsr__", (ftnlen)2358)]) {
		--findex;
	    }
	    dafbbs_(&fthan[(i__1 = findex - 1) < 110 && 0 <= i__1 ? i__1 : 
		    s_rnge("fthan", i__1, "t_sbsr__", (ftnlen)2362)]);
	    if (failed_()) {
		chkout_("T_SSFS", (ftnlen)6);
		return 0;
	    }
...
/*              to be the current file, and go check the current list. */
		...
		btlfs[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btlfs", i__1, "t_sbsr__", (ftnlen)2407)] = ftnum[(
			i__2 = findex - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge(
			"ftnum", i__2, "t_sbsr__", (ftnlen)2407)];
		btexp[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btexp", i__1, "t_sbsr__", (ftnlen)2408)] = btexp[(
			i__2 = bindex - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btexp", i__2, "t_sbsr__", (ftnlen)2408)] + cost;
		s_copy(status, "CHECK LIST", (ftnlen)15, (ftnlen)10);
	    } else {
		dafgs_(descr);
		dafus_(descr, &c__2, &c__6, dcd, icd);
...
/*           associated with the current body. */
	    ...
	    btlb[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btlb",
		     i__1, "t_sbsr__", (ftnlen)2455)] = dpmin_();
	    btub[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btub",
		     i__1, "t_sbsr__", (ftnlen)2456)] = dpmax_();
	    p = btbeg[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "btbeg", i__1, "t_sbsr__", (ftnlen)2457)];
	    while(p > 0) {
		if (*et > stdes[(i__1 = p * 5 - 4) < 500 && 0 <= i__1 ? i__1 :
			 s_rnge("stdes", i__1, "t_sbsr__", (ftnlen)2461)]) {
...
/* Computing MAX */
		    d__1 = btlb[(i__2 = bindex - 1) < 20 && 0 <= i__2 ? i__2 :
			     s_rnge("btlb", i__2, "t_sbsr__", (ftnlen)2466)], 
			    d__2 = stdes[(i__3 = p * 5 - 4) < 500 && 0 <= 
			    i__3 ? i__3 : s_rnge("stdes", i__3, "t_sbsr__", (
			    ftnlen)2466)];
		    btlb[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("btlb", i__1, "t_sbsr__", (ftnlen)2466)] = 
			    max(d__1,d__2);
		} else if (*et < stdes[(i__1 = p * 5 - 5) < 500 && 0 <= i__1 ?
			 i__1 : s_rnge("stdes", i__1, "t_sbsr__", (ftnlen)
			2469)]) {
...
/* Computing MIN */
		    d__1 = btub[(i__2 = bindex - 1) < 20 && 0 <= i__2 ? i__2 :
			     s_rnge("btub", i__2, "t_sbsr__", (ftnlen)2474)], 
			    d__2 = stdes[(i__3 = p * 5 - 5) < 500 && 0 <= 
			    i__3 ? i__3 : s_rnge("stdes", i__3, "t_sbsr__", (
			    ftnlen)2474)];
		    btub[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("btub", i__1, "t_sbsr__", (ftnlen)2474)] = 
			    min(d__1,d__2);
		} else {
...
/*                 The segment coverage interval includes ET. */
		    ...
		    s_copy(ident, stidnt + ((i__1 = p - 1) < 100 && 0 <= i__1 
			    ? i__1 : s_rnge("stidnt", i__1, "t_sbsr__", (
			    ftnlen)2481)) * 40, ident_len, (ftnlen)40);
		    *handle = sthan[(i__1 = p - 1) < 100 && 0 <= i__1 ? i__1 :
			     s_rnge("sthan", i__1, "t_sbsr__", (ftnlen)2482)];
		    *found = TRUE_;
...
/* Computing MAX */
		    d__1 = btlb[(i__2 = bindex - 1) < 20 && 0 <= i__2 ? i__2 :
			     s_rnge("btlb", i__2, "t_sbsr__", (ftnlen)2488)], 
			    d__2 = stdes[(i__3 = p * 5 - 5) < 500 && 0 <= 
			    i__3 ? i__3 : s_rnge("stdes", i__3, "t_sbsr__", (
			    ftnlen)2488)];
		    btlb[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("btlb", i__1, "t_sbsr__", (ftnlen)2488)] = 
			    max(d__1,d__2);
/* Computing MIN */
		    d__1 = btub[(i__2 = bindex - 1) < 20 && 0 <= i__2 ? i__2 :
			     s_rnge("btub", i__2, "t_sbsr__", (ftnlen)2489)], 
			    d__2 = stdes[(i__3 = p * 5 - 4) < 500 && 0 <= 
			    i__3 ? i__3 : s_rnge("stdes", i__3, "t_sbsr__", (
			    ftnlen)2489)];
		    btub[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("btub", i__1, "t_sbsr__", (ftnlen)2489)] = 
			    min(d__1,d__2);
...
/*              to speed up the operation. */
		...
		p = stpool[(i__1 = (p << 1) + 10) < 212 && 0 <= i__1 ? i__1 : 
			s_rnge("stpool", i__1, "t_sbsr__", (ftnlen)2509)];
	    }
...
/*           Otherwise, things are hopeless, set the status that way. */
	    ...
	    if (btlfs[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "btlfs", i__1, "t_sbsr__", (ftnlen)2520)] > ftnum[0]) {
		s_copy(status, "OLD FILES", (ftnlen)15, (ftnlen)9);
	    } else {
...
/*                 one. */
		    ...
		    if (btexp[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : 
			    s_rnge("btexp", i__2, "t_sbsr__", (ftnlen)2571)] <
			     minexp || cheap == 0) {
...
/*              'MAKE ROOM'. */
		...
		if (s_cmp(stack + ((i__1 = top - 1) < 2 && 0 <= i__1 ? i__1 : 
			s_rnge("stack", i__1, "t_sbsr__", (ftnlen)2595)) * 15,
...
/*                 node. */
		    ...
		    p = btbeg[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("btbeg", i__1, "t_sbsr__", (ftnlen)2620)];
...
/*                 Also, reset the suspended task stack to be empty. */
		    ...
		    btexp[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("btexp", i__1, "t_sbsr__", (ftnlen)2630)] =
			     0;
		    bthfs[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("bthfs", i__1, "t_sbsr__", (ftnlen)2631)] =
			     ftnum[(i__2 = nft - 1) < 110 && 0 <= i__2 ? i__2 
			    : s_rnge("ftnum", i__2, "t_sbsr__", (ftnlen)2631)]
			    ;
		    btlfs[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("btlfs", i__1, "t_sbsr__", (ftnlen)2632)] =
			     ftnum[(i__2 = nft - 1) < 110 && 0 <= i__2 ? i__2 
			    : s_rnge("ftnum", i__2, "t_sbsr__", (ftnlen)2632)]
			     + 1;
		    s_copy(status, "OLD FILES", (ftnlen)15, (ftnlen)9);
		    top = 0;
		}
...
/*              Return this cheapest list to the segment pool. */
		...
		p = btbeg[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : 
			s_rnge("btbeg", i__1, "t_sbsr__", (ftnlen)2642)];
		if (p > 0) {
		    tail = -lnkprv_(&p, stpool);
		    lnkfsl_(&p, &tail, stpool);
		}
...
/*              the final entry in the table. */
		...
		btbod[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btbod", i__1, "t_sbsr__", (ftnlen)2655)] = btbod[(
			i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btbod", i__2, "t_sbsr__", (ftnlen)2655)];
		btexp[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btexp", i__1, "t_sbsr__", (ftnlen)2656)] = btexp[(
			i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btexp", i__2, "t_sbsr__", (ftnlen)2656)];
		bthfs[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"bthfs", i__1, "t_sbsr__", (ftnlen)2657)] = bthfs[(
			i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"bthfs", i__2, "t_sbsr__", (ftnlen)2657)];
		btlfs[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btlfs", i__1, "t_sbsr__", (ftnlen)2658)] = btlfs[(
			i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btlfs", i__2, "t_sbsr__", (ftnlen)2658)];
		btbeg[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btbeg", i__1, "t_sbsr__", (ftnlen)2659)] = btbeg[(
			i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btbeg", i__2, "t_sbsr__", (ftnlen)2659)];
		btlb[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btlb", i__1, "t_sbsr__", (ftnlen)2660)] = btlb[(i__2 
			= nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge("btlb", 
			i__2, "t_sbsr__", (ftnlen)2660)];
		btub[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btub", i__1, "t_sbsr__", (ftnlen)2661)] = btub[(i__2 
			= nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge("btub", 
			i__2, "t_sbsr__", (ftnlen)2661)];
		btprvh[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btprvh", i__1, "t_sbsr__", (ftnlen)2662)] = btprvh[(
			i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btprvh", i__2, "t_sbsr__", (ftnlen)2662)];
		s_copy(btprvi + ((i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 :
			 s_rnge("btprvi", i__1, "t_sbsr__", (ftnlen)2663)) * 
			40, btprvi + ((i__2 = nbt - 1) < 20 && 0 <= i__2 ? 
			i__2 : s_rnge("btprvi", i__2, "t_sbsr__", (ftnlen)
			2663)) * 40, (ftnlen)40, (ftnlen)40);
		btruex[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btruex", i__1, "t_sbsr__", (ftnlen)2664)] = btruex[(
			i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btruex", i__2, "t_sbsr__", (ftnlen)2664)];
		btchkp[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btchkp", i__1, "t_sbsr__", (ftnlen)2665)] = btchkp[(
			i__2 = nbt - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"btchkp", i__2, "t_sbsr__", (ftnlen)2665)];
		if (cheap != nbt) {
		    moved_(&btprvd[(i__1 = nbt * 5 - 5) < 100 && 0 <= i__1 ? 
			    i__1 : s_rnge("btprvd", i__1, "t_sbsr__", (ftnlen)
			    2669)], &c__5, &btprvd[(i__2 = cheap * 5 - 5) < 
			    100 && 0 <= i__2 ? i__2 : s_rnge("btprvd", i__2, 
			    "t_sbsr__", (ftnlen)2669)]);
		}
...
/*              for the current body. */
		...
		sthan[(i__1 = new__ - 1) < 100 && 0 <= i__1 ? i__1 : s_rnge(
			"sthan", i__1, "t_sbsr__", (ftnlen)2721)] = fthan[(
			i__2 = findex - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge(
			"fthan", i__2, "t_sbsr__", (ftnlen)2721)];
		moved_(descr, &c__5, &stdes[(i__1 = new__ * 5 - 5) < 500 && 0 
			<= i__1 ? i__1 : s_rnge("stdes", i__1, "t_sbsr__", (
			ftnlen)2722)]);
		dafgn_(stidnt + ((i__1 = new__ - 1) < 100 && 0 <= i__1 ? i__1 
			: s_rnge("stidnt", i__1, "t_sbsr__", (ftnlen)2723)) * 
			40, (ftnlen)40);
		if (failed_()) {
		    chkout_("T_SSFS", (ftnlen)6);
		    return 0;
...
/*              is a no-op. */
		...
		lnkilb_(&new__, &btbeg[(i__1 = bindex - 1) < 20 && 0 <= i__1 ?
			 i__1 : s_rnge("btbeg", i__1, "t_sbsr__", (ftnlen)
			2734)], stpool);
		btbeg[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"btbeg", i__1, "t_sbsr__", (ftnlen)2735)] = new__;
		s_copy(status, "RESUME", (ftnlen)15, (ftnlen)6);
	    }
...
/*              Allocate a new node in the segment table pool. */
		...
		lnkan_(stpool, &new__);
		sthan[(i__1 = new__ - 1) < 100 && 0 <= i__1 ? i__1 : s_rnge(
			"sthan", i__1, "t_sbsr__", (ftnlen)2766)] = fthan[(
			i__2 = findex - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge(
			"fthan", i__2, "t_sbsr__", (ftnlen)2766)];
		moved_(descr, &c__5, &stdes[(i__1 = new__ * 5 - 5) < 500 && 0 
			<= i__1 ? i__1 : s_rnge("stdes", i__1, "t_sbsr__", (
			ftnlen)2767)]);
		dafgn_(stidnt + ((i__1 = new__ - 1) < 100 && 0 <= i__1 ? i__1 
			: s_rnge("stidnt", i__1, "t_sbsr__", (ftnlen)2768)) * 
			40, (ftnlen)40);
		if (failed_()) {
		    chkout_("T_SSFS", (ftnlen)6);
		    return 0;
...
/*                 Link the new node to the tail of the list. */
		    ...
		    tail = -lnkprv_(&btbeg[(i__1 = bindex - 1) < 20 && 0 <= 
			    i__1 ? i__1 : s_rnge("btbeg", i__1, "t_sbsr__", (
			    ftnlen)2785)], stpool);
...
/*           of the the segment list for the current body, and */
...
/*           the re-use interval. */
	    ...
	    btruex[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "btruex", i__1, "t_sbsr__", (ftnlen)2811)] = 0;
...
/*           remainder of the file's segments can't be added to the list. */
	    ...
	    crflbg = btbeg[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
		    s_rnge("btbeg", i__1, "t_sbsr__", (ftnlen)2819)];
	    fndhan = FALSE_;
	    while(! fndhan && crflbg > 0) {
		fndhan = sthan[(i__1 = crflbg - 1) < 100 && 0 <= i__1 ? i__1 :
			 s_rnge("sthan", i__1, "t_sbsr__", (ftnlen)2824)] == 
			fthan[(i__2 = findex - 1) < 110 && 0 <= i__2 ? i__2 : 
			s_rnge("fthan", i__2, "t_sbsr__", (ftnlen)2824)];
		if (! fndhan) {
...
/*                 to speed up the operation. */
		    ...
		    crflbg = stpool[(i__1 = (crflbg << 1) + 10) < 212 && 0 <= 
			    i__1 ? i__1 : s_rnge("stpool", i__1, "t_sbsr__", (
			    ftnlen)2831)];
		}
	    }
	    if (crflbg > 0) {
...
/*              a non-positive value to indicate an empty segment list. */
		...
		if (p == btbeg[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			s_rnge("btbeg", i__1, "t_sbsr__", (ftnlen)2852)]) {
		    btbeg[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("btbeg", i__1, "t_sbsr__", (ftnlen)2854)] =
			     0;
...
/*                 interval for this body. */
		    ...
		    btlb[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("btlb", i__1, "t_sbsr__", (ftnlen)2859)] = 
			    dpmin_();
		    btub[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("btub", i__1, "t_sbsr__", (ftnlen)2860)] = 
			    dpmax_();
		}
...
/*                 to the expense of the re-use interval. */
		    ...
		    btruex[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("btruex", i__1, "t_sbsr__", (ftnlen)2873)] 
			    = btruex[(i__2 = bindex - 1) < 20 && 0 <= i__2 ? 
			    i__2 : s_rnge("btruex", i__2, "t_sbsr__", (ftnlen)
			    2873)] + 1;
...
/* Computing MAX */
			d__1 = btlb[(i__2 = bindex - 1) < 20 && 0 <= i__2 ? 
				i__2 : s_rnge("btlb", i__2, "t_sbsr__", (
				ftnlen)2881)], d__2 = stdes[(i__3 = crflbg * 
				5 - 4) < 500 && 0 <= i__3 ? i__3 : s_rnge(
				"stdes", i__3, "t_sbsr__", (ftnlen)2881)];
			btlb[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("btlb", i__1, "t_sbsr__", (ftnlen)2881)
				] = max(d__1,d__2);
...
/* Computing MIN */
			d__1 = btub[(i__2 = bindex - 1) < 20 && 0 <= i__2 ? 
				i__2 : s_rnge("btub", i__2, "t_sbsr__", (
				ftnlen)2889)], d__2 = stdes[(i__3 = crflbg * 
				5 - 5) < 500 && 0 <= i__3 ? i__3 : s_rnge(
				"stdes", i__3, "t_sbsr__", (ftnlen)2889)];
			btub[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("btub", i__1, "t_sbsr__", (ftnlen)2889)
				] = min(d__1,d__2);
		    } else {
...
/* Computing MAX */
			d__1 = btlb[(i__2 = bindex - 1) < 20 && 0 <= i__2 ? 
				i__2 : s_rnge("btlb", i__2, "t_sbsr__", (
				ftnlen)2904)], d__2 = stdes[(i__3 = crflbg * 
				5 - 5) < 500 && 0 <= i__3 ? i__3 : s_rnge(
				"stdes", i__3, "t_sbsr__", (ftnlen)2904)];
			btlb[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("btlb", i__1, "t_sbsr__", (ftnlen)2904)
				] = max(d__1,d__2);
/* Computing MIN */
			d__1 = btub[(i__2 = bindex - 1) < 20 && 0 <= i__2 ? 
				i__2 : s_rnge("btub", i__2, "t_sbsr__", (
				ftnlen)2905)], d__2 = stdes[(i__3 = crflbg * 
				5 - 4) < 500 && 0 <= i__3 ? i__3 : s_rnge(
				"stdes", i__3, "t_sbsr__", (ftnlen)2905)];
			btub[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("btub", i__1, "t_sbsr__", (ftnlen)2905)
				] = min(d__1,d__2);
...
/*                    segment may be satisfy the next request. */
			...
			btprvh[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("btprvh", i__1, "t_sbsr__", (ftnlen)
				2911)] = *handle;
			s_copy(btprvi + ((i__1 = bindex - 1) < 20 && 0 <= 
				i__1 ? i__1 : s_rnge("btprvi", i__1, "t_sbsr"
				"__", (ftnlen)2912)) * 40, ident, (ftnlen)40, 
				ident_len);
			moved_(descr, &c__5, &btprvd[(i__1 = bindex * 5 - 5) <
				 100 && 0 <= i__1 ? i__1 : s_rnge("btprvd", 
				i__1, "t_sbsr__", (ftnlen)2913)]);
			btchkp[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("btchkp", i__1, "t_sbsr__", (ftnlen)
				2914)] = TRUE_;
...
/*                    the cost of locating this segment. */
			...
			btexp[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("btexp", i__1, "t_sbsr__", (ftnlen)
				2920)] = btexp[(i__2 = bindex - 1) < 20 && 0 
				<= i__2 ? i__2 : s_rnge("btexp", i__2, "t_sb"
				"sr__", (ftnlen)2920)] + btruex[(i__3 = bindex 
				- 1) < 20 && 0 <= i__3 ? i__3 : s_rnge("btru"
				"ex", i__3, "t_sbsr__", (ftnlen)2920)];
...
/*                 re-use interval. */
		    ...
		    btruex[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("btruex", i__1, "t_sbsr__", (ftnlen)2967)] 
			    = btruex[(i__2 = bindex - 1) < 20 && 0 <= i__2 ? 
			    i__2 : s_rnge("btruex", i__2, "t_sbsr__", (ftnlen)
			    2967)] + 1;
		    dafgs_(descr);
...
/* Computing MAX */
			    d__1 = btlb[(i__2 = bindex - 1) < 20 && 0 <= i__2 
				    ...
				    (ftnlen)2987)];
			    btlb[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 
				    : s_rnge("btlb", i__1, "t_sbsr__", (
				    ftnlen)2987)] = max(d__1,dcd[1]);
			} else if (*et < dcd[0]) {
...
/* Computing MIN */
			    d__1 = btub[(i__2 = bindex - 1) < 20 && 0 <= i__2 
				    ...
				    (ftnlen)2995)];
			    btub[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 
				    : s_rnge("btub", i__1, "t_sbsr__", (
				    ftnlen)2995)] = min(d__1,dcd[0]);
			} else {
...
/*                       The segment coverage interval includes ET. */
			    ...
			    *handle = fthan[(i__1 = findex - 1) < 110 && 0 <= 
				    i__1 ? i__1 : s_rnge("fthan", i__1, "t_s"
				    "bsr__", (ftnlen)3008)];
			    *found = TRUE_;
...
/*                       the cost of locating this segment. */
			    ...
			    btexp[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? 
				    i__1 : s_rnge("btexp", i__1, "t_sbsr__", (
				    ftnlen)3030)] = btexp[(i__2 = bindex - 1) 
				    < 20 && 0 <= i__2 ? i__2 : s_rnge("btexp",
				     i__2, "t_sbsr__", (ftnlen)3030)] + 
				    btruex[(i__3 = bindex - 1) < 20 && 0 <= 
				    i__3 ? i__3 : s_rnge("btruex", i__3, 
				    "t_sbsr__", (ftnlen)3030)];
			    chkout_("T_SSFS", (ftnlen)6);
			    return 0;
			}
		    }
...
/*              Try the next oldest file. */
		...
		if (findex > 0) {
		    dafbbs_(&fthan[(i__1 = findex - 1) < 110 && 0 <= i__1 ? 
			    i__1 : s_rnge("fthan", i__1, "t_sbsr__", (ftnlen)
			    3055)]);
		    daffpa_(&fnd);
		    if (failed_()) {
			chkout_("T_SSFS", (ftnlen)6);
...
/*           If you get to here, sorry. */
	    ...
	    btruex[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "btruex", i__1, "t_sbsr__", (ftnlen)3070)] = 0;
	    s_copy(status, "HOPELESS", (ftnlen)15, (ftnlen)8);
...
/*        some interrupts can be interrupted by others. */
	    ...
	    s_copy(stack + ((i__1 = top - 1) < 2 && 0 <= i__1 ? i__1 : s_rnge(
		    "stack", i__1, "t_sbsr__", (ftnlen)3081)) * 15, doing, (
		    ftnlen)15, (ftnlen)15);
...
/*           Pop the status stack. */
	    ...
	    s_copy(status, stack + ((i__1 = top - 1) < 2 && 0 <= i__1 ? i__1 :
		     s_rnge("stack", i__1, "t_sbsr__", (ftnlen)3088)) * 15, (
		    ftnlen)15, (ftnlen)15);
	    --top;
	}
    }
...
/*     at this point.  Also clear the re-use interval's expense. */
	...
	btchkp[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btchkp",
		 i__1, "t_sbsr__", (ftnlen)3102)] = FALSE_;
	btruex[(i__1 = bindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("btruex",
		 i__1, "t_sbsr__", (ftnlen)3103)] = 0;
    }
    chkout_("T_SSFS", (ftnlen)6);
    return 0;
} /* t_sbsr__ */

/* Subroutine */ int t_sbsr__(char *fname, integer *handle, integer *body, 
	doublereal *et, doublereal *descr, char *ident, logical *found, 
	ftnlen fname_len, ftnlen ident_len)
{
    return t_sbsr__0_(0, fname, handle, body, et, descr, ident, found, 
	    fname_len, ident_len);
    }

/* Subroutine */ int t_slef__(char *fname, integer *handle, ftnlen fname_len)
{
    return t_sbsr__0_(1, fname, handle, (integer *)0, (doublereal *)0, (
	    doublereal *)0, (char *)0, (logical *)0, fname_len, (ftnint)0);
    }

/* Subroutine */ int t_suef__(integer *handle)
{
    return t_sbsr__0_(2, (char *)0, handle, (integer *)0, (doublereal *)0, (
	    doublereal *)0, (char *)0, (logical *)0, (ftnint)0, (ftnint)0);
    }

/* Subroutine */ int t_ssfs__(integer *body, doublereal *et, integer *handle, 
	doublereal *descr, char *ident, logical *found, ftnlen ident_len)
{
    return t_sbsr__0_(3, (char *)0, handle, body, et, descr, ident, found, (
	    ftnint)0, ident_len);
    }
