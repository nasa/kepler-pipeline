/* f_swapad.f -- translated by f2c (version 19980913).
...
/* Subroutine */ int f_swapad__(logical *ok)
{
    ...
    extern /* Subroutine */ int t_swapad__(integer *, integer *, integer *, 
	    integer *, integer *, doublereal *, doublereal *);
    doublereal work[1000];
    integer i__, m, n;
    extern /* Subroutine */ int tcase_(char *, ftnlen), moved_(doublereal *, 
	    integer *, doublereal *), repmi_(char *, char *, integer *, char *
	    , ftnlen, ftnlen, ftnlen);
    char title[240];
    extern /* Subroutine */ int topen_(char *, ftnlen);
    integer start;
    extern /* Subroutine */ int t_success__(logical *);
    doublereal array0[1000], array1[1000];
    extern /* Subroutine */ int chckad_(char *, doublereal *, char *, 
	    doublereal *, integer *, doublereal *, logical *, ftnlen, ftnlen),
	     cleari_(integer *, doublereal *), chckxc_(logical *, char *, 
	    logical *, ftnlen), swapad_(integer *, integer *, integer *, 
	    integer *, doublereal *);
    doublereal xarray[1000];

/* $ Abstract */

/*     Exercise the SPICELIB sub-array swapping routine SWAPAD. */
...
/*     This routine tests the SPICELIB routine SWAPAD. */
...
/*     We'll do an exhaustive set of tests on a small array. */
	...
	array0[(i__2 = i__ - 1) < 1000 && 0 <= i__2 ? i__2 : s_rnge("array0", 
		i__2, "f_swapad__", (ftnlen)167)] = (doublereal) i__;
    }
...
/*                 Make two copies of the initial array. */
		    ...
		    moved_(array0, &size, array1);
		    moved_(array0, &size, xarray);

/*                 Swap the array slices indicated by LOCN, N, LOCM, */
/*                 and M. */

		    swapad_(&n, &locn, &m, &locm, array1);
		    chckxc_(&c_false, " ", ok, (ftnlen)1);

/*                 Produce our expected array. */

		    t_swapad__(&size, &n, &locn, &m, &locm, xarray, work);

/*                 Test our results. */

		    chckad_("swapped array (0)", array1, "=", xarray, &size, &
			    c_b17, ok, (ftnlen)17, (ftnlen)1);
...
/*                 and M. */
		    ...
		    swapad_(&m, &locm, &n, &locn, array1);
		    chckxc_(&c_false, " ", ok, (ftnlen)1);

/*                 Produce our expected array. */

		    t_swapad__(&size, &m, &locm, &n, &locn, xarray, work);

/*                 Test our results. */

		    chckad_("swapped array (1)", array1, "=", xarray, &size, &
			    c_b17, ok, (ftnlen)17, (ftnlen)1);
		}
	    }
	}
...
/* --- Case: ------------------------------------------------------ */
    ...
    tcase_("SWAPAD: overlapping array slices.", (ftnlen)33);
    swapad_(&c__2, &c__1, &c__2, &c__1, array1);
    chckxc_(&c_true, "SPICE(NOTDISTINCT)", ok, (ftnlen)18);

/* --- Case: ------------------------------------------------------ */

    tcase_("SWAPAD: non-positive value of LOCN, LOCM", (ftnlen)40);
    swapad_(&c__2, &c__0, &c__2, &c__1, array1);
    chckxc_(&c_true, "SPICE(INVALIDINDEX)", ok, (ftnlen)19);
    swapad_(&c__2, &c_n1, &c__2, &c__1, array1);
    chckxc_(&c_true, "SPICE(INVALIDINDEX)", ok, (ftnlen)19);
    swapad_(&c__2, &c__1, &c__2, &c_n1, array1);
    chckxc_(&c_true, "SPICE(INVALIDINDEX)", ok, (ftnlen)19);
    swapad_(&c__2, &c__1, &c__2, &c__0, array1);
    chckxc_(&c_true, "SPICE(INVALIDINDEX)", ok, (ftnlen)19);

/* --- Case: ------------------------------------------------------ */

    tcase_("SWAPAD: Negative value of N, M", (ftnlen)30);
    swapad_(&c_n2, &c__0, &c__2, &c__1, array1);
    chckxc_(&c_true, "SPICE(INVALIDARGUMENT)", ok, (ftnlen)22);
    swapad_(&c__2, &c__0, &c_n2, &c__1, array1);
    chckxc_(&c_true, "SPICE(INVALIDARGUMENT)", ok, (ftnlen)22);
...
/*     T_SWAPAD is a utility routine used for testing SWAPAD. T_SWAPAD */
...
/* Subroutine */ int t_swapad__(integer *size, integer *n, integer *locn, 
	integer *m, integer *locm, doublereal *array, doublereal *work)
{
    integer nlow;
...
/*     Move the elements below the lower slice into WORK. */
    ...
    loc = lower + nlow;
    nmove = *size - (loc - 1);
    moved_(&array[loc - 1], &nmove, &work[to - 1]);

/*     Copy WORK into ARRAY. */

    moved_(work, size, array);
    return 0;
} /* t_swapad__ */
