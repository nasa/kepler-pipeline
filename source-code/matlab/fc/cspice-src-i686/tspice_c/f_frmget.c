/* f_frmget.f -- translated by f2c (version 19980913).
...
static doublereal c_b202 = 1e-12;
static integer c_b204 = -111111;
static integer c_n9999 = -9999;
static doublereal c_b218 = 1e-14;

/* $Procedure      F_FRMGET ( Family of tests for FRMGET ) */
/* Subroutine */ int f_frmget__(logical *ok)
{
    ...
    extern /* Subroutine */ int tisbod_(char *, integer *, doublereal *, 
	    doublereal *, ftnlen), clpool_(void);
...
/*     This routine tests the primary frame utility FRMGET. */
...
/*     this test. */
    ...
    frmget_(&c_b84, &et, xform, &outfrm, &found);
	...
	frmget_(&i__, &et, xform, &outfrm, &found);
	irfrot_(&i__, &c__1, rot);
	for (k = 4; k <= 6; ++k) {
	    for (j = 1; j <= 3; ++j) {
		txform[(i__1 = k + j * 6 - 7) < 36 && 0 <= i__1 ? i__1 : 
			s_rnge("txform", i__1, "f_frmget__", (ftnlen)220)] = 
			0.;
		txform[(i__1 = j + k * 6 - 7) < 36 && 0 <= i__1 ? i__1 : 
			s_rnge("txform", i__1, "f_frmget__", (ftnlen)221)] = 
			0.;
	    }
	}
	for (k = 1; k <= 3; ++k) {
	    for (j = 1; j <= 3; ++j) {
		txform[(i__1 = k + j * 6 - 7) < 36 && 0 <= i__1 ? i__1 : 
			s_rnge("txform", i__1, "f_frmget__", (ftnlen)227)] = 
			rot[(i__2 = k + j * 3 - 4) < 9 && 0 <= i__2 ? i__2 : 
			s_rnge("rot", i__2, "f_frmget__", (ftnlen)227)];
		txform[(i__1 = k + 3 + (j + 3) * 6 - 7) < 36 && 0 <= i__1 ? 
			i__1 : s_rnge("txform", i__1, "f_frmget__", (ftnlen)
			228)] = rot[(i__2 = k + j * 3 - 4) < 9 && 0 <= i__2 ? 
			i__2 : s_rnge("rot", i__2, "f_frmget__", (ftnlen)228)]
			;
	    }
	}
	j = i__;
	...
	chcksi_("OUTFRM", &outfrm, "=", &c__1, &c__0, ok, (ftnlen)6, (ftnlen)
		1);
	chckad_("XFORM", xform, "=", txform, &c__36, &c_b114, ok, (ftnlen)5, (
		ftnlen)1);
    }
...
/*        Clear the extra message. */
    ...
    tcase_("Verify that inertial class frames that are not yet supported are"
	    " not found by FRMGET. ", (ftnlen)86);
    frmget_(&c__99, &et, xform, &outfrm, &found);
    chckxc_(&c_false, " ", ok, (ftnlen)1);
	...
	chcksi_("OUTFRM", &outfrm, "=", &c__1, &c__0, ok, (ftnlen)6, (ftnlen)
		1);
	chckad_("XFORM", xform, "=", tspmi, &c__36, &c_b114, ok, (ftnlen)5, (
		ftnlen)1);
    }
    tstmsg_("#", " ", (ftnlen)1, (ftnlen)1);
	...
	frinfo_(&xpctd[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		"xpctd", i__1, "f_frmget__", (ftnlen)294)], &xcent, &xclass, &
		xclsid, &found);
	chckxc_(&c_false, " ", ok, (ftnlen)1);
...
/*              replacement is unnecessary, no harm's done. */
	    ...
	    repmi_(deftxt + ((i__2 = j - 1) < 15 && 0 <= i__2 ? i__2 : s_rnge(
		    "deftxt", i__2, "f_frmget__", (ftnlen)323)) * 80, "INSTID"
		    , &id, deftxt + ((i__3 = j - 1) < 15 && 0 <= i__3 ? i__3 :
		     s_rnge("deftxt", i__3, "f_frmget__", (ftnlen)323)) * 80, 
		    (ftnlen)80, (ftnlen)6, (ftnlen)80);
	    repmi_(deftxt + ((i__2 = j - 1) < 15 && 0 <= i__2 ? i__2 : s_rnge(
		    "deftxt", i__2, "f_frmget__", (ftnlen)324)) * 80, "INSTID"
		    , &id, deftxt + ((i__3 = j - 1) < 15 && 0 <= i__3 ? i__3 :
		     s_rnge("deftxt", i__3, "f_frmget__", (ftnlen)324)) * 80, 
		    (ftnlen)80, (ftnlen)6, (ftnlen)80);
	    chckxc_(&c_false, " ", ok, (ftnlen)1);
...
/*              Replace the center ID when it comes up. */
	    ...
	    repmi_(deftxt + ((i__2 = j - 1) < 15 && 0 <= i__2 ? i__2 : s_rnge(
		    "deftxt", i__2, "f_frmget__", (ftnlen)329)) * 80, "XCENT",
		     &xcent, deftxt + ((i__3 = j - 1) < 15 && 0 <= i__3 ? 
		    i__3 : s_rnge("deftxt", i__3, "f_frmget__", (ftnlen)329)) 
...
/*           Insert the base frame ID. */
	...
	repmi_(deftxt + ((i__1 = ntext - 1) < 15 && 0 <= i__1 ? i__1 : s_rnge(
		"deftxt", i__1, "f_frmget__", (ftnlen)336)) * 80, "XPCT", &
		xpctd[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : s_rnge("xpc"
		"td", i__2, "f_frmget__", (ftnlen)336)], deftxt + ((i__3 = 
		ntext - 1) < 15 && 0 <= i__3 ? i__3 : s_rnge("deftxt", i__3, 
		"f_frmget__", (ftnlen)336)) * 80, (ftnlen)80, (ftnlen)4, (
		ftnlen)80);
	chckxc_(&c_false, " ", ok, (ftnlen)1);
...
/*           Load the CK_-10001 frame definition. */
	...
	tstmsg_("#", "Subcase: Frame ID = # ", (ftnlen)1, (ftnlen)22);
	tstmsi_(&id);
	frmget_(&id, &et, xform, &outfrm, &found);
	chckxc_(&c_false, " ", ok, (ftnlen)1);
	chcksl_("FOUND", &found, &c_true, ok, (ftnlen)5);
	chcksi_("OUTFRM", &outfrm, "=", &xpctd[(i__1 = i__ - 1) < 3 && 0 <= 
		i__1 ? i__1 : s_rnge("xpctd", i__1, "f_frmget__", (ftnlen)353)
		], &c__0, ok, (ftnlen)6, (ftnlen)1);
	chckad_("XFORM", xform, "~", txform, &c__36, &c_b202, ok, (ftnlen)5, (
		ftnlen)1);
    }
    tcase_("Verify that I-kernel frames are recognized. ", (ftnlen)44);
    frmget_(&c_b204, &et, xform, &outfrm, &found);

/*        Construct the expected output state transformation matrix. */

    for (i__ = 1; i__ <= 6; ++i__) {
	for (j = 1; j <= 6; ++j) {
	    txform[(i__1 = i__ + j * 6 - 7) < 36 && 0 <= i__1 ? i__1 : s_rnge(
		    "txform", i__1, "f_frmget__", (ftnlen)367)] = 0.;
	}
    }
    txform[0] = .48;
    txform[1] = .6;
    ...
    chcksi_("OUTFRM", &outfrm, "=", &c_n9999, &c__0, ok, (ftnlen)6, (ftnlen)1)
	    ;
    chckad_("XFORM", xform, "~", txform, &c__36, &c_b218, ok, (ftnlen)5, (
	    ftnlen)1);
