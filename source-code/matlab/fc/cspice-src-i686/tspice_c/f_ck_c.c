...
-Procedure f_ck_c ( Test wrappers for CK routines )

 
-Abstract
 
   Perform tests on CSPICE wrappers for the CK functions. 
...
-Particulars
   ...
   This routine tests the wrappers for the CSPICE CK routines.  These
   are:
...
-Version
   ...
   -tspice_c Version 1.2.0 01-FEB-2001 (EDW)
   
      Added a spkuf_c call for CK3 to insure the remove
      call executes on all OSs.
   ...
   -tspice_c Version 1.1.0 21-SEP-1999 (NJB) 
      ...
      Now unloads CKs before deleting them because the MS Visual C++/C
      compiler requires this (specifically, the files must be closed
      for remove() to delete them).

   -tspice_c Version 1.0.0 20-SEP-1999 (NJB)
...
{ /* Begin f_ck_c */
   ...
   */
   ...
   #define  CK3            "type3.bc"
   #define  FILEN          255
   #define  IFNAME         "Test CK created by f_ck_c"
   #define  INST1          -77701
   #define  INST2          -77702
   #define  INST3          -77703
   #define  MAXREC         201
