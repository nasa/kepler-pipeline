/* spkgeo_s.f -- translated by f2c (version 19980913).
...
/* Subroutine */ int spkgeo_s__(integer *targ, doublereal *et, char *ref, 
...
/*     body relative to an observing body.   THIS is for testing only. */
/*     It differs from SPKGEO_S in that CHLEN has been set to 3. */
...
/*     SPKGEO_S computes the geometric state, T(t), of the target */
...
/*               CALL SPKGEO_S ( MOON, ET, 'J2000', EARTH, STATE, LT ) */
...
/*     1) The ephemeris files to be used by SPKGEO_S must be loaded */
/*        by SPKLEF before SPKGEO_S is called. */
...
/*     we then call the more general routine NAMFRM. */
	...
	if (frstnp_(ref, ref_len) > 0) {
	    setmsg_("The string supplied to specify the reference frame, ('#"
		    ...
		    "call to SPKGEO_S; 2. an uninitialized variable. ", (
		    ftnlen)215);
...
/*     to itself. */
    ...
    i__ = 1;
    ctarg[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("ctarg", i__1, 
	    "spkgeo_s__", (ftnlen)559)] = *targ;
    found = TRUE_;
    cleard_(&c__6, &starg[(i__1 = i__ * 6 - 6) < 18 && 0 <= i__1 ? i__1 : 
	    s_rnge("starg", i__1, "spkgeo_s__", (ftnlen)562)]);
    while(found && i__ < 3 && ctarg[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 :
	     s_rnge("ctarg", i__1, "spkgeo_s__", (ftnlen)564)] != *obs && 
	    ctarg[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : s_rnge("ctarg", 
	    i__2, "spkgeo_s__", (ftnlen)564)] != 0) {

/*        Find a file and segment that has state */
/*        data for CTARG(I). */

	spksfs_(&ctarg[(i__1 = i__ - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		"ctarg", i__1, "spkgeo_s__", (ftnlen)573)], et, &handle, 
...
/*           CTARG(I+1) and the state is called STEMP. */
	    ...
	    ++i__;
	    spkpvn_(&handle, descr, et, &tframe[(i__1 = i__ - 1) < 3 && 0 <= 
		    i__1 ? i__1 : s_rnge("tframe", i__1, "spkgeo_s__", (
		    ftnlen)583)], &starg[(i__2 = i__ * 6 - 6) < 18 && 0 <= 
		    i__2 ? i__2 : s_rnge("starg", i__2, "spkgeo_s__", (ftnlen)
		    583)], &ctarg[(i__3 = i__ - 1) < 3 && 0 <= i__3 ? i__3 : 
		    s_rnge("ctarg", i__3, "spkgeo_s__", (ftnlen)583)]);
...
/*     alternatives. */
    ...
    if (i__ == 3) {
	while(found && ctarg[2] != 0 && ctarg[2] != *obs) {

/*           Find a file and segment that has state */
/*           data for CTARG(CHLEN). */

	    spksfs_(&ctarg[2], et, &handle, descr, ident, &found, (ftnlen)40);
	    if (found) {
...
/*              STEMP. */
		...
		spkpvn_(&handle, descr, et, &tmpfrm, stemp, &ctarg[2]);
...
/*              the last element of STARG. */
		...
		if (tframe[2] == tmpfrm) {
		    moved_(&starg[12], &c__6, vtemp);
		} else if (tmpfrm > 0 && tmpfrm <= 21 && tframe[2] > 0 && 
			tframe[2] <= 21) {
		    irfrot_(&tframe[2], &tmpfrm, rot);
		    mxv_(rot, &starg[12], vtemp);
		    mxv_(rot, &starg[15], &vtemp[3]);
		} else {
		    frmchg_(&tframe[2], &tmpfrm, et, stxfrm);
		    if (failed_()) {
			chkout_("SPKGEO_S", (ftnlen)8);
			return 0;
		    }
		    mxvg_(stxfrm, &starg[12], &c__6, &c__6, vtemp);
		}
		vaddg_(vtemp, stemp, &c__6, &starg[12]);
		tframe[2] = tmpfrm;
...
/*     be zero if COBS is not found in CTARG. */
    ...
    if (ctarg[(i__1 = nct - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("ctarg", i__1,
	     "spkgeo_s__", (ftnlen)719)] == cobs) {
	ctpos = nct;
	cframe = tframe[(i__1 = ctpos - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		"tframe", i__1, "spkgeo_s__", (ftnlen)721)];
    } else {
	ctpos = 0;
    }
...
/*     Compute the state of the target relative to CTARG(CTPOS) */
	...
	if (tframe[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : s_rnge("tframe",
		 i__2, "spkgeo_s__", (ftnlen)917)] == tframe[(i__3 = i__) < 3 
		&& 0 <= i__3 ? i__3 : s_rnge("tframe", i__3, "spkgeo_s__", (
		ftnlen)917)]) {
	    vaddg_(&starg[(i__2 = i__ * 6 - 6) < 18 && 0 <= i__2 ? i__2 : 
		    s_rnge("starg", i__2, "spkgeo_s__", (ftnlen)919)], &starg[
		    (i__3 = (i__ + 1) * 6 - 6) < 18 && 0 <= i__3 ? i__3 : 
		    s_rnge("starg", i__3, "spkgeo_s__", (ftnlen)919)], &c__6, 
		    vtemp);
	    moved_(vtemp, &c__6, &starg[(i__2 = (i__ + 1) * 6 - 6) < 18 && 0 
		    <= i__2 ? i__2 : s_rnge("starg", i__2, "spkgeo_s__", (
		    ftnlen)920)]);
	} else if (tframe[(i__3 = i__) < 3 && 0 <= i__3 ? i__3 : s_rnge("tfr"
		"ame", i__3, "spkgeo_s__", (ftnlen)922)] > 0 && tframe[(i__3 = 
		i__) < 3 && 0 <= i__3 ? i__3 : s_rnge("tframe", i__3, "spkge"
		"o_s__", (ftnlen)922)] <= 21 && tframe[(i__2 = i__ - 1) < 3 && 
		0 <= i__2 ? i__2 : s_rnge("tframe", i__2, "spkgeo_s__", (
		ftnlen)922)] > 0 && tframe[(i__2 = i__ - 1) < 3 && 0 <= i__2 ?
		 i__2 : s_rnge("tframe", i__2, "spkgeo_s__", (ftnlen)922)] <= 
		21) {
	    irfrot_(&tframe[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : s_rnge(
		    "tframe", i__2, "spkgeo_s__", (ftnlen)924)], &tframe[(
		    i__3 = i__) < 3 && 0 <= i__3 ? i__3 : s_rnge("tframe", 
		    i__3, "spkgeo_s__", (ftnlen)924)], rot);
	    mxv_(rot, &starg[(i__2 = i__ * 6 - 6) < 18 && 0 <= i__2 ? i__2 : 
		    s_rnge("starg", i__2, "spkgeo_s__", (ftnlen)925)], stemp);
	    mxv_(rot, &starg[(i__2 = i__ * 6 - 3) < 18 && 0 <= i__2 ? i__2 : 
		    s_rnge("starg", i__2, "spkgeo_s__", (ftnlen)926)], &stemp[
		    3]);
	    vaddg_(stemp, &starg[(i__2 = (i__ + 1) * 6 - 6) < 18 && 0 <= i__2 
		    ? i__2 : s_rnge("starg", i__2, "spkgeo_s__", (ftnlen)927)]
		    , &c__6, vtemp);
	    moved_(vtemp, &c__6, &starg[(i__2 = (i__ + 1) * 6 - 6) < 18 && 0 
		    <= i__2 ? i__2 : s_rnge("starg", i__2, "spkgeo_s__", (
		    ftnlen)928)]);
	} else {
	    frmchg_(&tframe[(i__2 = i__ - 1) < 3 && 0 <= i__2 ? i__2 : s_rnge(
		    "tframe", i__2, "spkgeo_s__", (ftnlen)932)], &tframe[(
		    i__3 = i__) < 3 && 0 <= i__3 ? i__3 : s_rnge("tframe", 
		    i__3, "spkgeo_s__", (ftnlen)932)], et, stxfrm);
	    if (failed_()) {
		chkout_("SPKGEO_S", (ftnlen)8);
		return 0;
	    }
	    mxvg_(stxfrm, &starg[(i__2 = i__ * 6 - 6) < 18 && 0 <= i__2 ? 
		    i__2 : s_rnge("starg", i__2, "spkgeo_s__", (ftnlen)939)], 
		    &c__6, &c__6, stemp);
	    vaddg_(stemp, &starg[(i__2 = (i__ + 1) * 6 - 6) < 18 && 0 <= i__2 
		    ? i__2 : s_rnge("starg", i__2, "spkgeo_s__", (ftnlen)940)]
		    , &c__6, vtemp);
	    moved_(vtemp, &c__6, &starg[(i__2 = (i__ + 1) * 6 - 6) < 18 && 0 
		    <= i__2 ? i__2 : s_rnge("starg", i__2, "spkgeo_s__", (
		    ftnlen)941)]);
...
/*     frame transformations. */
    ...
    if (tframe[(i__1 = ctpos - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge("tframe", 
	    i__1, "spkgeo_s__", (ftnlen)954)] == cframe) {
	vsubg_(&starg[(i__1 = ctpos * 6 - 6) < 18 && 0 <= i__1 ? i__1 : 
		s_rnge("starg", i__1, "spkgeo_s__", (ftnlen)956)], sobs, &
		c__6, state);
    } else if (tframe[(i__1 = ctpos - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
	    "tframe", i__1, "spkgeo_s__", (ftnlen)958)] == refid) {
...
/*        Set CFRAME to reflect this. */
	...
	cframe = refid;
	vsubg_(&starg[(i__1 = ctpos * 6 - 6) < 18 && 0 <= i__1 ? i__1 : 
		s_rnge("starg", i__1, "spkgeo_s__", (ftnlen)990)], stemp, &
		c__6, state);
    } else if (cframe > 0 && cframe <= 21 && tframe[(i__1 = ctpos - 1) < 3 && 
	    0 <= i__1 ? i__1 : s_rnge("tframe", i__1, "spkgeo_s__", (ftnlen)
	    993)] > 0 && tframe[(i__1 = ctpos - 1) < 3 && 0 <= i__1 ? i__1 : 
	    s_rnge("tframe", i__1, "spkgeo_s__", (ftnlen)993)] <= 21) {

/*        If both frames are inertial we use IRFROT instead of */
/*        FRMCHG to get things into a common frame. */

	irfrot_(&tframe[(i__1 = ctpos - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		"tframe", i__1, "spkgeo_s__", (ftnlen)999)], &cframe, rot);
	mxv_(rot, &starg[(i__1 = ctpos * 6 - 6) < 18 && 0 <= i__1 ? i__1 : 
		s_rnge("starg", i__1, "spkgeo_s__", (ftnlen)1000)], stemp);
	mxv_(rot, &starg[(i__1 = ctpos * 6 - 3) < 18 && 0 <= i__1 ? i__1 : 
		s_rnge("starg", i__1, "spkgeo_s__", (ftnlen)1001)], &stemp[3])
		;
...
/*        Use the more general routine FRMCHG to make the transformation. */
	...
	frmchg_(&tframe[(i__1 = ctpos - 1) < 3 && 0 <= i__1 ? i__1 : s_rnge(
		"tframe", i__1, "spkgeo_s__", (ftnlen)1008)], &cframe, et, 
		stxfrm);
	if (failed_()) {
	    chkout_("SPKGEO_S", (ftnlen)8);
	    return 0;
	}
	mxvg_(stxfrm, &starg[(i__1 = ctpos * 6 - 6) < 18 && 0 <= i__1 ? i__1 :
		 s_rnge("starg", i__1, "spkgeo_s__", (ftnlen)1015)], &c__6, &
		c__6, stemp);
	vsubg_(stemp, sobs, &c__6, state);
    }
