/* f_dyn03.f -- translated by f2c (version 19980913).
...
static doublereal c_b195 = 1e-19;

/* $Procedure F_DYN03 ( Dynamic Frame Test Family 03 ) */
/* Subroutine */ int f_dyn03__(logical *ok)
{
    /* Initialized data */
    ...
    doublereal dmag;
    integer case__;
    doublereal dmob, nmat[9]	/* was [3][3] */;
    ...
    extern /* Subroutine */ int eul2m_(doublereal *, doublereal *, doublereal 
	    *, integer *, integer *, integer *, doublereal *), zzmobliq_(
	    doublereal *, doublereal *, doublereal *), zzeprcss_(doublereal *,
	     doublereal *);
    integer i__, j;
    ...
    doublereal xform[36]	/* was [6][6] */, dvnut[4], r2[9]	/* 
	    was [3][3] */, rplus[9]	/* was [3][3] */, t0;
    ...
    doublereal deterr, drlerr, drvblk[9]	/* was [3][3] */, drverr, 
	    nrmerr, rminus[9]	/* was [3][3] */, todmat[9]	/* was [3][3] 
	    */;
    ...
    extern /* Subroutine */ int tstlsk_(void), tstspk_(char *, logical *, 
	    ...
	    ftnlen), zzwahr_(doublereal *, doublereal *);
    doublereal mob;
...
/*     subset of dynamic frame routines.  Only true equator and */
...
/*     As the set of true-of-date frames grows, this routine should */
...
/*        Set the rotation state. */
	...
	s_copy(rststr, rstate + ((i__2 = coords[0] - 1) < 3 && 0 <= i__2 ? 
		i__2 : s_rnge("rstate", i__2, "f_dyn03__", (ftnlen)496)) * 80,
		 (ftnlen)80, (ftnlen)80);

/*        Set the base frame. */

	s_copy(bframe, basefr + (((i__2 = coords[1] - 1) < 3 && 0 <= i__2 ? 
		i__2 : s_rnge("basefr", i__2, "f_dyn03__", (ftnlen)501)) << 5)
		, (ftnlen)32, (ftnlen)32);

/*        Set the frame center. */

	center = ctrcde[(i__2 = coords[2] - 1) < 1 && 0 <= i__2 ? i__2 : 
		s_rnge("ctrcde", i__2, "f_dyn03__", (ftnlen)506)];
...
/*           First comes the frame name-to-frame code assignment. */
	    ...
	    s_copy(frname, "TRUE_OF_DATE_DYN_FRAME", (ftnlen)32, (ftnlen)22);
	    frcode = -2000000000;
...
/* Writing concatenation */
	    ...
	    i__4[2] = 14, a__2[2] = "           = '";
	    i__4[3] = 32, a__2[3] = "TRUE_EQUATOR_AND_EQUINOX_OF_DATE";
	    i__4[4] = 1, a__2[4] = "'";
...
/*           to keep this code close to the associated SXFORM call. */
		...
		for (j = 1; j <= 3; ++j) {
		    drvblk[(i__2 = j + i__ * 3 - 4) < 9 && 0 <= i__2 ? i__2 : 
			    s_rnge("drvblk", i__2, "f_dyn03__", (ftnlen)667)] 
			    = xform[(i__6 = j + 3 + i__ * 6 - 7) < 36 && 0 <= 
			    i__6 ? i__6 : s_rnge("xform", i__6, "f_dyn03__", (
			    ftnlen)667)];
		}
	    }
...
/*           Extract the rotation block from XFORM into R2. */
		...
		for (j = 1; j <= 3; ++j) {
		    r2[(i__2 = j + i__ * 3 - 4) < 9 && 0 <= i__2 ? i__2 : 
			    s_rnge("r2", i__2, "f_dyn03__", (ftnlen)678)] = 
			    xform[(i__6 = j + i__ * 6 - 7) < 36 && 0 <= i__6 ?
			     i__6 : s_rnge("xform", i__6, "f_dyn03__", (
			    ftnlen)678)];
		}
	    }
	    chckad_("SXFORM vs PXFORM", r2, "~", r__, &c__9, &c_b157, ok, (
		    ftnlen)16, (ftnlen)1);
...
/*           up is the determinant error. */
	    ...
	    tol = 1e-14;
	    chcksd_("DETERR", &deterr, "~", &c_b170, &tol, ok, (ftnlen)6, (
		    ftnlen)1);

/*           Check norms of rows and columns in the rotation */
/*           blocks. */

	    chcksd_("NRMERR", &nrmerr, "~", &c_b170, &tol, ok, (ftnlen)6, (
		    ftnlen)1);
...
/*              Check the derivative absolute error. */
		...
		tol = 1e-12;
		chcksd_("DRVERR", &drverr, "~", &c_b170, &tol, ok, (ftnlen)6, 
			(ftnlen)1);

/*              Check the derivative relative error. */

		tol = 1e-5;
		chcksd_("DRLERR", &drlerr, "~", &c_b170, &tol, ok, (ftnlen)6, 
			(ftnlen)1);
...
/*              transformation is close to zero. */
		...
		for (i__ = 1; i__ <= 3; ++i__) {
			...
			drv2[(i__2 = j + i__ * 3 - 4) < 9 && 0 <= i__2 ? i__2 
				: s_rnge("drv2", i__2, "f_dyn03__", (ftnlen)
				756)] = xff2j[(i__6 = j + 3 + i__ * 6 - 7) < 
				36 && 0 <= i__6 ? i__6 : s_rnge("xff2j", i__6,
				 "f_dyn03__", (ftnlen)756)];
		    }
		}
		chckad_("Derivative block", drv2, "~", zr, &c__9, &c_b195, ok,
			 (ftnlen)16, (ftnlen)1);
	    } else {
...
/*              with respect to the base frame is zero. */
		...
		chckad_("Derivative block", drvblk, "=", zr, &c__9, &c_b170, 
...
/*           Get nutation angles.  Form nutation matrix. */
	    ...
	    d__1 = -mob - nuobl;
	    d__2 = -nulon;
	    eul2m_(&d__1, &d__2, &mob, &c__1, &c__3, &c__1, nmat);
	    chckxc_(&c_false, " ", ok, (ftnlen)1);

/*           Form the J2000 to true-of-date matrix. */

	    mxm_(nmat, precm, todmat);
