/* chgirf_o.f -- translated by f2c (version 19980913).
...
/* Subroutine */ int chgirf_o__0_(int n__, integer *refa, integer *refb, 
...
/*     REFB       I   IRFROT_O */
/*     ROTAB      O   IRFROT_O */
/*     NAME      I/O  IRFNUM_O, IRFNAM_O, IRFDEF_O */
/*     INDEX     I/O  IRFNUM_O, IRFNAM_O */

/* $ Detailed_Input */

/*     See entry points IRFROT_O, IRFNUM_O, IRFNAM_O, and IRFDEF_O. */

/* $ Detailed_Output */

/*     See entry points IRFROT_O, IRFNUM_O, and IRFNAM_O. */
...
/*     1) If CHGIRF_O is called directly, the signal SPICE(BOGUSENTRY) */
/*        is signalled. */

/*     2) See entry points IRFROT_O, IRFNUM_O, IRFNAM_O, and IRFDEF_O for */
/*        exceptions specific to those routines. */

/* $ Particulars */

/*     CHGIRF_O exists only as an umbrella for data to be shared */
/*     by its entry points (IRFROT_O, IRFNUM_O, IRFNAM_O, and IRFDEF_O). */
...
/*        Added DE-143 to header description for IRFROT_O. */

/* -    SPICELIB Version 4.2.0, 10-APR-1997 (WLT) */

/*        A descriptive diagnostic was added to the entry points */
/*        IRFROT_O and IRFDEF_O.  Before they simply signalled the error */
/*        with no diagnostic. */
...
/*     by CHGIRF_O must be updated whenever new frames are added. */
    ...
    /* Function Body */
    switch(n__) {
	case 1: goto L_irfrot_o;
	case 2: goto L_irfnum_o;
	case 3: goto L_irfnam_o;
	case 4: goto L_irfdef_o;
	}
...
/*     Standard SPICE error handling. */
	...
	chkin_("CHGIRF_O", (ftnlen)8);
    }
    sigerr_("SPICE(BOGUSENTRY)", (ftnlen)17);
    chkout_("CHGIRF_O", (ftnlen)8);
    return 0;
...
/*     IRFROT_O exists primarily for use by the ephemeris and star */
/*     catalog readers in the SPICELIB toolkit library. */

/* $ Examples */

/*     In the following code fragment, IRFROT_O is used to rotate */
...
/*         root->frame      base->frame    root->base */
	...
	for (i__ = 1; i__ <= 21; ++i__) {
	    rotate_(&c_b6, &c__1, &trans[(i__1 = i__ * 9 - 9) < 189 && 0 <= 
		    i__1 ? i__1 : s_rnge("trans", i__1, "chgirf_o__", (ftnlen)
		    868)]);
	    for (j = wdcnt_(defs + ((i__1 = i__ - 1) < 21 && 0 <= i__1 ? i__1 
		    : s_rnge("defs", i__1, "chgirf_o__", (ftnlen)870)) * 80, (
		    ftnlen)80); j >= 2; j += -2) {
		nthwd_(defs + ((i__1 = i__ - 1) < 21 && 0 <= i__1 ? i__1 : 
			s_rnge("defs", i__1, "chgirf_o__", (ftnlen)872)) * 80,
			 &j, word, &loc, (ftnlen)80, (ftnlen)25);
		nparsi_(word, &axis, error, &p, (ftnlen)25, (ftnlen)25);
		i__2 = j - 1;
		nthwd_(defs + ((i__1 = i__ - 1) < 21 && 0 <= i__1 ? i__1 : 
			s_rnge("defs", i__1, "chgirf_o__", (ftnlen)875)) * 80,
		...
		rotmat_(&trans[(i__1 = i__ * 9 - 9) < 189 && 0 <= i__1 ? i__1 
			: s_rnge("trans", i__1, "chgirf_o__", (ftnlen)880)], &
			radang, &axis, tmpmat);
		moved_(tmpmat, &c__9, &trans[(i__1 = i__ * 9 - 9) < 189 && 0 
			<= i__1 ? i__1 : s_rnge("trans", i__1, "chgirf_o__", (
			ftnlen)881)]);
	    }
	    b = isrchc_(bases + (((i__1 = i__ - 1) < 21 && 0 <= i__1 ? i__1 : 
		    s_rnge("bases", i__1, "chgirf_o__", (ftnlen)885)) << 4), &
		    i__, frames, (ftnlen)16, (ftnlen)16);
	    mxm_(&trans[(i__1 = i__ * 9 - 9) < 189 && 0 <= i__1 ? i__1 : 
		    s_rnge("trans", i__1, "chgirf_o__", (ftnlen)887)], &trans[
		    (i__2 = b * 9 - 9) < 189 && 0 <= i__2 ? i__2 : s_rnge(
		    "trans", i__2, "chgirf_o__", (ftnlen)887)], tmpmat);
	    moved_(tmpmat, &c__9, &trans[(i__1 = i__ * 9 - 9) < 189 && 0 <= 
		    i__1 ? i__1 : s_rnge("trans", i__1, "chgirf_o__", (ftnlen)
		    888)]);
	}
...
/*     should work, but why risk roundoff problems? */
	...
	mxmt_(&trans[(i__1 = *refb * 9 - 9) < 189 && 0 <= i__1 ? i__1 : 
		s_rnge("trans", i__1, "chgirf_o__", (ftnlen)943)], &trans[(
		i__2 = *refa * 9 - 9) < 189 && 0 <= i__2 ? i__2 : s_rnge(
		"trans", i__2, "chgirf_o__", (ftnlen)943)], rotab);
    }
    chkout_("IRFROT_O", (ftnlen)8);
    return 0;
...
/*                 reference frames supported by IRFROT_O, or */
/*                 'DEFAULT'. */
...
/*     IRFNUM_O is supplied as a convenience, to allow users to refer to */
...
/*        CALL IRFNUM_O ( 'DE-118', A ) */
/*        CALL IRFNUM_O ( 'FK4',    B ) */

/*        CALL IRFROT_O ( A, B, ROTAB ) */

/*     IRFNUM_O can be used to rotate vectors into the default frame, */
/*     as illustrated by the following code fragment. */

/*        CALL IRFNUM_O ( 'FK4',     A ) */
/*        CALL IRFNUM_O ( 'DEFAULT', B ) */
...
/*                 reference frames supported by IRFROT_O. */
...
/*     IRFNAM_O is supplied as a convenience, to allow users to determine */
...
/*        CALL IRFNAM_O ( A, NAME(1) ) */
/*        CALL IRFNAM_O ( B, NAME(2) ) */
...
/*        CALL IRFNUM_O ( 'DEFAULT', DINDEX        ) */
/*        CALL IRFNAM_O (            DINDEX, DNAME ) */
...
/*     Standard SPICE error handling. */
	...
	chkin_("IRFNAM_O", (ftnlen)8);
    }
    if (*index < 1 || *index > 21) {
	s_copy(name__, " ", name_len, (ftnlen)1);
    } else {
	s_copy(name__, frames + (((i__1 = *index - 1) < 21 && 0 <= i__1 ? 
		i__1 : s_rnge("frames", i__1, "chgirf_o__", (ftnlen)1348)) << 
		4), name_len, (ftnlen)16);
    }
    chkout_("IRFNAM_O", (ftnlen)8);
    return 0;
...
/*     IRFDEF_O allows tools to be written at a relatively high level */
...
/*        C */
/*              CALL IRFNUM_O ( 'DEFAULT', REFD   ) */

/*              DO I = 1, NVEC */
/*                 CALL IRFROT_O ( REFIN, REFD, ROT           ) */
...
/*     There's not much to do, except save the value for later use. */
    ...
    chkout_("IRFDEF_O", (ftnlen)8);
    return 0;
} /* chgirf_o__ */

/* Subroutine */ int chgirf_o__(integer *refa, integer *refb, doublereal *
	rotab, char *name__, integer *index, ftnlen name_len)
{
    return chgirf_o__0_(0, refa, refb, rotab, name__, index, name_len);
    }

/* Subroutine */ int irfrot_o__(integer *refa, integer *refb, doublereal *
	rotab)
{
    return chgirf_o__0_(1, refa, refb, rotab, (char *)0, (integer *)0, (
	    ftnint)0);
    }

/* Subroutine */ int irfnum_o__(char *name__, integer *index, ftnlen name_len)
{
    return chgirf_o__0_(2, (integer *)0, (integer *)0, (doublereal *)0, 
	    name__, index, name_len);
    }

/* Subroutine */ int irfnam_o__(integer *index, char *name__, ftnlen name_len)
{
    return chgirf_o__0_(3, (integer *)0, (integer *)0, (doublereal *)0, 
	    name__, index, name_len);
    }

/* Subroutine */ int irfdef_o__(integer *index)
{
    return chgirf_o__0_(4, (integer *)0, (integer *)0, (doublereal *)0, (char 
	    *)0, index, (ftnint)0);
    }
