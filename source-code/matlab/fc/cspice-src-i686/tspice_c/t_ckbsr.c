/* t_ckbsr.f -- translated by f2c (version 19980913).
...
/* Subroutine */ int t_cbr__0_(int n__, char *fname, integer *handle, integer 
	...
	ftnlen segid_len)
{
    ...
    /* Local variables */
    integer head, tail;
    static doublereal itlb[20], itub[20];
    integer cost;
    ...
    static integer itbeg[20], slbeg;
    extern /* Subroutine */ int chkin_(char *, ftnlen), dafps_(integer *, 
	    integer *, doublereal *, integer *, doublereal *);
    static integer fthan[110];
    static doublereal stdcd[200]	/* was [2][100] */;
    char doing[40], stack[40*2];
    static integer sticd[600]	/* was [6][100] */;
    extern doublereal dpmin_(void), dpmax_(void);
    extern /* Subroutine */ int moved_(doublereal *, integer *, doublereal *),
	     dafus_(doublereal *, integer *, integer *, doublereal *, integer 
	    *);
    static integer ithfs[20], sthan[100];
    extern /* Subroutine */ int lnkan_(integer *, integer *);
    static integer itlfs[20];
    extern integer lnktl_(integer *, integer *);
    static integer itins[20], ftnum[110], itexp[20];
    ...
    static char stidnt[40*100];
    char urgent[40];
    static doublereal itprvd[100]	/* was [5][20] */;
    static integer itprvf[20];
    integer nxtseg;
    extern integer lnkprv_(integer *, integer *);
    static char itprvi[40*20];
    extern integer lnknxt_(integer *, integer *);
    extern logical return_(void);
    static integer itprvh[20], itruex[20], stpool[212]	/* was [2][106] */, 
	    scinst;
...
/*     The purpose of entry points T_CBS and T_CSN is to search for */
...
/*     A segment matches the T_CBS/T_CSN search criteria when the */
...
/*                be loaded by T_CLF at any given time for use by the */
/*                readers. */

/*     ITSIZE     is the maximum number of instruments whose segments */
/*                are buffered by T_CSN. */

/*     STSIZE     is the maximum number of segments that can be buffered */
/*                at any given time by T_CSN. */

/* $ Files */

/*     C-kernel pointing files are indicated by filename before loading */
/*     (see T_CLF) and handle after loading (all other places). */

/* $ Exceptions */

/*     1) If T_CBR is called directly, the error SPICE(CKBOGUSENTRY) */
/*        is signaled. */

/*     2) See entry points T_CLF, T_CUF, T_CBS, and T_CSN for exceptions */
/*        specific to them. */

/* $ Particulars */

/*     T_CBR serves as an umbrella, allowing data to be shared by its */
/*     entry points: */

/*        T_CLF       Load pointing file. */
/*        T_CUF       Unload pointing file. */
/*        T_CBS       Begin search for segment. */
/*        T_CSN       Select next segment. */

/*     Before a file can be read by the C-kernel readers, it must be */
/*     loaded by T_CLF, which among other things load the file into */
/*     the DAF subsystem. */
...
/*     searching by unloading it via a call to T_CUF. */

/*     T_CBS and T_CSN are used together to search through loaded files */
/*     for segments. */

/*     T_CBS sets up the search. You tell it the instrument and time */
/*     that you are interested in, and whether you require segments */
/*     containing angular velocity data. */

/*     T_CSN finds segments matching the search criteria set up by */
/*     T_CBS. Last-loaded files get searched first, and individual files */
/*     are searched backwards. */

/*     When an applicable segment is found, T_CSN returns that segment's */
/*     descriptor and identifier, along with the handle of the file */
/*     containing the segment. */

/*     Subsequent calls to T_CSN continue the search, picking up where */
/*     the previous call to this routine left off. */

/*     T_CSN uses information on loaded files to manage a buffer */
...
/*     C        the original file. */
/*     C */
/*              CALL T_CLF ( UDFILE ( UPDATE ), HUPDT ) */
...
/*     C */
/*                 CALL T_CBS ( INST,   SCLKDP, TOL,   NEEDAV  ) */
/*                 CALL T_CSN ( HANDLE, DESCR,  SEGID, SFOUND  ) */
...
/* -    SPICELIB Version 4.2.0, 10-NOV-2005 (NJB) */

/*        Updated to remove non-standard use of duplicate arguments in */
/*        MOVED calls in entry points T_CKUF and T_CKSN. */

/* -    SPICELIB Version 4.1.0, 28-NOV-2001 (NJB) */
...
/*           4) The load routine T_CLF now resets all file numbers when */
/*              the next file number reaches INTMAX()-1, thereby */
/*              avoiding arithmetic overflow. */

/*           5) The unload routine T_CUF now calls RETURN() on entry and */
/*              returns if so directed. */

/*           6) In T_CSN, DAF calls are followed by tests of FAILED() */
...
/*        Added the Entry point T_CHAV */

/* -    SPICELIB Version 3.0.0, 03-MAR-1999 (WLT) */

/*        The parameter STSIZE was increased from 1000 to 4000 to */
/*        avoid the buffering error that exists in the T_CBR. */

/* -    SPICELIB Version 2.0.0, 25-NOV-1992 (JML) */

/*     1) When loading a file, T_CLF now checks if the file table is */
...
/*     2) A bug in the way that T_CLF and T_CUF clean up the instrument */
...
/*        that an error will be signaled if T_CSN is called */
/*        without T_CBS ever having been called to initiate the */
/*        search. */
...
/*     Nobody has any business calling T_CBR directly. */
    ...
    chkin_("T_CBR", (ftnlen)5);
    sigerr_("SPICE(CKBOGUSENTRY)", (ftnlen)19);
    chkout_("T_CBR", (ftnlen)5);
    return 0;
/* $Procedure T_CLF ( C-kernel, load pointing file ) */
...
/*     assigned an integer handle by T_CLF.  Other CK routines will refer */
/*     to this file by its handle. */

/* $ Particulars */

/*     See Particulars above, in T_CBR. */

/*     If there is room for a new file, T_CLF opens the file for */
/*     reading.  This routine must be called prior to a call to CKGP or */
/*     CKGPAV. */

/*     CK readers search files loaded with T_CLF in the reverse order */
/*     in which they were loaded.  That is, last-loaded files are */
/*     searched first. */

/* $ Examples */

/*     See the Example above, in T_CBR. */
...
/*     intervals are invalid. */
	...
	itchkp[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge("itchkp", 
		i__2, "t_cbr__", (ftnlen)1234)] = FALSE_;
    }
...
/*        Handle is already in the table.  Remove it. */
	...
	for (i__ = findex; i__ <= i__1; ++i__) {
	    fthan[(i__2 = i__ - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("fthan",
		     i__2, "t_cbr__", (ftnlen)1274)] = fthan[(i__3 = i__) < 
		    110 && 0 <= i__3 ? i__3 : s_rnge("fthan", i__3, "t_cbr__",
		     (ftnlen)1274)];
	    ftnum[(i__2 = i__ - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum",
		     i__2, "t_cbr__", (ftnlen)1275)] = ftnum[(i__3 = i__) < 
		    110 && 0 <= i__3 ? i__3 : s_rnge("ftnum", i__3, "t_cbr__",
		     (ftnlen)1275)];
	}
...
/*        Unlink any segments that came from this file. */
	...
	i__ = 1;
	while(i__ <= nit) {
	    p = itbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "itbeg", i__1, "t_cbr__", (ftnlen)1285)];
	    while(p > 0) {
...
/*              Find the successor of P, if any. */
		...
		if (sthan[(i__1 = p - 1) < 100 && 0 <= i__1 ? i__1 : s_rnge(
			"sthan", i__1, "t_cbr__", (ftnlen)1293)] == *handle) {
...
/*                 make the successor of P the head of the list. */
		    ...
		    lnkfsl_(&p, &p, stpool);
		    if (p == itbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 :
			     s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)1303)]) 
			    {
			itbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)1304)
				] = nxtseg;
		    }
		}
...
/*           one we've deleted. */
	    ...
	    if (itbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "itbeg", i__1, "t_cbr__", (ftnlen)1321)] <= 0) {
...
/*                 ITUB */
		...
		itins[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itins", i__1, "t_cbr__", (ftnlen)1336)] = itins[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itins", i__2, "t_cbr__", (ftnlen)1336)];
		itexp[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itexp", i__1, "t_cbr__", (ftnlen)1337)] = itexp[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itexp", i__2, "t_cbr__", (ftnlen)1337)];
		ithfs[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"ithfs", i__1, "t_cbr__", (ftnlen)1338)] = ithfs[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"ithfs", i__2, "t_cbr__", (ftnlen)1338)];
		itlfs[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itlfs", i__1, "t_cbr__", (ftnlen)1339)] = itlfs[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itlfs", i__2, "t_cbr__", (ftnlen)1339)];
		itbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itbeg", i__1, "t_cbr__", (ftnlen)1340)] = itbeg[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itbeg", i__2, "t_cbr__", (ftnlen)1340)];
		--nit;
	    } else {
		++i__;
	    }
...
/*           Re-map the HFS table for the Ith instrument. */
	    ...
	    j = isrchi_(&ithfs[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : 
		    s_rnge("ithfs", i__2, "t_cbr__", (ftnlen)1399)], &nft, 
		    ftnum);
	    if (j > 0) {
...
/*              file in the file table. */
		...
		ithfs[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"ithfs", i__2, "t_cbr__", (ftnlen)1406)] = j;
	    } else {
...
/*              I is performed. */
		...
		ithfs[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"ithfs", i__2, "t_cbr__", (ftnlen)1416)] = 0;
	    }

/*           Re-map the LFS table for the Ith instrument. */

	    j = isrchi_(&itlfs[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : 
		    s_rnge("itlfs", i__2, "t_cbr__", (ftnlen)1423)], &nft, 
		    ftnum);
	    if (j > 0) {
...
/*              in the file table. */
		...
		itlfs[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itlfs", i__2, "t_cbr__", (ftnlen)1430)] = j;
	    } else {
...
/*              list. */
		...
		itlfs[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itlfs", i__2, "t_cbr__", (ftnlen)1440)] = 0;
		ithfs[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"ithfs", i__2, "t_cbr__", (ftnlen)1441)] = 0;
	    }
	}
...
/*     Now add this file to file table. */
    ...
    ++nft;
    fthan[(i__1 = nft - 1) < 110 && 0 <= i__1 ? i__1 : s_rnge("fthan", i__1, 
	    "t_cbr__", (ftnlen)1467)] = *handle;
    ftnum[(i__1 = nft - 1) < 110 && 0 <= i__1 ? i__1 : s_rnge("ftnum", i__1, 
	    "t_cbr__", (ftnlen)1468)] = next;
    chkout_("T_CLF", (ftnlen)5);
    return 0;
/* $Procedure T_CUF ( C-kernel, Unload pointing file ) */
...
/*     See Particulars section above, in T_CBR. */

/*     Unloading a file with T_CUF removes that file from consideration */
...
/*     1) A bug in the way that T_CLF and T_CUF clean up the instrument */
/*        tables after a file is unloaded was fixed. */
...
/*     before wiping out the handle. */
    ...
    dafcls_(&fthan[(i__1 = findex - 1) < 110 && 0 <= i__1 ? i__1 : s_rnge(
	    "fthan", i__1, "t_cbr__", (ftnlen)1724)]);
    --nft;
    i__1 = nft;
    for (i__ = findex; i__ <= i__1; ++i__) {
	fthan[(i__2 = i__ - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("fthan", 
		i__2, "t_cbr__", (ftnlen)1730)] = fthan[(i__3 = i__) < 110 && 
		0 <= i__3 ? i__3 : s_rnge("fthan", i__3, "t_cbr__", (ftnlen)
		1730)];
	ftnum[(i__2 = i__ - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", 
		i__2, "t_cbr__", (ftnlen)1731)] = ftnum[(i__3 = i__) < 110 && 
		0 <= i__3 ? i__3 : s_rnge("ftnum", i__3, "t_cbr__", (ftnlen)
		1731)];
    }
...
/*     specially. */
    ...
    i__ = 1;
    while(i__ <= nit) {
	p = itbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itbeg", 
		i__1, "t_cbr__", (ftnlen)1743)];
	while(p > 0) {
	    nxtseg = lnknxt_(&p, stpool);
	    if (sthan[(i__1 = p - 1) < 100 && 0 <= i__1 ? i__1 : s_rnge("sth"
		    "an", i__1, "t_cbr__", (ftnlen)1749)] == *handle) {
		if (p == itbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : 
			s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)1751)]) {
		    itbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			    "itbeg", i__1, "t_cbr__", (ftnlen)1752)] = nxtseg;
		}
...
/*        one we've deleted. */
	...
	if (itbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itbeg", 
		i__1, "t_cbr__", (ftnlen)1771)] == 0) {
	    itins[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itins", 
		    i__1, "t_cbr__", (ftnlen)1773)] = itins[(i__2 = nit - 1) <
		     20 && 0 <= i__2 ? i__2 : s_rnge("itins", i__2, "t_cbr__",
		     (ftnlen)1773)];
	    itexp[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itexp", 
		    i__1, "t_cbr__", (ftnlen)1774)] = itexp[(i__2 = nit - 1) <
		     20 && 0 <= i__2 ? i__2 : s_rnge("itexp", i__2, "t_cbr__",
		     (ftnlen)1774)];
	    ithfs[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("ithfs", 
		    i__1, "t_cbr__", (ftnlen)1775)] = ithfs[(i__2 = nit - 1) <
		     20 && 0 <= i__2 ? i__2 : s_rnge("ithfs", i__2, "t_cbr__",
		     (ftnlen)1775)];
	    itlfs[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itlfs", 
		    i__1, "t_cbr__", (ftnlen)1776)] = itlfs[(i__2 = nit - 1) <
		     20 && 0 <= i__2 ? i__2 : s_rnge("itlfs", i__2, "t_cbr__",
		     (ftnlen)1776)];
	    itbeg[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itbeg", 
		    i__1, "t_cbr__", (ftnlen)1777)] = itbeg[(i__2 = nit - 1) <
		     20 && 0 <= i__2 ? i__2 : s_rnge("itbeg", i__2, "t_cbr__",
		     (ftnlen)1777)];
	    itlb[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itlb", 
		    i__1, "t_cbr__", (ftnlen)1778)] = itlb[(i__2 = nit - 1) < 
		    20 && 0 <= i__2 ? i__2 : s_rnge("itlb", i__2, "t_cbr__", (
		    ftnlen)1778)];
	    itub[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itub", 
		    i__1, "t_cbr__", (ftnlen)1779)] = itub[(i__2 = nit - 1) < 
		    20 && 0 <= i__2 ? i__2 : s_rnge("itub", i__2, "t_cbr__", (
		    ftnlen)1779)];
	    itprvf[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itprvf"
		    , i__1, "t_cbr__", (ftnlen)1780)] = itprvf[(i__2 = nit - 
		    1) < 20 && 0 <= i__2 ? i__2 : s_rnge("itprvf", i__2, 
		    "t_cbr__", (ftnlen)1780)];
	    itprvh[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itprvh"
		    , i__1, "t_cbr__", (ftnlen)1781)] = itprvh[(i__2 = nit - 
		    1) < 20 && 0 <= i__2 ? i__2 : s_rnge("itprvh", i__2, 
		    "t_cbr__", (ftnlen)1781)];
	    s_copy(itprvi + ((i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : 
		    s_rnge("itprvi", i__1, "t_cbr__", (ftnlen)1782)) * 40, 
		    itprvi + ((i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : 
		    s_rnge("itprvi", i__2, "t_cbr__", (ftnlen)1782)) * 40, (
		    ftnlen)40, (ftnlen)40);
	    itchkp[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itchkp"
		    , i__1, "t_cbr__", (ftnlen)1783)] = itchkp[(i__2 = nit - 
		    1) < 20 && 0 <= i__2 ? i__2 : s_rnge("itchkp", i__2, 
		    "t_cbr__", (ftnlen)1783)];
	    itruex[(i__1 = i__ - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itruex"
		    , i__1, "t_cbr__", (ftnlen)1784)] = itruex[(i__2 = nit - 
		    1) < 20 && 0 <= i__2 ? i__2 : s_rnge("itruex", i__2, 
		    "t_cbr__", (ftnlen)1784)];
	    if (i__ != nit) {
		moved_(&itprvd[(i__1 = nit * 5 - 5) < 100 && 0 <= i__1 ? i__1 
			: s_rnge("itprvd", i__1, "t_cbr__", (ftnlen)1787)], &
			c__5, &itprvd[(i__2 = i__ * 5 - 5) < 100 && 0 <= i__2 
			? i__2 : s_rnge("itprvd", i__2, "t_cbr__", (ftnlen)
			1787)]);
	    }
	    --nit;
	} else {
...
/*     we're unloading, indicate that the re-use interval is invalid. */
    ...
    i__1 = nit;
    for (i__ = 1; i__ <= i__1; ++i__) {
	if (itchkp[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge("itchkp"
		, i__2, "t_cbr__", (ftnlen)1809)]) {
	    if (itprvh[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
		    "itprvh", i__2, "t_cbr__", (ftnlen)1811)] == *handle) {
		itchkp[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itchkp", i__2, "t_cbr__", (ftnlen)1812)] = FALSE_;
	    }
	}
    }
    chkout_("T_CUF", (ftnlen)5);
    return 0;
/* $Procedure T_CBS ( C-kernel, begin search for segment ) */
...
/*     T_CBS sets up a search for segments. The four quantities below */
...
/*     All files loaded by T_CLF are potential search targets for */
/*     T_CSN. */

/* $ Particulars */

/*     T_CBS sets up a search for segments by T_CSN. It records the */
...
/*     data will be returned by T_CSN. If angular velocity data are */
/*     not required, segments returned by T_CSN may or may not contain */
/*     angular velocity data. */

/*     T_CBS determines the first task that T_CSN will have to perform */
/*     if it is called to get an applicable segment. */

/* $ Examples */

/*     See Examples in T_CBR. */
...
/*        deallocated here rather than in T_CSN.  Minor changes to */
...
/*     There must be at least one file loaded. */
    ...
    if (nft == 0) {
	setmsg_("At least one CK file needs must be loaded by T_CLF before b"
		"eginning a search.", (ftnlen)77);
	sigerr_("SPICE(NOLOADEDFILES)", (ftnlen)20);
	chkout_("T_CBS", (ftnlen)5);
	return 0;
    }
...
/*     1) If T_CSN is called without T_CBS ever having been called, */
/*        the error 'SPICE(CALLT_CBSFIRST)' is signaled. */
...
/*     All files loaded by T_CLF are potential search targets for */
/*     T_CSN. The files are all referred to by their integer handles. */

/* $ Particulars */

/*     T_CSN is used to locate segments based on the search criteria */
/*     established by the most recent call to T_CBS.  When a segment */
...
/*     T_CSN also buffers segment descriptors and identifiers, to */
...
/*        be started via a call to T_CBS before this routine may */
/*        be called. */
...
/*        T_CBS was never called to initiate a search. */
...
/*        error will be signaled. */
	...
	setmsg_("Must begin a search by calling T_CBS first.", (ftnlen)43);
	sigerr_("SPICE(CALLT_CBSFIRST)", (ftnlen)21);
	chkout_("T_CSN", (ftnlen)5);
	return 0;
    }
...
/*               a new file. */
	    ...
	    if (ithfs[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "ithfs", i__1, "t_cbr__", (ftnlen)2630)] < ftnum[(i__2 = 
		    nft - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", i__2,
		     "t_cbr__", (ftnlen)2630)]) {
...
/*              request.  Check whether this is the case. */
		...
		if (itchkp[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			s_rnge("itchkp", i__1, "t_cbr__", (ftnlen)2640)]) {
...
/*                 higher-priority segments. */
		    ...
		    if (reqt > itlb[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? 
			    i__1 : s_rnge("itlb", i__1, "t_cbr__", (ftnlen)
			    2652)] + savtol && reqt < itub[(i__2 = iindex - 1)
			     < 20 && 0 <= i__2 ? i__2 : s_rnge("itub", i__2, 
			    "t_cbr__", (ftnlen)2652)] - savtol) {
...
/*                    by TOL. */
			...
			if (! avneed || itprvf[(i__1 = iindex - 1) < 20 && 0 
				<= i__1 ? i__1 : s_rnge("itprvf", i__1, "t_c"
				"br__", (ftnlen)2661)] != 0) {
...
/*                       request. */
			    ...
			    *handle = itprvh[(i__1 = iindex - 1) < 20 && 0 <= 
				    i__1 ? i__1 : s_rnge("itprvh", i__1, 
				    "t_cbr__", (ftnlen)2668)];
			    s_copy(segid, itprvi + ((i__1 = iindex - 1) < 20 
				    && 0 <= i__1 ? i__1 : s_rnge("itprvi", 
				    i__1, "t_cbr__", (ftnlen)2669)) * 40, 
				    segid_len, (ftnlen)40);
			    moved_(&itprvd[(i__1 = iindex * 5 - 5) < 100 && 0 
				    <= i__1 ? i__1 : s_rnge("itprvd", i__1, 
				    "t_cbr__", (ftnlen)2671)], &c__5, descr);
			    *found = TRUE_;
...
/*                       we'll have to check the list.  Prepare now. */
			    ...
			    savep = itbeg[(i__1 = iindex - 1) < 20 && 0 <= 
				    i__1 ? i__1 : s_rnge("itbeg", i__1, "t_c"
				    "br__", (ftnlen)2680)];
...
/*                 that component from the expense. */
		    ...
		    itexp[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("itexp", i__1, "t_cbr__", (ftnlen)2699)] = 
			    itexp[(i__2 = iindex - 1) < 20 && 0 <= i__2 ? 
			    i__2 : s_rnge("itexp", i__2, "t_cbr__", (ftnlen)
			    2699)] - itruex[(i__3 = iindex - 1) < 20 && 0 <= 
			    i__3 ? i__3 : s_rnge("itruex", i__3, "t_cbr__", (
			    ftnlen)2699)];
		    itruex[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("itruex", i__1, "t_cbr__", (ftnlen)2700)] =
			     0;
...
/*                 Carry on with the usual search algorithm. */
		    ...
		    itchkp[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("itchkp", i__1, "t_cbr__", (ftnlen)2718)] =
			     FALSE_;
		}
...
/*              sure the expense is reset to 0. */
		...
		if (itbeg[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)2726)] == 0) 
			{
		    itexp[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("itexp", i__1, "t_cbr__", (ftnlen)2727)] = 
			    0;
		}
...
/*              this instrument. */
		...
		p = itbeg[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)2734)];
...
/*              expensive instrument in the table and remove it. */
		...
		for (i__ = 2; i__ <= i__1; ++i__) {
		    if (itexp[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : 
			    s_rnge("itexp", i__2, "t_cbr__", (ftnlen)2781)] < 
			    minexp) {
			cheap = i__;
			minexp = itexp[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? 
				i__2 : s_rnge("itexp", i__2, "t_cbr__", (
				ftnlen)2783)];
		    }
		}
...
/*              list. */
		...
		head = itbeg[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : 
			s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)2793)];
		if (head > 0) {
...
/*           Set up a table entry for the new instrument. */
	    ...
	    itins[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itins"
		    , i__1, "t_cbr__", (ftnlen)2807)] = scinst;
	    itexp[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itexp"
		    , i__1, "t_cbr__", (ftnlen)2808)] = 0;
	    ithfs[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("ithfs"
		    , i__1, "t_cbr__", (ftnlen)2809)] = ftnum[(i__2 = nft - 1)
		     < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", i__2, "t_cb"
		    "r__", (ftnlen)2809)];
	    itlfs[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itlfs"
		    , i__1, "t_cbr__", (ftnlen)2810)] = ftnum[(i__2 = nft - 1)
		     < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", i__2, "t_cb"
		    "r__", (ftnlen)2810)] + 1;
	    itbeg[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itbeg"
		    , i__1, "t_cbr__", (ftnlen)2811)] = 0;
	    itchkp[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itc"
		    "hkp", i__1, "t_cbr__", (ftnlen)2812)] = FALSE_;
	    iindex = cheap;
...
/*           compilers. */
	    ...
	    itruex[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itr"
		    "uex", i__1, "t_cbr__", (ftnlen)2831)] = 0;
	    itlb[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itlb", 
		    i__1, "t_cbr__", (ftnlen)2832)] = dpmin_();
	    itub[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itub", 
		    i__1, "t_cbr__", (ftnlen)2833)] = dpmax_();
	    itprvf[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itp"
		    "rvf", i__1, "t_cbr__", (ftnlen)2834)] = 0;
	    itprvh[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itp"
		    "rvh", i__1, "t_cbr__", (ftnlen)2835)] = 0;
	    s_copy(itprvi + ((i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : 
		    s_rnge("itprvi", i__1, "t_cbr__", (ftnlen)2836)) * 40, 
		    " ", (ftnlen)40, (ftnlen)1);
	    cleard_(&c__5, &itprvd[(i__1 = cheap * 5 - 5) < 100 && 0 <= i__1 ?
		     i__1 : s_rnge("itprvd", i__1, "t_cbr__", (ftnlen)2837)]);
...
/*           corresponding file table entry. */
	    findex = 1;
	    while(ithfs[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "ithfs", i__1, "t_cbr__", (ftnlen)2866)] >= ftnum[(i__2 = 
		    findex - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", 
		    i__2, "t_cbr__", (ftnlen)2866)]) {
		++findex;
	    }
	    ithfs[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("ith"
		    "fs", i__1, "t_cbr__", (ftnlen)2872)] = ftnum[(i__2 = 
		    findex - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", 
		    i__2, "t_cbr__", (ftnlen)2872)];
	    dafbfs_(&fthan[(i__1 = findex - 1) < 110 && 0 <= i__1 ? i__1 : 
		    s_rnge("fthan", i__1, "t_cbr__", (ftnlen)2874)]);
	    if (failed_()) {
		chkout_("T_CSN", (ftnlen)5);
		return 0;
	    }
...
/*              whether we're ready to check the list. */
		...
		s_copy(status, "?", (ftnlen)40, (ftnlen)1);
		itexp[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itexp", i__1, "t_cbr__", (ftnlen)2916)] = itexp[(
			i__2 = iindex - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itexp", i__2, "t_cbr__", (ftnlen)2916)] + cost;
	    } else {
		dafgs_(descr);
...
/*              have some files left that have not been seached. */
	    findex = nft;
	    while(itlfs[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "itlfs", i__1, "t_cbr__", (ftnlen)2968)] <= ftnum[(i__2 = 
		    findex - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge("ftnum", 
		    i__2, "t_cbr__", (ftnlen)2968)]) {
		--findex;
	    }
	    dafbbs_(&fthan[(i__1 = findex - 1) < 110 && 0 <= i__1 ? i__1 : 
		    s_rnge("fthan", i__1, "t_cbr__", (ftnlen)2972)]);
	    if (failed_()) {
		chkout_("T_CSN", (ftnlen)5);
		return 0;
	    }
...
/*              current list. */
		...
		itlfs[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itlfs", i__1, "t_cbr__", (ftnlen)3018)] = ftnum[(
			i__2 = findex - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge(
			"ftnum", i__2, "t_cbr__", (ftnlen)3018)];
		itexp[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itexp", i__1, "t_cbr__", (ftnlen)3019)] = itexp[(
			i__2 = iindex - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itexp", i__2, "t_cbr__", (ftnlen)3019)] + cost;
		p = itbeg[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)3020)];
		s_copy(status, "CHECK LIST", (ftnlen)40, (ftnlen)10);
	    } else {
		dafgs_(descr);
		dafus_(descr, &c__2, &c__6, dcd, icd);
...
/*           If we're resuming a search, the re-use interval is invalid. */
	    ...
	    if (newsch) {
		itlb[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itlb", i__1, "t_cbr__", (ftnlen)3075)] = dpmin_();
		itub[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itub", i__1, "t_cbr__", (ftnlen)3076)] = dpmax_();
	    }
	    while(p > 0) {
		if (newsch) {
...
/*                 outside of the current segment. */
		    ...
		    if (reqt > stdcd[(i__1 = (p << 1) - 1) < 200 && 0 <= i__1 
			    ? i__1 : s_rnge("stdcd", i__1, "t_cbr__", (ftnlen)
			    3087)]) {
...
/* Computing MAX */
			d__1 = itlb[(i__2 = iindex - 1) < 20 && 0 <= i__2 ? 
				i__2 : s_rnge("itlb", i__2, "t_cbr__", (
				ftnlen)3093)], d__2 = stdcd[(i__3 = (p << 1) 
				- 1) < 200 && 0 <= i__3 ? i__3 : s_rnge("std"
				"cd", i__3, "t_cbr__", (ftnlen)3093)];
			itlb[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("itlb", i__1, "t_cbr__", (ftnlen)3093)]
				 = max(d__1,d__2);
		    } else if (reqt < stdcd[(i__1 = (p << 1) - 2) < 200 && 0 
			    <= i__1 ? i__1 : s_rnge("stdcd", i__1, "t_cbr__", 
			    (ftnlen)3096)]) {
...
/* Computing MIN */
			d__1 = itub[(i__2 = iindex - 1) < 20 && 0 <= i__2 ? 
				i__2 : s_rnge("itub", i__2, "t_cbr__", (
				ftnlen)3102)], d__2 = stdcd[(i__3 = (p << 1) 
				- 2) < 200 && 0 <= i__3 ? i__3 : s_rnge("std"
				"cd", i__3, "t_cbr__", (ftnlen)3102)];
			itub[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("itub", i__1, "t_cbr__", (ftnlen)3102)]
				 = min(d__1,d__2);
		    }
		}
		if (omega >= stdcd[(i__1 = (p << 1) - 2) < 200 && 0 <= i__1 ? 
			i__1 : s_rnge("stdcd", i__1, "t_cbr__", (ftnlen)3109)]
			 && alpha <= stdcd[(i__2 = (p << 1) - 1) < 200 && 0 <=
			 i__2 ? i__2 : s_rnge("stdcd", i__2, "t_cbr__", (
			ftnlen)3109)]) {

/*                 The segment coverage interval intersects the request */
/*                 interval ALPHA:OMEGA. */

		    if (! avneed || sticd[(i__1 = p * 6 - 3) < 600 && 0 <= 
			    i__1 ? i__1 : s_rnge("sticd", i__1, "t_cbr__", (
			    ftnlen)3115)] != 0) {

/*                    This segment satisfies the request. */

			dafps_(&c__2, &c__6, &stdcd[(i__1 = (p << 1) - 2) < 
				200 && 0 <= i__1 ? i__1 : s_rnge("stdcd", 
				i__1, "t_cbr__", (ftnlen)3119)], &sticd[(i__2 
				= p * 6 - 6) < 600 && 0 <= i__2 ? i__2 : 
				s_rnge("sticd", i__2, "t_cbr__", (ftnlen)3119)
				], descr);
			s_copy(segid, stidnt + ((i__1 = p - 1) < 100 && 0 <= 
				i__1 ? i__1 : s_rnge("stidnt", i__1, "t_cbr__"
				, (ftnlen)3122)) * 40, segid_len, (ftnlen)40);
			*handle = sthan[(i__1 = p - 1) < 100 && 0 <= i__1 ? 
				i__1 : s_rnge("sthan", i__1, "t_cbr__", (
				ftnlen)3123)];
			*found = TRUE_;
...
/*                    cannot be one of the endpoints. */
			...
			if (newsch && reqt > stdcd[(i__1 = (p << 1) - 2) < 
				200 && 0 <= i__1 ? i__1 : s_rnge("stdcd", 
				i__1, "t_cbr__", (ftnlen)3133)] && reqt < 
				stdcd[(i__2 = (p << 1) - 1) < 200 && 0 <= 
				i__2 ? i__2 : s_rnge("stdcd", i__2, "t_cbr__",
				 (ftnlen)3133)]) {
...
/* Computing MAX */
			    d__1 = itlb[(i__2 = iindex - 1) < 20 && 0 <= i__2 
				    ? i__2 : s_rnge("itlb", i__2, "t_cbr__", (
				    ftnlen)3140)], d__2 = stdcd[(i__3 = (p << 
				    1) - 2) < 200 && 0 <= i__3 ? i__3 : 
				    s_rnge("stdcd", i__3, "t_cbr__", (ftnlen)
				    3140)];
			    itlb[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 
				    : s_rnge("itlb", i__1, "t_cbr__", (ftnlen)
				    3140)] = max(d__1,d__2);
/* Computing MIN */
			    d__1 = itub[(i__2 = iindex - 1) < 20 && 0 <= i__2 
				    ? i__2 : s_rnge("itub", i__2, "t_cbr__", (
				    ftnlen)3141)], d__2 = stdcd[(i__3 = (p << 
				    1) - 1) < 200 && 0 <= i__3 ? i__3 : 
				    s_rnge("stdcd", i__3, "t_cbr__", (ftnlen)
				    3141)];
			    itub[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 
				    : s_rnge("itub", i__1, "t_cbr__", (ftnlen)
				    3141)] = min(d__1,d__2);

/*                       Save the returned output items, in case this */
/*                       segment may satisfy the next request. */

			    itprvh[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? 
				    i__1 : s_rnge("itprvh", i__1, "t_cbr__", (
				    ftnlen)3147)] = *handle;
			    s_copy(itprvi + ((i__1 = iindex - 1) < 20 && 0 <= 
				    i__1 ? i__1 : s_rnge("itprvi", i__1, 
				    "t_cbr__", (ftnlen)3148)) * 40, segid, (
				    ftnlen)40, segid_len);
			    itprvf[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? 
				    i__1 : s_rnge("itprvf", i__1, "t_cbr__", (
				    ftnlen)3149)] = sticd[(i__2 = p * 6 - 3) <
				     600 && 0 <= i__2 ? i__2 : s_rnge("sticd",
				     i__2, "t_cbr__", (ftnlen)3149)];
			    moved_(descr, &c__5, &itprvd[(i__1 = iindex * 5 - 
				    5) < 100 && 0 <= i__1 ? i__1 : s_rnge(
				    "itprvd", i__1, "t_cbr__", (ftnlen)3151)])
				    ;
			    itchkp[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? 
				    i__1 : s_rnge("itchkp", i__1, "t_cbr__", (
				    ftnlen)3153)] = TRUE_;
			}
...
/*                    will start at the right place. */
			...
			savep = stpool[(i__1 = (p << 1) + 10) < 212 && 0 <= 
				i__1 ? i__1 : s_rnge("stpool", i__1, "t_cbr__"
				, (ftnlen)3162)];
...
/*              to speed up the operation. */
		...
		p = stpool[(i__1 = (p << 1) + 10) < 212 && 0 <= i__1 ? i__1 : 
			s_rnge("stpool", i__1, "t_cbr__", (ftnlen)3180)];
	    }
...
/*           Otherwise, things are hopeless, set the status that way. */
	    ...
	    if (itlfs[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "itlfs", i__1, "t_cbr__", (ftnlen)3191)] > ftnum[0]) {
		s_copy(status, "OLD FILES", (ftnlen)40, (ftnlen)9);
	    } else {
...
/*           instrument table. */
		...
		if (i__ != iindex) {
		    if (itexp[(i__2 = i__ - 1) < 20 && 0 <= i__2 ? i__2 : 
			    s_rnge("itexp", i__2, "t_cbr__", (ftnlen)3240)] < 
			    minexp || cheap == 0) {
...
/*              entering MAKE ROOM. */
		...
		if (s_cmp(stack + ((i__1 = top - 1) < 2 && 0 <= i__1 ? i__1 : 
			s_rnge("stack", i__1, "t_cbr__", (ftnlen)3264)) * 40, 
...
/*                 Note this list is non-empty. */
		    ...
		    p = itbeg[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)3287)];
...
/*                 INSTRUMENT'. */
		    ...
		    itexp[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("itexp", i__1, "t_cbr__", (ftnlen)3296)] = 
			    0;
		    ithfs[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("ithfs", i__1, "t_cbr__", (ftnlen)3297)] = 
			    ftnum[(i__2 = nft - 1) < 110 && 0 <= i__2 ? i__2 :
			     s_rnge("ftnum", i__2, "t_cbr__", (ftnlen)3297)];
		    itlfs[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("itlfs", i__1, "t_cbr__", (ftnlen)3298)] = 
			    ftnum[(i__2 = nft - 1) < 110 && 0 <= i__2 ? i__2 :
			     s_rnge("ftnum", i__2, "t_cbr__", (ftnlen)3298)] 
			    + 1;
		    s_copy(status, "OLD FILES", (ftnlen)40, (ftnlen)9);
		}
...
/*              list could be empty. */
		...
		head = itbeg[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : 
			s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)3314)];
		if (head > 0) {
		    tail = -lnkprv_(&head, stpool);
		    lnkfsl_(&head, &tail, stpool);
		}
...
/*              the final entry in the table. */
		...
		itins[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itins", i__1, "t_cbr__", (ftnlen)3328)] = itins[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itins", i__2, "t_cbr__", (ftnlen)3328)];
		itexp[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itexp", i__1, "t_cbr__", (ftnlen)3329)] = itexp[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itexp", i__2, "t_cbr__", (ftnlen)3329)];
		ithfs[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"ithfs", i__1, "t_cbr__", (ftnlen)3330)] = ithfs[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"ithfs", i__2, "t_cbr__", (ftnlen)3330)];
		itlfs[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itlfs", i__1, "t_cbr__", (ftnlen)3331)] = itlfs[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itlfs", i__2, "t_cbr__", (ftnlen)3331)];
		itbeg[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itbeg", i__1, "t_cbr__", (ftnlen)3332)] = itbeg[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itbeg", i__2, "t_cbr__", (ftnlen)3332)];
		itlb[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itlb", i__1, "t_cbr__", (ftnlen)3333)] = itlb[(i__2 =
			 nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge("itlb", 
			i__2, "t_cbr__", (ftnlen)3333)];
		itub[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itub", i__1, "t_cbr__", (ftnlen)3334)] = itub[(i__2 =
			 nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge("itub", 
			i__2, "t_cbr__", (ftnlen)3334)];
		itprvh[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itprvh", i__1, "t_cbr__", (ftnlen)3335)] = itprvh[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itprvh", i__2, "t_cbr__", (ftnlen)3335)];
		s_copy(itprvi + ((i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 :
			 s_rnge("itprvi", i__1, "t_cbr__", (ftnlen)3336)) * 
			40, itprvi + ((i__2 = nit - 1) < 20 && 0 <= i__2 ? 
			i__2 : s_rnge("itprvi", i__2, "t_cbr__", (ftnlen)3336)
			) * 40, (ftnlen)40, (ftnlen)40);
		itprvf[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itprvf", i__1, "t_cbr__", (ftnlen)3337)] = itprvf[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itprvf", i__2, "t_cbr__", (ftnlen)3337)];
		itchkp[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itchkp", i__1, "t_cbr__", (ftnlen)3338)] = itchkp[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itchkp", i__2, "t_cbr__", (ftnlen)3338)];
		itruex[(i__1 = cheap - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itruex", i__1, "t_cbr__", (ftnlen)3339)] = itruex[(
			i__2 = nit - 1) < 20 && 0 <= i__2 ? i__2 : s_rnge(
			"itruex", i__2, "t_cbr__", (ftnlen)3339)];
		if (cheap != nit) {
		    moved_(&itprvd[(i__1 = nit * 5 - 5) < 100 && 0 <= i__1 ? 
			    i__1 : s_rnge("itprvd", i__1, "t_cbr__", (ftnlen)
			    3342)], &c__5, &itprvd[(i__2 = cheap * 5 - 5) < 
			    100 && 0 <= i__2 ? i__2 : s_rnge("itprvd", i__2, 
			    "t_cbr__", (ftnlen)3342)]);
		}
		if (iindex == nit) {
		    iindex = cheap;
...
/*              for the current instrument. */
		...
		sthan[(i__1 = new__ - 1) < 100 && 0 <= i__1 ? i__1 : s_rnge(
			"sthan", i__1, "t_cbr__", (ftnlen)3387)] = fthan[(
			i__2 = findex - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge(
			"fthan", i__2, "t_cbr__", (ftnlen)3387)];
		dafgn_(stidnt + ((i__1 = new__ - 1) < 100 && 0 <= i__1 ? i__1 
			: s_rnge("stidnt", i__1, "t_cbr__", (ftnlen)3389)) * 
			40, (ftnlen)40);
		dafus_(descr, &c__2, &c__6, &stdcd[(i__1 = (new__ << 1) - 2) <
			 200 && 0 <= i__1 ? i__1 : s_rnge("stdcd", i__1, 
			"t_cbr__", (ftnlen)3391)], &sticd[(i__2 = new__ * 6 - 
			6) < 600 && 0 <= i__2 ? i__2 : s_rnge("sticd", i__2, 
			"t_cbr__", (ftnlen)3391)]);
		if (failed_()) {
		    chkout_("T_CSN", (ftnlen)5);
		    return 0;
...
/*              is a no-op. */
		...
		lnkilb_(&new__, &itbeg[(i__1 = iindex - 1) < 20 && 0 <= i__1 ?
			 i__1 : s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)3402)
			], stpool);
		itbeg[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
			"itbeg", i__1, "t_cbr__", (ftnlen)3403)] = new__;
		s_copy(status, "RESUME", (ftnlen)40, (ftnlen)6);
	    }
...
/*              Allocate a new node in the segment table pool. */
		...
		lnkan_(stpool, &new__);
		sthan[(i__1 = new__ - 1) < 100 && 0 <= i__1 ? i__1 : s_rnge(
			"sthan", i__1, "t_cbr__", (ftnlen)3430)] = fthan[(
			i__2 = findex - 1) < 110 && 0 <= i__2 ? i__2 : s_rnge(
			"fthan", i__2, "t_cbr__", (ftnlen)3430)];
		dafgn_(stidnt + ((i__1 = new__ - 1) < 100 && 0 <= i__1 ? i__1 
			: s_rnge("stidnt", i__1, "t_cbr__", (ftnlen)3432)) * 
			40, (ftnlen)40);
		dafus_(descr, &c__2, &c__6, &stdcd[(i__1 = (new__ << 1) - 2) <
			 200 && 0 <= i__1 ? i__1 : s_rnge("stdcd", i__1, 
			"t_cbr__", (ftnlen)3434)], &sticd[(i__2 = new__ * 6 - 
			6) < 600 && 0 <= i__2 ? i__2 : s_rnge("sticd", i__2, 
			"t_cbr__", (ftnlen)3434)]);
		if (failed_()) {
		    chkout_("T_CSN", (ftnlen)5);
		...
		if (itbeg[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)3441)] <= 0) 
			{

/*                 This is the first node in the list for this */
/*                 instrument. */

		    itbeg[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)3446)] = 
			    new__;
		} else {

/*                 Link the new node to the tail of the list. */

		    tail = -lnkprv_(&itbeg[(i__1 = iindex - 1) < 20 && 0 <= 
			    i__1 ? i__1 : s_rnge("itbeg", i__1, "t_cbr__", (
			    ftnlen)3452)], stpool);
...
/*           of the the segment list for the current instrument, and */
...
/*           the re-use interval. */
	    ...
	    itruex[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "itruex", i__1, "t_cbr__", (ftnlen)3477)] = 0;
...
/*           will point to the beginning of this sublist. */
	    ...
	    slbeg = itbeg[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
		    s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)3484)];
	    fndhan = FALSE_;
	    while(! fndhan && slbeg > 0) {
		fndhan = sthan[(i__1 = slbeg - 1) < 100 && 0 <= i__1 ? i__1 : 
			s_rnge("sthan", i__1, "t_cbr__", (ftnlen)3489)] == 
			fthan[(i__2 = findex - 1) < 110 && 0 <= i__2 ? i__2 : 
			s_rnge("fthan", i__2, "t_cbr__", (ftnlen)3489)];
		if (! fndhan) {

/*                 Get the next node.  We avoid LNKNXT here in order */
/*                 to speed up the operation. */

		    slbeg = stpool[(i__1 = (slbeg << 1) + 10) < 212 && 0 <= 
			    i__1 ? i__1 : s_rnge("stpool", i__1, "t_cbr__", (
			    ftnlen)3496)];
		}
	    }
...
/*              a non-positive value to indicate an empty segment list. */
		...
		if (p == itbeg[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)3530)]) {
		    itbeg[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("itbeg", i__1, "t_cbr__", (ftnlen)3532)] = 
			    0;
...
/*                 bounds for this instrument. */
		    ...
		    itlb[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("itlb", i__1, "t_cbr__", (ftnlen)3538)] = 
			    dpmin_();
		    itub[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("itub", i__1, "t_cbr__", (ftnlen)3539)] = 
			    dpmax_();
		}
...
/*                 to the expense of the re-use interval. */
		    ...
		    itruex[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
			    s_rnge("itruex", i__1, "t_cbr__", (ftnlen)3578)] =
			     itruex[(i__2 = iindex - 1) < 20 && 0 <= i__2 ? 
			    i__2 : s_rnge("itruex", i__2, "t_cbr__", (ftnlen)
			    3578)] + 1;
...
/* Computing MAX */
			d__1 = itlb[(i__2 = iindex - 1) < 20 && 0 <= i__2 ? 
				i__2 : s_rnge("itlb", i__2, "t_cbr__", (
				ftnlen)3590)], d__2 = stdcd[(i__3 = (p << 1) 
				- 1) < 200 && 0 <= i__3 ? i__3 : s_rnge("std"
				"cd", i__3, "t_cbr__", (ftnlen)3590)];
			itlb[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("itlb", i__1, "t_cbr__", (ftnlen)3590)]
				 = max(d__1,d__2);
		    } else if (reqt < stdcd[(i__1 = (p << 1) - 2) < 200 && 0 
			    <= i__1 ? i__1 : s_rnge("stdcd", i__1, "t_cbr__", 
			    (ftnlen)3593)]) {
...
/* Computing MIN */
			d__1 = itub[(i__2 = iindex - 1) < 20 && 0 <= i__2 ? 
				i__2 : s_rnge("itub", i__2, "t_cbr__", (
				ftnlen)3599)], d__2 = stdcd[(i__3 = (p << 1) 
				- 2) < 200 && 0 <= i__3 ? i__3 : s_rnge("std"
				"cd", i__3, "t_cbr__", (ftnlen)3599)];
			itub[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("itub", i__1, "t_cbr__", (ftnlen)3599)]
				 = min(d__1,d__2);
		    }
		}
...
/*              We've updated the re-use interval if so required. */
		...
		if (omega >= stdcd[(i__1 = (p << 1) - 2) < 200 && 0 <= i__1 ? 
			i__1 : s_rnge("stdcd", i__1, "t_cbr__", (ftnlen)3608)]
			 && alpha <= stdcd[(i__2 = (p << 1) - 1) < 200 && 0 <=
			 i__2 ? i__2 : s_rnge("stdcd", i__2, "t_cbr__", (
			ftnlen)3608)]) {
...
/*                    output arguments. */
			...
			dafps_(&c__2, &c__6, &stdcd[(i__1 = (p << 1) - 2) < 
				200 && 0 <= i__1 ? i__1 : s_rnge("stdcd", 
				i__1, "t_cbr__", (ftnlen)3619)], &sticd[(i__2 
				= p * 6 - 6) < 600 && 0 <= i__2 ? i__2 : 
				s_rnge("sticd", i__2, "t_cbr__", (ftnlen)3619)
				], descr);
			s_copy(segid, stidnt + ((i__1 = p - 1) < 100 && 0 <= 
				i__1 ? i__1 : s_rnge("stidnt", i__1, "t_cbr__"
				, (ftnlen)3622)) * 40, segid_len, (ftnlen)40);
			*handle = sthan[(i__1 = p - 1) < 100 && 0 <= i__1 ? 
				i__1 : s_rnge("sthan", i__1, "t_cbr__", (
				ftnlen)3623)];
			*found = TRUE_;
...
/*                       segment may satisfy the next request. */
			    ...
			    s_copy(itprvi + ((i__1 = iindex - 1) < 20 && 0 <= 
				    i__1 ? i__1 : s_rnge("itprvi", i__1, 
				    "t_cbr__", (ftnlen)3651)) * 40, segid, (
				    ftnlen)40, segid_len);
			    itprvf[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? 
				    i__1 : s_rnge("itprvf", i__1, "t_cbr__", (
				    ftnlen)3652)] = sticd[(i__2 = p * 6 - 3) <
				     600 && 0 <= i__2 ? i__2 : s_rnge("sticd",
				     i__2, "t_cbr__", (ftnlen)3652)];
			    moved_(descr, &c__5, &itprvd[(i__1 = iindex * 5 - 
				    5) < 100 && 0 <= i__1 ? i__1 : s_rnge(
				    "itprvd", i__1, "t_cbr__", (ftnlen)3654)])
				    ;
			    itchkp[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? 
				    i__1 : s_rnge("itchkp", i__1, "t_cbr__", (
				    ftnlen)3656)] = TRUE_;
...
/*                       the cost of locating this segment. */
			    ...
			    itexp[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? 
				    i__1 : s_rnge("itexp", i__1, "t_cbr__", (
				    ftnlen)3661)] = itexp[(i__2 = iindex - 1) 
				    < 20 && 0 <= i__2 ? i__2 : s_rnge("itexp",
				     i__2, "t_cbr__", (ftnlen)3661)] + itruex[
				    (i__3 = iindex - 1) < 20 && 0 <= i__3 ? 
				    i__3 : s_rnge("itruex", i__3, "t_cbr__", (
				    ftnlen)3661)];
			}
...
/*              segment in the list. */
		...
		p = stpool[(i__1 = (p << 1) + 10) < 212 && 0 <= i__1 ? i__1 : 
			s_rnge("stpool", i__1, "t_cbr__", (ftnlen)3703)];
	    }
...
/*                    the re-use interval. */
			...
			itruex[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : 
				s_rnge("itruex", i__1, "t_cbr__", (ftnlen)
				3751)] = itruex[(i__2 = iindex - 1) < 20 && 0 
				<= i__2 ? i__2 : s_rnge("itruex", i__2, "t_c"
				"br__", (ftnlen)3751)] + 1;
		    }
		    dafgs_(descr);
		    dafus_(descr, &c__2, &c__6, dcd, icd);
...
/* Computing MAX */
				d__1 = itlb[(i__2 = iindex - 1) < 20 && 0 <= 
					i__2 ? i__2 : s_rnge("itlb", i__2, 
					"t_cbr__", (ftnlen)3778)];
				itlb[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? 
					i__1 : s_rnge("itlb", i__1, "t_cbr__",
					 (ftnlen)3778)] = max(d__1,dcd[1]);
			    } else if (reqt < dcd[0]) {
...
/* Computing MIN */
				d__1 = itub[(i__2 = iindex - 1) < 20 && 0 <= 
					i__2 ? i__2 : s_rnge("itub", i__2, 
					"t_cbr__", (ftnlen)3787)];
				itub[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? 
					i__1 : s_rnge("itub", i__1, "t_cbr__",
					 (ftnlen)3787)] = min(d__1,dcd[0]);
			    }
			}
...
/*                          the output arguments. */
				...
				*handle = fthan[(i__1 = findex - 1) < 110 && 
					0 <= i__1 ? i__1 : s_rnge("fthan", 
					i__1, "t_cbr__", (ftnlen)3811)];
				*found = TRUE_;
				if (newsch) {
...
/* Computing MAX */
				    d__1 = itlb[(i__2 = iindex - 1) < 20 && 0 
					    ...
					    i__2, "t_cbr__", (ftnlen)3819)];
				    itlb[(i__1 = iindex - 1) < 20 && 0 <= 
					    i__1 ? i__1 : s_rnge("itlb", i__1,
					     "t_cbr__", (ftnlen)3819)] = max(
					    d__1,dcd[0]);
/* Computing MIN */
				    d__1 = itub[(i__2 = iindex - 1) < 20 && 0 
					    <= i__2 ? i__2 : s_rnge("itub", 
					    i__2, "t_cbr__", (ftnlen)3820)];
				    itub[(i__1 = iindex - 1) < 20 && 0 <= 
					    i__1 ? i__1 : s_rnge("itub", i__1,
					     "t_cbr__", (ftnlen)3820)] = min(
					    d__1,dcd[1]);

/*                             Save the returned output items, in case */
/*                             this segment may satisfy the next request. */

				    itprvh[(i__1 = iindex - 1) < 20 && 0 <= 
					    i__1 ? i__1 : s_rnge("itprvh", 
					    i__1, "t_cbr__", (ftnlen)3826)] = 
					    *handle;
				    s_copy(itprvi + ((i__1 = iindex - 1) < 20 
					    && 0 <= i__1 ? i__1 : s_rnge(
					    "itprvi", i__1, "t_cbr__", (
					    ftnlen)3827)) * 40, segid, (
					    ftnlen)40, segid_len);
				    itprvf[(i__1 = iindex - 1) < 20 && 0 <= 
					    i__1 ? i__1 : s_rnge("itprvf", 
					    i__1, "t_cbr__", (ftnlen)3828)] = 
					    icd[3];
				    moved_(descr, &c__5, &itprvd[(i__1 = 
					    iindex * 5 - 5) < 100 && 0 <= 
					    i__1 ? i__1 : s_rnge("itprvd", 
					    i__1, "t_cbr__", (ftnlen)3830)]);
				    itchkp[(i__1 = iindex - 1) < 20 && 0 <= 
					    i__1 ? i__1 : s_rnge("itchkp", 
					    i__1, "t_cbr__", (ftnlen)3833)] = 
					    TRUE_;

/*                             Update the expense of the list to reflect */
/*                             cost of locating this segment. */

				    itexp[(i__1 = iindex - 1) < 20 && 0 <= 
					    i__1 ? i__1 : s_rnge("itexp", 
					    i__1, "t_cbr__", (ftnlen)3839)] = 
					    itexp[(i__2 = iindex - 1) < 20 && 
					    0 <= i__2 ? i__2 : s_rnge("itexp",
					     i__2, "t_cbr__", (ftnlen)3839)] 
					    + itruex[(i__3 = iindex - 1) < 20 
					    && 0 <= i__3 ? i__3 : s_rnge(
					    "itruex", i__3, "t_cbr__", (
					    ftnlen)3839)];
				}
...
/*              Try the next oldest file. */
		...
		--findex;
		if (findex > 0) {
		    dafbbs_(&fthan[(i__1 = findex - 1) < 110 && 0 <= i__1 ? 
			    i__1 : s_rnge("fthan", i__1, "t_cbr__", (ftnlen)
			    3888)]);
		    daffpa_(&fnd);
		}
	    }
...
/*           There's nothing nowhere if you get to here. */
	    ...
	    itruex[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge(
		    "itruex", i__1, "t_cbr__", (ftnlen)3898)] = 0;
	    s_copy(status, "HOPELESS", (ftnlen)40, (ftnlen)8);
...
/*           since some interrupts can be interrupted by others. */
	    ...
	    s_copy(stack + ((i__1 = top - 1) < 2 && 0 <= i__1 ? i__1 : s_rnge(
		    "stack", i__1, "t_cbr__", (ftnlen)3909)) * 40, doing, (
		    ftnlen)40, (ftnlen)40);
	    ...
	    s_copy(status, stack + ((i__1 = top - 1) < 2 && 0 <= i__1 ? i__1 :
		     s_rnge("stack", i__1, "t_cbr__", (ftnlen)3914)) * 40, (
		    ftnlen)40, (ftnlen)40);
	    --top;
	}
    }
...
/*     interval is zeroed out. */
    ...
    if (iindex > 0) {
	itchkp[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itchkp",
		 i__1, "t_cbr__", (ftnlen)3935)] = FALSE_;
	itruex[(i__1 = iindex - 1) < 20 && 0 <= i__1 ? i__1 : s_rnge("itruex",
		 i__1, "t_cbr__", (ftnlen)3936)] = 0;
    }
...
/*     called again, but it's possible. */
    ...
    chkout_("T_CSN", (ftnlen)5);
    return 0;
/* $Procedure      T_CHAV ( C-kernels --- Have some ) */
...
/* Subroutine */ int t_cbr__(char *fname, integer *handle, integer *inst, 
	doublereal *sclkdp, doublereal *tol, logical *needav, doublereal *
	descr, char *segid, logical *found, ftnlen fname_len, ftnlen 
	segid_len)
{
    return t_cbr__0_(0, fname, handle, inst, sclkdp, tol, needav, descr, 
	    segid, found, fname_len, segid_len);
    }

/* Subroutine */ int t_clf__(char *fname, integer *handle, ftnlen fname_len)
{
    return t_cbr__0_(1, fname, handle, (integer *)0, (doublereal *)0, (
	    doublereal *)0, (logical *)0, (doublereal *)0, (char *)0, (
	    logical *)0, fname_len, (ftnint)0);
    }

/* Subroutine */ int t_cuf__(integer *handle)
{
    return t_cbr__0_(2, (char *)0, handle, (integer *)0, (doublereal *)0, (
	    doublereal *)0, (logical *)0, (doublereal *)0, (char *)0, (
	    logical *)0, (ftnint)0, (ftnint)0);
    }

/* Subroutine */ int t_cbs__(integer *inst, doublereal *sclkdp, doublereal *
	tol, logical *needav)
{
    return t_cbr__0_(3, (char *)0, (integer *)0, inst, sclkdp, tol, needav, (
	    doublereal *)0, (char *)0, (logical *)0, (ftnint)0, (ftnint)0);
    }

/* Subroutine */ int t_csn__(integer *handle, doublereal *descr, char *segid, 
	logical *found, ftnlen segid_len)
{
    return t_cbr__0_(4, (char *)0, handle, (integer *)0, (doublereal *)0, (
	    doublereal *)0, (logical *)0, descr, segid, found, (ftnint)0, 
	    segid_len);
    }

/* Subroutine */ int t_chav__(logical *found)
{
    return t_cbr__0_(5, (char *)0, (integer *)0, (integer *)0, (doublereal *)
	    0, (doublereal *)0, (logical *)0, (doublereal *)0, (char *)0, 
