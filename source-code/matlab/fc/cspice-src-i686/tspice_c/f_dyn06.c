/* f_dyn06.f -- translated by f2c (version 19980913).
...
static doublereal c_b159 = 1e-12;
static doublereal c_b172 = 0.;
static integer c__6 = 6;
static doublereal c_b197 = 1e-19;

/* $Procedure F_DYN06 ( Dynamic Frame Test Family 06 ) */
/* Subroutine */ int f_dyn06__(logical *ok)
{
    /* Initialized data */
    ...
    extern /* Subroutine */ int zzmobliq_(doublereal *, doublereal *, 
	    doublereal *), zzeprcss_(doublereal *, doublereal *);
    integer i__, j;
    ...
    doublereal deterr, drlerr, drvblk[9]	/* was [3][3] */, drverr, 
	    nrmerr, oblmat[9]	/* was [3][3] */, rminus[9]	/* was [3][3] 
	    */;
    integer coords[3], center;
    extern /* Subroutine */ int tstlsk_(void), tstspk_(char *, logical *, 
	    integer *, ftnlen), t_pck08__(char *, logical *, logical *, 
	    ftnlen), cmprss_(char *, integer *, char *, char *, ftnlen, 
	    ftnlen, ftnlen), replch_(char *, char *, char *, char *, ftnlen, 
    ...
    doublereal mob;
    extern /* Subroutine */ int dvpool_(char *, ftnlen), spkuef_(integer *);
    doublereal tol;
    extern /* Subroutine */ int mxm_(doublereal *, doublereal *, doublereal *)
	    , t_xform__(doublereal *, doublereal *, doublereal *, doublereal *
...
/*     Test family to exercise the logic and code in the third */
/*     subset of dynamic frame routines.  Only mean ecliptic and */
...
/*     Open the test family. */
    ...
    topen_("F_DYN06", (ftnlen)7);

/* --- Case: ------------------------------------------------------ */

    tcase_("Create test inputs for comprehensive true-of-date test.", (ftnlen)
	    55);
...
/*        Set the rotation state. */
	...
	s_copy(rststr, rstate + ((i__2 = coords[0] - 1) < 3 && 0 <= i__2 ? 
		i__2 : s_rnge("rstate", i__2, "f_dyn06__", (ftnlen)491)) * 80,
		 (ftnlen)80, (ftnlen)80);

/*        Set the base frame. */

	s_copy(bframe, basefr + (((i__2 = coords[1] - 1) < 3 && 0 <= i__2 ? 
		i__2 : s_rnge("basefr", i__2, "f_dyn06__", (ftnlen)496)) << 5)
		, (ftnlen)32, (ftnlen)32);

/*        Set the frame center. */

	center = ctrcde[(i__2 = coords[2] - 1) < 1 && 0 <= i__2 ? i__2 : 
		s_rnge("ctrcde", i__2, "f_dyn06__", (ftnlen)501)];
...
/*           First comes the frame name-to-frame code assignment. */
	    ...
	    s_copy(frname, "MEAN_ECL_OF_DATE", (ftnlen)32, (ftnlen)16);
	    frcode = -2000000000;
...
/* Writing concatenation */
	    ...
	    i__4[2] = 14, a__2[2] = "           = '";
	    i__4[3] = 33, a__2[3] = "MEAN_ECLIPTIC_AND_EQUINOX_OF_DATE";
	    i__4[4] = 1, a__2[4] = "'";
...
/*           Set the frame's obliquity model. */

/* Writing concatenation */
	    i__4[0] = 18, a__2[0] = frstem;
	    i__4[1] = 11, a__2[1] = "OBLIQ_MODEL";
	    i__4[2] = 4, a__2[2] = " = '";
	    i__4[3] = 14, a__2[3] = "EARTH_IAU_1980";
	    i__4[4] = 1, a__2[4] = "'";
...
/* Writing concatenation */
		...
		s_cat(deftxt + 800, a__3, i__5, &c__4, (ftnlen)80);
	    } else {
/* Writing concatenation */
		i__4[0] = 18, a__2[0] = frstem;
		...
		i__4[4] = 1, a__2[4] = "'";
		s_cat(deftxt + 800, a__2, i__4, &c__5, (ftnlen)80);
	    }
	    dx = 11;
...
/*           to keep this code close to the associated SXFORM call. */
		...
		for (j = 1; j <= 3; ++j) {
		    drvblk[(i__2 = j + i__ * 3 - 4) < 9 && 0 <= i__2 ? i__2 : 
			    s_rnge("drvblk", i__2, "f_dyn06__", (ftnlen)654)] 
			    = xform[(i__6 = j + 3 + i__ * 6 - 7) < 36 && 0 <= 
			    i__6 ? i__6 : s_rnge("xform", i__6, "f_dyn06__", (
			    ftnlen)654)];
		}
	    }
...
/*           Extract the rotation block from XFORM into R2. */
		...
		for (j = 1; j <= 3; ++j) {
		    r2[(i__2 = j + i__ * 3 - 4) < 9 && 0 <= i__2 ? i__2 : 
			    s_rnge("r2", i__2, "f_dyn06__", (ftnlen)665)] = 
			    xform[(i__6 = j + i__ * 6 - 7) < 36 && 0 <= i__6 ?
			     i__6 : s_rnge("xform", i__6, "f_dyn06__", (
			    ftnlen)665)];
		}
	    }
	    chckad_("SXFORM vs PXFORM", r2, "~", r__, &c__9, &c_b159, ok, (
		    ftnlen)16, (ftnlen)1);
...
/*           up is the determinant error. */
	    ...
	    tol = 1e-14;
	    chcksd_("DETERR", &deterr, "~", &c_b172, &tol, ok, (ftnlen)6, (
		    ftnlen)1);

/*           Check norms of rows and columns in the rotation */
/*           blocks. */

	    chcksd_("NRMERR", &nrmerr, "~", &c_b172, &tol, ok, (ftnlen)6, (
		    ftnlen)1);
...
/*              Check the derivative absolute error. */
		...
		tol = 1e-12;
		chcksd_("DRVERR", &drverr, "~", &c_b172, &tol, ok, (ftnlen)6, 
			(ftnlen)1);

/*              Check the derivative relative error. */

		tol = 1e-5;
		chcksd_("DRLERR", &drlerr, "~", &c_b172, &tol, ok, (ftnlen)6, 
			(ftnlen)1);
...
/*              transformation is close to zero. */
		...
		for (i__ = 1; i__ <= 3; ++i__) {
			...
			drv2[(i__2 = j + i__ * 3 - 4) < 9 && 0 <= i__2 ? i__2 
				: s_rnge("drv2", i__2, "f_dyn06__", (ftnlen)
				743)] = xff2j[(i__6 = j + 3 + i__ * 6 - 7) < 
				36 && 0 <= i__6 ? i__6 : s_rnge("xff2j", i__6,
				 "f_dyn06__", (ftnlen)743)];
		    }
		}
		chckad_("Derivative block", drv2, "~", zr, &c__9, &c_b197, ok,
			 (ftnlen)16, (ftnlen)1);
	    } else {
...
/*              with respect to the base frame is zero. */
		...
		chckad_("Derivative block", drvblk, "=", zr, &c__9, &c_b172, 
...
/*           Get mean obliquity. */
	    ...
	    zzmobliq_(&t, &mob, &dmob);
	    chckxc_(&c_false, " ", ok, (ftnlen)1);

/*           Form obliquity transformation matrix. */

	    rotate_(&mob, &c__1, oblmat);
	    chckxc_(&c_false, " ", ok, (ftnlen)1);

/*           Form the J2000 to mean-ECL-of-date matrix. */

	    mxm_(oblmat, precm, ecleqn);

/*           Now form the transformation from the mean-ECL-of-date */
/*           frame to the base frame. */

	    pxform_("J2000", bframe, &t, rj2b, (ftnlen)5, (ftnlen)32);
	    chckxc_(&c_false, " ", ok, (ftnlen)1);
	    mxmt_(rj2b, ecleqn, r2);
