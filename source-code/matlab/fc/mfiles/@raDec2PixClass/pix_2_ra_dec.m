function [ra dec] = pix_2_ra_dec(raDec2PixObject, module, output, row, column, mjds, aberrateFlag)
% [ra dec] = pix_2_ra_dec(raDec2PixObject, module, output, row, column, mjds, aberrateFlag)
%
...
% The raPointing, decPointing, and rollPointing must be the same length as julianTime.
...
%   mjds --The modified julian date of for the coordinate transformation dates.  Must be the same size as raPointing, decPointing, and rollPointing.  
%
% OUTPUTS:
...
%
    ...
    if nargin < 6 || nargin > 7
        error('MATLAB:FC:raDec2PixClass:pix_2_ra_dec', 'pix_2_ra_dec takes 6 or 7 arguments');
    end
    
    [ra dec] = pix_2_ra_dec_absolute(raDec2PixObject, module, output, row, column, mjds, aberrateFlag);
return
