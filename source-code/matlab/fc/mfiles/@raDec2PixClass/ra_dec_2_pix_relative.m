function [module output row column] = ra_dec_2_pix_relative(raDec2PixObject, ra, dec, mjds, deltaRaPointing, deltaDecPointing, deltaRollPointing, aberrateFlag)
% [module output row column] = ra_dec_2_pix_relative(raDec2PixObject, ra, dec, mjds, deltaRaPointing, deltaDecPointing, deltaRollPointing, aberrateFlag)
%
...
%   mjds --The MJD for the coordinate transformation.  Must be the same size as raPointing, decPointing, and rollPointing.  
%   raPointing --The pointing RA of the spacecraft for the vector of Julian dates given in the mjds argument.  Must be the same size as mjds. This is relative to the FC model's pointing.
%   decPointing --The pointing declination of the spacecraft for the vector of modified Julian dates given in the mjds argument.  Must be the same size as mjds. This is relative to the FC model's pointing.
%   rollPointing --The pointing roll of the spacecraft for the vector of modified Julian dates given in the mjds argument.  Must be the same size as mjds. This is relative to the FC model's pointing.
%
% OUTPUTS:
...
%
    ...
    if nargin < 7 || nargin > 8
        error('MATLAB:FC:raDec2Pix:ra_dec_2_pix_relative', 'ra_dec_2_pix_relative takes 7 or 8 arguments');
    end
    ...
    [module output row column] = ra_dec_2_pix_absolute(raDec2PixObject, ra, dec, mjds, adjustedRaPointing, adjustedDecPointing, adjustedRollPointing, aberrateFlag);
return
