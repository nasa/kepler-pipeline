function pointingSegments = generate_pointing_segment_array(pointingObject)
%
% function pointingSegments = generate_pointing_segment_array(pointingObject)
%
...
ras              = get(pointingObject, 'ras');
declinations     = get(pointingObject, 'declinations');
rolls            = get(pointingObject, 'rolls');
segmentStartMjds = get(pointingObject, 'segmentStartMjds');

% Intialize the output struct array
startMjds = unique(segmentStartMjds);
nSegments = length(startMjds);

pointingSegments = repmat(struct( ...
    'startMjd', 0, ...
    'endMjd', 0, ...
    'mjds', [], ...
    'ras', [], ...
...
for iSegment = 1 : nSegments
    ...
    if iSegment > 1
        ...
        pointingSegments(iSegment).startMjd = mjds(1);
    end
...
for iSegment = 1 : nSegments
    ...
    if iSegment < nSegments
        pointingSegments(iSegment).endMjd = ...
            pointingSegments(iSegment+1).startMjd;
    else
        pointingSegments(iSegment).endMjd = ...
            pointingSegments(iSegment).mjds(end);
    end
