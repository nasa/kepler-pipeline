function [koiAndTceStruct,koiOnlyStruct,tceOnlyStruct] = ...
    match_multiplanet_tce_with_multiplanet_koi( koiDataStruct, tceStruct )
%
...
% loop over the targets
  ...
  for iTarget = 1:length(keplerIdList) ;
      ...
      koiPointer = find(koiDataStruct.keplerId == keplerId) ;
      tcePointer = find(tceStruct.keplerId == keplerId & tceStruct.trueTceFlag) ;
...
%     two sets of information
      ...
      for iKoi = 1:length(koiPointer)
          for iTce = 1:length(tcePointer)
              ...
              if tceStruct.tceIndex(thisTce) == 0
                  tceStruct.tceIndex(thisTce) = iTce ;
              end
              ephemerisMatch( iKoi, iTce ) = ephemeris_match( ...
                  koiDataStruct.epochKjd( thisKoi ), koiDataStruct.periodDays( thisKoi ), ...
                  tceStruct.epochKjd( thisTce ), tceStruct.periodDays( thisTce ), ...
                  tceStruct.maxMesPulseDurationHours( thisTce ), tceStruct.unitOfWorkKjd ) ;
          end
      end
