function [tpsInputStruct, tpsOutputStruct] = generate_random_plotter_inputs ( numTargets, ...
...
for i=1:numTargets
    inputFile = char(strcat(topDir,'/tps-matlab',tpsDawgStruct.taskfile(randIndices(i)),'/tps-inputs-0.mat'));
    outputFile = char(strcat(topDir,'/tps-matlab',tpsDawgStruct.taskfile(randIndices(i)),'/tps-outputs-0.mat'));
    ...
    if i==1
        ...
        tpsOutputStruct.tpsResults=outputsStruct.tpsResults(1);
        load(diagnosticFile);
        ...
        load(outputFile);
        load(diagnosticFile);
        tempResults=outputsStruct.tpsResults(1);
        tempResults.detrendedFluxTimeSeries = tpsDiagnosticStruct(1).detrendedFluxTimeSeries;
        tpsInputStruct.tpsTargets(i) = inputsStruct.tpsTargets(1);
        tpsOutputStruct.tpsResults(i) = tempResults;
    end
    clear tpsDiagnosticStruct tempResults inputFile outputFile diagnosticFile inputsStruct outputsStruct;
end
