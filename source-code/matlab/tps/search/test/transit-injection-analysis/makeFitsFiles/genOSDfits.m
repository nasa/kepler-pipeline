...
fptr=fits.createFile(['!',outputOSDfile]);
% Get the date
...
fits.createTbl(fptr,'binary',0,{'PERIOD','ONESIGDEP'},{'1E','1E'},{'Days','PPM'});
...
%fits.writeComment(fptr,'This file contains the D.3 Occurrence Rate Product for the one sigma depth function.  The one sigma depth data specify the averaged over phase depth of a signal (in ppm) that will result in a one sigma detection as a function of orbital period.  There are 14 binary table extensions corresponding to the 14 transit durations searched for transits.  This file is relevant to the SOC 9.2 Kepler pipeline run, and generation of this file is tracked on the internal ticket KSO-299');
...
fits.writeCol(fptr,1,1,tpsqualityvectors{1}.periodSearchedOSD);
fits.writeCol(fptr,2,1,tpsqualityvectors{1}.oneSigmaDepthFunction);
for k=2:length(pulsedurs)
 ...
 fits.writeKey(fptr,'EXTNAME',sprintf('DURATION%02d_%1d',pulsehr,pulsefrac),'extension name');
 fits.writeCol(fptr,1,1,tpsqualityvectors{k}.periodSearchedOSD);
 fits.writeCol(fptr,2,1,tpsqualityvectors{k}.oneSigmaDepthFunction);
