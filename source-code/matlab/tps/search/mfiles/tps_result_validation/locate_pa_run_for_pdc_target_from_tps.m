% locate_pa_run_for_pdc_target_from_tps.m
% 
...
for iDir  = 1:length(paDir)
    ...
    for iMatFile = 1:length(paInputMatList)
        ...
        if(inputsStruct.ccdModule ~= ccdModule || inputsStruct.ccdOutput ~= ccdOutput)
            break;
        end

        if(~isempty(inputsStruct.targetStarDataStruct))

            matchingIndex = find(cat(1,inputsStruct.targetStarDataStruct.keplerId) == keplerId);
