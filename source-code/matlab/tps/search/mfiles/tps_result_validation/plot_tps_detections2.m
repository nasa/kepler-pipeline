...
function plot_tps_detections2(inputsStruct, tpsOutputStruct)
...
for j = 1:length(isp),
    ...
    xaxisInDays = (1:length(inputsStruct.tpsTargets(isp(j)).fluxValue))./48.939;

    subplot(2,1,1);

    plot(xaxisInDays, inputsStruct.tpsTargets(isp(j)).fluxValue, '.-'),
