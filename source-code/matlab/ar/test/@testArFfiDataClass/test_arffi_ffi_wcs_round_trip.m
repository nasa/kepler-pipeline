function self = test_arffi_ffi_wcs_round_trip(self)
%
...
%
    ...
    % Read the inputs
    %
    load(fullfile(inputDir, 'arffi-inputs-0.mat'));
    write_FfiArchiveInputs(outfile, inputsStruct);
    inputsStruct = read_FfiArchiveInputs(outfile);

    % Run the controller, and display the time it consumed:
    %
    disp('Running AR controller');
    os = arffi_matlab_controller(inputsStruct);
