...
/**
 * This class contains business logic for files stored by DR.
 * 
 * @author Miles Cote
 */
public class DrFileOperations {

    private LogCrud logCrud;
    private FileStoreClient fsClient;

    public DrFileOperations() {
        logCrud = new LogCrud(DatabaseServiceFactory.getInstance());
        fsClient = FileStoreClientFactory.getInstance();
    }

    public BlobResult retrieveLatest(DispatcherType dispatcherType) {
        FileLog fileLog = logCrud.retrieveLatestFileLog(dispatcherType);

        return fsClient.readBlob(DrFsIdFactory.getFile(dispatcherType,
            fileLog.getFilename()));
    }
