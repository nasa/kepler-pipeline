...
/**
 * This dispatcher processes and stores target list sets sent by the SO.
 * 
 * @author Miles Cote
 */
public class TargetListSetDispatcher implements Dispatcher {

    public TargetListSetDispatcher() {
    }
    ...
    public void dispatch(Set<String> filenames, String sourceDirectory,
        ...
        for (String filename : filenames) {
            try {
                ...
                TargetListSet targetListSet = importer.importFile(new File(
                    sourceDirectory + File.separator + filename));

                TargetSelectionCrud targetSelectionCrud = new TargetSelectionCrud();

                // Make sure the tls doesn't already exist in the database.
                TargetListSet existingTls = targetSelectionCrud.retrieveTargetListSet(targetListSet.getName());
                if (existingTls != null) {
                    throw new PipelineException(
                        "New tls names need to not exist in the database.\n  existingTlsName: "
                            + existingTls.getName());
                }
