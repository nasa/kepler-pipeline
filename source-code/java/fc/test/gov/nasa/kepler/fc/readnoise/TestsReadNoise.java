...
public class TestsReadNoise {
    @SuppressWarnings("unused")
	private static final Log log = LogFactory.getLog(TestsReadNoise.class);
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        try {
            ...
            importer.rewriteHistory("running importer from TestsReadNoise");
    ...
    @Test
    public void testRetrieveModelNoRange() {
        ReadNoiseOperations ops = new ReadNoiseOperations();
        ReadNoiseModel model = ops.retrieveReadNoiseModelAll();
        assertTrue(model.size() >= 1);
        assertTrue(model.getConstants().length == model.size());
    }

    @Test
    public void testRetrieveModelRange() {
        ReadNoiseOperations ops = new ReadNoiseOperations();
        ReadNoiseModel model = ops.retrieveReadNoiseModel(50000, 56000);
        assertTrue(model.size() >= 1);
        assertTrue(model.getConstants().length == model.size());
    }
}
