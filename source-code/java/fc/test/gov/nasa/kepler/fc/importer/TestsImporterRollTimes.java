...
public class TestsImporterRollTimes {
    ...
    public void loadStuff() throws IOException {
        ...
        try {
            ...
            importer.appendNew("TestImporterRollTimes load", new Date());
    ...
    public void testImportIntegrity() throws Exception {
        ...
        String[] rollTimeFiles = ImporterParentNonImage.getDataFilenames(
            ImporterRollTime.DATAFILE_REGEX,
            ImporterRollTime.DATAFILE_DIRECTORY_NAME);
        for (String rollTimeFile : rollTimeFiles) {

            // Get rollTimes from file, via importer:
            //
            List<RollTime> importerRollTimes = importer.parseFile(rollTimeFile);

            // Get rollTimes from db for the MJD from the rollTimeFile:
            //
            List<RollTime> databaseRollTimes = new ArrayList<RollTime>();
            RollTimeOperations ops = new RollTimeOperations(dbService);
            for (RollTime imRollTime : importerRollTimes) {
                RollTime rollTime = ops.retrieveRollTimeExact(imRollTime.getMjd());
                databaseRollTimes.add(rollTime);
            }
            ...
            reflectionEquals.assertEquals(importerRollTimes, databaseRollTimes);
        }
    }
    ...
    public void TestAppendNewNominal() throws IOException {
        ...
        try {
            ...
            importer.appendNew(SocEnvVars.getLocalTestDataDir()
                + "/fc/roll-time/v1/", "this should also work", new Date());
            dbService.commitTransaction();
        } finally {
            dbService.rollbackTransactionIfActive();
            dbService.closeCurrentSession();
