...
public class TestsFlatFieldAft {
    @SuppressWarnings("unused")
	private static final Log log = LogFactory.getLog(TestsFlatField.class);
    
    private static int TEST_MODULE = 7;
    private static int TEST_OUTPUT = 1;
    
    private static double TEST_MJD      = 55553.5;
    private static double TEST_MJD_END  = 55644.5;
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        try {
            ...
            importerSmall.rewriteHistory(TEST_MODULE, TEST_OUTPUT, "TestsFlatFieldAft");
            importerLarge.rewriteHistory(TEST_MODULE, TEST_OUTPUT, "TestsFlatFieldAft");
    ...
    @Test
    public void testAftFramework() {
        FlatFieldOperations ops = new FlatFieldOperations();

        try {
            dbService.beginTransaction();
            FlatFieldModel flatFieldModel = ops.retrieveFlatFieldModel(
                TEST_MJD, TEST_MJD_END, TEST_MODULE, TEST_OUTPUT); 
            dbService.commitTransaction();
            assertTrue(flatFieldModel.getMjds().length > 0);
        } catch (Exception e) {
            e.printStackTrace();
            assertTrue(false);
        } finally {
