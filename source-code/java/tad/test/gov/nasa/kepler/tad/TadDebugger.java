...
public class TadDebugger {

    private static final int TARGET_TABLE_ID = 10;

    public static void main(String[] args) {
        TargetCrud targetCrud = new TargetCrud(
            DatabaseServiceFactory.getInstance());

        TargetTable targetTable = targetCrud.retrieveTargetTable(TARGET_TABLE_ID);
            ...
            for (TargetDefinition targetDef : target.getTargetDefinitions()) {
                ...
                for (Offset offset : targetDef.getMask()
                    ...
                    int absCol = targetDef.getReferenceColumn()
                        + offset.getColumn();
                    System.out.print("[" + absRow + "," + absCol + "] ");
                }

                System.out.print("\n    refRow: " + targetDef.getReferenceRow()
                    + ", refCol: " + targetDef.getReferenceColumn()
                    + ", offsets: " + targetDef.getMask()
                        .getOffsets());
            }
        }
    }
