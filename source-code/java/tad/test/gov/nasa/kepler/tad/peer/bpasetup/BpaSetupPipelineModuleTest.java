 ...
 * 
 */
public class BpaSetupPipelineModuleTest extends JMockTest {
    ...
    private static final int SATURATED_ROW_COUNT = 31;
    ...
    private static final boolean APERTURE_USED = false;
    ...
    private BpaSetupPipelineModule bpaSetupPipelineModule = new BpaSetupPipelineModule(
        targetCrud, targetSelectionCrud, maskTableFactory, targetTableFactory,
        mask);
    ...
    public void testFrameworkMethods() {
        PipelineModuleTest.testFrameworkMethods(bpaSetupPipelineModule);
    }
    ...
    public void testProcessTask() {
        ...
        oneOf(mask).setMaskTable(maskTable);

        oneOf(mask).setPipelineTask(pipelineTask);

        oneOf(mask).setIndexInTable(BpaSetupPipelineModule.INDEX_IN_TABLE);

        oneOf(targetCrud).createTargetTable(backgroundTable);

        oneOf(backgroundTable).setObservingSeason(OBSERVING_SEASON);

        oneOf(backgroundTable).setMaskTable(maskTable);

        oneOf(targetListSet).setBackgroundTable(backgroundTable);

        oneOf(backgroundTable).setPlannedStartTime(START);

        oneOf(backgroundTable).setPlannedEndTime(END);

        oneOf(maskTable).setPlannedStartTime(START);

        oneOf(maskTable).setPlannedEndTime(END);

        oneOf(targetCrud).delete(backgroundTableOld);

        oneOf(targetCrud).delete(maskTableOld);

        bpaSetupPipelineModule.processTask(pipelineInstance, pipelineTask);
    }
    ...
    private void setAllowances() {
        ...
        will(returnValue(APERTURE_USED));
        ...
        allowing(maskTableFactory).create(BpaSetupPipelineModule.MASK_TYPE);
        ...
        allowing(targetTableFactory).create(BpaSetupPipelineModule.TARGET_TYPE);
