...
/**
 * Implements the following KID rule: <br>
 * A sufficient set of base investigations must be specified for the
 * investigations found in the KTC.<br>
 * 
 * @author Miles Cote
 * 
 */
public class KidRuleBaseInvestigationsExist implements KidRule {
    ...
    public void apply(List<InvestigationType> kidInvestigations,
        ...
        for (InvestigationType kidInvestigation : kidInvestigations) {
            ...
            for (String baseInvestigationId : baseInvestigationIds) {
                ...
                if (baseInvestigation == null) {
                    errorStringBuilder.append("\nA sufficient set of base investigations must be specified for the investigations found in the KTC.\n  ktcInvestigationId: "
                        + kidInvestigation.getId()
                        + "\n  baseInvestigationIdNotSpecifiedAsInput: "
                        + baseInvestigationId + "\n--------");
                }
            }

            if (baseInvestigationIds.length != 1) {
                if (investigationIdToKtcEntries.get(kidInvestigation.getId()) == null) {
                    errorStringBuilder.append("\nShared investigations must exist in the KTC.\n  sharedInvestigationIdMissingFromKtc: "
                        + kidInvestigation.getId() + "\n--------");
                }
            }
        }
