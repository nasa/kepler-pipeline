...
/**
 * Implements the title KID rule: <br>
 * - If the investigation is a base investigation, copy the title from the base
 * investigation file.<br>
 * - If the investigation is a shared investigation, the title should be
 * specified as "Overlap of iiiii and jjjj" where iiii and jjjj are replaced by
 * the component base investigation ids.<br>
 * 
 * @author Miles Cote
 * 
 */
public class KidRuleTitle implements KidRule {

    static final String SHARED_TITLE_PREFIX = "Overlap of ";
    ...
    public void apply(List<InvestigationType> kidInvestigations,
        ...
        for (InvestigationType kidInvestigation : kidInvestigations) {
            ...
            if (!kidInvestigation.getId()
                ...
                title = baseInvestigation.getTitle();
            } else {
                String[] baseInvestigationIds = kidInvestigation.getId()
                    .split(INVESTIGATION_ID_SEPARATOR);
                List<String> baseInvestigationIdsList = Arrays.asList(baseInvestigationIds);
                title = SHARED_TITLE_PREFIX
                    + EnglishFormatter.format(baseInvestigationIdsList);
            }

            kidInvestigation.setTitle(title);
        }
    }
