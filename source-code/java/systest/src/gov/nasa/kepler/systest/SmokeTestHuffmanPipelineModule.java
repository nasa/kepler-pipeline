...
import gov.nasa.kepler.gar.huffman.HuffmanInputs;
import gov.nasa.kepler.gar.huffman.HuffmanOutputs;
...
public class SmokeTestHuffmanPipelineModule extends HuffmanPipelineModule {

    public static final String MODULE_NAME = "systest-huffman";

    private static final Log log = LogFactory.getLog(SmokeTestHuffmanPipelineModule.class);
    ...
    private void validateInputs(Persistable inputs) {
        ...
        if (huffmanInputs.getFcConstants() == null
            || huffmanInputs.getHistogram().length == 0
            || huffmanInputs.getHuffmanModuleParameters() == null) {
            throw new PipelineException(
                "Unexpected empty input in smoke test huffman ("
    ...
    private void populateOutputs(Persistable outputs) {
        ...
        log.info("Retrieving src huffman table...");
        HuffmanTable srcHuffmanTable = srcCrud.retrieveAllHuffmanTables()
            .get(0);

        if (srcHuffmanTable == null) {
            throw new PipelineException("No huffman table found.");
        }

        List<String> bitstrings = new ArrayList<String>();
        List<Integer> bitstringLengths = newArrayList();
        for (HuffmanEntry entry : srcHuffmanTable.getEntries()) {
            bitstrings.add(entry.getBitstring());
            bitstringLengths.add(entry.getBitstring()
                .length());
        }

        HuffmanOutputs huffmanOutputs = (HuffmanOutputs) outputs;
        huffmanOutputs.setTheoreticalCompressionRate(srcHuffmanTable.getTheoreticalCompressionRate());
        huffmanOutputs.setEffectiveCompressionRate(srcHuffmanTable.getEffectiveCompressionRate());
        huffmanOutputs.setHuffmanCodeStrings(bitstrings.toArray(new String[0]));
        huffmanOutputs.setHuffmanCodeLengths(toArray(bitstringLengths));
    }
