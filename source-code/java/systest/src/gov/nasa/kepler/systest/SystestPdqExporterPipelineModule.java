...
public class SystestPdqExporterPipelineModule extends PipelineModule {

    public static final String MODULE_NAME = "pdq-exporter";
    ...
    public void processTask(PipelineInstance pipelineInstance,
        ...
        try {
            ...
            String pdqExportDir = dataGenDirManager.getPdqExportDir();

            // Clean output dir.
            FileUtil.cleanDir(pdqExportDir);

            // Retrieve attitude adjustment.
            AttitudeAdjustment attitudeAdjustment = null;
            PdqCrud pdqCrud = new PdqCrud();
            attitudeAdjustment = pdqCrud.retrieveLatestAttitudeAdjustment();

            if (attitudeAdjustment == null) {
                throw new ModuleFatalProcessingException(
                    "The attitudeAdjustment must be non-null.");
            }

            // Export attitude adjustment.
            AttitudeAdjustmentExporter exporter = new AttitudeAdjustmentExporter();
            exporter.export(pdqExportDir, attitudeAdjustment);

        } catch (Exception e) {
            AlertServiceFactory.getInstance()
                .generateAlert(MODULE_NAME, pipelineTask.getId(),
                    Severity.WARNING, "Unable to export data:\n" + e);
        }
    }
