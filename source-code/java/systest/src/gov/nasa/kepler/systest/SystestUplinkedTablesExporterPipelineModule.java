...
public class SystestUplinkedTablesExporterPipelineModule extends PipelineModule {

    public static final String MODULE_NAME = "uplinked-tables-exporter";
    ...
    public void processTask(PipelineInstance pipelineInstance,
        ...
        try {
            ...
            String xmlDir = dataGenDirManager.getUplinkedTablesExportDir();

            // Clean output dir.
            FileUtil.cleanDir(xmlDir);

            TargetCrud targetCrud = new TargetCrud();

            TargetTable lct = targetCrud.retrieveUplinkedTargetTable(
                photometerConfigParams.getLctExternalId(),
                TargetType.LONG_CADENCE);
            TargetTable bgp = targetCrud.retrieveUplinkedTargetTable(
                photometerConfigParams.getBgpExternalId(),
                TargetType.BACKGROUND);
            MaskTable tad = targetCrud.retrieveUplinkedMaskTable(
                photometerConfigParams.getTadExternalId(), MaskType.TARGET);
            MaskTable bad = targetCrud.retrieveUplinkedMaskTable(
                photometerConfigParams.getBadExternalId(), MaskType.BACKGROUND);
            TargetTable sct = targetCrud.retrieveUplinkedTargetTable(
                photometerConfigParams.getSctExternalId(),
                TargetType.SHORT_CADENCE);
            TargetTable rpt = targetCrud.retrieveUplinkedTargetTable(
                photometerConfigParams.getRptExternalId(),
                TargetType.REFERENCE_PIXEL);

            CompressionCrud compressionCrud = new CompressionCrud();
            HuffmanTable huffmanTable = compressionCrud.retrieveUplinkedHuffmanTable(photometerConfigParams.getCompressionExternalId());
            RequantTable requantTable = compressionCrud.retrieveUplinkedRequantTable(photometerConfigParams.getCompressionExternalId());

            // Export tad.
            TargetExporter targetExporter = new TargetExporter();
            targetExporter.export(lct, bgp, tad, bad, rpt, sct, null, null,
                xmlDir);

            // Export compression.
            GarOperations garOperations = new GarOperations();
            garOperations.export(huffmanTable, requantTable, xmlDir, false);
