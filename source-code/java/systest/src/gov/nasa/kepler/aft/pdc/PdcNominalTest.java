...
package gov.nasa.kepler.aft.pdc;
...
/**
 * Nominal (uses all the defaults) PDC AFT.
 * 
 * @author Forrest Girouard
 */
public class PdcNominalTest extends AutomatedFeatureTest {

    static final Log log = LogFactory.getLog(PdcNominalTest.class);

    private static final String PDC_LC_TRIGGER_NAME = "PDC_LC";
    private static final String PDC_SC_TRIGGER_NAME = "PDC_SC";

    public PdcNominalTest() {
        super(PdcPipelineModule.MODULE_NAME, "Nominal");
    }

    @Override
    protected void createDatabaseContents() throws Exception {

        TriggerDefinition trigger = new TriggerDefinitionCrud().retrieve(PDC_LC_TRIGGER_NAME);
        if (trigger != null) {
            return;
        }
        ...
        new PipelineConfigurationOperations().importPipelineConfiguration(new File(
            SocEnvVars.getLocalDataDir(), AFT_PIPELINE_CONFIGURATION_ROOT
                + PdcPipelineModule.MODULE_NAME + ".xml"));
    }

    @Override
    protected void process() throws Exception {
        if (getCadenceType() == CadenceType.LONG) {
            runPipeline(PDC_LC_TRIGGER_NAME);
        } else {
            runPipeline(PDC_SC_TRIGGER_NAME);
        }
    }
}
