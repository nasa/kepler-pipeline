 ...
 */
public abstract class AbstractCalFeatureTest extends AutomatedFeatureTest {

    @SuppressWarnings("unused")
    private static final Log log = LogFactory.getLog(AbstractCalFeatureTest.class);

    protected static final String CAL_LC_TRIGGER_NAME = "CAL_LC";
    protected static final String CAL_SC_TRIGGER_NAME = "CAL_SC";

    protected AbstractCalFeatureTest(String testName) {
        super(CalPipelineModule.MODULE_NAME, testName);
    }

    @Override
    protected void createDatabaseContents() throws Exception {

        TriggerDefinition trigger = new TriggerDefinitionCrud().retrieve(CAL_LC_TRIGGER_NAME);
        if (trigger == null) {
            new PipelineConfigurationOperations().importPipelineConfiguration(new File(
                SocEnvVars.getLocalDataDir(), AFT_PIPELINE_CONFIGURATION_ROOT
                    + CalPipelineModule.MODULE_NAME + ".xml"));
        }
