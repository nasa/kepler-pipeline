...
/**
 * Tests PA's batch processing.
 * 
 * @author Forrest Girouard
 */
public class PaMultipleBatchTest extends AbstractPaFeatureTest {

    private static final Log log = LogFactory.getLog(PaMultipleBatchTest.class);

    public PaMultipleBatchTest() {
        super("MultipleBatch");
    }
    ...
    protected void createDatabaseContents() throws Exception {
        ...
        TriggerDefinition trigger = new TriggerDefinitionCrud().retrieve(PA_LC_TRIGGER_NAME);
        if (trigger == null) {
            ...
            new PipelineConfigurationOperations().importPipelineConfiguration(new File(
                SocEnvVars.getLocalDataDir(), AFT_PIPELINE_CONFIGURATION_ROOT
                    + PaPipelineModule.MODULE_NAME + ".xml"));
        }

        updatePaModuleParameters();
    }
    ...
    protected void process() throws Exception {
        ...
        if (getCadenceType() == CadenceType.LONG) {
            runPipeline(PA_LC_TRIGGER_NAME);
        } else {
            runPipeline(PA_SC_TRIGGER_NAME);
        }
    }
}
