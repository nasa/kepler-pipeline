 ...
 */
public abstract class AbstractTpsTest extends AutomatedFeatureTest {

    private static final Logger log = Logger.getLogger(AbstractTpsTest.class);

    private static final String TPS_FULL_TRIGGER_NAME = "TPS_FULL";

    public AbstractTpsTest(String dirName, String testName) {
    ...
    protected void createDatabaseContents() throws Exception {
        ...
        TriggerDefinition trigger = new TriggerDefinitionCrud().retrieve(TPS_FULL_TRIGGER_NAME);
        if (trigger != null) {
            return;
        }
        ...
        new PipelineConfigurationOperations().importPipelineConfiguration(new File(
            SocEnvVars.getLocalDataDir(), AFT_PIPELINE_CONFIGURATION_ROOT
                + TpsPipelineModule.MODULE_NAME + ".xml"));
    }

    @Override
    protected void process() throws Exception {
        runPipeline(TPS_FULL_TRIGGER_NAME);
    }
}
