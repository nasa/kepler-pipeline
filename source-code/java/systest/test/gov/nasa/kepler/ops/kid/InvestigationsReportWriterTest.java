 ...
 * 
 */
public class InvestigationsReportWriterTest extends JMockTest {
    ...
    public void testReadWrite() throws IOException, XmlException {
        ...
        List<String> row = ImmutableList.of(id, name,
            investigationTypeType.toString(), ("" + collaboratorTypes.size()),
            warnings);

        List<List<String>> table = ImmutableList.of(
            InvestigationsReportWriter.COLUMN_NAMES, row);

        CsvWriter csvWriter = mock(CsvWriter.class);
        InvestigationWarningsGenerator investigationWarningsGenerator = mock(InvestigationWarningsGenerator.class);

        allowing(csvWriter).write(table);
        will(returnValue(file));

        allowing(investigationWarningsGenerator).generate(investigationType);
        will(returnValue(warnings));

        InvestigationsReportWriter investigationsReportWriter = new InvestigationsReportWriter(
            csvWriter, investigationWarningsGenerator);
        File actualFile = investigationsReportWriter.write(investigations);
