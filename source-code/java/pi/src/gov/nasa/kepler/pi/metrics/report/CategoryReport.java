...
public class CategoryReport extends Report {
    ...
    private static final Log log = LogFactory.getLog(CategoryReport.class);
    ...
    public void generateReport(String moduleName, DescriptiveStatistics stats, TopNList topTen) throws Exception{

        String label = moduleName + " : " + category;

        Pair<String, List<Double>> histogramValues = null;
        String unitsLabel = "";
        Format f;
        
        if(isTime){
            histogramValues = millisToHumanReadable(stats);
            unitsLabel = "Time (" + histogramValues.left + ")";
        ...
        JFreeChart histogram = generateHistogram(label, unitsLabel, "Tasks", histogramValues.right, 20);
