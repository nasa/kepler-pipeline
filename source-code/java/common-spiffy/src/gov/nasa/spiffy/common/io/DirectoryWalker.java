...
/**
 * In order traversal of directories and the files contained in them. Kind of
 * like the UNIX command "find". Note that Java does not know anything about
...
public class DirectoryWalker {
    ...
    public DirectoryWalker(File rootDir) {
        if (!rootDir.isDirectory()) {
            throw new IllegalArgumentException("\"" + rootDir.getAbsolutePath()
                + "\" is not a directory.");
        }
    ...
    public void traverse(FileVisitor v) throws IOException {
        ...
        while (unvisited.size() != 0) {
            ...
            File[] filesInThisDir = nextDir.listFiles();
            if (filesInThisDir == null) {
                continue;
            }
            for (File visitingFile : filesInThisDir) {
                if (visitingFile.isDirectory()) {
                    unvisited.add(visitingFile);
                } else {
                    v.visitFile(nextDir, visitingFile);
                    if (v.prune()) {
                        break;
                    }
                }
        ...
        // directories, this will happen when we get to the last unopened
        // directory.
