...
/**
 * Command line to export the Kepler Input Catalog (KIC).
 * 
 * @author Sean McCauliff
 * 
 */
public class KicExportCli {

    private static final Log log = LogFactory.getLog(KicExportCli.class);

    private final SystemProvider system;
    private File outputDir;

    public KicExportCli(SystemProvider system) {
        this.system = system;
    }
    ...
    private void printUsage() {
        system
            .out()
            .println(
                "java -cp ... gov.nasa.kepler.ar.cli.KicExportCli <output directory>");
    }
    ...
    public void execute() {
        InputCatalogExporter kicExporter = new InputCatalogExporter();
        FileNameFormatter fileNameFormatter = new FileNameFormatter();
        File kicFile = new File(outputDir,fileNameFormatter.kicName());
        ExportOptions xo = new ExportOptions(-1,-1, kicFile, "");
        ...
        kicExporter.export(progress, xo);
    }
    ...
    public static void main(String[] argv) {
        KicExportCli cli = new KicExportCli(new DefaultSystemProvider());
        if (cli.parseCommandLine(argv)) {
            cli.execute();
        }
