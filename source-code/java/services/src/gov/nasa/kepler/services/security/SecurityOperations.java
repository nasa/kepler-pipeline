...
public class SecurityOperations {
    ...
    public boolean validateLogin(User user, String password) {
        ...
        // MR can pass the already-encrypted password from a cookie.
        if (user.getPassword()
            .equals(password) || user.getPassword()
            .equals(user.encryptPassword(password))) {
            this.user = user;
            return true;
        }
     ...
     * 
     * @return the user object, or {@code null} if a user has not yet been
     * validated
     */
