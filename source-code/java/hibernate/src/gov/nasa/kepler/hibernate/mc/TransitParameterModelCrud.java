...
/**
 * Contains {@link TransitParameterModel} data access operations.
 * 
 * @author Forrest Girouard
 */
public class TransitParameterModelCrud extends AbstractCrud implements
    ModelCrud<TransitParameterModel> {

    public static final String TYPE = "TRANSIT_PARAMETER";

    public TransitParameterModelCrud() {
        this(null);
    }

    public TransitParameterModelCrud(DatabaseService databaseService) {
    ...
    @Override
    public void create(TransitParameterModel model) {
        List<TransitParameter> transitParameters = create(model.getTransitParameters());

        TransitParameterModel transitParameterModelToCreate = new TransitParameterModel(
            model.getRevision(), transitParameters);
        getSession().save(transitParameterModelToCreate);
    }
     ...
     * @return new {@link List} of {@link TransitParameter}s which should be
     * used in place of the input {@link List} {@link TransitParameter}s.
     */
    private List<TransitParameter> create(
        ...
        for (TransitParameter transitParameter : retrieveAllTransitParameters()) {
            transitParameterKeyToTransitParameterValue.put(transitParameter,
                transitParameter);
        }
        ...
        for (TransitParameter transitParameter : transitParameters) {
            ...
            if (transitParameterValue == null) {
                ...
                transitParameterKeyToTransitParameterValue.put(
    ...
    @Override
    public TransitParameterModel retrieve(int revision) {
        Query q = getSession().createQuery(
            "from TransitParameterModel where " + "revision = :revision ");
        q.setParameter("revision", revision);

        TransitParameterModel model = uniqueResult(q);

        return model;
    }

    @Override
    public void delete(TransitParameterModel model) {
