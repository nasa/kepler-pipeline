...
/**
 * {@link TriggerDefinition} uses a {@link PipelineDefinitionName} to refer 
 * to the {@link PipelineDefinition} that is launched by that trigger.  
 * This is a 'soft reference' because there may be several instances of the 
 * {@link PipelineDefinition} with the name in the {@link PipelineDefinitionName} 
 * (one for each version).  An association to a specific version (usually latest version)
 * is only made when a trigger is launched (the {@link PipelineInstance} 
...
public class PipelineDefinitionName {
     ...
     * @param name
     */
    PipelineDefinitionName(String name) {
        this.name = name;
    }
