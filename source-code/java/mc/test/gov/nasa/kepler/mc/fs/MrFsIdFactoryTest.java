...
/**
 * Tests the {@link MrFsIdFactory} class.
 * 
 * @author Bill Wohler
 */
public class MrFsIdFactoryTest {

    private String moduleName = "foo";
    private long pipelineInstanceId;
    private long pipelineTaskId;
    private String identifier = "12345";

    @Test
    public void testGetReportId() {
        FsId expectedFsId = new FsId(MrFsIdFactory.MR_PATH + "/" + moduleName
            + "/" + pipelineInstanceId + "/" + pipelineTaskId);

        PipelineTask pipelineTask = createPipelineTask();
        FsId actualFsId = MrFsIdFactory.getReportId(pipelineTask);
        assertEquals(expectedFsId, actualFsId);
    }

    @Test
    public void testGetReportIdWithIdentifier() {
        FsId expectedFsId = new FsId(MrFsIdFactory.MR_PATH + "/" + moduleName
            + "/" + pipelineInstanceId + "/" + pipelineTaskId + "/" + identifier);

        PipelineTask pipelineTask = createPipelineTask();
        FsId actualFsId = MrFsIdFactory.getReportId(pipelineTask, identifier);
        assertEquals(expectedFsId, actualFsId);
    }
