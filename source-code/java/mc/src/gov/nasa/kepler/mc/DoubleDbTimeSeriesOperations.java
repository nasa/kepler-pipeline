...
/**
 * Operations on {@link DoubleDbTimeSeries}.
 * 
 * @author Bill Wohler
 * @author Forrest Girouard
 */
public class DoubleDbTimeSeriesOperations {

    /**
     * Adds every originator for all the time series to {@code producerTaskIds}.
     * 
     * @param doubleTimeSeries all the time series
     * @param producerTaskIds the producer task IDs
     * @throws NullPointerException if either {@code timeSeries} or {@code
     * producerTaskIds} is {@code null}.
     */
    public static void addToDataAccountability(
        DoubleDbTimeSeries[] doubleTimeSeries, Set<Long> producerTaskIds) {
        ...
        for (DoubleDbTimeSeries timeSeries : doubleTimeSeries) {
            for (int i = 0; i < timeSeries.getGapIndicators().length; i++) {
                if (!timeSeries.getGapIndicators()[i]) {
                    producerTaskIds.add(timeSeries.getOriginators()[i]);
                }
            }
        }
    }
