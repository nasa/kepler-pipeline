...
public class PipelineLauncherPipelineModule extends PipelineModule {
    private static final Log log = LogFactory.getLog(PipelineLauncherPipelineModule.class);

    public static final String MODULE_NAME = "launcher";

    public PipelineLauncherPipelineModule() {
    }
    ...
    public List<Class<? extends Parameters>> requiredParameters() {
        ...
        requiredParams.add(PipelineLauncherParameters.class);
        return requiredParams;
    }

    @Override
    public void processTask(PipelineInstance pipelineInstance, PipelineTask pipelineTask) throws PipelineException {
        PipelineLauncherParameters launcherParameters = pipelineTask.getParameters(PipelineLauncherParameters.class);

        if(launcherParameters.isEnabled()){
            TriggerDefinitionCrud triggerCrud = new TriggerDefinitionCrud();      
            PipelineOperations pipelineOps = new PipelineOperations();
            
            String instanceName = launcherParameters.getInstanceName();
            String triggerName = launcherParameters.getTriggerName();
            ...
            if(triggerName != null && triggerName.length() > 0){
                ...
                throw new ModuleFatalProcessingException("triggerName is null or empty");
            }
        }else{
            log.info("launcherParameters.isEnabled() == false, doing nothing");
        }
    }
}
