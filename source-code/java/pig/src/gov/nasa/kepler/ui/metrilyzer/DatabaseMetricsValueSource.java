...
import gov.nasa.kepler.ui.proxy.MetricsLogCrudProxy;
...
public class DatabaseMetricsValueSource implements MetricsValueSource {
    ...
    @Override
    public Map<MetricType, Collection<MetricValue>> metricValues(List<MetricType> selectedMetricTypes,
        ...
        for (MetricType type : selectedMetricTypes) {
            List<MetricValue> values = 
                    metricsLogCrud.retrieveAllMetricValuesForType(type, windowStart, windowEnd);
            rv.put(type, values);
        }
        return rv;
    }
    ...
    public Map<MetricType, Pair<Date, Date>> metricStartEndDates(
        ...
        MetricsLogCrudProxy metricsLogCrud = new MetricsLogCrudProxy();
        Map<MetricType, Pair<Date, Date>> rv = Maps.newHashMapWithExpectedSize(selectedMetricTypes.size());
        for (MetricType type : selectedMetricTypes) {
            Pair<Date, Date> interval = 
                metricsLogCrud.getTimestampRange(type);
