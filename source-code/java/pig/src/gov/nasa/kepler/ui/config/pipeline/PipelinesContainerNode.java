...
import gov.nasa.kepler.ui.config.TreeContainerNode;
...
public class PipelinesContainerNode extends TreeContainerNode {
    ...
    @Override
    public void expand(DefaultTreeModel model, DefaultMutableTreeNode node) throws PipelineUIException {

        if (!loaded) {
            deleteAllChildren(model, node);

            int index = 0;
            Collection<PipelineDefinition> pipelinesCollection = new LinkedList<PipelineDefinition>();
            try {
                //TODO: update when pipelines are added/deleted, or when the db session is invalidated
                try{
                    pipelinesCollection = pipelineDefinitionCrud.retrieveLatestVersions();
                }catch(PigSecurityException ignore){
                }
            } catch (Throwable e) {
                throw new PipelineUIException("Failed to retrieve Pipeline list from datastore.", e);
            }
