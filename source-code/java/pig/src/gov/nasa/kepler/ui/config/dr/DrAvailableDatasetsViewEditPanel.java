...
public class DrAvailableDatasetsViewEditPanel extends AbstractViewEditPanel {
    private static final Log log = LogFactory.getLog(DrAvailableDatasetsViewEditPanel.class);
    
    // do NOT init to null! (see getTableModel)
    private AvailableDatasetsTableModel datasetsTableModel;

    public DrAvailableDatasetsViewEditPanel() throws PipelineUIException {
        super();
    ...
    protected AbstractTableModel getTableModel() throws PipelineUIException {
        ...
        if (datasetsTableModel == null) {
            datasetsTableModel = new AvailableDatasetsTableModel();
            datasetsTableModel.register();
        }

        log.debug("getTableModel() - end");
        return datasetsTableModel;
    }
    ...
    protected void doEdit(int row) {
        ...
        DrDataset dataset = datasetsTableModel.getDatasetAtRow(row);
        showEditDialog(dataset);
        
        datasetsTableModel.loadFromDatabase();

        log.debug("doEdit(int) - end");
    }

    private void showEditDialog(DrDataset dataset) {
