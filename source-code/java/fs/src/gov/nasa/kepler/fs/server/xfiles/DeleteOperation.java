...
class DeleteOperation extends Operation {
    ...
    static {
        addReader(new DeleteOperationReader());
    }
    
    DeleteOperation(long start, long end) {
        super(start, end);
    }

    @Override
    void merge(IntervalSet<SimpleInterval, Factory> iset) {
        iset.deleteInterval(new SimpleInterval(start, end));
    }

    @Override
    void mergeOriginators(IntervalSet<TaggedInterval, gov.nasa.spiffy.common.intervals.TaggedInterval.Factory> iset) {
        iset.deleteInterval(new TaggedInterval(start, end, -1));
    }

    
    @Override
    void writeTo(DataOutput dout) throws IOException {
        dout.writeUTF(OpType.OP_TYPE_DELETE.name());
        dout.writeLong(start);
        dout.writeLong(end);
    }
    
    
    private static class DeleteOperationReader implements OperationReader {
        ...
        public Operation readFrom(DataInput din) throws IOException {
            ...
            DeleteOperation delOp = new DeleteOperation(start, end);
            return delOp;
        }
