...
import static gov.nasa.kepler.fs.FileStoreConstants.FS_PROTOCOL_VERSION_CHECK_DEFAULT;
import static gov.nasa.kepler.fs.FileStoreConstants.FS_PROTOCOL_VERSION_CHECK_PROP;
import static gov.nasa.kepler.fs.transport.ProtocolVersion.CURRENT_PROTOCOL_VERSION;
...
/**
 * The server side of the file store transport protocol.
 * 
 * @author Sean McCauliff
 *
 */
class TransportServerV2 extends TransportV2 implements TransportServer {

    private final static Log log = LogFactory.getLog(TransportServerV2.class);
     ...
     * @throws IOException
     */
    public TransportServerV2(SocketChannel serverChannel, boolean admit) throws IOException {
    ...
    /**
     * @see gov.nasa.kepler.fs.transport.TransportV2#initConnection()
     */
    @Override
    ...
    private void doProtocolVersionCheck() throws IOException {
        ...
        if (serverChannel.read(versionBuf) == -1) {
        ...
        clientProtocolVersion = ProtocolVersion.ordinalToEnum(versionBuf.getInt());
        ...
        protocolVersionInUse = 
            ProtocolVersion.ordinalToEnum(Math.min(clientProtocolVersion.ordinal(), CURRENT_PROTOCOL_VERSION.ordinal()));
    }
    ...
    public void close() throws IOException {
        ...
        serverChannel.close();
        clientByteOrder = null;
    }
    
    /**
     * The server should call this method when it has completed sending
     * all the data for the current method invocation.
     */
    @Override
    public void doneWithMethod() {
        methodOrder++;
    }
