...
################################################################################
# SCRIPT NAME: set_java_paths.hpy 
#
# PURPOSE: Sets jave paths in ~/.bashrc, ~/.modulesbeginenv, and
#          /path/to/dist/bin/nas-build.sh 
#
# RUNS ON: host
#          
# USAGE: ./set_java_paths.py -j [java_version] -u [user_name] -h       
#
...
# PARAMETERS: p_bashRcFilePath: Path to .modulesbeginenv file.
...
def updateBashRcFile(p_bashRcFilePath, p_versionJava):
   ...
   l_jvmRegExp = re.compile("^module\s+add\s+jvm\/.*")
...
def updateModulesBeginEnvFile(p_modulesBeginEnvFilePath, p_versionJava):
   ...
   l_regexpMatch = \
   re.search(".*jdk\d+\.(\d+)\.\d+_\d+$", p_versionJava)
   ...
   l_alphaNumRegExp = re.compile("^" + l_version_number + "[a-z]\d+$")
